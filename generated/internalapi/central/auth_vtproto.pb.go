...
func (m *ServiceCertAuth) CloneVT() *ServiceCertAuth {
	...
	if rhs := m.CurrentTime; rhs != nil {
		if vtpb, ok := interface{}(rhs).(interface{ CloneVT() *timestamppb.Timestamp }); ok {
			r.CurrentTime = vtpb.CloneVT()
		} else {
			r.CurrentTime = proto.Clone(rhs).(*timestamppb.Timestamp)
		}
	}
...
func (m *ServiceCertAuth) CloneGenericVT() proto.Message {
	return m.CloneVT()
}
...
func (this *ServiceCertAuth) EqualVT(that *ServiceCertAuth) bool {
	...
	if equal, ok := interface{}(this.CurrentTime).(interface {
		EqualVT(*timestamppb.Timestamp) bool
	}); ok {
		if !equal.EqualVT(that.CurrentTime) {
			return false
		}
	} else if !proto.Equal(this.CurrentTime, that.CurrentTime) {
		return false
	}
...
func (m *ServiceCertAuth) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if m.CurrentTime != nil {
		if vtmsg, ok := interface{}(m.CurrentTime).(interface {
			...
			encoded, err := proto.Marshal(m.CurrentTime)
			if err != nil {
				return 0, err
			}
			i -= len(encoded)
...
func (m *ServiceCertAuth) SizeVT() (n int) {
	...
	if m.CurrentTime != nil {
		if size, ok := interface{}(m.CurrentTime).(interface {
			SizeVT() int
		}); ok {
			l = size.SizeVT()
		} else {
			l = proto.Size(m.CurrentTime)
		}
		n += 1 + l + sov(uint64(l))
	}
...
func (m *ServiceCertAuth) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 2:
			...
			if unmarshal, ok := interface{}(m.CurrentTime).(interface {
				...
				if err := proto.Unmarshal(dAtA[iNdEx:postIndex], m.CurrentTime); err != nil {
					return err
				}
			}
			iNdEx = postIndex
