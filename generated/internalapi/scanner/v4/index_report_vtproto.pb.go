...
func (this *IndexReport) EqualVT(that *IndexReport) bool {
	...
	if this.Rhcoscpe != that.Rhcoscpe {
		return false
	}
...
func (m *IndexReport) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if len(m.Rhcoscpe) > 0 {
		i -= len(m.Rhcoscpe)
		copy(dAtA[i:], m.Rhcoscpe)
		i = protohelpers.EncodeVarint(dAtA, i, uint64(len(m.Rhcoscpe)))
		i--
		dAtA[i] = 0x32
	}
	if m.Contents != nil {
...
func (m *IndexReport) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 6:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field Rhcoscpe", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.Rhcoscpe = string(dAtA[iNdEx:postIndex])
			iNdEx = postIndex
		default:
			iNdEx = preIndex
