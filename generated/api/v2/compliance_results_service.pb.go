// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.11
// 	protoc        v6.32.1
// source: api/v2/compliance_results_service.proto

package v2

import (
	_ "google.golang.org/genproto/googleapis/api/annotations"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	timestamppb "google.golang.org/protobuf/types/known/timestamppb"
	reflect "reflect"
	sync "sync"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

// ClusterCheckStatus groups the result of the check by cluster
type ClusterCheckStatus struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Cluster       *ComplianceScanCluster `protobuf:"bytes,1,opt,name=cluster,proto3" json:"cluster,omitempty"`
	Status        ComplianceCheckStatus  `protobuf:"varint,2,opt,name=status,proto3,enum=v2.ComplianceCheckStatus" json:"status,omitempty"`
	CreatedTime   *timestamppb.Timestamp `protobuf:"bytes,3,opt,name=created_time,json=createdTime,proto3" json:"created_time,omitempty"`
	CheckUid      string                 `protobuf:"bytes,4,opt,name=check_uid,json=checkUid,proto3" json:"check_uid,omitempty"`
	LastScanTime  *timestamppb.Timestamp `protobuf:"bytes,5,opt,name=last_scan_time,json=lastScanTime,proto3" json:"last_scan_time,omitempty"`
	DataState     ComplianceDataState    `protobuf:"varint,6,opt,name=data_state,json=dataState,proto3,enum=v2.ComplianceDataState" json:"data_state,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ClusterCheckStatus) Reset() {
	*x = ClusterCheckStatus{}
	mi := &file_api_v2_compliance_results_service_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ClusterCheckStatus) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ClusterCheckStatus) ProtoMessage() {}

func (x *ClusterCheckStatus) ProtoReflect() protoreflect.Message {
	mi := &file_api_v2_compliance_results_service_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ClusterCheckStatus.ProtoReflect.Descriptor instead.
func (*ClusterCheckStatus) Descriptor() ([]byte, []int) {
	return file_api_v2_compliance_results_service_proto_rawDescGZIP(), []int{0}
}

func (x *ClusterCheckStatus) GetCluster() *ComplianceScanCluster {
	if x != nil {
		return x.Cluster
	}
	return nil
}

func (x *ClusterCheckStatus) GetStatus() ComplianceCheckStatus {
	if x != nil {
		return x.Status
	}
	return ComplianceCheckStatus_UNSET_CHECK_STATUS
}

func (x *ClusterCheckStatus) GetCreatedTime() *timestamppb.Timestamp {
	if x != nil {
		return x.CreatedTime
	}
	return nil
}

func (x *ClusterCheckStatus) GetCheckUid() string {
	if x != nil {
		return x.CheckUid
	}
	return ""
}

func (x *ClusterCheckStatus) GetLastScanTime() *timestamppb.Timestamp {
	if x != nil {
		return x.LastScanTime
	}
	return nil
}

func (x *ClusterCheckStatus) GetDataState() ComplianceDataState {
	if x != nil {
		return x.DataState
	}
	return ComplianceDataState_COMPLIANCE_DATA_STATE_UNKNOWN
}

// ComplianceCheckResult details of an instance of a compliance check result
type ComplianceCheckResult struct {
	state        protoimpl.MessageState `protogen:"open.v1"`
	CheckId      string                 `protobuf:"bytes,1,opt,name=check_id,json=checkId,proto3" json:"check_id,omitempty"`
	CheckName    string                 `protobuf:"bytes,2,opt,name=check_name,json=checkName,proto3" json:"check_name,omitempty"`
	CheckUid     string                 `protobuf:"bytes,3,opt,name=check_uid,json=checkUid,proto3" json:"check_uid,omitempty"`
	Description  string                 `protobuf:"bytes,4,opt,name=description,proto3" json:"description,omitempty"`
	Instructions string                 `protobuf:"bytes,5,opt,name=instructions,proto3" json:"instructions,omitempty"`
	Rationale    string                 `protobuf:"bytes,8,opt,name=rationale,proto3" json:"rationale,omitempty"`
	ValuesUsed   []string               `protobuf:"bytes,9,rep,name=valuesUsed,proto3" json:"valuesUsed,omitempty"`
	Warnings     []string               `protobuf:"bytes,10,rep,name=warnings,proto3" json:"warnings,omitempty"`
	Status       ComplianceCheckStatus  `protobuf:"varint,11,opt,name=status,proto3,enum=v2.ComplianceCheckStatus" json:"status,omitempty"`
	RuleName     string                 `protobuf:"bytes,12,opt,name=rule_name,json=ruleName,proto3" json:"rule_name,omitempty"`
	Labels       map[string]string      `protobuf:"bytes,13,rep,name=labels,proto3" json:"labels,omitempty" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"`
	Annotations  map[string]string      `protobuf:"bytes,14,rep,name=annotations,proto3" json:"annotations,omitempty" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"`
	Controls     []*ComplianceControl   `protobuf:"bytes,15,rep,name=controls,proto3" json:"controls,omitempty"`
	// Per-check freshness for this cluster: OUTDATED if the check's assessment
	// time predates the config's most recent expected scan.
	DataState     ComplianceDataState `protobuf:"varint,16,opt,name=data_state,json=dataState,proto3,enum=v2.ComplianceDataState" json:"data_state,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ComplianceCheckResult) Reset() {
	*x = ComplianceCheckResult{}
	mi := &file_api_v2_compliance_results_service_proto_msgTypes[1]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ComplianceCheckResult) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ComplianceCheckResult) ProtoMessage() {}

func (x *ComplianceCheckResult) ProtoReflect() protoreflect.Message {
	mi := &file_api_v2_compliance_results_service_proto_msgTypes[1]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ComplianceCheckResult.ProtoReflect.Descriptor instead.
func (*ComplianceCheckResult) Descriptor() ([]byte, []int) {
	return file_api_v2_compliance_results_service_proto_rawDescGZIP(), []int{1}
}

func (x *ComplianceCheckResult) GetCheckId() string {
	if x != nil {
		return x.CheckId
	}
	return ""
}

func (x *ComplianceCheckResult) GetCheckName() string {
	if x != nil {
		return x.CheckName
	}
	return ""
}

func (x *ComplianceCheckResult) GetCheckUid() string {
	if x != nil {
		return x.CheckUid
	}
	return ""
}

func (x *ComplianceCheckResult) GetDescription() string {
	if x != nil {
		return x.Description
	}
	return ""
}

func (x *ComplianceCheckResult) GetInstructions() string {
	if x != nil {
		return x.Instructions
	}
	return ""
}

func (x *ComplianceCheckResult) GetRationale() string {
	if x != nil {
		return x.Rationale
	}
	return ""
}

func (x *ComplianceCheckResult) GetValuesUsed() []string {
	if x != nil {
		return x.ValuesUsed
	}
	return nil
}

func (x *ComplianceCheckResult) GetWarnings() []string {
	if x != nil {
		return x.Warnings
	}
	return nil
}

func (x *ComplianceCheckResult) GetStatus() ComplianceCheckStatus {
	if x != nil {
		return x.Status
	}
	return ComplianceCheckStatus_UNSET_CHECK_STATUS
}

func (x *ComplianceCheckResult) GetRuleName() string {
	if x != nil {
		return x.RuleName
	}
	return ""
}

func (x *ComplianceCheckResult) GetLabels() map[string]string {
	if x != nil {
		return x.Labels
	}
	return nil
}

func (x *ComplianceCheckResult) GetAnnotations() map[string]string {
	if x != nil {
		return x.Annotations
	}
	return nil
}

func (x *ComplianceCheckResult) GetControls() []*ComplianceControl {
	if x != nil {
		return x.Controls
	}
	return nil
}

func (x *ComplianceCheckResult) GetDataState() ComplianceDataState {
	if x != nil {
		return x.DataState
	}
	return ComplianceDataState_COMPLIANCE_DATA_STATE_UNKNOWN
}

type ComplianceCheckData struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	ClusterId     string                 `protobuf:"bytes,1,opt,name=cluster_id,json=clusterId,proto3" json:"cluster_id,omitempty"`
	ScanName      string                 `protobuf:"bytes,2,opt,name=scan_name,json=scanName,proto3" json:"scan_name,omitempty"`
	Result        *ComplianceCheckResult `protobuf:"bytes,3,opt,name=result,proto3" json:"result,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ComplianceCheckData) Reset() {
	*x = ComplianceCheckData{}
	mi := &file_api_v2_compliance_results_service_proto_msgTypes[2]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ComplianceCheckData) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ComplianceCheckData) ProtoMessage() {}

func (x *ComplianceCheckData) ProtoReflect() protoreflect.Message {
	mi := &file_api_v2_compliance_results_service_proto_msgTypes[2]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ComplianceCheckData.ProtoReflect.Descriptor instead.
func (*ComplianceCheckData) Descriptor() ([]byte, []int) {
	return file_api_v2_compliance_results_service_proto_rawDescGZIP(), []int{2}
}

func (x *ComplianceCheckData) GetClusterId() string {
	if x != nil {
		return x.ClusterId
	}
	return ""
}

func (x *ComplianceCheckData) GetScanName() string {
	if x != nil {
		return x.ScanName
	}
	return ""
}

func (x *ComplianceCheckData) GetResult() *ComplianceCheckResult {
	if x != nil {
		return x.Result
	}
	return nil
}

// ComplianceClusterCheckStatus provides the status of a compliance check result across clusters
type ComplianceClusterCheckStatus struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	CheckId       string                 `protobuf:"bytes,1,opt,name=check_id,json=checkId,proto3" json:"check_id,omitempty"`
	CheckName     string                 `protobuf:"bytes,2,opt,name=check_name,json=checkName,proto3" json:"check_name,omitempty"`
	Clusters      []*ClusterCheckStatus  `protobuf:"bytes,3,rep,name=clusters,proto3" json:"clusters,omitempty"`
	Description   string                 `protobuf:"bytes,4,opt,name=description,proto3" json:"description,omitempty"`
	Instructions  string                 `protobuf:"bytes,5,opt,name=instructions,proto3" json:"instructions,omitempty"`
	Rationale     string                 `protobuf:"bytes,8,opt,name=rationale,proto3" json:"rationale,omitempty"`
	ValuesUsed    []string               `protobuf:"bytes,9,rep,name=valuesUsed,proto3" json:"valuesUsed,omitempty"`
	Warnings      []string               `protobuf:"bytes,10,rep,name=warnings,proto3" json:"warnings,omitempty"`
	Labels        map[string]string      `protobuf:"bytes,11,rep,name=labels,proto3" json:"labels,omitempty" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"`
	Annotations   map[string]string      `protobuf:"bytes,12,rep,name=annotations,proto3" json:"annotations,omitempty" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"`
	Controls      []*ComplianceControl   `protobuf:"bytes,13,rep,name=controls,proto3" json:"controls,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ComplianceClusterCheckStatus) Reset() {
	*x = ComplianceClusterCheckStatus{}
	mi := &file_api_v2_compliance_results_service_proto_msgTypes[3]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ComplianceClusterCheckStatus) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ComplianceClusterCheckStatus) ProtoMessage() {}

func (x *ComplianceClusterCheckStatus) ProtoReflect() protoreflect.Message {
	mi := &file_api_v2_compliance_results_service_proto_msgTypes[3]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ComplianceClusterCheckStatus.ProtoReflect.Descriptor instead.
func (*ComplianceClusterCheckStatus) Descriptor() ([]byte, []int) {
	return file_api_v2_compliance_results_service_proto_rawDescGZIP(), []int{3}
}

func (x *ComplianceClusterCheckStatus) GetCheckId() string {
	if x != nil {
		return x.CheckId
	}
	return ""
}

func (x *ComplianceClusterCheckStatus) GetCheckName() string {
	if x != nil {
		return x.CheckName
	}
	return ""
}

func (x *ComplianceClusterCheckStatus) GetClusters() []*ClusterCheckStatus {
	if x != nil {
		return x.Clusters
	}
	return nil
}

func (x *ComplianceClusterCheckStatus) GetDescription() string {
	if x != nil {
		return x.Description
	}
	return ""
}

func (x *ComplianceClusterCheckStatus) GetInstructions() string {
	if x != nil {
		return x.Instructions
	}
	return ""
}

func (x *ComplianceClusterCheckStatus) GetRationale() string {
	if x != nil {
		return x.Rationale
	}
	return ""
}

func (x *ComplianceClusterCheckStatus) GetValuesUsed() []string {
	if x != nil {
		return x.ValuesUsed
	}
	return nil
}

func (x *ComplianceClusterCheckStatus) GetWarnings() []string {
	if x != nil {
		return x.Warnings
	}
	return nil
}

func (x *ComplianceClusterCheckStatus) GetLabels() map[string]string {
	if x != nil {
		return x.Labels
	}
	return nil
}

func (x *ComplianceClusterCheckStatus) GetAnnotations() map[string]string {
	if x != nil {
		return x.Annotations
	}
	return nil
}

func (x *ComplianceClusterCheckStatus) GetControls() []*ComplianceControl {
	if x != nil {
		return x.Controls
	}
	return nil
}

// ComplianceScanResult provides the results of a scan
type ComplianceScanResult struct {
	state         protoimpl.MessageState          `protogen:"open.v1"`
	ScanName      string                          `protobuf:"bytes,1,opt,name=scan_name,json=scanName,proto3" json:"scan_name,omitempty"`
	ProfileName   string                          `protobuf:"bytes,2,opt,name=profile_name,json=profileName,proto3" json:"profile_name,omitempty"`
	CheckResults  []*ComplianceClusterCheckStatus `protobuf:"bytes,3,rep,name=check_results,json=checkResults,proto3" json:"check_results,omitempty"`
	ScanConfigId  string                          `protobuf:"bytes,4,opt,name=scan_config_id,json=scanConfigId,proto3" json:"scan_config_id,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ComplianceScanResult) Reset() {
	*x = ComplianceScanResult{}
	mi := &file_api_v2_compliance_results_service_proto_msgTypes[4]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ComplianceScanResult) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ComplianceScanResult) ProtoMessage() {}

func (x *ComplianceScanResult) ProtoReflect() protoreflect.Message {
	mi := &file_api_v2_compliance_results_service_proto_msgTypes[4]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ComplianceScanResult.ProtoReflect.Descriptor instead.
func (*ComplianceScanResult) Descriptor() ([]byte, []int) {
	return file_api_v2_compliance_results_service_proto_rawDescGZIP(), []int{4}
}

func (x *ComplianceScanResult) GetScanName() string {
	if x != nil {
		return x.ScanName
	}
	return ""
}

func (x *ComplianceScanResult) GetProfileName() string {
	if x != nil {
		return x.ProfileName
	}
	return ""
}

func (x *ComplianceScanResult) GetCheckResults() []*ComplianceClusterCheckStatus {
	if x != nil {
		return x.CheckResults
	}
	return nil
}

func (x *ComplianceScanResult) GetScanConfigId() string {
	if x != nil {
		return x.ScanConfigId
	}
	return ""
}

// ListComplianceScanResultsResponse provides the complete scan results
type ListComplianceScanResultsResponse struct {
	state         protoimpl.MessageState  `protogen:"open.v1"`
	ScanResults   []*ComplianceScanResult `protobuf:"bytes,1,rep,name=scan_results,json=scanResults,proto3" json:"scan_results,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListComplianceScanResultsResponse) Reset() {
	*x = ListComplianceScanResultsResponse{}
	mi := &file_api_v2_compliance_results_service_proto_msgTypes[5]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListComplianceScanResultsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListComplianceScanResultsResponse) ProtoMessage() {}

func (x *ListComplianceScanResultsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_api_v2_compliance_results_service_proto_msgTypes[5]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListComplianceScanResultsResponse.ProtoReflect.Descriptor instead.
func (*ListComplianceScanResultsResponse) Descriptor() ([]byte, []int) {
	return file_api_v2_compliance_results_service_proto_rawDescGZIP(), []int{5}
}

func (x *ListComplianceScanResultsResponse) GetScanResults() []*ComplianceScanResult {
	if x != nil {
		return x.ScanResults
	}
	return nil
}

// ListComplianceResultsResponse provides the complete scan results
type ListComplianceResultsResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	ScanResults   []*ComplianceCheckData `protobuf:"bytes,1,rep,name=scan_results,json=scanResults,proto3" json:"scan_results,omitempty"`
	TotalCount    int32                  `protobuf:"varint,2,opt,name=total_count,json=totalCount,proto3" json:"total_count,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListComplianceResultsResponse) Reset() {
	*x = ListComplianceResultsResponse{}
	mi := &file_api_v2_compliance_results_service_proto_msgTypes[6]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListComplianceResultsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListComplianceResultsResponse) ProtoMessage() {}

func (x *ListComplianceResultsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_api_v2_compliance_results_service_proto_msgTypes[6]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListComplianceResultsResponse.ProtoReflect.Descriptor instead.
func (*ListComplianceResultsResponse) Descriptor() ([]byte, []int) {
	return file_api_v2_compliance_results_service_proto_rawDescGZIP(), []int{6}
}

func (x *ListComplianceResultsResponse) GetScanResults() []*ComplianceCheckData {
	if x != nil {
		return x.ScanResults
	}
	return nil
}

func (x *ListComplianceResultsResponse) GetTotalCount() int32 {
	if x != nil {
		return x.TotalCount
	}
	return 0
}

// ListComplianceCheckClusterResponse provides stats per cluster
type ListComplianceCheckClusterResponse struct {
	state        protoimpl.MessageState `protogen:"open.v1"`
	CheckResults []*ClusterCheckStatus  `protobuf:"bytes,1,rep,name=check_results,json=checkResults,proto3" json:"check_results,omitempty"`
	ProfileName  string                 `protobuf:"bytes,2,opt,name=profile_name,json=profileName,proto3" json:"profile_name,omitempty"`
	CheckName    string                 `protobuf:"bytes,3,opt,name=check_name,json=checkName,proto3" json:"check_name,omitempty"`
	TotalCount   int32                  `protobuf:"varint,4,opt,name=total_count,json=totalCount,proto3" json:"total_count,omitempty"`
	Controls     []*ComplianceControl   `protobuf:"bytes,5,rep,name=controls,proto3" json:"controls,omitempty"`
	// Number of clusters with OUTDATED data for this check in the current
	// filtered scope (computed over the unpaginated result set).
	OutdatedClusterCount int32 `protobuf:"varint,6,opt,name=outdated_cluster_count,json=outdatedClusterCount,proto3" json:"outdated_cluster_count,omitempty"`
	unknownFields        protoimpl.UnknownFields
	sizeCache            protoimpl.SizeCache
}

func (x *ListComplianceCheckClusterResponse) Reset() {
	*x = ListComplianceCheckClusterResponse{}
	mi := &file_api_v2_compliance_results_service_proto_msgTypes[7]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListComplianceCheckClusterResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListComplianceCheckClusterResponse) ProtoMessage() {}

func (x *ListComplianceCheckClusterResponse) ProtoReflect() protoreflect.Message {
	mi := &file_api_v2_compliance_results_service_proto_msgTypes[7]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListComplianceCheckClusterResponse.ProtoReflect.Descriptor instead.
func (*ListComplianceCheckClusterResponse) Descriptor() ([]byte, []int) {
	return file_api_v2_compliance_results_service_proto_rawDescGZIP(), []int{7}
}

func (x *ListComplianceCheckClusterResponse) GetCheckResults() []*ClusterCheckStatus {
	if x != nil {
		return x.CheckResults
	}
	return nil
}

func (x *ListComplianceCheckClusterResponse) GetProfileName() string {
	if x != nil {
		return x.ProfileName
	}
	return ""
}

func (x *ListComplianceCheckClusterResponse) GetCheckName() string {
	if x != nil {
		return x.CheckName
	}
	return ""
}

func (x *ListComplianceCheckClusterResponse) GetTotalCount() int32 {
	if x != nil {
		return x.TotalCount
	}
	return 0
}

func (x *ListComplianceCheckClusterResponse) GetControls() []*ComplianceControl {
	if x != nil {
		return x.Controls
	}
	return nil
}

func (x *ListComplianceCheckClusterResponse) GetOutdatedClusterCount() int32 {
	if x != nil {
		return x.OutdatedClusterCount
	}
	return 0
}

type ListComplianceCheckResultResponse struct {
	state        protoimpl.MessageState   `protogen:"open.v1"`
	CheckResults []*ComplianceCheckResult `protobuf:"bytes,1,rep,name=check_results,json=checkResults,proto3" json:"check_results,omitempty"`
	ProfileName  string                   `protobuf:"bytes,2,opt,name=profile_name,json=profileName,proto3" json:"profile_name,omitempty"`
	ClusterId    string                   `protobuf:"bytes,3,opt,name=cluster_id,json=clusterId,proto3" json:"cluster_id,omitempty"`
	TotalCount   int32                    `protobuf:"varint,4,opt,name=total_count,json=totalCount,proto3" json:"total_count,omitempty"`
	LastScanTime *timestamppb.Timestamp   `protobuf:"bytes,5,opt,name=last_scan_time,json=lastScanTime,proto3" json:"last_scan_time,omitempty"`
	// 1 if this cluster's data is OUTDATED, 0 otherwise (single-cluster scope).
	OutdatedClusterCount int32 `protobuf:"varint,6,opt,name=outdated_cluster_count,json=outdatedClusterCount,proto3" json:"outdated_cluster_count,omitempty"`
	unknownFields        protoimpl.UnknownFields
	sizeCache            protoimpl.SizeCache
}

func (x *ListComplianceCheckResultResponse) Reset() {
	*x = ListComplianceCheckResultResponse{}
	mi := &file_api_v2_compliance_results_service_proto_msgTypes[8]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListComplianceCheckResultResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListComplianceCheckResultResponse) ProtoMessage() {}

func (x *ListComplianceCheckResultResponse) ProtoReflect() protoreflect.Message {
	mi := &file_api_v2_compliance_results_service_proto_msgTypes[8]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListComplianceCheckResultResponse.ProtoReflect.Descriptor instead.
func (*ListComplianceCheckResultResponse) Descriptor() ([]byte, []int) {
	return file_api_v2_compliance_results_service_proto_rawDescGZIP(), []int{8}
}

func (x *ListComplianceCheckResultResponse) GetCheckResults() []*ComplianceCheckResult {
	if x != nil {
		return x.CheckResults
	}
	return nil
}

func (x *ListComplianceCheckResultResponse) GetProfileName() string {
	if x != nil {
		return x.ProfileName
	}
	return ""
}

func (x *ListComplianceCheckResultResponse) GetClusterId() string {
	if x != nil {
		return x.ClusterId
	}
	return ""
}

func (x *ListComplianceCheckResultResponse) GetTotalCount() int32 {
	if x != nil {
		return x.TotalCount
	}
	return 0
}

func (x *ListComplianceCheckResultResponse) GetLastScanTime() *timestamppb.Timestamp {
	if x != nil {
		return x.LastScanTime
	}
	return nil
}

func (x *ListComplianceCheckResultResponse) GetOutdatedClusterCount() int32 {
	if x != nil {
		return x.OutdatedClusterCount
	}
	return 0
}

type ComplianceScanResultsRequest struct {
	state          protoimpl.MessageState `protogen:"open.v1"`
	ScanConfigName string                 `protobuf:"bytes,1,opt,name=scan_config_name,json=scanConfigName,proto3" json:"scan_config_name,omitempty"`
	Query          *RawQuery              `protobuf:"bytes,2,opt,name=query,proto3" json:"query,omitempty"`
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *ComplianceScanResultsRequest) Reset() {
	*x = ComplianceScanResultsRequest{}
	mi := &file_api_v2_compliance_results_service_proto_msgTypes[9]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ComplianceScanResultsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ComplianceScanResultsRequest) ProtoMessage() {}

func (x *ComplianceScanResultsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_api_v2_compliance_results_service_proto_msgTypes[9]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ComplianceScanResultsRequest.ProtoReflect.Descriptor instead.
func (*ComplianceScanResultsRequest) Descriptor() ([]byte, []int) {
	return file_api_v2_compliance_results_service_proto_rawDescGZIP(), []int{9}
}

func (x *ComplianceScanResultsRequest) GetScanConfigName() string {
	if x != nil {
		return x.ScanConfigName
	}
	return ""
}

func (x *ComplianceScanResultsRequest) GetQuery() *RawQuery {
	if x != nil {
		return x.Query
	}
	return nil
}

type ComplianceProfileClusterRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	ProfileName   string                 `protobuf:"bytes,1,opt,name=profile_name,json=profileName,proto3" json:"profile_name,omitempty"`
	ClusterId     string                 `protobuf:"bytes,2,opt,name=cluster_id,json=clusterId,proto3" json:"cluster_id,omitempty"`
	Query         *RawQuery              `protobuf:"bytes,3,opt,name=query,proto3" json:"query,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ComplianceProfileClusterRequest) Reset() {
	*x = ComplianceProfileClusterRequest{}
	mi := &file_api_v2_compliance_results_service_proto_msgTypes[10]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ComplianceProfileClusterRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ComplianceProfileClusterRequest) ProtoMessage() {}

func (x *ComplianceProfileClusterRequest) ProtoReflect() protoreflect.Message {
	mi := &file_api_v2_compliance_results_service_proto_msgTypes[10]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ComplianceProfileClusterRequest.ProtoReflect.Descriptor instead.
func (*ComplianceProfileClusterRequest) Descriptor() ([]byte, []int) {
	return file_api_v2_compliance_results_service_proto_rawDescGZIP(), []int{10}
}

func (x *ComplianceProfileClusterRequest) GetProfileName() string {
	if x != nil {
		return x.ProfileName
	}
	return ""
}

func (x *ComplianceProfileClusterRequest) GetClusterId() string {
	if x != nil {
		return x.ClusterId
	}
	return ""
}

func (x *ComplianceProfileClusterRequest) GetQuery() *RawQuery {
	if x != nil {
		return x.Query
	}
	return nil
}

type ComplianceCheckDetailRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	ProfileName   string                 `protobuf:"bytes,1,opt,name=profile_name,json=profileName,proto3" json:"profile_name,omitempty"`
	CheckName     string                 `protobuf:"bytes,2,opt,name=check_name,json=checkName,proto3" json:"check_name,omitempty"`
	Query         *RawQuery              `protobuf:"bytes,3,opt,name=query,proto3" json:"query,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ComplianceCheckDetailRequest) Reset() {
	*x = ComplianceCheckDetailRequest{}
	mi := &file_api_v2_compliance_results_service_proto_msgTypes[11]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ComplianceCheckDetailRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ComplianceCheckDetailRequest) ProtoMessage() {}

func (x *ComplianceCheckDetailRequest) ProtoReflect() protoreflect.Message {
	mi := &file_api_v2_compliance_results_service_proto_msgTypes[11]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ComplianceCheckDetailRequest.ProtoReflect.Descriptor instead.
func (*ComplianceCheckDetailRequest) Descriptor() ([]byte, []int) {
	return file_api_v2_compliance_results_service_proto_rawDescGZIP(), []int{11}
}

func (x *ComplianceCheckDetailRequest) GetProfileName() string {
	if x != nil {
		return x.ProfileName
	}
	return ""
}

func (x *ComplianceCheckDetailRequest) GetCheckName() string {
	if x != nil {
		return x.CheckName
	}
	return ""
}

func (x *ComplianceCheckDetailRequest) GetQuery() *RawQuery {
	if x != nil {
		return x.Query
	}
	return nil
}

var File_api_v2_compliance_results_service_proto protoreflect.FileDescriptor

const file_api_v2_compliance_results_service_proto_rawDesc = "" +
	"\n" +
	"'api/v2/compliance_results_service.proto\x12\x02v2\x1a\x13api/v2/common.proto\x1a\x1eapi/v2/compliance_common.proto\x1a\x19api/v2/search_query.proto\x1a\x1cgoogle/api/annotations.proto\x1a\x1fgoogle/protobuf/timestamp.proto\"\xd2\x02\n" +
	"\x12ClusterCheckStatus\x123\n" +
	"\acluster\x18\x01 \x01(\v2\x19.v2.ComplianceScanClusterR\acluster\x121\n" +
	"\x06status\x18\x02 \x01(\x0e2\x19.v2.ComplianceCheckStatusR\x06status\x12=\n" +
	"\fcreated_time\x18\x03 \x01(\v2\x1a.google.protobuf.TimestampR\vcreatedTime\x12\x1b\n" +
	"\tcheck_uid\x18\x04 \x01(\tR\bcheckUid\x12@\n" +
	"\x0elast_scan_time\x18\x05 \x01(\v2\x1a.google.protobuf.TimestampR\flastScanTime\x126\n" +
	"\n" +
	"data_state\x18\x06 \x01(\x0e2\x17.v2.ComplianceDataStateR\tdataState\"\xdd\x05\n" +
	"\x15ComplianceCheckResult\x12\x19\n" +
	"\bcheck_id\x18\x01 \x01(\tR\acheckId\x12\x1d\n" +
	"\n" +
	"check_name\x18\x02 \x01(\tR\tcheckName\x12\x1b\n" +
	"\tcheck_uid\x18\x03 \x01(\tR\bcheckUid\x12 \n" +
	"\vdescription\x18\x04 \x01(\tR\vdescription\x12\"\n" +
	"\finstructions\x18\x05 \x01(\tR\finstructions\x12\x1c\n" +
	"\trationale\x18\b \x01(\tR\trationale\x12\x1e\n" +
	"\n" +
	"valuesUsed\x18\t \x03(\tR\n" +
	"valuesUsed\x12\x1a\n" +
	"\bwarnings\x18\n" +
	" \x03(\tR\bwarnings\x121\n" +
	"\x06status\x18\v \x01(\x0e2\x19.v2.ComplianceCheckStatusR\x06status\x12\x1b\n" +
	"\trule_name\x18\f \x01(\tR\bruleName\x12=\n" +
	"\x06labels\x18\r \x03(\v2%.v2.ComplianceCheckResult.LabelsEntryR\x06labels\x12L\n" +
	"\vannotations\x18\x0e \x03(\v2*.v2.ComplianceCheckResult.AnnotationsEntryR\vannotations\x121\n" +
	"\bcontrols\x18\x0f \x03(\v2\x15.v2.ComplianceControlR\bcontrols\x126\n" +
	"\n" +
	"data_state\x18\x10 \x01(\x0e2\x17.v2.ComplianceDataStateR\tdataState\x1a9\n" +
	"\vLabelsEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x12\x14\n" +
	"\x05value\x18\x02 \x01(\tR\x05value:\x028\x01\x1a>\n" +
	"\x10AnnotationsEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x12\x14\n" +
	"\x05value\x18\x02 \x01(\tR\x05value:\x028\x01J\x04\b\x06\x10\aJ\x04\b\a\x10\b\"\x84\x01\n" +
	"\x13ComplianceCheckData\x12\x1d\n" +
	"\n" +
	"cluster_id\x18\x01 \x01(\tR\tclusterId\x12\x1b\n" +
	"\tscan_name\x18\x02 \x01(\tR\bscanName\x121\n" +
	"\x06result\x18\x03 \x01(\v2\x19.v2.ComplianceCheckResultR\x06result\"\x81\x05\n" +
	"\x1cComplianceClusterCheckStatus\x12\x19\n" +
	"\bcheck_id\x18\x01 \x01(\tR\acheckId\x12\x1d\n" +
	"\n" +
	"check_name\x18\x02 \x01(\tR\tcheckName\x122\n" +
	"\bclusters\x18\x03 \x03(\v2\x16.v2.ClusterCheckStatusR\bclusters\x12 \n" +
	"\vdescription\x18\x04 \x01(\tR\vdescription\x12\"\n" +
	"\finstructions\x18\x05 \x01(\tR\finstructions\x12\x1c\n" +
	"\trationale\x18\b \x01(\tR\trationale\x12\x1e\n" +
	"\n" +
	"valuesUsed\x18\t \x03(\tR\n" +
	"valuesUsed\x12\x1a\n" +
	"\bwarnings\x18\n" +
	" \x03(\tR\bwarnings\x12D\n" +
	"\x06labels\x18\v \x03(\v2,.v2.ComplianceClusterCheckStatus.LabelsEntryR\x06labels\x12S\n" +
	"\vannotations\x18\f \x03(\v21.v2.ComplianceClusterCheckStatus.AnnotationsEntryR\vannotations\x121\n" +
	"\bcontrols\x18\r \x03(\v2\x15.v2.ComplianceControlR\bcontrols\x1a9\n" +
	"\vLabelsEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x12\x14\n" +
	"\x05value\x18\x02 \x01(\tR\x05value:\x028\x01\x1a>\n" +
	"\x10AnnotationsEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x12\x14\n" +
	"\x05value\x18\x02 \x01(\tR\x05value:\x028\x01J\x04\b\x06\x10\aJ\x04\b\a\x10\b\"\xc3\x01\n" +
	"\x14ComplianceScanResult\x12\x1b\n" +
	"\tscan_name\x18\x01 \x01(\tR\bscanName\x12!\n" +
	"\fprofile_name\x18\x02 \x01(\tR\vprofileName\x12E\n" +
	"\rcheck_results\x18\x03 \x03(\v2 .v2.ComplianceClusterCheckStatusR\fcheckResults\x12$\n" +
	"\x0escan_config_id\x18\x04 \x01(\tR\fscanConfigId\"`\n" +
	"!ListComplianceScanResultsResponse\x12;\n" +
	"\fscan_results\x18\x01 \x03(\v2\x18.v2.ComplianceScanResultR\vscanResults\"|\n" +
	"\x1dListComplianceResultsResponse\x12:\n" +
	"\fscan_results\x18\x01 \x03(\v2\x17.v2.ComplianceCheckDataR\vscanResults\x12\x1f\n" +
	"\vtotal_count\x18\x02 \x01(\x05R\n" +
	"totalCount\"\xad\x02\n" +
	"\"ListComplianceCheckClusterResponse\x12;\n" +
	"\rcheck_results\x18\x01 \x03(\v2\x16.v2.ClusterCheckStatusR\fcheckResults\x12!\n" +
	"\fprofile_name\x18\x02 \x01(\tR\vprofileName\x12\x1d\n" +
	"\n" +
	"check_name\x18\x03 \x01(\tR\tcheckName\x12\x1f\n" +
	"\vtotal_count\x18\x04 \x01(\x05R\n" +
	"totalCount\x121\n" +
	"\bcontrols\x18\x05 \x03(\v2\x15.v2.ComplianceControlR\bcontrols\x124\n" +
	"\x16outdated_cluster_count\x18\x06 \x01(\x05R\x14outdatedClusterCount\"\xbe\x02\n" +
	"!ListComplianceCheckResultResponse\x12>\n" +
	"\rcheck_results\x18\x01 \x03(\v2\x19.v2.ComplianceCheckResultR\fcheckResults\x12!\n" +
	"\fprofile_name\x18\x02 \x01(\tR\vprofileName\x12\x1d\n" +
	"\n" +
	"cluster_id\x18\x03 \x01(\tR\tclusterId\x12\x1f\n" +
	"\vtotal_count\x18\x04 \x01(\x05R\n" +
	"totalCount\x12@\n" +
	"\x0elast_scan_time\x18\x05 \x01(\v2\x1a.google.protobuf.TimestampR\flastScanTime\x124\n" +
	"\x16outdated_cluster_count\x18\x06 \x01(\x05R\x14outdatedClusterCount\"l\n" +
	"\x1cComplianceScanResultsRequest\x12(\n" +
	"\x10scan_config_name\x18\x01 \x01(\tR\x0escanConfigName\x12\"\n" +
	"\x05query\x18\x02 \x01(\v2\f.v2.RawQueryR\x05query\"\x87\x01\n" +
	"\x1fComplianceProfileClusterRequest\x12!\n" +
	"\fprofile_name\x18\x01 \x01(\tR\vprofileName\x12\x1d\n" +
	"\n" +
	"cluster_id\x18\x02 \x01(\tR\tclusterId\x12\"\n" +
	"\x05query\x18\x03 \x01(\v2\f.v2.RawQueryR\x05query\"\x84\x01\n" +
	"\x1cComplianceCheckDetailRequest\x12!\n" +
	"\fprofile_name\x18\x01 \x01(\tR\vprofileName\x12\x1d\n" +
	"\n" +
	"check_name\x18\x02 \x01(\tR\tcheckName\x12\"\n" +
	"\x05query\x18\x03 \x01(\v2\f.v2.RawQueryR\x05query2\xa3\t\n" +
	"\x18ComplianceResultsService\x12{\n" +
	"\x1cGetComplianceScanCheckResult\x12\x10.v2.ResourceByID\x1a .v2.ComplianceClusterCheckStatus\"'\x82\xd3\xe4\x93\x02!\x12\x1f/v2/compliance/scan/result/{id}\x12p\n" +
	"\x18GetComplianceScanResults\x12\f.v2.RawQuery\x1a!.v2.ListComplianceResultsResponse\"#\x82\xd3\xe4\x93\x02\x1d\x12\x1b/v2/compliance/scan/results\x12\xa4\x01\n" +
	"%GetComplianceScanConfigurationResults\x12 .v2.ComplianceScanResultsRequest\x1a!.v2.ListComplianceResultsResponse\"6\x82\xd3\xe4\x93\x020\x12./v2/compliance/scan/results/{scan_config_name}\x12\xa8\x01\n" +
	"\x1bGetComplianceProfileResults\x12#.v2.ComplianceProfileResultsRequest\x1a .v2.ListComplianceProfileResults\"B\x82\xd3\xe4\x93\x02<\x12:/v2/compliance/scan/results/profiles/{profile_name}/checks\x12\xbd\x01\n" +
	"\x1fGetComplianceProfileCheckResult\x12!.v2.ComplianceProfileCheckRequest\x1a&.v2.ListComplianceCheckClusterResponse\"O\x82\xd3\xe4\x93\x02I\x12G/v2/compliance/scan/results/profiles/{profile_name}/checks/{check_name}\x12\xc3\x01\n" +
	"\"GetComplianceProfileClusterResults\x12#.v2.ComplianceProfileClusterRequest\x1a%.v2.ListComplianceCheckResultResponse\"Q\x82\xd3\xe4\x93\x02K\x12I/v2/compliance/scan/results/profiles/{profile_name}/clusters/{cluster_id}\x12\xbf\x01\n" +
	" GetComplianceProfileCheckDetails\x12 .v2.ComplianceCheckDetailRequest\x1a .v2.ComplianceClusterCheckStatus\"W\x82\xd3\xe4\x93\x02Q\x12O/v2/compliance/scan/results/profiles/{profile_name}/checks/{check_name}/detailsB'\n" +
	"\x18io.stackrox.proto.api.v2Z\v./api/v2;v2X\x03b\x06proto3"

var (
	file_api_v2_compliance_results_service_proto_rawDescOnce sync.Once
	file_api_v2_compliance_results_service_proto_rawDescData []byte
)

func file_api_v2_compliance_results_service_proto_rawDescGZIP() []byte {
	file_api_v2_compliance_results_service_proto_rawDescOnce.Do(func() {
		file_api_v2_compliance_results_service_proto_rawDescData = protoimpl.X.CompressGZIP(unsafe.Slice(unsafe.StringData(file_api_v2_compliance_results_service_proto_rawDesc), len(file_api_v2_compliance_results_service_proto_rawDesc)))
	})
	return file_api_v2_compliance_results_service_proto_rawDescData
}

var file_api_v2_compliance_results_service_proto_msgTypes = make([]protoimpl.MessageInfo, 16)
var file_api_v2_compliance_results_service_proto_goTypes = []any{
	(*ClusterCheckStatus)(nil),                 // 0: v2.ClusterCheckStatus
	(*ComplianceCheckResult)(nil),              // 1: v2.ComplianceCheckResult
	(*ComplianceCheckData)(nil),                // 2: v2.ComplianceCheckData
	(*ComplianceClusterCheckStatus)(nil),       // 3: v2.ComplianceClusterCheckStatus
	(*ComplianceScanResult)(nil),               // 4: v2.ComplianceScanResult
	(*ListComplianceScanResultsResponse)(nil),  // 5: v2.ListComplianceScanResultsResponse
	(*ListComplianceResultsResponse)(nil),      // 6: v2.ListComplianceResultsResponse
	(*ListComplianceCheckClusterResponse)(nil), // 7: v2.ListComplianceCheckClusterResponse
	(*ListComplianceCheckResultResponse)(nil),  // 8: v2.ListComplianceCheckResultResponse
	(*ComplianceScanResultsRequest)(nil),       // 9: v2.ComplianceScanResultsRequest
	(*ComplianceProfileClusterRequest)(nil),    // 10: v2.ComplianceProfileClusterRequest
	(*ComplianceCheckDetailRequest)(nil),       // 11: v2.ComplianceCheckDetailRequest
	nil,                                        // 12: v2.ComplianceCheckResult.LabelsEntry
	nil,                                        // 13: v2.ComplianceCheckResult.AnnotationsEntry
	nil,                                        // 14: v2.ComplianceClusterCheckStatus.LabelsEntry
	nil,                                        // 15: v2.ComplianceClusterCheckStatus.AnnotationsEntry
	(*ComplianceScanCluster)(nil),              // 16: v2.ComplianceScanCluster
	(ComplianceCheckStatus)(0),                 // 17: v2.ComplianceCheckStatus
	(*timestamppb.Timestamp)(nil),              // 18: google.protobuf.Timestamp
	(ComplianceDataState)(0),                   // 19: v2.ComplianceDataState
	(*ComplianceControl)(nil),                  // 20: v2.ComplianceControl
	(*RawQuery)(nil),                           // 21: v2.RawQuery
	(*ResourceByID)(nil),                       // 22: v2.ResourceByID
	(*ComplianceProfileResultsRequest)(nil),    // 23: v2.ComplianceProfileResultsRequest
	(*ComplianceProfileCheckRequest)(nil),      // 24: v2.ComplianceProfileCheckRequest
	(*ListComplianceProfileResults)(nil),       // 25: v2.ListComplianceProfileResults
}
var file_api_v2_compliance_results_service_proto_depIdxs = []int32{
	16, // 0: v2.ClusterCheckStatus.cluster:type_name -> v2.ComplianceScanCluster
	17, // 1: v2.ClusterCheckStatus.status:type_name -> v2.ComplianceCheckStatus
	18, // 2: v2.ClusterCheckStatus.created_time:type_name -> google.protobuf.Timestamp
	18, // 3: v2.ClusterCheckStatus.last_scan_time:type_name -> google.protobuf.Timestamp
	19, // 4: v2.ClusterCheckStatus.data_state:type_name -> v2.ComplianceDataState
	17, // 5: v2.ComplianceCheckResult.status:type_name -> v2.ComplianceCheckStatus
	12, // 6: v2.ComplianceCheckResult.labels:type_name -> v2.ComplianceCheckResult.LabelsEntry
	13, // 7: v2.ComplianceCheckResult.annotations:type_name -> v2.ComplianceCheckResult.AnnotationsEntry
	20, // 8: v2.ComplianceCheckResult.controls:type_name -> v2.ComplianceControl
	19, // 9: v2.ComplianceCheckResult.data_state:type_name -> v2.ComplianceDataState
	1,  // 10: v2.ComplianceCheckData.result:type_name -> v2.ComplianceCheckResult
	0,  // 11: v2.ComplianceClusterCheckStatus.clusters:type_name -> v2.ClusterCheckStatus
	14, // 12: v2.ComplianceClusterCheckStatus.labels:type_name -> v2.ComplianceClusterCheckStatus.LabelsEntry
	15, // 13: v2.ComplianceClusterCheckStatus.annotations:type_name -> v2.ComplianceClusterCheckStatus.AnnotationsEntry
	20, // 14: v2.ComplianceClusterCheckStatus.controls:type_name -> v2.ComplianceControl
	3,  // 15: v2.ComplianceScanResult.check_results:type_name -> v2.ComplianceClusterCheckStatus
	4,  // 16: v2.ListComplianceScanResultsResponse.scan_results:type_name -> v2.ComplianceScanResult
	2,  // 17: v2.ListComplianceResultsResponse.scan_results:type_name -> v2.ComplianceCheckData
	0,  // 18: v2.ListComplianceCheckClusterResponse.check_results:type_name -> v2.ClusterCheckStatus
	20, // 19: v2.ListComplianceCheckClusterResponse.controls:type_name -> v2.ComplianceControl
	1,  // 20: v2.ListComplianceCheckResultResponse.check_results:type_name -> v2.ComplianceCheckResult
	18, // 21: v2.ListComplianceCheckResultResponse.last_scan_time:type_name -> google.protobuf.Timestamp
	21, // 22: v2.ComplianceScanResultsRequest.query:type_name -> v2.RawQuery
	21, // 23: v2.ComplianceProfileClusterRequest.query:type_name -> v2.RawQuery
	21, // 24: v2.ComplianceCheckDetailRequest.query:type_name -> v2.RawQuery
	22, // 25: v2.ComplianceResultsService.GetComplianceScanCheckResult:input_type -> v2.ResourceByID
	21, // 26: v2.ComplianceResultsService.GetComplianceScanResults:input_type -> v2.RawQuery
	9,  // 27: v2.ComplianceResultsService.GetComplianceScanConfigurationResults:input_type -> v2.ComplianceScanResultsRequest
	23, // 28: v2.ComplianceResultsService.GetComplianceProfileResults:input_type -> v2.ComplianceProfileResultsRequest
	24, // 29: v2.ComplianceResultsService.GetComplianceProfileCheckResult:input_type -> v2.ComplianceProfileCheckRequest
	10, // 30: v2.ComplianceResultsService.GetComplianceProfileClusterResults:input_type -> v2.ComplianceProfileClusterRequest
	11, // 31: v2.ComplianceResultsService.GetComplianceProfileCheckDetails:input_type -> v2.ComplianceCheckDetailRequest
	3,  // 32: v2.ComplianceResultsService.GetComplianceScanCheckResult:output_type -> v2.ComplianceClusterCheckStatus
	6,  // 33: v2.ComplianceResultsService.GetComplianceScanResults:output_type -> v2.ListComplianceResultsResponse
	6,  // 34: v2.ComplianceResultsService.GetComplianceScanConfigurationResults:output_type -> v2.ListComplianceResultsResponse
	25, // 35: v2.ComplianceResultsService.GetComplianceProfileResults:output_type -> v2.ListComplianceProfileResults
	7,  // 36: v2.ComplianceResultsService.GetComplianceProfileCheckResult:output_type -> v2.ListComplianceCheckClusterResponse
	8,  // 37: v2.ComplianceResultsService.GetComplianceProfileClusterResults:output_type -> v2.ListComplianceCheckResultResponse
	3,  // 38: v2.ComplianceResultsService.GetComplianceProfileCheckDetails:output_type -> v2.ComplianceClusterCheckStatus
	32, // [32:39] is the sub-list for method output_type
	25, // [25:32] is the sub-list for method input_type
	25, // [25:25] is the sub-list for extension type_name
	25, // [25:25] is the sub-list for extension extendee
	0,  // [0:25] is the sub-list for field type_name
}

func init() { file_api_v2_compliance_results_service_proto_init() }
func file_api_v2_compliance_results_service_proto_init() {
	if File_api_v2_compliance_results_service_proto != nil {
		return
	}
	file_api_v2_common_proto_init()
	file_api_v2_compliance_common_proto_init()
	file_api_v2_search_query_proto_init()
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_api_v2_compliance_results_service_proto_rawDesc), len(file_api_v2_compliance_results_service_proto_rawDesc)),
			NumEnums:      0,
			NumMessages:   16,
			NumExtensions: 0,
			NumServices:   1,
		},
		GoTypes:           file_api_v2_compliance_results_service_proto_goTypes,
		DependencyIndexes: file_api_v2_compliance_results_service_proto_depIdxs,
		MessageInfos:      file_api_v2_compliance_results_service_proto_msgTypes,
	}.Build()
	File_api_v2_compliance_results_service_proto = out.File
	file_api_v2_compliance_results_service_proto_goTypes = nil
	file_api_v2_compliance_results_service_proto_depIdxs = nil
}
