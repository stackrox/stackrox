...
func (m *LocalSensorPolicies) CloneMessageVT() proto.Message {
	return m.CloneVT()
}
...
func (this *LocalSensorPolicies) EqualMessageVT(thatMsg proto.Message) bool {
	that, ok := thatMsg.(*LocalSensorPolicies)
	if !ok {
		return false
	}
...
func (m *LocalSensorPolicies) UnmarshalVTUnsafe(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		preIndex := iNdEx
