...
func (m *CosignSignature) CloneVT() *CosignSignature {
	...
	if rhs := m.Rfc3161Timestamp; rhs != nil {
		tmpBytes := make([]byte, len(rhs))
		copy(tmpBytes, rhs)
		r.Rfc3161Timestamp = tmpBytes
	}
	if len(m.unknownFields) > 0 {
		r.unknownFields = make([]byte, len(m.unknownFields))
		copy(r.unknownFields, m.unknownFields)
...
func (this *CosignSignature) EqualVT(that *CosignSignature) bool {
	...
	if string(this.Rfc3161Timestamp) != string(that.Rfc3161Timestamp) {
		return false
	}
	return string(this.unknownFields) == string(that.unknownFields)
}
...
func (m *CosignSignature) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if len(m.Rfc3161Timestamp) > 0 {
		i -= len(m.Rfc3161Timestamp)
		copy(dAtA[i:], m.Rfc3161Timestamp)
		i = protohelpers.EncodeVarint(dAtA, i, uint64(len(m.Rfc3161Timestamp)))
		i--
		dAtA[i] = 0x32
	}
...
func (m *CosignSignature) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 6:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field Rfc3161Timestamp", wireType)
			}
			var byteLen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.Rfc3161Timestamp = append(m.Rfc3161Timestamp[:0], dAtA[iNdEx:postIndex]...)
			if m.Rfc3161Timestamp == nil {
				m.Rfc3161Timestamp = []byte{}
			}
			iNdEx = postIndex
		default:
			iNdEx = preIndex
...
func (m *CosignSignature) UnmarshalVTUnsafe(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 6:
			...
			m.Rfc3161Timestamp = dAtA[iNdEx:postIndex]
			iNdEx = postIndex
		default:
			iNdEx = preIndex
			skippy, err := protohelpers.Skip(dAtA[iNdEx:])
