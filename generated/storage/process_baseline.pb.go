...
type ProcessBaselineKey struct {
	...
	DeploymentId  string `protobuf:"bytes,1,opt,name=deployment_id,json=deploymentId,proto3" json:"deployment_id,omitempty" search:"Deployment ID" sql:"type(uuid),index=hash"` // @gotags: search:"Deployment ID" sql:"type(uuid),index=hash"
...
const file_storage_process_baseline_proto_rawDesc = "" +
	"\n" +
	"\x1estorage/process_baseline.proto\x12\astorage\x1a\x1fgoogle/protobuf/timestamp.proto\"\x9d\x01\n" +
	...
	"\x11baseline_statuses\x18\x02 \x03(\v2'.storage.ContainerNameAndBaselineStatusR\x10baselineStatusesB1\n" +
...
var (
	...
	file_storage_process_baseline_proto_rawDescData []byte
)

func file_storage_process_baseline_proto_rawDescGZIP() []byte {
	file_storage_process_baseline_proto_rawDescOnce.Do(func() {
		file_storage_process_baseline_proto_rawDescData = protoimpl.X.CompressGZIP(unsafe.Slice(unsafe.StringData(file_storage_process_baseline_proto_rawDesc), len(file_storage_process_baseline_proto_rawDesc)))
	})
