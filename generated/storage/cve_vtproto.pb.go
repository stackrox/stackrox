...
func (m *ImageCVEV2) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if len(m.RHSA) > 0 {
		i -= len(m.RHSA)
		copy(dAtA[i:], m.RHSA)
		i = protohelpers.EncodeVarint(dAtA, i, uint64(len(m.RHSA)))
		i--
		dAtA[i] = 0x7a
	}
...
func (m *ImageCVEV2) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 15:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field RHSA", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.RHSA = string(dAtA[iNdEx:postIndex])
			iNdEx = postIndex
		default:
			iNdEx = preIndex
			skippy, err := protohelpers.Skip(dAtA[iNdEx:])
