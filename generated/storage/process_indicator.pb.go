...
type ProcessIndicator struct {
	...
	Id            string `protobuf:"bytes,1,opt,name=id,proto3" json:"id,omitempty" search:"Process ID,hidden" sql:"pk,type(uuid)"`                                            // @gotags: search:"Process ID,hidden"  sql:"pk,type(uuid)"
	DeploymentId  string `protobuf:"bytes,2,opt,name=deployment_id,json=deploymentId,proto3" json:"deployment_id,omitempty" search:"Deployment ID,hidden" policy:",prefer-parent" sql:"index,fk(Deployment:id),no-fk-constraint,type(uuid)"`    // @gotags: search:"Deployment ID,hidden" policy:",prefer-parent" sql:"index,fk(Deployment:id),no-fk-constraint,type(uuid)"
	...
	PodUid string `protobuf:"bytes,11,opt,name=pod_uid,json=podUid,proto3" json:"pod_uid,omitempty" search:"Pod UID,hidden" sql:"index,type(uuid)"` // @gotags: search:"Pod UID,hidden" sql:"index,type(uuid)"
	...
	ClusterId          string                 `protobuf:"bytes,7,opt,name=cluster_id,json=clusterId,proto3" json:"cluster_id,omitempty" search:"Cluster ID,hidden" policy:",prefer-parent" sql:"type(uuid)"`                              // @gotags: search:"Cluster ID,hidden" policy:",prefer-parent" sql:"type(uuid)"
	Namespace          string                 `protobuf:"bytes,8,opt,name=namespace,proto3" json:"namespace,omitempty" search:"Namespace,hidden" policy:",prefer-parent"`                                               // @gotags: search:"Namespace,hidden" policy:",prefer-parent"
...
func (x *ProcessIndicator) ProtoReflect() protoreflect.Message {
	file_storage_process_indicator_proto_init_ensure()
	mi := &file_storage_process_indicator_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
...
var file_storage_process_indicator_proto_init_once sync.Once
func file_storage_process_indicator_proto_init_ensure() { file_storage_process_indicator_proto_init_once.Do(file_storage_process_indicator_proto_init) }
func init() {} // proto registration is lazy — triggered by first ProtoReflect() call
func file_storage_process_indicator_proto_init() {
	if File_storage_process_indicator_proto != nil || skipProtoInit {
		return
	}
	type x struct{}
