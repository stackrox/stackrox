...
func (m *AuthProvider_RequiredAttribute) CloneVT() *AuthProvider_RequiredAttribute {
	...
	r := new(AuthProvider_RequiredAttribute)
	r.AttributeKey = m.AttributeKey
	r.AttributeValue = m.AttributeValue
...
func (m *AuthProvider_RequiredAttribute) CloneMessageVT() proto.Message {
	return m.CloneVT()
}
...
func (m *AuthProvider) CloneVT() *AuthProvider {
	...
	if rhs := m.ClaimMappings; rhs != nil {
		tmpContainer := make(map[string]string, len(rhs))
		for k, v := range rhs {
			tmpContainer[k] = v
		}
		r.ClaimMappings = tmpContainer
	}
	if len(m.unknownFields) > 0 {
		r.unknownFields = make([]byte, len(m.unknownFields))
		copy(r.unknownFields, m.unknownFields)
...
func (m *AuthProvider) CloneMessageVT() proto.Message {
	return m.CloneVT()
}
...
func (this *AuthProvider_RequiredAttribute) EqualMessageVT(thatMsg proto.Message) bool {
	that, ok := thatMsg.(*AuthProvider_RequiredAttribute)
	if !ok {
		return false
	}
...
func (this *AuthProvider) EqualVT(that *AuthProvider) bool {
	...
	if len(this.ClaimMappings) != len(that.ClaimMappings) {
		return false
	}
	for i, vx := range this.ClaimMappings {
		vy, ok := that.ClaimMappings[i]
		if !ok {
			return false
		}
		if vx != vy {
...
func (this *AuthProvider) EqualMessageVT(thatMsg proto.Message) bool {
	that, ok := thatMsg.(*AuthProvider)
	if !ok {
		return false
	}
	return this.EqualVT(that)
...
func (m *AuthProvider_RequiredAttribute) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if len(m.AttributeKey) > 0 {
		...
		i = protohelpers.EncodeVarint(dAtA, i, uint64(len(m.AttributeKey)))
		i--
		dAtA[i] = 0xa
	}
...
func (m *AuthProvider) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if len(m.ExtraUiEndpoints) > 0 {
		for iNdEx := len(m.ExtraUiEndpoints) - 1; iNdEx >= 0; iNdEx-- {
			...
			i = protohelpers.EncodeVarint(dAtA, i, uint64(len(m.ExtraUiEndpoints[iNdEx])))
			i--
			dAtA[i] = 0x4a
		}
	}
...
func (m *AuthProvider) UnmarshalVTUnsafe(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		preIndex := iNdEx
		...
		case 9:
			...
			m.ExtraUiEndpoints = append(m.ExtraUiEndpoints, stringValue)
			iNdEx = postIndex
		case 10:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field Active", wireType)
		...
		case 11:
			...
			m.RequiredAttributes = append(m.RequiredAttributes, &AuthProvider_RequiredAttribute{})
			if err := m.RequiredAttributes[len(m.RequiredAttributes)-1].UnmarshalVTUnsafe(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		case 12:
