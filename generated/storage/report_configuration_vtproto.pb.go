...
func (m *VulnerabilityReportFilters) CloneVT() *VulnerabilityReportFilters {
	...
	r.IncludeKnownExploit = m.IncludeKnownExploit
...
func (this *VulnerabilityReportFilters) EqualVT(that *VulnerabilityReportFilters) bool {
	...
	if this.IncludeKnownExploit != that.IncludeKnownExploit {
		return false
	}
	return string(this.unknownFields) == string(that.unknownFields)
}
...
func (m *VulnerabilityReportFilters) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 12:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field IncludeKnownExploit", wireType)
			}
			var v int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.IncludeKnownExploit = bool(v != 0)
		default:
			iNdEx = preIndex
			skippy, err := protohelpers.Skip(dAtA[iNdEx:])
			if err != nil {
