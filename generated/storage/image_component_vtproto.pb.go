...
func (this *ImageComponentV2) EqualVT(that *ImageComponentV2) bool {
	...
	if this.LayerIdx != that.LayerIdx {
		return false
	}
	return string(this.unknownFields) == string(that.unknownFields)
}
...
func (m *ImageComponentV2) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if m.LayerIdx != 0 {
		i = protohelpers.EncodeVarint(dAtA, i, uint64(m.LayerIdx))
		i--
		dAtA[i] = 0x78
	}
...
func (m *ImageComponentV2) SizeVT() (n int) {
	...
	if m.LayerIdx != 0 {
		n += 1 + protohelpers.SizeOfVarint(uint64(m.LayerIdx))
	}
	n += len(m.unknownFields)
	return n
}
...
func (m *ImageComponentV2) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 15:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field LayerIdx", wireType)
			}
			m.LayerIdx = 0
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
