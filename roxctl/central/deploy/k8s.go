...
func k8sBasedOrchestrator(k8sConfig *renderer.K8sConfig, shortName, longName string, cluster storage.ClusterType) *cobra.Command {
	...
	flagWrap.StringVarP(&k8sConfig.Monitoring.Password, "monitoring-password", "p", "", "enter a monitoring password (default: autogenerated)", "monitoring", "monitoring-type=on-prem")
	flagWrap.StringVarP(&k8sConfig.MonitoringImage, "monitoring-image", "", "", "monitoring image to use (default: same repository as main)", "monitoring", "monitoring-type=on-prem")
	...
	flagWrap.StringVarP(&k8sConfig.Monitoring.External.Name, "monitoring-persistence-name", "", "monitoring-db", "external volume name", "monitoring", "monitoring-type=on-prem", "monitoring-persistence-type=pvc")
	flagWrap.StringVarP(&k8sConfig.Monitoring.External.StorageClass, "monitoring-persistence-storage-class", "", "", "monitoring storage class name (optional if you have a default StorageClass configured)", "monitoring", "monitoring-type=on-prem", "monitoring-persistence-type=pvc")

	flagWrap.StringVarP(&k8sConfig.Monitoring.HostPath.HostPath, "monitoring-persistence-hostpath", "", "/var/lib/stackrox/monitoring", "monitoring path on the host", "monitoring", "monitoring-type=on-prem", "monitoring-persistence-type=hostpath")
	flagWrap.StringVarP(&k8sConfig.Monitoring.HostPath.NodeSelectorKey, "monitoring-node-selector-key", "", "", "monitoring node selector key (e.g. kubernetes.io/hostname)", "monitoring", "monitoring-type=on-prem", "monitoring-persistence-type=hostpath")
	flagWrap.StringVarP(&k8sConfig.Monitoring.HostPath.NodeSelectorValue, "monitoring-node-selector-value", "", "", "monitoring node selector value", "monitoring", "monitoring-type=on-prem", "monitoring-persistence-type=hostpath")
