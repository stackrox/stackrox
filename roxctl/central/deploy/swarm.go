...
func dockerBasedOrchestrator(shortName, longName string, cluster storage.ClusterType) *cobra.Command {
	...
	c.PersistentFlags().StringVarP(&swarmConfig.ClairifyImage, "clairify-image", "", defaults.ClairifyImage, "Clairify image to use")
	c.PersistentFlags().StringVarP(&swarmConfig.MainImage, "main-image", "i", defaults.MainImage, "Tmage to use")
