...
func (l *loginCommand) verifyLoginAuthProviders() error {
	...
	// When using the default endpoint, disable retries to fail fast on connection errors.
	clientOpts := []common.HttpClientOption{common.WithAuthMethod(auth.Anonymous())}
	if !flags.EndpointWasExplicitlyProvided() {
		clientOpts = append(clientOpts, common.WithRetryCount(0))
	}
	httpClient, err := l.env.HTTPClient(l.timeout, clientOpts...)
