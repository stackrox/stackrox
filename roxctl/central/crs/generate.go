...
func generateCRS(cliEnvironment environment.Environment, name string,
	...
	validFor time.Duration, validUntil time.Time, maxClusters uint32,
) error {
	var err error
	var outFile *os.File
...
func generateCommand(cliEnvironment environment.Environment) *cobra.Command {
	...
	c.PersistentFlags().Uint32Var(&maxClusters, "max-clusters", 0, "Specify maximum number of clusters which can be registered with the new CRS (0 means no limit).")
