...
func Command(cliEnvironment environment.Environment) *cobra.Command {
	...
	c.RunE = func(cmd *cobra.Command, args []string) error {
		...
		for {
			...
			jsonBytes, err := protocompat.MarshalToProtoJSONBytes(image)
			if err != nil {
				return errors.Wrap(err, "unable to serialize image")
			}
			if _, err := cliEnvironment.InputOutput().Out().Write(jsonBytes); err != nil {
				return errors.Wrap(err, "unable to write serialized image")
			}
		}
		return nil
	}
