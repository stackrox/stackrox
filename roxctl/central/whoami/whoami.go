...
func (cmd *centralWhoAmICommand) checkVersionCompatibility(ctx context.Context, conn *grpc.ClientConn) {
	...
	if err != nil {
		cmd.env.Logger().WarnfLn("Could not parse roxctl version %q: %v", roxctlVersion, err)
		return
	}
	...
	if err != nil {
		cmd.env.Logger().WarnfLn("Could not parse Central version %q: %v", centralVersion, err)
		return
	}
	...
	compatRange := formatVersionRange(versioncompatibility.CompatibleVersions(roxctlXY))
