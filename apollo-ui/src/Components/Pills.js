...
class Pills extends Component {
    ...
    displayData() {
        ...
        return this.state.data.map((item) => {
            ...
            return <div className={pillClass} key={`${item.value}`} onClick={this.activatePillHandler(item)} onKeyPress={this.activatePillHandler(item)} role="presentation">{item.text}</div>;
        });
    }
