...
Capture ALL test output as evidence. If `freeze` is available, render key outputs
as images for PR attachment:

```bash
# Pipe command output through freeze to produce a PNG
oc -n <ns> get pods 2>&1 | freeze --language "" --output /tmp/verify-pods.png
# Or capture a command directly
freeze --execute "curl -sk -u admin:<password> https://<endpoint>/v1/..." \
  --language json --output /tmp/verify-api.png
```

Save images to `/tmp/verify-*.png`. When freeze is not available, capture
command output and log snippets as plain text.
...
6. **Proof images**: List any `/tmp/verify-*.png` files generated by freeze
...
If freeze-generated images exist, mention their paths so the caller can attach them to the PR.
