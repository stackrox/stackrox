...
require (
	...
	github.com/ComplianceAsCode/compliance-operator v1.7.1-0.20260723162409-9b37150f1d06
...
// compliance-operator v1.9.0 directly requires github.com/itchyny/gojq v0.12.19 (used only by
// CO's cmd/manager, which we never import — we only use pkg/apis/compliance/v1alpha1). MVS
// otherwise elevates gojq repo-wide, but github.com/stackrox/helmtest v0.0.9 still uses the
// gojq.Suffix.Bind field that v0.12.19 removed (helmtest's main branch hasn't picked up a fix
// either). Pin back to the version helmtest supports until one side updates.
replace github.com/itchyny/gojq => github.com/itchyny/gojq v0.12.17
