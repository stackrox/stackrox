...
func (m *MockTransformer) Transform(config declarativeconfig.Configuration) (map[reflect.Type][]protocompat.Message, error) {
	m.ctrl.T.Helper()
	ret := m.ctrl.Call(m, "Transform", config)
	ret0, _ := ret[0].(map[reflect.Type][]protocompat.Message)
	ret1, _ := ret[1].(error)
	return ret0, ret1
}
