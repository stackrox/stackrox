...
func TestUniversalTransformGenericNotifier(t *testing.T) {
	notifier := &declarativeconfig.Notifier{
		Name: "test-notifier",
		GenericConfig: &declarativeconfig.GenericConfig{
			Endpoint:      "endpoint",
	...
	for _, m := range protos[notifierType] {
		casted, ok := m.(*storage.Notifier)
		if ok {
...
func TestUniversalTransformSplunkNotifier(t *testing.T) {
	notifier := &declarativeconfig.Notifier{
		Name: "test-notifier",
		SplunkConfig: &declarativeconfig.SplunkConfig{
			HTTPEndpoint: "endpoint",
