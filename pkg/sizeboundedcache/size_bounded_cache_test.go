...
func testSizeFunc(k string, v interface{}) int64 {
	return int64(len(k) + len(v.(string)))
}

func TestSizeBoundedCacheErrors(t *testing.T) {
	var cases = []struct {
		maxSize, maxItemSize int64
		costFunc             func(k string, v interface{}) int64
	}{
		{
			maxSize:     0,
			maxItemSize: 10,
...
func TestSizeBoundedCache(t *testing.T) {
	...
	cacheImpl := cache.(*sizeBoundedCache[string])
