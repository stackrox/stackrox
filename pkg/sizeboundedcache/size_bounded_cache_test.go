...
func TestSizeBoundedCache(t *testing.T) {
	...
	// simple add
	cacheImpl.Add("a", "b")
	assert.Equal(t, int64(2), cacheImpl.currSize.Load())
	...
	cacheImpl.Add("a", "bcd")
	assert.Equal(t, int64(4), cacheImpl.currSize.Load())
	assert.Equal(t, 1, cacheImpl.cache.Len())

	// Adding new value should increment
	cacheImpl.Add("b", "bcd")
	assert.Equal(t, int64(8), cacheImpl.currSize.Load())
	...
	assert.Equal(t, int64(9), cacheImpl.currSize.Load())
