...
func (s *PolicyValueValidator) TestRegex() {
	cases := []struct {
		...
		{
			name:    "file operation",
			valid:   []string{"OPEN", "CREATE", "UNLINK", "OWNERSHIP_CHANGE", "PERMISSION_CHANGE", "open", "create", "unlink", "ownership_change", "permission_change", "Open", "Create", "rename", "RENAME", "ACL_CHANGE", "acl_change", "Acl_Change"},
