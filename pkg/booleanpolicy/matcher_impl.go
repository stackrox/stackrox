...
type hostFileEventMatcher struct {
	fileEventOnlyMatchers []evaluator.Evaluator
	matcherImpl
}

func (m *hostFileEventMatcher) checkWhetherFileMatches(cache *CacheReceptacle, activity *storage.FileActivity) (bool, error) {
	var augmentedFile *pathutil.AugmentedObj
	if cache != nil && cache.augmentedFile != nil {
		augmentedFile = cache.augmentedFile
	} else {
		var err error
		augmentedFile, err = augmentedobjs.ConstructFileActivity(activity)
		if err != nil {
			return false, err
		}
		if cache != nil {
			cache.augmentedFile = augmentedFile
		}
	}

	for _, eval := range m.fileEventOnlyMatchers {
		_, matched := eval.Evaluate(augmentedFile.Value())
		if matched {
			return true, nil
		}
	}
...
func (m *hostFileEventMatcher) MatchHostFileActivity(cache *CacheReceptacle, activity *storage.FileActivity) (Violations, error) {
	if cache == nil || cache.augmentedObj == nil {
		matched, err := m.checkWhetherFileMatches(cache, activity)
		if err != nil || !matched {
			return Violations{}, err
		}
	}
	...
	violations, err := m.getViolations(cache, func() (*pathutil.AugmentedObj, error) {
		return augmentedobjs.ConstructFileActivity(activity)
	}, nil, nil, nil, nil, activity)
	if err != nil || violations == nil {
		return Violations{}, err
	}
...
func (m *matcherImpl) getViolations(
	...
	var processIndicatorMatched, kubeOrAuditEventMatched, networkFlowMatched, networkPolicyMatched, fileActivityMatched bool
	for _, eval := range m.evaluators {
		...
		alertViolations, isProcessViolation, isKubeOrAuditEventViolation, isNetworkFlowViolation, isNetworkPolicyViolation, isFileActivityViolation, err :=
			violationmessages.Render(eval.section, result, indicator, kubeEvent, networkFlow, networkPolicy, fileActivity)
		if err != nil {
			return nil, err
		}
		...
		} else if isFileActivityViolation {
			fileActivityMatched = true
		}

		v.AlertViolations = append(v.AlertViolations, alertViolations...)
	}
	if !atLeastOneMatched && !processIndicatorMatched && !kubeOrAuditEventMatched && !networkFlowMatched && !networkPolicyMatched && !fileActivityMatched {
		return nil, nil
	}
	...
	} else if fileActivityMatched {
		v.FileActivityViolation = &storage.Alert_FileActivityViolation{
			Activity: []*storage.FileActivity{fileActivity},
		}
		printer.UpdateFileActivityAlertViolationMessage(v.FileActivityViolation)
	}
	return v, nil
}
