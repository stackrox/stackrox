	...
	for _, tc := range []struct {
		...
		},
		{
			...
			events: []eventWrapper{
				...
				{
					access:      newEffectiveFileAccessEvent("/etc/passwd", storage.FileAccess_XATTR_SET),
					expectAlert: true,
				},
				{
					access:      newEffectiveFileAccessEvent("/etc/passwd", storage.FileAccess_XATTR_REMOVE),
					expectAlert: true,
				},
			},
		},
