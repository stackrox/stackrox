...
// This file is mostly copied verbatim from Helm, with some tweaks to stop
// roxvet from complaining.
...
func (r *Rules) Ignore(path string, fi os.FileInfo) bool {
	...
	for _, p := range r.patterns {
		...
		if p.negate {
			// StackRox change - without this, negated patterns are pretty much useless.
			// Basically, since in the presence of a .helmignore file there is no concept of
			// "ignored by default", a negation can _only_ make sense if there is more
			// general non-negated match as well. As such, a negated rule should never be
			// sufficient alone to exclude a file, or rather, we never need to make that
			// decision for a negated rule, as there should always be a more general
			// non-negated rule.
