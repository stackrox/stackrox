...
// dictionary.
func ValuesForKVPair(configPath string, val any) (chartutil.Values, error) {
	if configPath == "" {
		return nil, errors.New("empty config path")
	}
	keyPath := strings.Split(configPath, ".")

	mapForSet := map[string]any{keyPath[len(keyPath)-1]: val}
	for i := len(keyPath) - 2; i >= 0; i-- {
		mapForSet = map[string]any{keyPath[i]: mapForSet}
	}
