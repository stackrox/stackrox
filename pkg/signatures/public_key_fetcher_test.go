...
func TestOpenshiftImage_NotWorking(t *testing.T) {
	...
	cimg, err := imgUtils.GenerateImageFromString("quay.io/openshift-release-dev/ocp-v4.0-art-dev@sha256:ac6b699c17280bde04ae86f2598b5322a2c057787ab4df38ee216426041d3de8")
	require.NoError(t, err)
	img := types.ToImage(cimg)

	f := &cosignPublicKeySignatureFetcher{}

	sigs, err := f.FetchSignatures(context.Background(), img, reg)
	assert.NotEmpty(t, sigs)
	assert.NoError(t, err)
}

func TestSomeAdditionalImages_WithoutAuthenticationRequired(t *testing.T) {
	reg, err := docker.NewDockerRegistry(&storage.ImageIntegration{
		Id:         "",
		Name:       "",
		Type:       "",
	...
	cimg, err := imgUtils.GenerateImageFromString("docker.io/istio/operator@sha256:4b368d1ecb9c5fbf73969258cd00a60b6c92fca19cbc5cfe9bebc841483c8a72")
	require.NoError(t, err)
	img := types.ToImage(cimg)
...
func TestAdditionalImages_WithAuthenticationRequired(t *testing.T) {
	reg, err := docker.NewDockerRegistry(&storage.ImageIntegration{
		Id:         "",
		Name:       "Private Docker",
		Type:       "docker",
