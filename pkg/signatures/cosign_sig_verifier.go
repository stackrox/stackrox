...
import (
	...
	pkgTuf "github.com/sigstore/sigstore-go/pkg/tuf"
...
func newCosignSignatureVerifier(config *storage.SignatureIntegration) (*cosignSignatureVerifier, error) {
	...
	return &cosignSignatureVerifier{
		tufRepositoryURL: config.GetTufRepositoryUrl(),
		parsedPublicKeys: parsedKeys,
		certs:            certsWithChains,
		transparencyLog:  config.GetTransparencyLog(),
	}, nil
}
...
func (c *cosignSignatureVerifier) createVerifierOpts(ctx context.Context) error {
	...
	tufClient, err := pkgTuf.New(pkgTuf.DefaultOptions().
		WithCachePath("/tmp/tuf-roots").
		WithRepositoryBaseURL(c.tufRepositoryURL),
	)
	if err != nil {
		verifierErrs = multierror.Append(verifierErrs, errors.Wrap(err, "creating tuf client"))
	}
	if tufClient != nil {
		tufClient.Refresh()
	}
...
func getSCLogPublicKeys(ctx context.Context, publicKey string) (*cosign.TrustedTransparencyLogPubKeys, error) {
	if publicKey == "" {
...
func (c *cosignSignatureVerifier) defaultCosignCheckOpts(ctx context.Context) (cosign.CheckOpts, error) {
	...
	opts.IgnoreTlog = !c.transparencyLog.GetEnabled()
	if !opts.IgnoreTlog {
		opts.RekorClient, err = rekorClient.GetRekorClient(
			urlfmt.FormatURL(c.transparencyLog.GetRekorUrl(), urlfmt.HTTPS, urlfmt.NoTrailingSlash),
		)
		if err != nil {
			return cosign.CheckOpts{}, errors.Wrap(err, "creating rekor client")
		}
		opts.RekorPubKeys, err = getRekorPublicKeys(ctx, c.transparencyLog.GetRekorPublicKeyPemEnc())
		if err != nil {
			return cosign.CheckOpts{}, errors.Wrap(err, "getting rekor public keys")
		}
		opts.IgnoreSCT = c.transparencyLog.GetIgnoreSct()
		if !opts.IgnoreSCT {
			opts.CTLogPubKeys, err = getSCLogPublicKeys(ctx, c.transparencyLog.GetCtlogPublicKeyPemEnc())
			if err != nil {
				return cosign.CheckOpts{}, errors.Wrap(err, "getting ctlog public keys")
			}
		}
		opts.Offline = c.transparencyLog.GetValidateOffline()
	}
	log.Infof("cosign check opts: %+v", opts)
	return opts, nil
}
...
func getBundleFromSignature(signature *storage.Signature) (*bundle.RekorBundle, error) {
	byteBundle := signature.GetCosign().GetRekorBundle()
...
func retrieveVerificationDataFromImage(image *storage.Image) ([]oci.Signature, gcrv1.Hash, error) {
	...
	for _, imgSig := range image.GetSignature().GetSignatures() {
		...
		log.Infof("raw rekor bundle for image %q: %+v", image.GetName().GetFullName(), imgSig.GetCosign().GetRekorBundle())
		rekorBundle, err := getBundleFromSignature(imgSig)
		log.Infof("typed rekor bundle for image %q: %+v", image.GetName().GetFullName(), rekorBundle)
		if err != nil {
		...
		bundle, _ := sig.Bundle()
		log.Infof("sig bundle for image %q: %+v", image.GetName().GetFullName(), bundle)
		if err != nil {
