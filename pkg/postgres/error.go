...
func toErrox(err error) error {
	if pgErr := (*pgconn.PgError)(nil); errors.As(err, &pgErr) {
		...
		case "23503":
			// Special case: for insert operations a FK constraint violation can occur when the referenced FK does not
			// exist. Instead of returning errox.ReferencedByAnotherObject, we shall return errox.NotFound here.
			...
			if strings.Contains(pgErr.Detail, "is not present in table") {
				return errors.Wrap(errox.NotFound, err.Error())
			}
