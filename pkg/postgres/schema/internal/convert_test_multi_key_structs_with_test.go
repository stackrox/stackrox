...
func convertTestMultiKeyStruct_NestedFromProto(obj *storage.TestMultiKeyStruct_Nested, test_multi_key_structs_Key1 string, test_multi_key_structs_Key2 string) (*schema.TestMultiKeyStructsNesteds, error) {
	serialized, err := obj.Marshal()
	if err != nil {
		return nil, err
	}
...
func convertTestMultiKeyStruct_NestedToProto(m *schema.TestMultiKeyStructsNesteds) (*storage.TestMultiKeyStruct_Nested, error) {
	var msg storage.TestMultiKeyStruct_Nested
	if err := msg.Unmarshal(m.Serialized); err != nil {
		return nil, err
	}
	return &msg, nil
...
func TestTestMultiKeyStruct_NestedConversion(t *testing.T) {
	obj := &storage.TestMultiKeyStruct_Nested{}
	assert.NoError(t, testutils.FullInit(obj, testutils.UniqueInitializer(), testutils.JSONFieldsFilter))
	m, err := convertTestMultiKeyStruct_NestedFromProto(obj)
	assert.NoError(t, err)
	conv, err := convertTestMultiKeyStruct_NestedToProto(m)
	assert.NoError(t, err)
	assert.Equal(t, obj, conv)
}
