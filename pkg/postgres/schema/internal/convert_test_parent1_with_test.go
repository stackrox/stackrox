...
func convertTestParent1_Child1RefFromProto(obj *storage.TestParent1_Child1Ref, test_parent1_Id string) (*schema.TestParent1Childrens, error) {
	serialized, err := obj.Marshal()
	if err != nil {
		return nil, err
	}
...
func convertTestParent1_Child1RefToProto(m *schema.TestParent1Childrens) (*storage.TestParent1_Child1Ref, error) {
	var msg storage.TestParent1_Child1Ref
	if err := msg.Unmarshal(m.Serialized); err != nil {
		return nil, err
	}
	return &msg, nil
...
func TestTestParent1_Child1RefConversion(t *testing.T) {
	obj := &storage.TestParent1_Child1Ref{}
	assert.NoError(t, testutils.FullInit(obj, testutils.UniqueInitializer(), testutils.JSONFieldsFilter))
	m, err := convertTestParent1_Child1RefFromProto(obj)
	assert.NoError(t, err)
	conv, err := convertTestParent1_Child1RefToProto(m)
	assert.NoError(t, err)
	assert.Equal(t, obj, conv)
}
