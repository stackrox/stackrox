...
type ProcessBaselines struct {
	...
	KeyDeploymentID string `gorm:"column:key_deploymentid;type:uuid;index:processbaselines_key_deploymentid,type:hash,option:CONCURRENTLY"`
	KeyClusterID    string `gorm:"column:key_clusterid;type:uuid;index:processbaselines_sac_filter,type:btree,option:CONCURRENTLY"`
	KeyNamespace    string `gorm:"column:key_namespace;type:varchar;index:processbaselines_sac_filter,type:btree,option:CONCURRENTLY"`
