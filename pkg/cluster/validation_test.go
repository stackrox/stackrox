...
func TestGetAutoLockProcessBaselinesEnabled(t *testing.T) {
	t.Run("returns true when enabled for manually managed cluster", func(t *testing.T) {
		...
		assert.True(t, GetAutoLockProcessBaselinesEnabled(cluster))
	})

	t.Run("returns false when disabled for manually managed cluster", func(t *testing.T) {
		cluster := &storage.Cluster{
			ManagedBy: storage.ManagerType_MANAGER_TYPE_MANUAL,
			DynamicConfig: &storage.DynamicClusterConfig{
				AutoLockProcessBaselinesConfig: &storage.AutoLockProcessBaselinesConfig{
		...
		assert.False(t, GetAutoLockProcessBaselinesEnabled(cluster))
	})

	t.Run("returns true when enabled for unknown manager type cluster", func(t *testing.T) {
		cluster := &storage.Cluster{
	...
	t.Run("returns true when enabled for helm managed cluster", func(t *testing.T) {
		cluster := &storage.Cluster{
	...
	t.Run("returns false when disabled for helm managed cluster", func(t *testing.T) {
		cluster := &storage.Cluster{
			ManagedBy: storage.ManagerType_MANAGER_TYPE_HELM_CHART,
			HelmConfig: &storage.CompleteClusterConfig{
				DynamicConfig: &storage.DynamicClusterConfig{
	...
	t.Run("returns true when enabled for operator managed cluster", func(t *testing.T) {
		cluster := &storage.Cluster{
