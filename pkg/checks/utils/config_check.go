...
func (c *CommandCheck) contains(result *v1.CheckResult, actualValue ConfigParams, exists bool) {
	Pass(result)
...
func (c *CommandCheck) notContains(result *v1.CheckResult, actualValue ConfigParams, exists bool) {
	Pass(result)
	if contains(actualValue, c.DesiredValue) {
		Warn(result)
...
func (c *CommandCheck) matches(result *v1.CheckResult, actualValue ConfigParams, exists bool) {
	Pass(result)
...
func (c *CommandCheck) notMatches(result *v1.CheckResult, actualValue ConfigParams, exists bool) {
	Pass(result)
	if !matches(actualValue, c.DesiredValue) {
		Warn(result)
...
func (c *CommandCheck) set(result *v1.CheckResult, actualValue ConfigParams, exists bool) {
	Pass(result)
	if !exists {
		Warn(result)
...
func (c *CommandCheck) unset(result *v1.CheckResult, actualValue ConfigParams, exists bool) {
	Pass(result)
	if exists {
		Warn(result)
...
func (c *CommandCheck) setAsAppropriate(result *v1.CheckResult, actualValue ConfigParams, exists bool) {
	Pass(result)
	if exists {
		Info(result)
	} else {
...
func (c *CommandCheck) note(result *v1.CheckResult, actualValue string, exists bool, verb string) {
...
func (c *CommandCheck) Run() (result v1.CheckResult) {
	if c.EvalFunc == Skip {
		Note(&result)
...
// Run evaluates the check
func (c *MultipleCommandChecks) Run() (result v1.CheckResult) {
	Pass(&result)
	for _, check := range c.Checks {
		...
		res := check.Run()
		if res.Result != v1.CheckStatus_PASS {
