...
func toProtoV4Contents(
	...
	// Use the lightweight index conversion
	contents, err := toProtoV4IndexContents(pkgs, dists, repos, envs)
	if err != nil {
		return nil, err
	}

	// Add enrichment data (pkgFixedBy) to packages
	if len(pkgFixedBy) > 0 {
		for id, fixedIn := range pkgFixedBy {
			if pkg, ok := contents.Packages[id]; ok {
				pkg.FixedInVersion = fixedIn
			}
		}
		// Also update deprecated slice
		for _, pkg := range contents.PackagesDEPRECATED {
			if fixedIn, ok := pkgFixedBy[pkg.Id]; ok {
				pkg.FixedInVersion = fixedIn
			}
		}
	}
