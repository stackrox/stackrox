...
func Test_getMaxBaseScore(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			args: args{
				cvssMetrics: []*v4.VulnerabilityReport_Vulnerability_CVSS{
					{
						V3: &v4.VulnerabilityReport_Vulnerability_CVSS_V3{BaseScore: 5.0},
					},
					{
		...
		{
			name: "Red Hat score takes priority over non-Red Hat",
			args: args{
				cvssMetrics: []*v4.VulnerabilityReport_Vulnerability_CVSS{
					{
						Source: v4.VulnerabilityReport_Vulnerability_CVSS_SOURCE_NVD,
					...
					{
						Source: v4.VulnerabilityReport_Vulnerability_CVSS_SOURCE_RED_HAT,
						V2:     &v4.VulnerabilityReport_Vulnerability_CVSS_V2{BaseScore: 7.0},
					},
					{
						Source: v4.VulnerabilityReport_Vulnerability_CVSS_SOURCE_OSV,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			assert.Equalf(t, tt.want, getMaxBaseScore(tt.args.cvssMetrics), "getMaxBaseScore(%v)", tt.args.cvssMetrics)
		})
	}
}
...
func Test_sortBySeverity(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			ids := sortBySeverity(tt.args.ids, tt.args.vulnerabilities)
			assert.Equalf(t, tt.want, ids, "sortBySeverity(%v, %v)", tt.args.ids, tt.args.vulnerabilities)
		})
	}
}
