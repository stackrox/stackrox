...
func filterProcesses(processes []*storage.ProcessIndicator, maxSize int, currSize *int) []*storage.ProcessIndicator {
	...
	for i := len(processes) - 1; i >= 0; i-- {
		jsonBytes, err := protocompat.MarshalToProtoJSONBytes(processes[i])
		if err != nil {
			log.Error(err)
		}
		*currSize -= len(jsonBytes)
		if *currSize < maxSize {
			return processes[:i]
		}
	}
...
func filterViolations(violations []*storage.Alert_Violation, maxSize int, currSize *int) []*storage.Alert_Violation {
	...
	for i := len(violations) - 1; i >= 0; i-- {
		jsonBytes, err := protocompat.MarshalToProtoJSONBytes(violations[i])
		if err != nil {
			log.Error(err)
		}
		*currSize -= len(jsonBytes)
...
func PruneAlert(alert *storage.Alert, maxSize int) {
	...
	jsonBytes, err := protocompat.MarshalToProtoJSONBytes(alert)
	if err != nil {
		log.Error(err)
	}
	currSize := len(jsonBytes)
