...
func TestMultiTree(t *testing.T) {
	...
	*/
	...
	assert.True(t, protocompat.Equal(e1, multiTree.Get("1")))

	assert.True(t, protocompat.Equal(e2, multiTree.GetSupernet("1")))
	assert.True(t, protocompat.Equal(e6, multiTree.GetSupernet("5")))
	...
	assert.True(t, protocompat.Equal(e6, multiTree.GetSupernetForCIDR("36.188.144.0/24")))
