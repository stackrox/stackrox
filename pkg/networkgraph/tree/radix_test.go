...
func TestNRadixTreeIPv4(t *testing.T) {
	...
	assert.True(t, protocompat.Equal(e1, tree.Get("1")))
	assert.True(t, protocompat.Equal(e2, tree.Get("2")))
	assert.True(t, protocompat.Equal(e3, tree.Get("3")))
	assert.True(t, protocompat.Equal(e4, tree.Get("4")))
	assert.True(t, protocompat.Equal(e5, tree.Get("5")))
	assert.True(t, protocompat.Equal(e6, tree.Get("6")))
	assert.True(t, protocompat.Equal(e7, tree.Get("7")))
	assert.True(t, protocompat.Equal(e8, tree.Get("8")))

	assert.Error(t, tree.Insert(testutils.GetExtSrcNetworkEntityInfo("60", "60", "36.188.144.0/16", true)))

	assert.True(t, protocompat.Equal(e2, tree.GetSupernet(e1.GetId())))
	assert.True(t, protocompat.Equal(e1, tree.GetSupernet(e4.GetId())))
	assert.True(t, protocompat.Equal(e7, tree.GetSupernet(e6.GetId())))

	assert.True(t, protocompat.Equal(e2, tree.GetMatchingSupernet(e4.GetId(), func(e *storage.NetworkEntityInfo) bool {
		return !e.GetExternalSource().GetDefault()
	})))

	assert.True(t, protocompat.Equal(e1, tree.GetMatchingSupernet(e4.GetId(), func(e *storage.NetworkEntityInfo) bool {
		return e.GetExternalSource().GetDefault()
	})))

	assert.Nil(t, tree.GetSupernetForCIDR("0.0.0.0/0"))
	assert.True(t, protocompat.Equal(e2, tree.GetSupernetForCIDR("35.187.144.0/20")))
	assert.True(t, protocompat.Equal(e2, tree.GetSupernetForCIDR("35.187.144.0/18")))

	assert.True(t, protocompat.Equal(e3, tree.GetMatchingSupernetForCIDR("35.187.144.0/18", func(e *storage.NetworkEntityInfo) bool {
		return e.GetId() != e2.GetId()
	})))

	assert.True(t, protocompat.Equal(e2, tree.GetMatchingSupernetForCIDR("35.187.144.0/18", nil)))
...
func TestNRadixTreeIPv6(t *testing.T) {
	...
	assert.True(t, protocompat.Equal(e1, tree.GetSupernet(e2.GetId())))
	assert.True(t, protocompat.Equal(networkgraph.InternetEntity().ToProto(), tree.GetSupernet(e1.GetId())))
	assert.True(t, protocompat.Equal(e5, tree.GetSupernet(e6.GetId())))

	assert.True(t, protocompat.Equal(e3, tree.GetMatchingSupernet(e4.GetId(), func(e *storage.NetworkEntityInfo) bool {
		return !e.GetExternalSource().GetDefault()
	})))
	...
	assert.True(t, protocompat.Equal(e2, tree.GetSupernetForCIDR("2001:db8:3333:4444:5555:6666:7777:8888/100")))
	assert.True(t, protocompat.Equal(e2, tree.GetSupernetForCIDR("2001:db8:3333:4444:5555:6666:7777:8888/90")))

	assert.True(t, protocompat.Equal(e1, tree.GetMatchingSupernetForCIDR("2001:db8:3333:4444:5555:6666:7777:8888/90", func(e *storage.NetworkEntityInfo) bool {
		return e.GetId() != e2.GetId()
	})))

	assert.True(t, protocompat.Equal(e2, tree.GetMatchingSupernetForCIDR("2001:db8:3333:4444:5555:6666:7777:8888/90", nil)))
