...
func GetImageV2ID(img *storage.ImageV2) (string, error) {
	if img.GetId() != "" {
		...
		if img.GetId() != expectedID {
			return "", utils.ShouldErr(errors.Errorf("image ID '%s' does not match expected UUID V5 '%s' generated by combining image fullname and digest", img.GetId(), expectedID))
		}
