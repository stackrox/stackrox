...
func (e *enricherImpl) delegateEnrichImage(ctx context.Context, enrichCtx EnrichmentContext, image *storage.Image) (bool, error) {
	...
	if exists && cachedImageIsValid(existingImg) {
		updated := e.updateImageWithExistingImage(ctx, image, existingImg, enrichCtx.FetchOpt)
		if updated {
			...
			caudit.AddEvent(ctx, caudit.StatusSuccess, "Using cached image from central-db")
...
func (e *enricherImpl) updateImageWithExistingImage(ctx context.Context, image *storage.Image, existingImage *storage.Image, option FetchOption) bool {
	...
	e.useExistingSignature(ctx, image, existingImage, option)
	e.useExistingSignatureVerificationData(ctx, image, existingImage, option, hasChangedNames)
	e.useExistingImageName(ctx, image, existingImage, option)
	return e.useExistingScan(ctx, image, existingImage, option)
}
...
func (e *enricherImpl) updateImageFromDatabase(ctx context.Context, img *storage.Image, option FetchOption) bool {
	...
	return e.updateImageWithExistingImage(ctx, img, existingImg, option)
}
...
func (e *enricherImpl) enrichWithMetadata(ctx context.Context, enrichmentContext EnrichmentContext, image *storage.Image) (bool, error) {
	...
	if !metadataOutOfDate {
		caudit.AddSuccessEvent(ctx, "Using existing metadata")
		return false, nil
	}
	...
	if !enrichmentContext.FetchOpt.forceRefetchCachedValues() &&
		...
		if metadataValue, ok := e.metadataCache.Get(getRef(image)); ok {
			...
			caudit.AddSuccessEvent(ctx, "Pulled metadata from central in-memory cache")
			return true, nil
		}
	...
	for _, registry := range registries {
		...
		if err != nil {
			caudit.AddFailureEvent(ctx, "Pulling metadata using integration %q: %v", registry.DataSource().Name, err)
		...
		if updated {
			...
			caudit.AddSuccessEvent(ctx, "Pulled metadata with integration %q", registry.DataSource().Name)
			return true, nil
		}
	}
...
func (e *enricherImpl) fetchFromDatabase(ctx context.Context, img *storage.Image, option FetchOption) (*storage.Image, bool) {
	...
	if err != nil {
		...
		caudit.AddFailureEvent(ctx, "Fetching image from central-db: %v", err)
		return img, false
	}
...
func (e *enricherImpl) useExistingScan(ctx context.Context, img *storage.Image, existingImg *storage.Image, option FetchOption) bool {
	if option == ForceRefetchScansOnly {
		return false
	}

	if existingImg.GetScan() != nil {
		caudit.AddSuccessEvent(ctx, "Using existing scan from central-db")
...
func (e *enricherImpl) useExistingSignature(ctx context.Context, img *storage.Image, existingImg *storage.Image, option FetchOption) {
	...
	if existingImg.GetSignature() != nil {
		caudit.AddSuccessEvent(ctx, "Using existing signature from central-db")
		img.Signature = existingImg.GetSignature()
	}
}

func (e *enricherImpl) useExistingSignatureVerificationData(ctx context.Context, img *storage.Image, existingImg *storage.Image,
	...
	if existingImg.GetSignatureVerificationData() != nil {
		caudit.AddSuccessEvent(ctx, "Using existing signature verification data from central-db")
		img.SignatureVerificationData = existingImg.GetSignatureVerificationData()
	}
}

func (e *enricherImpl) useExistingImageName(ctx context.Context, img *storage.Image, existingImg *storage.Image, option FetchOption) {
...
func (e *enricherImpl) enrichWithScan(ctx context.Context, enrichmentContext EnrichmentContext,
	...
	if enrichmentContext.FetchOnlyIfScanEmpty() && image.GetScan() != nil {
		caudit.AddSuccessEvent(ctx, "Using existing scan")
	...
	for _, scanner := range scanners.GetAll() {
		...
		if err != nil {
			caudit.AddFailureEvent(ctx, "Scanning image with %q: %v", scanner.DataSource().Name, err)
		...
		if result != ScanNotDone {
			...
			caudit.AddSuccessEvent(ctx, "Scanned image with %q", scanner.DataSource().Name)
			return result, nil
		}
	}
	return ScanNotDone, errorList.ToError()
...
func (e *enricherImpl) enrichWithSignature(ctx context.Context, enrichmentContext EnrichmentContext,
	...
	// Short-circuit if no integrations are available.
	if len(sigIntegrations) == 0 {
		caudit.AddSuccessEvent(ctx, "Signature pull skipped (no signature integrations found)")
...
func (e *enricherImpl) enrichImageWithScanner(ctx context.Context, image *storage.Image, imageScanner scannerTypes.ImageScannerWithDataSource) (ScanResult, error) {
	...
	scanStartTime := time.Now()
	scan, err := scanner.GetScan(ctx, image)
