...
var (
	ScanSemaphoreQueueSize = prometheus.NewGaugeVec(prometheus.GaugeOpts{
		...
		Help:      "A gauge that shows how many scan tasks are currently waiting for the semaphores to be available.",
	}, []string{"entity"})
	...
	ScanSemaphoreLimit = prometheus.NewGaugeVec(prometheus.GaugeOpts{
		...
		Help:      "A gauge that shows the maximum number of available scan semaphores. It is configured through env and can only change on pod restart.",
	}, []string{"entity"})
)
