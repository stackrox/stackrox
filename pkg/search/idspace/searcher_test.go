...
func (s *idTransformedSearcherTestSuite) TestTransformParentToEdge() {
	...
	s.mockSearcher.EXPECT().Search(context.Background(), gomock.Any()).Return(results, nil)
	...
	transformer := WithKeyTransformations(s.mockSearcher, fakeTransform)

	transformedResults, err := transformer.Search(context.Background(), search.EmptyQuery())
	s.NoError(err)
