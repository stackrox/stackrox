...
type DirectScanConfig struct {
	// ScanDests returns scan destination pointers for a row. The order must
	// match the SELECT columns generated by the query's Selects. The same
	// slice may be returned across rows to avoid per-row allocation; values
	// are written into the pointed-to destinations before OnRow is called.
	ScanDests func() []any
