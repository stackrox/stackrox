...
func TestGloballyScopedDeleteByQuery(t *testing.T) {
	...
	assert.Equal(t, getTestReportConfig(identifier2, name2), obj2)
	...
	assert.Equal(t, getTestReportConfig(identifier4, name4), obj4)
	...
	assert.True(t, foundAfter1)
	assert.NoError(t, errAfter1)
	objAfter2, foundAfter2, errAfter2 := reportConfigStore.Get(ctx, identifier2)
	assert.Nil(t, objAfter2)
	assert.False(t, foundAfter2)
	assert.NoError(t, errAfter2)
	objAfter3, foundAfter3, errAfter3 := reportConfigStore.Get(ctx, identifier3)
	assert.Equal(t, getTestReportConfig(identifier3, name3), objAfter3)
	assert.True(t, foundAfter3)
	assert.NoError(t, errAfter3)
	objAfter4, foundAfter4, errAfter4 := reportConfigStore.Get(ctx, identifier4)
	assert.Nil(t, objAfter4)
	assert.False(t, foundAfter4)
	assert.NoError(t, errAfter4)
	objAfter5, foundAfter5, errAfter5 := reportConfigStore.Get(ctx, identifier5)
	assert.Equal(t, getTestReportConfig(identifier5, name5), objAfter5)
	assert.True(t, foundAfter5)
	assert.NoError(t, errAfter5)
}
