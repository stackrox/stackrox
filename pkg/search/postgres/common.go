...
func runQueryForSchemaWithCursorFn[T any, PT pgutils.Unmarshaler[T]](ctx context.Context, schema *walker.Schema, q *v1.Query, db postgres.DB, callback func(obj PT) error) error {
...
// RunQueryForSchemaFn creates a cursor against the database
func RunQueryForSchemaFn[T any, PT pgutils.Unmarshaler[T]](ctx context.Context, schema *walker.Schema, q *v1.Query, db postgres.DB, callback func(obj PT) error) error {
	limit := q.GetPagination().GetLimit()
	if limit == 0 || limit > cursorBatchSize {
		return runQueryForSchemaWithCursorFn(ctx, schema, q, db, callback)
	}

	return runQueryForSchemaFn(ctx, schema, q, db, callback)
}
