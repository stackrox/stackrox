...
// RowScanner scans a single pgx.Row into an object.
type RowScanner[T any] func(row pgx.Row) (*T, error)

// RowsScanner scans a single row from pgx.Rows into an object.
type RowsScanner[T any] func(rows pgx.Rows) (*T, error)

// RunGetQueryForSchemaWithScanner is like RunGetQueryForSchema but uses a custom scanner
// instead of protobuf unmarshaling. Used by NoSerializedStore for column-based reads.
...
func handleRowsWithCallbackScanner[T any](ctx context.Context, rows pgx.Rows, scanner RowsScanner[T], callback func(obj *T) error) (int64, error) {
	var count int64
	defer rows.Close()
	for rows.Next() {
		if ctx.Err() != nil {
...
// RunQueryForSchemaFnWithScanner is like RunQueryForSchemaFn but uses a custom scanner.
func RunQueryForSchemaFnWithScanner[T any](ctx context.Context, schema *walker.Schema, q *v1.Query, db postgres.Queryable, scanner RowsScanner[T], callback func(obj *T) error) error {
	...
	_, err = handleRowsWithCallbackScanner(ctx, rows, scanner, callback)
	if err != nil {
...
// RunCursorQueryForSchemaFnWithScanner is like RunCursorQueryForSchemaFn but uses a custom scanner.
func RunCursorQueryForSchemaFnWithScanner[T any](ctx context.Context, schema *walker.Schema, q *v1.Query, db postgres.DB, scanner RowsScanner[T], callback func(obj *T) error) error {
	...
	for {
		...
		rowsAffected, err := handleRowsWithCallbackScanner(ctx, rows, scanner, func(obj *T) error {
