...
func condenseList(children []*searchRequestSpec, combineQueries func(q ...*v1.Query) *v1.Query) ([]*searchRequestSpec, error) {
	...
	specQueryIndex := make(map[*walker.Schema]int)
	...
	for _, child := range children {
		...
		index, hasIndex := specQueryIndex[condensed.base.schema]
		if !hasIndex {
			index = len(queriesPerSpec)
			specQueryIndex[condensed.base.schema] = index
...
func condenseMap(specQueryIndex map[*walker.Schema]int, queriesPerSpec [][]*v1.Query, combineQueries func(q ...*v1.Query) *v1.Query) []*searchRequestSpec {
