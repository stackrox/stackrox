...
// All proto fields are individual DB columns, and reads reconstruct the proto
// from column values via custom scanner functions.
...
// ChildFetcher fetches child table data for a set of parent objects.
type ChildFetcher[T any] func(ctx context.Context, db postgres.DB, objs []*T) error

// BulkInserter queues unnest-based bulk INSERT statements into a batch.
...
func NewNoSerializedStore[T any](
	...
	if len(opts) > 0 {
		s.childFetcher = opts[0].ChildFetcher
		s.bulkInsert = opts[0].BulkInsert
	}
	return s
}
...
func (s *noSerializedStore[T]) DeleteMany(ctx context.Context, identifiers []string) error {
	...
	for identifierBatch := range slices.Chunk(identifiers, deleteBatchSize) {
...
func (s *noSerializedStore[T]) PruneMany(ctx context.Context, identifiers []string) error {
	...
	for identifierBatch := range slices.Chunk(identifiers, pruneBatchSize) {
		q := search.NewQueryBuilder().AddDocIDs(identifierBatch...).ProtoQuery()
		if err := RunDeleteRequestForSchema(ctx, s.schema, q, s.db); err != nil {
...
// defaultUpsertChecker returns a globally-scoped upsert checker for NoSerializedStore.
func defaultUpsertChecker[T any](targetResource permissions.ResourceMetadata) func(ctx context.Context, objs ...*T) error {
