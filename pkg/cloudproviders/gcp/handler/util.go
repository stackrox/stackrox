...
func CreateStorageHandlerFromConfig(ctx context.Context, conf *storage.GCSConfig) (Handler[*googleStorage.Client], error) {
...
func CreateStorageHandlerFromConfigWithManager(ctx context.Context, conf *storage.GCSConfig,
	manager auth.STSClientManager) (Handler[*googleStorage.Client], error) {
	if conf.GetUseWorkloadId() {
...
func CreateSecurityCenterHandlerFromConfig(ctx context.Context, decCreds []byte,
	wifEnabled bool) (Handler[*securitycenter.Client], error) {
	if wifEnabled {
...
func createStaticHandler[T types.GcpSDKClients](ctx context.Context, credentialBytes []byte, scope string) (Handler[T], error) {
	creds, err := google.CredentialsFromJSON(ctx, credentialBytes, scope)
	if err != nil {
		return nil, errors.Wrap(err, "failed to create credentials")
	}
	return NewHandler[T](ctx, creds)
}

func createDefaultCredsHandler[T types.GcpSDKClients](ctx context.Context, scope string) (Handler[T], error) {
