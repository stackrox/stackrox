...
func nodeScan(vr *v4.VulnerabilityReport, osImage string) *storage.NodeScan {
...
func nodeNotes(report *v4.VulnerabilityReport, osImage string) []storage.NodeScan_Note {
	notes := make([]storage.NodeScan_Note, 0, len(report.GetNotes()))
	...
	for _, note := range report.GetNotes() {
		...
		case v4.VulnerabilityReport_NOTE_UNSPECIFIED:
			notes = append(notes, storage.NodeScan_UNSET)
		default:
			// Ignore unknown/unsupported note.
		}
	}

	return filterNodeNotes(notes, osImage)
}
