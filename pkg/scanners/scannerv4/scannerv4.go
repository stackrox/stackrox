...
func (s *scannerv4) GetNodeInventoryScan(_ *storage.Node, _ *storage.NodeInventory) (*storage.NodeScan, error) {
	return nil, errors.New("not implemented for Scanner V4")
}

func (s *scannerv4) GetNodeIndexReportScan(node *storage.Node, ir *v4.IndexReport) (*storage.NodeScan, error) {
	ctx, cancel := context.WithTimeout(context.Background(), scanTimeout)
	defer cancel()
	...
	return nodeScan(vr, node.GetOsImage()), nil
}

func (s *scannerv4) TestNodeScanner() error {
	log.Debug("Scanner V4 - Returning FAKE 'success' to Test")
	return nil
}
