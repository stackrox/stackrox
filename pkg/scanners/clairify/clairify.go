...
func (c *clairify) GetNodeScan(node *storage.Node) (*storage.NodeScan, error) {
	...
	log.Infof("Request GetNodeVulnerabilities.NodeInventory for Node '%s': %+v", node.GetName(), req.GetComponents())
	ctx, cancel := context.WithTimeout(context.Background(), clientTimeout)
	defer cancel()
	resp, err := c.nodeScanServiceClient.GetNodeVulnerabilities(ctx, req)
	log.Infof("Reply GetNodeVulnerabilities.InventoryFeatures: %+v, ERROR: %v", resp.GetFeatures(), err)
