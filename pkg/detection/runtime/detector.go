...
type Detector interface {
	...
	Detect(deployment *storage.Deployment, images []*storage.Image, process *storage.ProcessIndicator, processNotInBaseline bool) ([]*storage.Alert, error)
}
...
func (d *detectorImpl) Detect(deployment *storage.Deployment, images []*storage.Image, indicator *storage.ProcessIndicator, processNotInBaseline bool) ([]*storage.Alert, error) {
	...
	err := d.policySet.ForEach(func(compiled detection.CompiledPolicy) error {
		...
		violation, err := compiled.MatchAgainstDeploymentAndProcess(&cacheReceptable, deployment, images, indicator, processNotInBaseline)
		if err != nil {
