...
func Context(ctx context.Context) []zap.Field {
	...
	if cause := context.Cause(ctx); cause != nil {
		fields = append(fields, ContextCause(cause.Error()))
	}
	if clusterID := metadata.ValueFromIncomingContext(ctx, ClusterIDContextValue); clusterID != nil {
		fields = append(fields, ClusterID(clusterID[0]))
	}
	return fields
}

// ContextCause provides the cause of a context cancellation as a structured log field.
func ContextCause(cause string) zap.Field {
	if cause == "" {
		return zap.Skip()
	}
	return zap.Any("context", map[string]string{"cause": cause})
}
