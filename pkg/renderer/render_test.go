...
func TestRenderCentralWorkerEnvNotDuplicated(t *testing.T) {
	...
	assert.Equal(t, 1, countContainerEnv(central, env.CentralWorkerEnabled.EnvVar()),
		"chart already writes ROX_CENTRAL_WORKER_ENABLED; copying it from Environment duplicates the env name")
	assert.Equal(t, 1, countContainerEnv(central, "ROX_CUSTOM_TEST_FLAG"))

	workerFile := filterNamedFile(files, "central/03-central-worker-01-deployment.yaml")
