...
func DefaultBackend(ctx context.Context, backendFactoryPool map[string]BackendFactory) ProviderOption {
	return func(pr *providerImpl) error {
		...
		backend, effectiveConfig, err := backendFactory.CreateBackend(ctx, pr.storedInfo.Id, AllUIEndpoints(&pr.storedInfo), pr.storedInfo.Config)
		if err != nil {
			return fmt.Errorf("unable to create backend for provider id: %s", pr.storedInfo.Id)
		}
		pr.backend = backend
