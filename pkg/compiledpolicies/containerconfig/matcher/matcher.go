...
type Matcher func(*storage.ContainerConfig) []*storage.Alert_Violation
...
func (f orMatcherImpl) do(config *storage.ContainerConfig) []*storage.Alert_Violation {
...
func (f andMatcherImpl) do(config *storage.ContainerConfig) []*storage.Alert_Violation {
	violations1 := f.p1(config)
	if violations1 == nil {
		return nil
	}
