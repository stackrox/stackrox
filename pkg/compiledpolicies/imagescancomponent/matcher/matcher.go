...
type Matcher func(*storage.ImageScanComponent) []*storage.Alert_Violation
...
func (f orMatcherImpl) do(isc *storage.ImageScanComponent) []*storage.Alert_Violation {
	violations1 := f.p1(isc)
	violations2 := f.p2(isc)
...
func (f andMatcherImpl) do(isc *storage.ImageScanComponent) []*storage.Alert_Violation {
	violations1 := f.p1(isc)
	if violations1 == nil {
		return nil
	}
