...
type Matcher func(*storage.SecurityContext) []*storage.Alert_Violation
...
func (f orMatcherImpl) do(sc *storage.SecurityContext) []*storage.Alert_Violation {
	violations1 := f.p1(sc)
	violations2 := f.p2(sc)
...
func (f andMatcherImpl) do(sc *storage.SecurityContext) []*storage.Alert_Violation {
	violations1 := f.p1(sc)
	if violations1 == nil {
		return nil
	}
