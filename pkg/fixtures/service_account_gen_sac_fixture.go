...
func GetSACTestStorageServiceAccountSet(t *testing.T, scopedStorageServiceAccountCreator func(t *testing.T, id string, clusterID string, namespace string) *storage.ServiceAccount) []*storage.ServiceAccount {
	...
	for _, cluster := range clusters {
		for _, namespace := range namespaces {
			for i := 0; i < numberOfAccounts; i++ {
				storageServiceAccounts = append(storageServiceAccounts, scopedStorageServiceAccountCreator(t, uuid.NewV4().String(), cluster, namespace))
			}
		}
	}
