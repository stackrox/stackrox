...
// ConvertBindingsToSimpleAccessScope converts ClusterBinding entries from
// an ACM UserPermission object to a *storage.SimpleAccessScope
//
...
//
// Note: In ACM, the Cluster field in ClusterBinding contains the managed
// cluster name, which corresponds to a namespace name on the hub cluster.
	...
	for _, binding := range bindings {
		...
		case clusterviewv1alpha1.BindingScopeNamespace:
			...
			for _, ns := range binding.Namespaces {
				// Convert wildcard namespace to full cluster access
				if ns == "*" {
					clusterSet.Add(binding.Cluster)
					break
				}
