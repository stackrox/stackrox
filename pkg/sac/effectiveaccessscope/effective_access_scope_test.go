...
func TestComputeEffectiveClusterAccessScope(t *testing.T) {
	inputClusters := make([]Cluster, 0, len(clusters))
	for _, c := range clusters {
		inputClusters = append(inputClusters, c)
	}
	...
	for name, tc := range map[string]struct {
		...
		"nil rules excludes all clusters": {
			rules: nil,
			expected: &ScopeTree{
				State:           Excluded,
				clusterIDToName: clusterIDs,
		...
		t.Run(name, func(t *testing.T) {
			result, err := ComputeEffectiveClusterAccessScope(tc.rules, inputClusters, v1.ComputeEffectiveAccessScopeRequest_HIGH)
			assert.NoError(t, err)
