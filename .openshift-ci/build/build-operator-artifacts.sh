...
build_operator_bundle_and_binary() {
    ...
    mkdir -p "$(make --quiet default-image-registry)"
    cp -a build/bundle "$(make --quiet default-image-registry)/stackrox-operator-bundle:v$(make --quiet --no-print-directory -C operator tag)"
