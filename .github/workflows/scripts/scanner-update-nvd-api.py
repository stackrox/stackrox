    ...
    def start(dirpath):
        try:
            for year in range(2002, datetime.now().year + 1):
                ...
                for start, end in ApiLoader.get_dates(year, 3):  # fetch quarterly
                    vulnerabilities.extend(ApiLoader.fetch_data(start, end))

                file_path = os.path.join(dirpath, f"{year}.nvd.json")
                with open(file_path, "w") as file:
                    for start, end in ApiLoader.get_dates(year, 3):  # fetch quarterly
                        for vulnerability in ApiLoader.fetch_data(start, end):
