...
#
# Usage (locally):
# bash local-env.sh check-scanner-updater-version <version>
...
if ! grep -Fqx "$VERSION" <<<"$SCANNER_VERSION"; then
    gh_log error "Release version $VERSION (inferred from the tag '$TAG') is not present in $SCANNER_VERSION_PATH in master branch"
