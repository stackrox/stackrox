...
class TestRailconfig {
    ...
    private static Map<String, Integer> casesToAdd = [:]
    ...
    static void setProjectSectionId(String projectName, String sectionName) {
        try {
            ...
            if (pid != 0) {
                java.util.List sectionList = testRail.sections().list(pid).execute()
...
// ***** Create Run Function *********
        ...
        try {
            ...
            for (Case c : cases) {
                if (c.sectionId == sectionId) {
                    casesToAdd.put(c.title, c.id)
                }
            }
            List<Integer> caseIds = casesToAdd.values().asList()
            run = new Run()
    ...
    static void addResult(String comment, int caseId) {
        try {
            if (null != testRail()) {
                ...
                testRail.results().addForCase(getRun().getId(), caseId, new Result().setComment(comment), customResultFields).execute()
            }
        }
        catch (Exception e) {
            e.printStackTrace()
    ...
    static void addStatusForCase(int statusId, int caseId){
        ...
        try{
            List customResultFields = testRail.resultFields().list().execute()
            testRail.results().addForCase(getRun().getId(), caseId, new Result().setStatusId(statusId), customResultFields).execute()
        }
        catch(Exception e){
            e.printStackTrace()
        }
