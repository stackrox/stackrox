...
class ImageScanningTest extends BaseSpecification {
    ...
    def "Verify Image Scanner Integrations: #testName"() {
        given:
        ...
        if (IMAGE_PULL_SECRETS.containsKey(integration)) {
            ...
            autogeneratedId = autoGenerated.id
        }
        ...
        "quay-keep-autogenerated"       | "quay"      |
                [{ ImageIntegrationService.addDefaultQuayRegistry() },]                                           |
                165        | 184       | 28
        ...
        "quay-fully-qualified-endpoint" | "quay"      |
                [{ ImageIntegrationService.addDefaultQuayRegistry("quay-fully-qualified", "https://quay.io/") },] |
                165        | 184       | 28

        "quay-insecure"                 | "quay"      |
                [{ ImageIntegrationService.addDefaultQuayRegistry("quay-insecure", "quay.io", true, true) },]     |
                165        | 184       | 28

        "quay-duplicate"                | "quay"      |
                [{ ImageIntegrationService.addDefaultQuayRegistry() },
                 { ImageIntegrationService.addDuplicateQuayRegistry() }]                                          |
                165        | 184       | 28

        "quay-dupe-invalid"             | "quay"      |
                [{ ImageIntegrationService.addDefaultQuayRegistry() },
                 { ImageIntegrationService.addSecondaryQuayRegistry() }]                                          |
                165        | 184       | 28

        "quay-and-other"             | "quay"      |
                [{ ImageIntegrationService.addGcrRegistry() },
                 { ImageIntegrationService.addDefaultQuayRegistry() }]                                          |
                165        | 184       | 28
        ...
        "gcr-fully-qualified-endpoint"  | "gcr"       |
                [{ ImageIntegrationService.addGcrRegistry("gcr-fully-qualified", "https://us.gcr.io/") },]        |
                44         | 204       | 49

        "gcr-duplicate"                 | "gcr"       |
                [{ ImageIntegrationService.addGcrRegistry() },
                 { ImageIntegrationService.addGcrRegistry("gcr-duplicate") },]                                    |
                44         | 204       | 49

        "gcr-dupe-invalid"              | "gcr"       |
                [{ ImageIntegrationService.addGcrRegistry() },
                 { ImageIntegrationService.addGcrNoAccessRegistry() },]                                           |
                44         | 204       | 49
    ...
    def "Verify Image Scan Results - #scanner.name() - #component:#version - #image - #cve - #layerIdx"() {
        ...
        when:
        "Add scanner"
        def integrationId = ImageIntegrationService.createImageIntegration(
                scanner.getBuilder(scanner.getScannerConfig()).build()
        )
    ...
    def "Verify image scan exceptions - #scanner.name() - #testAspect"() {
        ...
        and:
        "Scan image"
        def image = IMAGES_FOR_ERROR_TESTS[scanner.name()][testAspect]
        assert image
