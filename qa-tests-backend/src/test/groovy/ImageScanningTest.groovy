...
class ImageScanningTest extends BaseSpecification {
    ...
    @Tag("BAT")
    @Tag("Integration")
    // GCR doesn't have MA images to verify the GCR-image-integrations
...
//        new StackroxScannerIntegration() | /status=401/ | "no access"
    ...
    @Tag("BAT")
    @Tag("Integration")
    // ECR, ACR, GCR throwing invalid argument / auth issues during tests
    // ecr  : INVALID_ARGUMENT: registry integration: MissingEndpoint: 'Endpoint' configuration is required for this service
    // ecr  : tcp: lookup ecr."us-east-2".amazonaws.com: no such host
    // acr  : "stackroxci.azurecr.io/stackroxci/registry-image:0.3", reason=ImagePullBackOff
    // acr  : unauthorized: authentication required
    // acr  : http: non-successful response (status=401 body="{\"errors\":[{\"code\":\"UNAUTHORIZED\",\"message\":\"authentication required
    // quay : imageDetail.metadata?.v1?.layersCount >= 1
    // gcr  : "us.gcr.io/stackrox-ci/qa/registry-image:0.3", reason=ImagePullBackOff
    //@IgnoreIf({ Env.REMOTE_CLUSTER_ARCH == "ppc64le" || Env.REMOTE_CLUSTER_ARCH == "s390x" })
