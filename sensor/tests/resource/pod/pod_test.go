...
func (s *PodHierarchySuite) Test_ContainerSpecOnDeployment() {
	s.testContext.RunWithResources([]resource.YamlTestFile{
		...
		resource.NginxServiceAccount,
	}, func(t *testing.T, testC *resource.TestContext, objects map[string]k8s.Object) {
		// wait until pods are created
		err := wait.For(conditions.New(testC.Resources()).ResourceMatch(objects[resource.NginxDeployment.File], func(object k8s.Object) bool {
		...
		deployment := resource.GetLastMessageWithDeploymentName(messages, "nginx-deployment").
			GetEvent().
			GetDeployment()
		...
		uniquePodNames := resource.GetUniquePodNamesFromPrefix(messages, "nginx-")
...
func (s *PodHierarchySuite) Test_ParentlessPodsAreTreatedAsDeployments() {
	s.testContext.RunWithResources([]resource.YamlTestFile{
		...
		uniqueDeployments := resource.GetUniqueDeploymentNames(messages)
		...
		deployment := resource.GetLastMessageWithDeploymentName(messages, "nginx-rogue").
			GetEvent().
			GetDeployment()
