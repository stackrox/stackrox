...
func Test_SensorReconnects(t *testing.T) {
	...
	c.RunTest(helper.WithTestCase(func(t *testing.T, testContext *helper.TestContext, _ map[string]k8s.Object) {
		...
		// is up to Central.
		_, err = testContext.ApplyDeploymentAndWait(context.Background(), NginxDeployment1)
		require.NoError(t, err)
		...
		_, err = testContext.ApplyDeploymentAndWait(context.Background(), NginxDeployment2)
		require.NoError(t, err)
	}))
}
