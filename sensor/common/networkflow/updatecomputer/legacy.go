...
func (l *Legacy) ComputeUpdatedConns(current map[indicator.NetworkConn]timestamp.MicroTS) ([]*storage.NetworkFlow, error) {
...
func (l *Legacy) RecordSizeMetrics(name string, gv *prometheus.GaugeVec) {
	...
	connsSize := 480 * lenConn // calculated with Cursor
	epSize := 330 * lenEp      // calculated with Cursor
	gv.WithLabelValues("legacy", "lastSent", "conns").Set(float64(connsSize))
	gv.WithLabelValues("legacy", "lastSent", "endpoints").Set(float64(epSize))
}
