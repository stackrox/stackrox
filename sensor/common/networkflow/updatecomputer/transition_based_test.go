...
func TestTransitionBasedComputeUpdatedProcesses(t *testing.T) {
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			...
			initialProcMap := map[indicator.ContainerEndpoint]*indicator.ProcessListening{}
			l.ComputeUpdatedEndpointsAndProcesses(initialEp, initialProc, initialProcMap)
			l.OnSuccessfulSend(nil, initialEp, initialProc)

			var gotProc []*storage.ProcessListeningOnPortFromSensor

			// Apply all updates except the last one in offline mode
			for _, upd := range tc.updates {
				epMap := map[indicator.ContainerEndpoint]timestamp.MicroTS{}
				procMap := map[indicator.ProcessListening]timestamp.MicroTS{}
				procEpMap := map[indicator.ContainerEndpoint]*indicator.ProcessListening{}

				if upd.p != nil {
					epMap[ep] = upd.ts
					procMap[*upd.p] = upd.ts
					procEpMap[ep] = upd.p
				}
				_, updatedProc := l.ComputeUpdatedEndpointsAndProcesses(epMap, procMap, procEpMap)
				gotProc = append(gotProc, updatedProc...)
				l.OnSuccessfulSend(nil, epMap, procMap)
			}

			assert.Len(t, gotProc, tc.expectNumUpdates)

			// Empty update to ensure that any caches for offline mode are cleared
			uEp, uProc := l.ComputeUpdatedEndpointsAndProcesses(
				map[indicator.ContainerEndpoint]timestamp.MicroTS{},
				map[indicator.ProcessListening]timestamp.MicroTS{},
				map[indicator.ContainerEndpoint]*indicator.ProcessListening{})
			l.OnSuccessfulSend(nil, map[indicator.ContainerEndpoint]timestamp.MicroTS{}, map[indicator.ProcessListening]timestamp.MicroTS{})
			assert.Len(t, uEp, 0)
