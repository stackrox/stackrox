...
var (
	UpdateEvents = prometheus.NewCounterVec(prometheus.CounterOpts{
		...
		Help: "Counts the internal update events for the shallUpdate method in Categorized updateComputer. " +
	...
	}, []string{"transition", "entity", "action"})
	UpdateEventsGauge = prometheus.NewGaugeVec(prometheus.GaugeOpts{
		Namespace: metrics.PrometheusNamespace,
		Subsystem: metrics.SensorSubsystem.String(),
		Name:      "update_computer_update_events_current",
		Help: "Counts the internal update events for the shallUpdate method in Categorized updateComputer in a single tick. " +
