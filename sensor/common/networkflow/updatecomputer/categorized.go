...
func (c *Categorized) ComputeUpdatedEndpoints(current map[indicator.ContainerEndpoint]timestamp.MicroTS) []*storage.NetworkEndpoint {
	...
	for ep, currTS := range current {
		...
		// Look up previous timestamp - use infinity for open connections, or actual value for recently closed ones
		prevTsFound, prevTS := c.lookupPrevTimestamp(epKey)
		// Based on the categorization, apply direct action. Execute expensive checks only if necessary.
		_ = shallUpdate(prevTS, currTS, prevTsFound, epHashedKey,
			EndpointHashEnrichedEntity, c.openTracker, &c.openTrackerMutex)

		if shallUpdate(prevTS, currTS, prevTsFound, epKey,
...
func (c *Categorized) ComputeUpdatedProcesses(current map[indicator.ProcessListening]timestamp.MicroTS) []*storage.ProcessListeningOnPortFromSensor {
	...
	for proc, currTS := range current {
		...
		// Look up previous timestamp - use infinity for open connections, or actual value for recently closed ones
		prevTsFound, prevTS := c.lookupPrevTimestamp(procKey)
		// Based on the categorization, apply direct action. Execute expensive checks only if necessary.
		if shallUpdate(prevTS, currTS, prevTsFound, procKey,
			ProcessEnrichedEntity, c.openTracker, &c.openTrackerMutex) {
...
func (c *Categorized) GetStateMetrics() map[string]map[string]int {
	...
	data["open"] = make(map[string]int)
	for _, entity := range allEntities {
		data["open"][string(entity)] = concurrency.WithRLock1(&c.openTrackerMutex, func() int {
...
func (c *Categorized) RecordSizeMetrics(name string, gv *prometheus.GaugeVec) {
	// Calculate byte metrics
	// TODO: Remove this code before merging
	for _, entity := range allEntities {
		...
		gv.WithLabelValues(name, "open", string(entity)).Set(float64(c.openTrackerEstBytes[entity]))
	}
}
