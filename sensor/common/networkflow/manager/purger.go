	...
	for {
		...
		case <-tickerC:
			...
			concurrency.WithLock(&m.connectionsByHostMutex, func() {
				log.Debug("Purging hostConn endpoints")
				purgeHostConnsNoLock(maxAge, m.connectionsByHost, m.clusterEntities)
			})
...
func purgeHostConnsNoLock(maxAge time.Duration,
	connectionsByHost map[string]*hostConnections,
	store EntityStore) {
	for _, conns := range connectionsByHost {
