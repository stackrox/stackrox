...
func TestEnrichContainerEndpoint_EdgeCases(t *testing.T) {
	...
	tests := map[string]struct {
		...
		validateEnrichment func(*testing.T,
		...
		"Endpoint with duplicate timestamp should be marked as duplicate with result EnrichmentResultSuccess for both Network Graph and PLOP": {
			...
			validateEnrichment: func(t *testing.T, enrichedEndpoints map[*indicator.ContainerEndpoint]timestamp.MicroTS, processesListening map[*indicator.ProcessListening]timestamp.MicroTS) {
