...
func authFuncOverride(ctx context.Context, fullMethodName string) (context.Context, error) {
	...
	return ctx, errors.Wrapf(err, "authorization failed for networkflow method %q", fullMethodName)
}
...
func (s *serviceImpl) receiveMessages(stream sensor.NetworkConnectionInfoService_PushNetworkConnectionInfoServer) error {
	...
	for {
		...
		case <-stream.Context().Done():
			return errors.Wrap(stream.Context().Err(), "networkflow receiveMessages context done")
