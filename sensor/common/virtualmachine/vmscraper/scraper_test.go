...
func TestVMScraper_TrustsUnchangedWhenEpochZero(t *testing.T) {
	...
	require.Len(t, client.calls, 1, "epoch 0 Unchanged is trusted in a single dial")
	assert.Len(t, sender.sent, 1, "should not forward unchanged report")
}
...
func TestVMScraper_PrunesStaleState(t *testing.T) {
	...
	assert.False(t, s.IsActivelyScraped("ns1/vm-a"), "vm-a should no longer be active")
	assert.True(t, s.IsActivelyScraped("ns2/vm-b"))
}
