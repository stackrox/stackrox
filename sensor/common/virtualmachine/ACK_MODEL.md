...
A short tick (`ROX_VIRTUAL_MACHINES_SCRAPER_TICK_INTERVAL`, default 10s) walks the per-VM schedule. Each VM is due at its own `nextAttemptAt`. Success and permanent failures reschedule at pollInterval + U(0, W), where W is `ROX_VIRTUAL_MACHINES_SCRAPER_STEADY_SPREAD_FRACTION` times the poll interval (default 2/3). A tick starts at most `startBudget` of the due set (`ceil(nTracked × tick / C)`, at least 1), then capped by concurrency. `nTracked` is the fleet in `vmState`, not the leftover due pile, so the drain rate stays stable as due VMs are started. Retryable pull failures and NACKs share one exponential backoff (`ROX_VIRTUAL_MACHINES_SCRAPER_INITIAL_BACKOFF`, default 10s, doubled each time, capped at `min(poll interval, 30m)`).
