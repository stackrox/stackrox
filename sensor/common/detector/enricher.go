...
func scanImage(ctx context.Context, svc v1.ImageServiceClient, req *scanImageRequest, _ *scan.LocalScan) (*v1.ScanImageInternalResponse, error) {
	...
	internalReq.SetImage(req.containerImage)
	if features.SourcedAutogeneratedIntegrations.Enabled() {
		ss := &v1.ScanImageInternalRequest_Source{}
		ss.SetClusterId(req.clusterID)
		ss.SetNamespace(req.namespace)
		ss.SetImagePullSecrets(req.pullSecrets)
		internalReq.SetSource(ss)
	}
...
func scanImageLocal(ctx context.Context, svc v1.ImageServiceClient, req *scanImageRequest, localScan *scan.LocalScan) (*v1.ScanImageInternalResponse, error) {
	...
	resp := &v1.ScanImageInternalResponse{}
	resp.SetImage(img)
	if err != nil {
...
func (c *cacheValue) updateImageNoLock(image *storage.Image) {
	...
	c.image.SetNames(protoutils.SliceUnique(append(c.image.GetNames(), existingNames...)))
}
...
func (e *enricher) getImages(ctx context.Context, deployment *storage.Deployment) []*storage.Image {
	...
	for i := 0; i < len(deployment.GetContainers()); i++ {
		...
		// exact same ID
		image.SetName(deployment.GetContainers()[imgResult.containerIdx].GetImage().GetName())
