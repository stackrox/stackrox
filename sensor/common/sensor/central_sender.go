...
type CentralSender interface {
	Start(stream central.SensorService_CommunicateClient, sendUnchangedIDs bool, initialDeduperState map[deduperkey.Key]uint64, onSyncDoneCallback func(), onStops ...func(error))
	Stop(err error)
