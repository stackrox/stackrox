...
func NewSensor(
	clusterIDSetter clusterIDHandler,
	configHandler config.Handler,
	detector detector.Detector,
	imageService image.Service,
	centralConnectionFactory centralclient.CentralConnectionFactory,
...
func (s *Sensor) Start() {
	...
	go s.centralConnectionFactory.SetCentralConnectionWithRetries(s.clusterIDHandler, s.centralConnection, centralclient.StaticCertLoader(centralCertificates))
...
func (s *Sensor) communicationWithCentral(centralReachable *concurrency.Flag) {
	s.centralCommunication = NewCentralCommunication(s.clusterIDHandler, false, false, s.components...)
...
func (s *Sensor) communicationWithCentralWithRetries(centralReachable *concurrency.Flag) {
	...
	err := backoff.RetryNotify(func() error {
		...
		case <-s.centralConnectionFactory.StopSignal().WaitC():
			...
			go s.centralConnectionFactory.SetCentralConnectionWithRetries(s.clusterIDHandler, s.centralConnection, s.certLoader)
			return err
		}
		...
		// suddenly broke.
		centralCommunication := NewCentralCommunication(s.clusterIDHandler, s.reconnect.Load(), s.reconcile.Load(), s.components...)
		syncDone := concurrency.NewSignal()
