...
func NewProcessPipeline(indicators chan *message.ExpiringMessage, clusterEntities *clusterentities.Store, processFilter filter.Filter, detector detector.Detector) *Pipeline {
	...
	msgCtx, cancelMsgCtx := context.WithCancelCause(status.DefaultContext())
	enricherCtx, cancelEnricherCtx := context.WithCancelCause(status.DefaultContext())
...
func (p *Pipeline) createNewContext() {
	...
	p.msgCtx, p.msgCtxCancel = context.WithCancelCause(status.DefaultContext())
}
