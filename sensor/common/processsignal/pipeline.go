...
func NewProcessPipeline(indicators chan *message.ExpiringMessage, clusterEntities *clusterentities.Store, processFilter filter.Filter, detector detector.Detector) *Pipeline {
	...
	msgCtx, cancelMsgCtx := context.WithCancelCause(trace.DefaultContext())
	enricherCtx, cancelEnricherCtx := context.WithCancelCause(trace.DefaultContext())
...
func (p *Pipeline) createNewContext() {
	...
	p.msgCtx, p.msgCtxCancel = context.WithCancelCause(trace.DefaultContext())
}
