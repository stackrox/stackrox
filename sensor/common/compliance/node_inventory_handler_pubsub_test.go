...
// TestConcurrentPubSubDeliverySerializedThroughLane publishes interleaved NodeInventoryEvent and
...
// goroutine -- the same guarantee the legacy single-goroutine select loop provided.
// Run with -race to catch a regression if that guarantee is ever broken.
func (s *NodeInventoryHandlerTestSuite) TestConcurrentPubSubDeliverySerializedThroughLane() {
	t := s.T()
	t.Setenv(features.SensorInternalPubSub.EnvVar(), "true")
