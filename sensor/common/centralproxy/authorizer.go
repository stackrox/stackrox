...
// headerGetter wraps http.Header to satisfy the getter interface (Get returns []string).
type headerGetter http.Header

func (h headerGetter) Get(key string) []string { return http.Header(h).Values(key) }

func extractBearerToken(r *http.Request) (string, error) {
	token := authn.ExtractToken(headerGetter(r.Header), "Bearer")
	if token == "" {
