...
func (f *centralConnectionFactoryImpl) pingCentral() error {
	ctx, cancel := context.WithTimeout(status.DefaultContext(), 10*time.Second)
	defer cancel()
...
func (f *centralConnectionFactoryImpl) SetCentralConnectionWithRetries(conn *util.LazyClientConn, certLoader CertLoader) {
	...
	centralConnection, err := clientconn.AuthenticatedGRPCConnection(status.DefaultContext(), env.CentralEndpoint.Setting(), mtls.CentralSubject, opts...)
	if err != nil {
