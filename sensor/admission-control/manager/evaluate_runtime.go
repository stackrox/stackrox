...
func (m *manager) evaluatePodEvent(s *state, req *admission.AdmissionRequest, event *storage.KubernetesEvent) ([]*storage.Alert, bool, error) {
	...
	if deployment != nil {
		...
		if len(s.deployFieldK8sDetector.PolicySet().GetCompiledPolicies()) == 0 {
			alerts, err := s.eventOnlyK8sDetector.DetectForDeploymentAndKubeEvent(
		...
		getAlertsFunc := func(dep *storage.Deployment, imgs []*storage.Image) ([]*storage.Alert, error) {
			...
			return s.allK8sEventDetector.DetectForDeploymentAndKubeEvent(enhancedDeployment, event)
		}
	...
	alerts, err := s.eventOnlyK8sDetector.DetectForDeploymentAndKubeEvent(booleanpolicy.EnhancedDeployment{}, event)
	if err != nil {
...
func (m *manager) waitForDeploymentAndDetect(s *state, event *storage.KubernetesEvent) {
	...
	case <-m.initialSyncSig.Done():
		...
		getAlertsFunc := func(dep *storage.Deployment, imgs []*storage.Image) ([]*storage.Alert, error) {
			...
			return s.deployFieldK8sDetector.DetectForDeploymentAndKubeEvent(enhancedDeployment, event)
		}
