...
var (
	alternativeCAPathSetting = env.RegisterSetting("ROX_ALTERNATIVE_CA_FILE",
		env.WithDefault("/run/secrets/stackrox.io/ca/ca.pem"))
)
...
func configureCA() error {
	...
	if exists, err := fileutils.Exists(alternativeCAPathSetting.Setting()); err != nil {
	...
	if err := utils.ShouldErr(os.Setenv(mtls.CAFileEnvName, alternativeCAPathSetting.Setting())); err != nil {
