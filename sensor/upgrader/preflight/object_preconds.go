...
func (objectPreconditionsCheck) Check(ctx *upgradectx.UpgradeContext, execPlan *plan.ExecutionPlan, reporter checkReporter) error {
	...
	for gvk, acts := range groupedActions {
		...
		for _, act := range acts {
			resClient, err := ctx.DynamicClientForResource(res, act.ObjectRef.Namespace)
			if err != nil {
				return errors.Wrapf(err, "could not retrieve dynamic client for resource type %v", gvk)
			}
