...
func CreateSensor(cfg *CreateOptions) (*sensor.Sensor, error) {
	...
	components := []common.SensorComponent{
		...
		clusterstatus.NewUpdater(cfg.k8sClient, internalMessageDispatcher),
		clusterhealth.NewUpdater(cfg.k8sClient.Kubernetes(), 0, internalMessageDispatcher),
		clustermetrics.New(clusterID, cfg.k8sClient.Kubernetes(), internalMessageDispatcher),
		complianceCommandHandler,
		processSignals,
		telemetry.NewCommandHandler(cfg.k8sClient.Kubernetes(), storeProvider, internalMessageDispatcher),
