...
const (
	...
	defaultUpdateRetryTimeout = 2 * time.Minute
)
...
type handlerImpl struct {
	...
	handlerUpdateRetryTimeout time.Duration
}
...
func NewRequestHandler(client dynamic.Interface, complianceOperatorInfo StatusInfo, coIsReady *concurrency.Signal) common.SensorComponent {
	return &handlerImpl{
		...
		handlerUpdateRetryTimeout: defaultUpdateRetryTimeout,
	}
}
	...
	for _, scan := range complianceSuite.Spec.Scans {
		if err := func() error {
			...
			complianceScan, scanObj, err := m.getScanForReRun(func() (*unstructured.Unstructured, error) {
				err := m.callWithRetryWithContext(ctx, func(ctx context.Context) error {
					var err error
...
func (m *handlerImpl) reconcileCreateOrUpdateResource(
	...
	if resource, isInCluster := inClusterResources[namespaceNameIndex]; isInCluster {
		...
		ctx, cancel := context.WithTimeout(m.ctx(), m.handlerUpdateRetryTimeout)
		defer cancel()
		err = m.callWithRetryWithOnErrorCallback(
			ctx,
			func(ctx context.Context) error {
...
func (m *handlerImpl) callWithRetryWithContext(ctx context.Context, fn func(context.Context) error) error {
	return m.callWithRetryWithOnErrorCallback(ctx, fn, nil)
}
