...
func newCertificatesRefresher(certsDescription string, requestCertificates requestCertificatesFunc, repository certrepo.ServiceCertificatesRepo,
	timeout time.Duration, backoff wait.Backoff, dynClient dynamic.Interface) concurrency.RetryTicker {

	return concurrency.NewRetryTicker(func(ctx context.Context) (timeToNextTick time.Duration, err error) {
		return refreshCertificates(ctx, certsDescription, requestCertificates, GetCertsRenewalTime, repository, dynClient)
	}, timeout, backoff)
}
...
func refreshCertificates(ctx context.Context, certsDescription string, requestCertificates requestCertificatesFunc,
	getCertsRenewalTime getCertsRenewalTimeFunc, repository certrepo.ServiceCertificatesRepo, dynClient dynamic.Interface) (timeToNextRefresh time.Duration, err error) {

	timeToNextRefresh, err = ensureCertificatesAreFresh(ctx, certsDescription, requestCertificates, getCertsRenewalTime, repository, dynClient)
	if err != nil {
...
func ensureCertificatesAreFresh(ctx context.Context, certsDescription string, requestCertificates requestCertificatesFunc,
	getCertsRenewalTime getCertsRenewalTimeFunc, repository certrepo.ServiceCertificatesRepo, dynClient dynamic.Interface) (time.Duration, error) {
	...
	if len(certificates.GetCaBundlePem()) > 0 {
		...
		if err := updateCABundleConfigMap(ctx, certificates.GetCaBundlePem(), dynClient); err != nil {
...
func updateCABundleConfigMap(ctx context.Context, caBundlePem []byte, dynClient dynamic.Interface) error {
	namespace := pods.GetPodNamespace()
	podName := os.Getenv("POD_NAME")

	ownerRef, err := FetchSensorDeploymentOwnerRef(ctx, podName, namespace, dynClient, wait.Backoff{})
	if err != nil {
		log.Warnf("Failed to fetch sensor deployment owner reference for CA bundle update: %v", err)
		ownerRef = nil
	}

	return CreateTLSCABundleConfigMapFromPEM(ctx, caBundlePem, dynClient, namespace, ownerRef)
}
