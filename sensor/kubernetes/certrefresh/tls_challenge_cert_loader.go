...
func TLSChallengeCertLoader(centralClient *centralclient.Client, dynClient dynamic.Interface) centralclient.CertLoader {
	return func() []*x509.Certificate {
		...
		if err != nil {
			...
			handleCABundleConfigMapUpdate(ctx, centralCAs, dynClient)
...
func handleCABundleConfigMapUpdate(ctx context.Context, centralCAs []*x509.Certificate, dynClient dynamic.Interface) {
	namespace := pods.GetPodNamespace()
	podName := os.Getenv("POD_NAME")
	ownerRef, err := FetchSensorDeploymentOwnerRef(ctx, podName, namespace,
		dynClient, wait.Backoff{})
	if err != nil {
	...
	if err := CreateTLSCABundleConfigMapFromCerts(ctx, centralCAs,
		dynClient, namespace, ownerRef); err != nil {
