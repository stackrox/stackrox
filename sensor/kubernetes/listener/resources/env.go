...
var (
	...
	// The default value of 5 results in between 2 to 2.5 minutes (4-5 * 30s) - the precision of 1 tick is necessary,
	// as we have no influence in which part of the 30s window a deletion event from K8s informer arrives.
	// A higher memory window (for example ~10 minutes) may be necessary if Collector is used with
	// ROX_ENABLE_AFTERGLOW=true and its default settings.
	...
	pastClusterEntitiesMemorySize = env.RegisterIntegerSetting("ROX_PAST_CLUSTER_ENTITIES_MEMORY_SIZE", 5)
