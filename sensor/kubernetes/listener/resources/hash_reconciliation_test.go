...
func (s *HashReconciliationSuite) TestProcessHashes() {
	// ImageIntegrations need this feature flag enabled
	s.T().Setenv(features.SourcedAutogeneratedIntegrations.EnvVar(), "true")
	cases := map[string]struct {
	...
	for n, c := range cases {
		s.Run(n, func() {
			// We need to skip image integrations if this is not enabled.
			// This feature flag cannot be set in release builds.
			if !features.SourcedAutogeneratedIntegrations.Enabled() {
				for k := range c.dstate {
					if strings.Contains(k.ResourceType.String(), deduper.TypeImageIntegration.String()) {
						s.T().Skipf("%s should be enabled to test %s", features.SourcedAutogeneratedIntegrations.EnvVar(), deduper.TypeImageIntegration.String())
					}
				}
			}
