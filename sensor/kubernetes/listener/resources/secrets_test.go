...
func TestSkipIntegrationCreate(t *testing.T) {
	...
	// No integration creations should be skipped for these test cases.
	noSkipTcs := []struct {
		sourcedAutogenEnabled bool
		globalPullEnabled     bool
	}{
		{},
	}
	for i, tc := range noSkipTcs {
		t.Run(fmt.Sprint(i), func(t *testing.T) {
			testutils.MustUpdateFeature(t, features.SourcedAutogeneratedIntegrations, tc.sourcedAutogenEnabled)
			t.Setenv(env.AutogenerateGlobalPullSecRegistries.EnvVar(), strconv.FormatBool(tc.globalPullEnabled))

			assert.False(t, skipIntegrationCreate(globalPullSecret))
			assert.False(t, skipIntegrationCreate(globalPullSecretAlt))
		})
	}

	// The global pull secret dupe should be skipped if only creating sourced integrations
	// for the global pull secret.
	t.Run("skip when sourced feature disabled and global pull feature enabled", func(t *testing.T) {
		...
		assert.False(t, skipIntegrationCreate(globalPullSecret))
		assert.True(t, skipIntegrationCreate(globalPullSecretAlt))
	})
}
