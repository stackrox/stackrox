...
func TestConvertDifferentContainerNumbers(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			actual := newDeploymentEventFromResource(c.inputObj, &c.action, c.deploymentType, testClusterID, c.podLister, getMockNamespaceStore(t), hierarchyFromPodLister(c.podLister), "", c.systemNamespaces, c.registryStore).GetDeployment()
			if actual != nil {
