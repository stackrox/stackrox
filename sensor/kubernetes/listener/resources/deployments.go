...
func (d *deploymentHandler) getImageIntegrationEvent(registry string) *central.SensorEvent {
	...
	// type = "ecr".
	return central.SensorEvent_builder{
		...
		ImageIntegration: storage.ImageIntegration_builder{
			...
			Ecr: storage.ECRConfig_builder{
				...
				AuthorizationData: storage.ECRConfig_AuthorizationData_builder{
					ExpiresAt: expiresAt,
...
func (d *deploymentHandler) processPodEvent(owningDeploymentID string, k8sPod *v1.Pod, action central.ResourceAction) *central.SensorEvent {
	...
	if action == central.ResourceAction_REMOVE_RESOURCE {
		...
		pod := &storage.Pod{}
		pod.SetId(uid)
		pod.SetName(k8sPod.GetName())
		pod.SetDeploymentId(owningDeploymentID)
		pod.SetNamespace(k8sPod.GetNamespace())
		event := &central.SensorEvent{}
		event.SetId(uid)
		event.SetAction(action)
		event.SetPod(proto.ValueOrDefault(pod))
		return event
	}
	...
	p := &storage.Pod{}
	p.SetId(uid)
	p.SetName(k8sPod.GetName())
	p.SetDeploymentId(owningDeploymentID)
	p.SetClusterId(d.clusterID)
	p.SetNamespace(k8sPod.Namespace)
	p.SetStarted(started)

	// Assume we only receive one status per live container, so we can blindly append.
	p.SetLiveInstances(containerInstances(k8sPod))
	...
	event.SetId(p.GetId())
	event.SetAction(action)
	event.SetPod(proto.ValueOrDefault(p))
	return event
}
