...
func (p *InMemoryStoreProvider) Reconcile(resType, resID string, resHash uint64) (*reconcile.SensorReconciliationEvent, error) {
	for _, r := range p.reconcilableStores {
		done, err := r.Reconcile(resType, resID, resHash)
		if done {
			return done, err
		}
	}
	return false, ErrUnableToReconcile
}
