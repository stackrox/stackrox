...
func TestBenchmarkFixturesIncludeInitContainers(t *testing.T) {
	t.Setenv(features.InitContainerSupport.EnvVar(), "true")

	containers := makeBenchmarkContainers(2)
	pods := makeBenchmarkPods(2, 1)

	require.Len(t, containers, 3)
	assert.Equal(t, storage.ContainerType_INIT, containers[0].GetType())
	require.Len(t, pods, 1)
	assert.Len(t, pods[0].Spec.InitContainers, 1)
	assert.Len(t, pods[0].Status.InitContainerStatuses, 1)
}
