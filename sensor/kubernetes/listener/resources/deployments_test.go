	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.want == nil {
				...
				assert.True(t, i.GetAutogenerated())
			}
		})
	}
}

func Test_isOwnerBeingDeleted(t *testing.T) {
	now := metaV1.Now()
	...
	tests := map[string]struct {
		...
		"Deployment with DeletionTimestamp is being deleted": {
			original: &appsV1.Deployment{
				ObjectMeta: metaV1.ObjectMeta{DeletionTimestamp: &now},
			},
			expected: true,
		},
		"Deployment without DeletionTimestamp is not being deleted": {
			original: &appsV1.Deployment{
				ObjectMeta: metaV1.ObjectMeta{Name: "my-deploy"},
			},
			expected: false,
		},
		"DaemonSet with DeletionTimestamp is being deleted": {
			original: &appsV1.DaemonSet{
				ObjectMeta: metaV1.ObjectMeta{DeletionTimestamp: &now},
			},
			expected: true,
		},
		"StatefulSet without DeletionTimestamp is not being deleted": {
			original: &appsV1.StatefulSet{
				ObjectMeta: metaV1.ObjectMeta{Name: "my-sts"},
			},
			expected: false,
		},
		"Pod with DeletionTimestamp is being deleted": {
			original: &v1.Pod{
				ObjectMeta: metaV1.ObjectMeta{DeletionTimestamp: &now},
			},
			expected: true,
		},
		"non-Kubernetes object returns false": {
			original: "not-a-k8s-object",
			expected: false,
		},
	}
	...
	for name, tt := range tests {
		t.Run(name, func(t *testing.T) {
			assert.Equal(t, tt.expected, d.isOwnerBeingDeleted(tt.original))
		})
	}
}
