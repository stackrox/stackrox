...
var (
	...
	// skipSecretIntegrations contains namespace/secret pairs from which to
	// not create autogenerated integrations for.
	skipSecretIntegrations = map[string]bool{
		// This is a copy of the OCP global pull secret, synced by the cluster-image-registry-operator.
		"openshift-image-registry|installation-pull-secrets": true,
	}
)
...
func skipSecretKey(secret *v1.Secret) string {
...
func (s *secretDispatcher) processDockerConfigEvent(secret, oldSecret *v1.Secret, action central.ResourceAction) *component.ResourceEvent {
	...
	for registryAddress, dce := range dockerConfig {
		...
		if !features.SourcedAutogeneratedIntegrations.Enabled() && env.AutogenerateGlobalPullSecRegistries.BooleanSetting() && skipSecretIntegrations[skipSecretKey(secret)] {
			// Skip creating an autogenerated integration for this secret.
