...
func DockerConfigToImageIntegration(secret *v1.Secret, registry string, dce config.DockerConfigEntry) (*storage.ImageIntegration, error) {
	...
	isGlobalPullSecret := isOCPGlobalPullSecret(secret)

	var id string
	if !features.SourcedAutogeneratedIntegrations.Enabled() && !isGlobalPullSecret {
...
func isOCPGlobalPullSecret(secret *v1.Secret) bool {
	if secret == nil {
		return false
	}

	return secret.Name == "pull-secret" && secret.Namespace == "openshift-config"
}
