...
func convertCFSSLName(name certinfo.Name) *storage.CertName {
	certName := &storage.CertName{}
	certName.SetCommonName(name.CommonName)
	certName.SetCountry(name.Country)
	certName.SetOrganization(name.Organization)
	certName.SetOrganizationUnit(name.OrganizationalUnit)
	certName.SetLocality(name.Locality)
	certName.SetProvince(name.Province)
	certName.SetStreetAddress(name.StreetAddress)
	certName.SetPostalCode(name.PostalCode)
	certName.SetNames(convertInterfaceSliceToStringSlice(name.Names))
	return certName
}
...
func parseCertData(data string) *storage.Cert {
	...
	cert := &storage.Cert{}
	cert.SetSubject(convertCFSSLName(info.Subject))
	cert.SetIssuer(convertCFSSLName(info.Issuer))
	cert.SetSans(info.SANs)
	cert.SetStartDate(protoconv.ConvertTimeToTimestampOrNil(info.NotBefore))
	cert.SetEndDate(protoconv.ConvertTimeToTimestampOrNil(info.NotAfter))
	cert.SetAlgorithm(info.SignatureAlgorithm)
	return cert
}
...
func populateTypeData(secret *storage.Secret, dataFiles map[string][]byte) {
	for file, rawData := range dataFiles {
		...
		file.SetName(sdf)
		file.SetType(secretType)

		switch secretType {
		case storage.SecretType_PUBLIC_CERTIFICATE:
			file.SetCert(proto.ValueOrDefault(parseCertData(data)))
		}
		secret.SetFiles(append(secret.GetFiles(), file))
	}
...
func DockerConfigToImageIntegration(clusterID string, secret *v1.Secret, registry string, dce config.DockerConfigEntry) (*storage.ImageIntegration, error) {
	...
	ii.SetAutogenerated(true)

	if sourcedIntegration {
		is := &storage.ImageIntegration_Source{}
		is.SetClusterId(clusterID)
		is.SetNamespace(secret.GetNamespace())
		is.SetImagePullSecretName(secret.GetName())
		ii.SetSource(is)
	}
...
func (s *secretDispatcher) processDockerConfigEvent(secret, oldSecret *v1.Secret, action central.ResourceAction) *component.ResourceEvent {
	...
	for registryAddress, dce := range dockerConfig {
		...
		ir := &storage.ImagePullSecret_Registry{}
		ir.SetName(registryAddr)
		ir.SetUsername(dce.Username)
		registries = append(registries, ir)
		...
		se.SetId(utils.IfThenElse(sourcedIntegration, ii.GetId(), ""))
		se.SetAction(action)
		se.SetImageIntegration(proto.ValueOrDefault(ii))
		sensorEvents = append(sensorEvents, se)
	...
	if sourcedIntegration {
		...
		if err != nil {
			...
			for id := range oldIntegrations.Difference(newIntegrationSet) {
				sensorEvents = append(sensorEvents, central.SensorEvent_builder{
					Id:     id,
					Action: central.ResourceAction_REMOVE_RESOURCE,
					ImageIntegration: storage.ImageIntegration_builder{
						Id: id,
						// Add a source to the remove event so that Central can determine
						// if this integrations is from the OCP global pull secret if
						// needed.
						Source: storage.ImageIntegration_Source_builder{
	...
	protoSecret.SetFiles([]*storage.SecretDataFile{storage.SecretDataFile_builder{
		Name: v1.DockerConfigKey,
		Type: storage.SecretType_IMAGE_PULL_SECRET,
		ImagePullSecret: storage.ImagePullSecret_builder{
...
func getProtoSecret(secret *v1.Secret) *storage.Secret {
	secret2 := &storage.Secret{}
	secret2.SetId(string(secret.GetUID()))
	secret2.SetName(secret.GetName())
	secret2.SetNamespace(secret.GetNamespace())
	secret2.SetLabels(secret.GetLabels())
	secret2.SetAnnotations(secret.GetAnnotations())
	secret2.SetCreatedAt(protoconv.ConvertTimeToTimestamp(secret.GetCreationTimestamp().Time))
	return secret2
}

func secretToSensorEvent(action central.ResourceAction, secret *storage.Secret) *central.SensorEvent {
	se := &central.SensorEvent{}
	se.SetId(secret.GetId())
	se.SetAction(action)
