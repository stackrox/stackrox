...
func newSecretDispatcher(regStore *registry.Store, reconciliationStore reconciliation.Store) *secretDispatcher {
...
func (s *secretDispatcher) processDockerConfigEvent(secret, oldSecret *v1.Secret, action central.ResourceAction) *component.ResourceEvent {
	...
	log.Infof("docker config: %s %s", action.String(), secret.GetName())
		...
		if fromDefaultSA {
			...
			if err != nil {
				...
				if action == central.ResourceAction_REMOVE_RESOURCE {
					s.reconciliationStore.Remove(deduper.TypeImageIntegration.String(), ii.GetId())
				} else {
					s.reconciliationStore.Upsert(deduper.TypeImageIntegration.String(), ii.GetId())
				}
				if features.SourcedAutogeneratedIntegrations.Enabled() {
	...
	if features.SourcedAutogeneratedIntegrations.Enabled() {
		...
		if err != nil {
			...
			for id := range oldIntegrations.Difference(newIntegrationSet) {
				s.reconciliationStore.Remove(deduper.TypeImageIntegration.String(), id)
				sensorEvents = append(sensorEvents, &central.SensorEvent{
					Id:     id,
					Action: central.ResourceAction_REMOVE_RESOURCE,
					Resource: &central.SensorEvent_ImageIntegration{
