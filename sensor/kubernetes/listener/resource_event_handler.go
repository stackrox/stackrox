...
func (k *listenerImpl) handleAllEvents() {
	...
	handle(resyncingSif.Apps().V1().ReplicaSets().Informer(), dispatchers.ForDeployments(kubernetesPkg.ReplicaSet), k.eventsC, &syncingResources, preTopLevelDeploymentWaitGroup, stopSignal, &eventLock)
	handle(resyncingSif.Core().V1().ReplicationControllers().Informer(), dispatchers.ForDeployments(kubernetesPkg.ReplicationController), k.eventsC, &syncingResources, preTopLevelDeploymentWaitGroup, stopSignal, &eventLock)
	...
	// Deployment types.
	handle(resyncingSif.Apps().V1().DaemonSets().Informer(), dispatchers.ForDeployments(kubernetesPkg.DaemonSet), k.eventsC, &syncingResources, wg, stopSignal, &eventLock)
	handle(resyncingSif.Apps().V1().Deployments().Informer(), dispatchers.ForDeployments(kubernetesPkg.Deployment), k.eventsC, &syncingResources, wg, stopSignal, &eventLock)
	handle(resyncingSif.Apps().V1().StatefulSets().Informer(), dispatchers.ForDeployments(kubernetesPkg.StatefulSet), k.eventsC, &syncingResources, wg, stopSignal, &eventLock)
	...
	if ok, err := hasAPI(k.client.Kubernetes(), "batch/v1", kubernetesPkg.CronJob); err != nil {
		...
		handle(resyncingSif.Batch().V1().CronJobs().Informer(), dispatchers.ForDeployments(kubernetesPkg.CronJob), k.eventsC, &syncingResources, wg, stopSignal, &eventLock)
	} else {
		handle(resyncingSif.Batch().V1beta1().CronJobs().Informer(), dispatchers.ForDeployments(kubernetesPkg.CronJob), k.eventsC, &syncingResources, wg, stopSignal, &eventLock)
	}
	if osAppsFactory != nil {
		handle(osAppsFactory.Apps().V1().DeploymentConfigs().Informer(), dispatchers.ForDeployments(kubernetesPkg.DeploymentConfig), k.eventsC, &syncingResources, wg, stopSignal, &eventLock)
	}
	...
	handle(podInformer.Informer(), dispatchers.ForDeployments(kubernetesPkg.Pod), k.eventsC, &syncingResources, podWaitGroup, stopSignal, &eventLock)
	sif.Start(stopSignal.Done())
