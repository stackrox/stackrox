...
// handleAllEvents starts the dispatchers for all the kubernetes resources tracked by Sensor.
// For each dispatcher, we wait until it is fully synced, meaning we have received and processed the initial resources
// from the cluster. This is a synchronous process and can be time costly. It is encouraged to keep the resource
// dispatchers as quick as possible to avoid having sensor in an unready state for long. This is also encouraged by the go-client documentation
// (see: https://github.com/kubernetes/client-go/blob/592d891671b2a09e5f81781b28ebe078d8115e41/tools/cache/shared_informer.go#L128-L132).
// We did some stress testing to determine what would happen if a dispatcher is blocked for long periods of time. The results
// indicated that sensor recovers eventually, but we should not take this as a valid reason to disregard the previous
// comment since the tests were only focused on the startup of secrets, and there are many unknowns regarding other resources.
// Plus, having this function take long periods of time is not ideal since other components of Sensor rely on the
// dispatchers to be synced.
// (see: https://github.com/stackrox/stackrox/pull/11662)
//
// The order in the startup process is important since some resources depend on others. For example: the pod's informer
// needs to sync before the deployment's since the PodLister is used to populate the image ids of deployments.
