...
func ObjectReaction(tracker ObjectTracker) testing.ReactionFunc {
	return func(action testing.Action) (bool, runtime.Object, error) {
		...
		case testing.PatchActionImpl:
			...
			default:
				return true, nil, pkgErrors.New("PatchType is not supported")
			}
...
type tracker struct {
	...
	watchers map[schema.GroupVersionResource]map[string][]*RaceFreeFakeWatcher
}
...
func NewObjectTracker(scheme ObjectScheme, decoder runtime.Decoder) ObjectTracker {
	return &tracker{
		...
		watchers: make(map[schema.GroupVersionResource]map[string][]*RaceFreeFakeWatcher),
	}
}
...
func (t *tracker) Watch(gvr schema.GroupVersionResource, ns string) (watch.Interface, error) {
	...
	fakewatcher := NewRaceFreeFake()

	if _, exists := t.watchers[gvr]; !exists {
		t.watchers[gvr] = make(map[string][]*RaceFreeFakeWatcher)
	}
...
func (t *tracker) getWatches(gvr schema.GroupVersionResource, ns string) []*RaceFreeFakeWatcher {
	watches := []*RaceFreeFakeWatcher{}
