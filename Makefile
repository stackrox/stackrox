...
roxagent_%: build-prep
	...
	$(eval arch := $(lastword  $(w)))
ifdef SKIP_CLI_BUILD
	test -f bin/$(os)_$(arch)/roxagent || RACE=0 CGO_ENABLED=0 GOOS=$(os) GOARCH=$(arch) GOTAGS=roxagent $(GOBUILD) ./compliance/virtualmachines/roxagent
else
	RACE=0 CGO_ENABLED=0 GOOS=$(os) GOARCH=$(arch) GOTAGS=roxagent $(GOBUILD) ./compliance/virtualmachines/roxagent
endif
