...
config-controller-gen:
	...
	# The CRD should not be installed only when it is explicitly disabled
	# Need to avoid removing the CRD after SecurityPolicy CRs have been created
	echo '{{- if ._rox.configAsCode.installCRDs }}' >> image/templates/helm/stackrox-central/templates/00-securitypolicies-crd.yaml
	echo '# DO NOT EDIT: Generated by make config-controller-gen' >> image/templates/helm/stackrox-central/templates/00-securitypolicies-crd.yaml
	...
	echo '{{- end }}' >> image/templates/helm/stackrox-central/templates/00-securitypolicies-crd.yaml
