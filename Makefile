...
go-unit-tests: build-prep test-prep
	set -o pipefail ; \
	CGO_ENABLED=1 GOEXPERIMENT=cgocheck2 MUTEX_WATCHDOG_TIMEOUT_SECS=30 GOTAGS=$(GOTAGS),test scripts/go-test.sh -tags=test_all -timeout 15m -race -cover -coverprofile test-output/coverage.out -v \
...
sensor-integration-test: build-prep test-prep
	...
	for package in $(shell git ls-files ./sensor/tests | grep '_test.go' | xargs -n 1 dirname | uniq | sort | sed -e 's/sensor\/tests\///'); do \
		CGO_ENABLED=1 GOEXPERIMENT=cgocheck2 MUTEX_WATCHDOG_TIMEOUT_SECS=30 GOTAGS=$(GOTAGS),test scripts/go-test.sh -tags=test_sensor_integration -p 4 -race -cover -coverprofile test-output/coverage.out -v ./sensor/tests/$$package \
