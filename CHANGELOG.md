...
- ROX-9614: Add `file` query parameter to Central's `/api/extensions/scannerdefinitions`, allowing retrieval of individual files (not directories) from Scanner's Definition bundle using their full path within the archive. Add `senßsorEndpoint` to Scanner's configmap, so Scanner in slim mode knows how to reach Sensor from its cluster.
...
- ROX-8277: changed UserAgent Header for all requests from stackrox operator to kubernetes API server
## [70.0]
