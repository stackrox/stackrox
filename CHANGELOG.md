...
- Support for system policies with no versioning and version less than 1.0 will be removed in 3.71. It is recommended to upgrade the policies to version 1.0 or higher before upgrading to 3.71. To do so, export the policies. Now ensure that the system is upgraded to a supported version (refer to `Life Cycle` at https://access.redhat.com/node/5822721). Import them back into the system.
