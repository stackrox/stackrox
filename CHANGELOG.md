...
- Support for system policies with no version or version less than 1.0 will be removed in 3.71. All externally stored policies with no version or version less than 1.0 must be converted to policy version 1.0 or higher. To do so, import the policies into the system, and then export the previously imported policies. The exported policies will now be in supported policy version format (>=1.0), which can be ensured by checking the `policyVersion` field.
