...
- ROX-29582: A `kubectl get` on a Central CR now shows the following additional columns: Version, AdminPassword, Message.
- ROX-31738: Global NodeSelectors and Tolerations: Added the `spec.customize.deploymentDefaults` field to Central and SecuredCluster CRDs,
  for configuring default scheduling constraints for Deployments. This was previously possible on a per-component basis only.
