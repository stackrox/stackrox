...
- The default policy "systemctl Execution" has been updated to not trigger when the process argument `--version` is used as it's not a security issue, because the information printed relates to features supported by systemd at the build time and not the capabilities of the host OS.
