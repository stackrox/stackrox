...
func TestParseCrioConfig(t *testing.T) {
	const configData = `
# Ansible managed
...
pause_image = "docker.io/openshift/origin-pod:v3.11.0"
...
`
	...
	cfg, err := parseCRIOConfig([]byte(configData))
	require.NoError(t, err)

	insecureRegs := cfg.Image.InsecureRegistries
	require.ElementsMatch(t, insecureRegs, []string{"foo.bar"})
}
