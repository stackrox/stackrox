...
for i in 50.0 51.0 52.0 54.0 53.0; do
  ...
  yq e -j values.yaml.orig | jq '.cluster.name="test" | .image.registry.main="docker.io" | .image.repository.main="stackrox/main" | .image.registry.collector="docker.io" | .image.repository.collector="stackrox/collector" | .config.slimCollector=true' | yq e -P - >values.yaml
