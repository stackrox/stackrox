...
ssh_probe_with_key() {
    ...
    output=$(virtctl ssh \
        ...
        "${SSH_USER}@vmi/${vm_name}" 2>/dev/null) || return 1
    grep -q "SSH_PROBE_OK" <<< "$output"
}
...
adopt_vm_with_password() {
    ...
    local output
    if output=$(sshpass -p "$password" virtctl ssh \
        --namespace "$NAMESPACE" \
        --local-ssh-opts="-o StrictHostKeyChecking=no" \
        --local-ssh-opts="-o UserKnownHostsFile=/dev/null" \
        --local-ssh-opts="-o ConnectTimeout=10" \
        ...
        "${SSH_USER}@vmi/${vm_name}" 2>/dev/null) && grep -q "ADOPT_OK" <<< "$output"; then
