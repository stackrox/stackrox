...
function invoke_go() {
  ...
  if [[ "$RACE" == "true" ]]; then
    GOEXPERIMENT=strictfipsruntime CGO_ENABLED=1 go "$tool" -race -ldflags="${ldflags[*]}" -tags "$GOTAGS" "$@"
  else
    GOEXPERIMENT=strictfipsruntime go "$tool" -ldflags="${ldflags[*]}" -tags "$GOTAGS" "$@"
  fi
}
