...
// ===========================================================================
// IMP-CLI-005 / IMP-MAP-022 / IMP-MAP-023: unified --cluster flag
// ===========================================================================

func TestIMP_CLI_005_ClusterUUID(t *testing.T) {
	clearAuthEnv(t)
	setenv(t, "ROX_API_TOKEN", "tok")
	...
	if cfg.ACSClusterID != "65640fbb-ac7c-42a8-9e65-883c3f35f23b" {
		t.Errorf("IMP-MAP-023: expected UUID used directly, got %q", cfg.ACSClusterID)
	}
	if cfg.AutoDiscoverClusterID {
		t.Error("IMP-MAP-023: auto-discover should be disabled when UUID given")
	}
}

func TestIMP_CLI_005_ClusterUUID_UpperCase(t *testing.T) {
	clearAuthEnv(t)
	setenv(t, "ROX_API_TOKEN", "tok")
	...
	if cfg.ACSClusterID != "65640FBB-AC7C-42A8-9E65-883C3F35F23B" {
		t.Errorf("expected uppercase UUID accepted, got %q", cfg.ACSClusterID)
	}
}

func TestIMP_CLI_005_ClusterName(t *testing.T) {
	clearAuthEnv(t)
	setenv(t, "ROX_API_TOKEN", "tok")
	...
	if cfg.ClusterNameLookup != "production-cluster" {
		t.Errorf("IMP-MAP-022: expected ClusterNameLookup='production-cluster', got %q", cfg.ClusterNameLookup)
	}
	if cfg.ACSClusterID != "" {
		t.Errorf("expected empty ACSClusterID for name lookup, got %q", cfg.ACSClusterID)
	}
	if cfg.AutoDiscoverClusterID {
		t.Error("auto-discover should be disabled when cluster name given")
	}
}

func TestIMP_CLI_005_ClusterOverride(t *testing.T) {
	clearAuthEnv(t)
	setenv(t, "ROX_API_TOKEN", "tok")

	cfg, err := ParseAndValidate(minimalValidArgs(
		"--kubecontext", "ctx1",
		"--cluster", "ctx1=my-cluster",
	))
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(cfg.ClusterOverrides) != 1 || cfg.ClusterOverrides[0] != "ctx1=my-cluster" {
		t.Errorf("expected cluster override, got %v", cfg.ClusterOverrides)
	}
}

func TestIMP_CLI_005_MultipleOverrides(t *testing.T) {
	clearAuthEnv(t)
	setenv(t, "ROX_API_TOKEN", "tok")
	...
	cfg, err := ParseAndValidate(minimalValidArgs(
		"--kubecontext", "ctx1",
		"--kubecontext", "ctx2",
		"--cluster", "ctx1=cluster-a",
		"--cluster", "ctx2=cluster-b",
	))
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(cfg.ClusterOverrides) != 2 {
		t.Errorf("expected 2 overrides, got %d", len(cfg.ClusterOverrides))
	}
}

func TestIMP_CLI_005_MultipleShorthandsRejected(t *testing.T) {
	clearAuthEnv(t)
	setenv(t, "ROX_API_TOKEN", "tok")
	...
	if err == nil {
		t.Fatal("expected error for multiple --cluster shorthands, got nil")
	}
}

func TestIMP_CLI_005_MixedShorthandAndOverride(t *testing.T) {
	clearAuthEnv(t)
	setenv(t, "ROX_API_TOKEN", "tok")

	// One shorthand (name) + one override should work:
	// shorthand applies to current context, override applies to named context.
	cfg, err := ParseAndValidate(minimalValidArgs(
		"--cluster", "production",
		"--cluster", "ctx1=staging",
	))
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if cfg.ClusterNameLookup != "production" {
		t.Errorf("expected shorthand name, got %q", cfg.ClusterNameLookup)
	}
	if len(cfg.ClusterOverrides) != 1 || cfg.ClusterOverrides[0] != "ctx1=staging" {
		t.Errorf("expected override, got %v", cfg.ClusterOverrides)
	}
}

func TestIMP_CLI_005_NoClusterAutoDiscover(t *testing.T) {
	clearAuthEnv(t)
	setenv(t, "ROX_API_TOKEN", "tok")
	...
	if !cfg.AutoDiscoverClusterID {
		t.Fatal("expected AutoDiscoverClusterID=true when no --cluster given")
	}
}

func TestIMP_CLI_005_OverrideWithUUIDValue(t *testing.T) {
	clearAuthEnv(t)
	setenv(t, "ROX_API_TOKEN", "tok")

	cfg, err := ParseAndValidate(minimalValidArgs(
		"--kubecontext", "ctx1",
		"--cluster", "ctx1=65640fbb-ac7c-42a8-9e65-883c3f35f23b",
	))
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	// Should be stored as an override, not as a UUID shorthand.
	if len(cfg.ClusterOverrides) != 1 {
		t.Errorf("expected 1 override, got %d", len(cfg.ClusterOverrides))
	}
	if cfg.ACSClusterID != "" {
		t.Errorf("expected empty ACSClusterID (UUID in override, not shorthand), got %q", cfg.ACSClusterID)
	}
}

func TestIMP_CLI_005_NotUUID_LooksLikeUUID(t *testing.T) {
	clearAuthEnv(t)
	setenv(t, "ROX_API_TOKEN", "tok")

	// Missing one hex digit in the last segment — not a valid UUID.
	cfg, err := ParseAndValidate(minimalValidArgs(
		"--cluster", "65640fbb-ac7c-42a8-9e65-883c3f35f23",
	))
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	// Should be treated as a name, not UUID.
	if cfg.ACSClusterID != "" {
		t.Errorf("expected empty ACSClusterID for invalid UUID, got %q", cfg.ACSClusterID)
	}
	if cfg.ClusterNameLookup != "65640fbb-ac7c-42a8-9e65-883c3f35f23" {
		t.Errorf("expected name lookup for invalid UUID, got %q", cfg.ClusterNameLookup)
	}
}
