...
categorize_references() {
    ...
    while IFS= read -r line; do
        [[ -z "$line" ]] && continue
        if echo "$line" | grep -q "^\s*#.*apollo-ci"; then
            comment_refs+="$line"$'\n'
        else
            direct_refs+="$line"$'\n'
        fi
...
update_direct_references() {
    ...
    while IFS= read -r file; do
        [[ -z "$file" ]] && continue
        sed -i.bak -E "s|(quay\.io/stackrox-io/apollo-ci:stackrox-test-)[0-9]+\.[0-9]+\.[0-9]+|\1$version|g" "$GIT_REPO_TOP/$file"
        sed -i.bak -E "s|(quay\.io/stackrox-io/apollo-ci:scanner-test-)[0-9]+\.[0-9]+\.[0-9]+|\1$version|g" "$GIT_REPO_TOP/$file"
        sed -i.bak -E "s|(quay\.io/stackrox-io/apollo-ci:stackrox-ui-test-)[0-9]+\.[0-9]+\.[0-9]+|\1$version|g" "$GIT_REPO_TOP/$file"
...
print_comment_references() {
    ...
    if [[ -n "$comment_refs" ]]; then
        echo "Files with apollo-ci in comments (not updated):"
        echo "$comment_refs"
    fi
}
...
push_and_create_pr() {
    ...
    if [[ -n "$existing_pr" ]]; then
        ...
        echo "Updating PR title to match new version..."

        gh pr edit "$pr_number" --title "chore: bump apollo-ci to $version"

        echo "Updated the existing PR with new commits and title"
    else
        local pr_url
        pr_url=$(gh pr create \
            --draft \
...
Verified all image references are updated consistently. CI will validate the new image works correctly.
