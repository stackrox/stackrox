...
func main() {
	...
	c.Flags().StringVar(&props.MigrateRoot, "migrate-root", "", "Root for migrations")
	c.Flags().StringVar(&props.MigrateTo, "migrate-to", "", "where the data are migrated from in the format of \"<database>:<bucket>\", eg, \"rocksdb:alerts\" or \"boltdb:version\"")
	c.Flags().IntVar(&props.MigrateSeq, "migrate-seq", 0, "the unique sequence number to migrate to Postgres")
	...
	c.RunE = func(*cobra.Command, []string) error {
		...
		migrationDir := fmt.Sprintf("n_%02d_to_n_%02d_postgres_%s", props.MigrateSeq, props.MigrateSeq+1, props.MigrateTo)
		...
		templateMap["Migration"] = migrationOptions{
			...
			Package:         fmt.Sprintf("n%dton%d", props.MigrateSeq, props.MigrateSeq+1),
		}
