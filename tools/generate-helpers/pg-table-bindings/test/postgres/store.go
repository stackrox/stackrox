...
func (s *storeImpl) GetByQuery(ctx context.Context, query *v1.Query) ([]*storage.TestSingleKeyStruct, error) {
	...
	rows, err := postgres.RunGetManyQueryForSchema[storage.TestSingleKeyStruct](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.TestSingleKeyStruct) error) error {
	var sacQueryFilter *v1.Query
	fetcher, closer, err := postgres.RunCursorQueryForSchema[storage.TestSingleKeyStruct](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
