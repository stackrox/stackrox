...
func (s *storeImpl) Get(ctx context.Context, key string) (*storage.TestSingleKeyStruct, bool, error) {
	...
	data, err := postgres.RunGetQueryForSchemaType[storage.TestSingleKeyStruct](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetMany(ctx context.Context, ids []string) ([]*storage.TestSingleKeyStruct, []int, error) {
	...
	rows, err := postgres.RunGetManyQueryForSchemaType[storage.TestSingleKeyStruct](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.TestSingleKeyStruct) error) error {
	var sacQueryFilter *v1.Query
	fetcher, closer, err := postgres.RunCursorQueryForSchemaType[storage.TestSingleKeyStruct](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
