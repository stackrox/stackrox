...
type Store interface {
	...
	GetMany(ctx context.Context, identifiers []string) ([]*storage.TestSingleKeyStruct, []int, error)
...
func (s *storeImpl) Delete(ctx context.Context, key string) error {
	...
	q := search.NewQueryBuilder().AddDocIDs(key).ProtoQuery()
...
func (s *storeImpl) Get(ctx context.Context, key string) (*storage.TestSingleKeyStruct, bool, error) {
	...
	data, err := pgSearch.RunGetQueryForSchema[storage.TestSingleKeyStruct](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetByQuery(ctx context.Context, query *v1.Query) ([]*storage.TestSingleKeyStruct, error) {
	...
	rows, err := pgSearch.RunGetManyQueryForSchema[storage.TestSingleKeyStruct](ctx, schema, query, s.db)
	if err != nil {
...
func (s *storeImpl) GetMany(ctx context.Context, identifiers []string) ([]*storage.TestSingleKeyStruct, []int, error) {
	...
	rows, err := pgSearch.RunGetManyQueryForSchema[storage.TestSingleKeyStruct](ctx, schema, q, s.db)
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
	...
	resultsByID := make(map[string]*storage.TestSingleKeyStruct, len(rows))
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.TestSingleKeyStruct) error) error {
	fetcher, closer, err := pgSearch.RunCursorQueryForSchema[storage.TestSingleKeyStruct](ctx, schema, search.EmptyQuery(), s.db)
	if err != nil {
		return err
	}
	defer closer()
