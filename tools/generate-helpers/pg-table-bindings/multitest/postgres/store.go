...
func (s *storeImpl) Get(ctx context.Context, key1 string, key2 string) (*storage.TestMultiKeyStruct, bool, error) {
	...
	data, err := postgres.RunGetQueryForSchemaType[storage.TestMultiKeyStruct](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetMany(ctx context.Context, ids []string) ([]*storage.TestMultiKeyStruct, []int, error) {
	...
	rows, err := postgres.RunGetManyQueryForSchemaType[storage.TestMultiKeyStruct](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.TestMultiKeyStruct) error) error {
	var sacQueryFilter *v1.Query
	fetcher, closer, err := postgres.RunCursorQueryForSchemaType[storage.TestMultiKeyStruct](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
