...
const (
	...
	getWithRollupStmt = "select row_to_json((select record from (select table0.Key1 as Key1, table0.Key2 as Key2, table0.StringSlice as StringSlice, table0.Bool as Bool, table0.Uint64 as Uint64, table0.Int64 as Int64, table0.Float as Float, table0.Labels as Labels, table0.Timestamp as Timestamp, table0.Enum as Enum, table0.Enums as Enums, table0.String_ as String_, table0.IntSlice as IntSlice, table0.Embedded_Embedded as Embedded_Embedded, table0.Oneofstring as Oneofstring, table0.Oneofnested_Nested as Oneofnested_Nested, to_json(join0)->'array' as join0 from multikey table0 left join lateral (select array(select json_build_object('idx', table1.idx, 'Nested', table1.Nested, 'IsNested', table1.IsNested, 'Int64', table1.Int64, 'Nested2_Nested2', table1.Nested2_Nested2, 'Nested2_IsNested', table1.Nested2_IsNested, 'Nested2_Int64', table1.Nested2_Int64) from multikey_Nested table1 where (table0.Key1 = table1.multikey_Key1 and table0.Key2 = table1.multikey_Key2))) join0 on true where (table0.Key1 = $1 and table0.Key2 = $2)) record ))"
)
...
func (s *storeImpl) GetWithRollup(ctx context.Context, key1 string, key2 string) (map[string]interface{}, bool, error) {
	defer metrics.SetPostgresOperationDurationTime(time.Now(), ops.Get, "TestMultiKeyStruct")

	row := s.db.QueryRow(ctx, getWithRollupStmt, key1, key2)
