...
type Store interface {
	...
	GetMany(ctx context.Context, identifiers []string) ([]*storage.TestParent2, []int, error)
...
type storeImpl struct {
	*pgSearch.GenericStore[storage.TestParent2, *storage.TestParent2]
	db    postgres.DB
	mutex sync.RWMutex
}
...
func New(db postgres.DB) Store {
	return &storeImpl{
		db: db,
		GenericStore: pgSearch.NewGenericStore[storage.TestParent2, *storage.TestParent2](
			db,
			schema,
			pkGetter,
...
func pkGetter(obj *storage.TestParent2) string {
	return obj.GetId()
}

func metricsSetPostgresOperationDurationTime(start time.Time, op ops.Op) {
	metrics.SetPostgresOperationDurationTime(start, op, "TestParent2")
}

func metricsSetAcquireDBConnDuration(start time.Time, op ops.Op) {
	metrics.SetAcquireDBConnDuration(start, op, "TestParent2")
}
...
func (s *storeImpl) copyFromTestParent2(ctx context.Context, tx *postgres.Tx, objs ...*storage.TestParent2) error {
...
func (s *storeImpl) Get(ctx context.Context, id string) (*storage.TestParent2, bool, error) {
	...
	data, err := pgSearch.RunGetQueryForSchema[storage.TestParent2](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetByQuery(ctx context.Context, query *v1.Query) ([]*storage.TestParent2, error) {
	...
	rows, err := pgSearch.RunGetManyQueryForSchema[storage.TestParent2](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetMany(ctx context.Context, identifiers []string) ([]*storage.TestParent2, []int, error) {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.TestParent2) error) error {
	...
	fetcher, closer, err := pgSearch.RunCursorQueryForSchema[storage.TestParent2](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
...
func dropTableTestParent2(ctx context.Context, db postgres.DB) {
