...
func insertIntoTestParent2(ctx context.Context, batch *pgx.Batch, obj *storage.TestParent2) error {
	...
	finalStr := "INSERT INTO test_parent2 (Id, ParentId, Val, serialized, tenant_id) VALUES($1, $2, $3, $4, $5) ON CONFLICT(Id) DO UPDATE SET Id = EXCLUDED.Id, ParentId = EXCLUDED.ParentId, Val = EXCLUDED.Val, serialized = EXCLUDED.serialized, tenant_id = EXCLUDED.tenant_id"
...
func copyFromTestParent2(ctx context.Context, s pgSearch.Deleter, tx *postgres.Tx, objs ...*storage.TestParent2) error {
	...
	for idx, obj := range objs {
		...
		if (idx+1)%batchSize == 0 || idx == len(objs)-1 {
			...
			if _, err := tx.CopyFrom(ctx, pgx.Identifier{"test_parent2"}, copyCols, pgx.CopyFromRows(inputRows)); err != nil {
				return err
			}
