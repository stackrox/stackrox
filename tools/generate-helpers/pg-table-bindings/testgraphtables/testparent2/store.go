...
const (
	...
	countStmt  = "SELECT COUNT(*) FROM testparent2"
	existsStmt = "SELECT EXISTS(SELECT 1 FROM testparent2 WHERE Id = $1)"
...
func (s *storeImpl) GetMany(ctx context.Context, ids []string) ([]*storage.TestParent2, []int, error) {
	...
	conn, release, err := s.acquireConn(ctx, ops.GetMany, "TestParent2")
	if err != nil {
		return nil, nil, err
	}
	defer release()
