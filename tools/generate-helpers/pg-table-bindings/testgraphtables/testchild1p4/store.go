...
func New(db postgres.DB) Store {
	return pgSearch.NewGloballyScopedGenericStore[storeType, *storeType](
		...
		schema,
		pkGetter,
		insertIntoTestChild1P4,
		copyFromTestChild1P4,
...
func insertIntoTestChild1P4(batch *pgx.Batch, pool pgSearch.BufferPool, obj *storage.TestChild1P4) (*[]byte, error) {
...
func copyFromTestChild1P4(ctx context.Context, s pgSearch.Deleter, tx *postgres.Tx, pool pgSearch.BufferPool, objs ...*storage.TestChild1P4) error {
	if len(objs) == 0 {
		return nil
	}
	...
	for objBatch := range slices.Chunk(objs, batchSize) {
		...
		if _, err := tx.CopyFrom(ctx, pgx.Identifier{"test_child1_p4"}, copyCols, pgx.CopyFromRows(inputRows)); err != nil {
			return err
		}
