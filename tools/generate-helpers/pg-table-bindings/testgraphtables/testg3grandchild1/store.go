...
func New(db postgres.DB) Store {
	return pgSearch.NewGloballyScopedGenericStore[storeType, *storeType](
		...
		schema,
		pkGetter,
		insertIntoTestG3GrandChild1,
		copyFromTestG3GrandChild1,
...
func insertIntoTestG3GrandChild1(batch *pgx.Batch, pool pgSearch.BufferPool, obj *storage.TestG3GrandChild1) (*[]byte, error) {
...
func copyFromTestG3GrandChild1(ctx context.Context, s pgSearch.Deleter, tx *postgres.Tx, pool pgSearch.BufferPool, objs ...*storage.TestG3GrandChild1) error {
	if len(objs) == 0 {
		return nil
	}
	...
	for objBatch := range slices.Chunk(objs, batchSize) {
		...
		if _, err := tx.CopyFrom(ctx, pgx.Identifier{"test_g3_grand_child1"}, copyCols, pgx.CopyFromRows(inputRows)); err != nil {
			return err
		}
