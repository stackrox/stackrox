...
func (s *storeImpl) GetByQuery(ctx context.Context, query *v1.Query) ([]*storage.TestGrandparent, error) {
	...
	rows, err := pgSearch.RunGetManyQueryForSchema[storage.TestGrandparent](ctx, schema, query, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.TestGrandparent) error) error {
	fetcher, closer, err := pgSearch.RunCursorQueryForSchema[storage.TestGrandparent](ctx, schema, search.EmptyQuery(), s.db)
	if err != nil {
		return err
	}
	defer closer()
