...
func (s *storeImpl) Get(ctx context.Context, id string) (*storage.TestGrandparent, bool, error) {
	...
	data, err := postgres.RunGetQueryForSchemaType[storage.TestGrandparent](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetMany(ctx context.Context, ids []string) ([]*storage.TestGrandparent, []int, error) {
	...
	rows, err := postgres.RunGetManyQueryForSchemaType[storage.TestGrandparent](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.TestGrandparent) error) error {
	var sacQueryFilter *v1.Query
	fetcher, closer, err := postgres.RunCursorQueryForSchemaType[storage.TestGrandparent](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
