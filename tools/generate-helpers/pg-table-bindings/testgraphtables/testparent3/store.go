...
func metricsSetPostgresOperationDurationTime(start time.Time, op ops.Op) {
	metrics.SetPostgresOperationDurationTime(start, op, "TestParent3")
}

func metricsSetAcquireDBConnDuration(start time.Time, op ops.Op) {
	metrics.SetAcquireDBConnDuration(start, op, "TestParent3")
}
...
func (s *storeImpl) Get(ctx context.Context, id string) (*storage.TestParent3, bool, error) {
	...
	data, err := pgSearch.RunGetQueryForSchema[storage.TestParent3](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetByQuery(ctx context.Context, query *v1.Query) ([]*storage.TestParent3, error) {
	...
	rows, err := pgSearch.RunGetManyQueryForSchema[storage.TestParent3](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetMany(ctx context.Context, identifiers []string) ([]*storage.TestParent3, []int, error) {
	...
	resultsByID := make(map[string]*storage.TestParent3, len(rows))
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.TestParent3) error) error {
	...
	fetcher, closer, err := pgSearch.RunCursorQueryForSchema[storage.TestParent3](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
