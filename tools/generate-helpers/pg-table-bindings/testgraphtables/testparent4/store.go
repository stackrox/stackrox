...
func insertIntoTestParent4(batch *pgx.Batch, pool pgSearch.BufferPool, obj *storage.TestParent4) (*[]byte, error) {
...
func copyFromTestParent4(ctx context.Context, s pgSearch.Deleter, tx *postgres.Tx, pool pgSearch.BufferPool, objs ...*storage.TestParent4) error {
	if len(objs) == 0 {
		return nil
	}
	...
	for objBatch := range slices.Chunk(objs, batchSize) {
		...
		if _, err := tx.CopyFrom(ctx, pgx.Identifier{"test_parent4"}, copyCols, pgx.CopyFromRows(inputRows)); err != nil {
			return err
		}
