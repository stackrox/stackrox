...
func (s *storeImpl) Get(ctx context.Context, id string) (*storage.TestChild2, bool, error) {
	...
	data, err := pgSearch.RunGetQueryForSchema[storage.TestChild2](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetByQuery(ctx context.Context, query *v1.Query) ([]*storage.TestChild2, error) {
	...
	rows, err := pgSearch.RunGetManyQueryForSchema[storage.TestChild2](ctx, schema, query, s.db)
	if err != nil {
...
func (s *storeImpl) GetMany(ctx context.Context, identifiers []string) ([]*storage.TestChild2, []int, error) {
	...
	rows, err := pgSearch.RunGetManyQueryForSchema[storage.TestChild2](ctx, schema, q, s.db)
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
	...
	resultsByID := make(map[string]*storage.TestChild2, len(rows))
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.TestChild2) error) error {
	fetcher, closer, err := pgSearch.RunCursorQueryForSchema[storage.TestChild2](ctx, schema, search.EmptyQuery(), s.db)
	if err != nil {
		return err
	}
	defer closer()
