...
const (
	...
	existsStmt = "SELECT EXISTS(SELECT 1 FROM test_child2 WHERE Id = $1)"

	getStmt     = "SELECT serialized FROM test_child2 WHERE Id = $1"
	deleteStmt  = "DELETE FROM test_child2 WHERE Id = $1"
	walkStmt    = "SELECT serialized FROM test_child2"
	getManyStmt = "SELECT serialized FROM test_child2 WHERE Id = ANY($1::text[])"

	deleteManyStmt = "DELETE FROM test_child2 WHERE Id = ANY($1::text[])"
...
func (s *storeImpl) Delete(ctx context.Context, id string) error {
	...
	conn, release, err := s.acquireConn(ctx, ops.Remove, "TestChild2")
	if err != nil {
		return err
	}
	defer release()
...
func (s *storeImpl) GetMany(ctx context.Context, ids []string) ([]*storage.TestChild2, []int, error) {
	...
	conn, release, err := s.acquireConn(ctx, ops.GetMany, "TestChild2")
	if err != nil {
		return nil, nil, err
	}
	defer release()
...
func (s *storeImpl) DeleteMany(ctx context.Context, ids []string) error {
	...
	conn, release, err := s.acquireConn(ctx, ops.RemoveMany, "TestChild2")
	if err != nil {
		return err
	}
	defer release()
