...
func (s *storeImpl) Get(ctx context.Context, id string) (*storage.TestChild2, bool, error) {
	...
	data, err := postgres.RunGetQueryForSchemaType[storage.TestChild2](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetMany(ctx context.Context, ids []string) ([]*storage.TestChild2, []int, error) {
	...
	rows, err := postgres.RunGetManyQueryForSchemaType[storage.TestChild2](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.TestChild2) error) error {
	var sacQueryFilter *v1.Query
	fetcher, closer, err := postgres.RunCursorQueryForSchemaType[storage.TestChild2](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
