...
func (s *storeImpl) Get(ctx context.Context, id string) (*storage.TestParent1, bool, error) {
	...
	data, err := postgres.RunGetQueryForSchemaType[storage.TestParent1](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetMany(ctx context.Context, ids []string) ([]*storage.TestParent1, []int, error) {
	...
	rows, err := postgres.RunGetManyQueryForSchemaType[storage.TestParent1](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.TestParent1) error) error {
	var sacQueryFilter *v1.Query
	fetcher, closer, err := postgres.RunCursorQueryForSchemaType[storage.TestParent1](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
