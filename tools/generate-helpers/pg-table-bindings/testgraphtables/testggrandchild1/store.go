...
func (s *storeImpl) Get(ctx context.Context, id string) (*storage.TestGGrandChild1, bool, error) {
	...
	data, err := postgres.RunGetQueryForSchemaType[storage.TestGGrandChild1](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetMany(ctx context.Context, ids []string) ([]*storage.TestGGrandChild1, []int, error) {
	...
	rows, err := postgres.RunGetManyQueryForSchemaType[storage.TestGGrandChild1](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.TestGGrandChild1) error) error {
	var sacQueryFilter *v1.Query
	fetcher, closer, err := postgres.RunCursorQueryForSchemaType[storage.TestGGrandChild1](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
