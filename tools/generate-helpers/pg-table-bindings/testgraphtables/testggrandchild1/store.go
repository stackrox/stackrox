...
type Store interface {
	...
	GetMany(ctx context.Context, identifiers []string) ([]*storage.TestGGrandChild1, []int, error)
	GetIDs(ctx context.Context) ([]string, error)

	Walk(ctx context.Context, fn func(obj *storage.TestGGrandChild1) error) error
}

type storeImpl struct {
	*pgSearch.GenericStore[storage.TestGGrandChild1, *storage.TestGGrandChild1]
	db    postgres.DB
	mutex sync.RWMutex
}
...
func New(db postgres.DB) Store {
	return &storeImpl{
		db: db,
		GenericStore: pgSearch.NewGenericStore[storage.TestGGrandChild1, *storage.TestGGrandChild1](
			db,
			targetResource,
			schema,
...
func pkGetter(obj *storage.TestGGrandChild1) string {
	return obj.GetId()
}

func metricsSetPostgresOperationDurationTime(start time.Time, op ops.Op) {
	metrics.SetPostgresOperationDurationTime(start, op, "TestGGrandChild1")
}

func metricsSetAcquireDBConnDuration(start time.Time, op ops.Op) {
	metrics.SetAcquireDBConnDuration(start, op, "TestGGrandChild1")
}
...
func (s *storeImpl) copyFromTestGGrandChild1(ctx context.Context, tx *postgres.Tx, objs ...*storage.TestGGrandChild1) error {
...
func (s *storeImpl) upsert(ctx context.Context, objs ...*storage.TestGGrandChild1) error {
	conn, err := s.AcquireConn(ctx, ops.Get)
	if err != nil {
		return err
	}
	...
	for _, obj := range objs {
		batch := &pgx.Batch{}
		if err := insertIntoTestGGrandChild1(ctx, batch, obj); err != nil {
			return err
		}
...
func (s *storeImpl) Get(ctx context.Context, id string) (*storage.TestGGrandChild1, bool, error) {
	...
	data, err := pgSearch.RunGetQueryForSchema[storage.TestGGrandChild1](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetByQuery(ctx context.Context, query *v1.Query) ([]*storage.TestGGrandChild1, error) {
	...
	rows, err := pgSearch.RunGetManyQueryForSchema[storage.TestGGrandChild1](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetMany(ctx context.Context, identifiers []string) ([]*storage.TestGGrandChild1, []int, error) {
	...
	resultsByID := make(map[string]*storage.TestGGrandChild1, len(rows))
	...
	elems := make([]*storage.TestGGrandChild1, 0, len(resultsByID))
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.TestGGrandChild1) error) error {
	...
	fetcher, closer, err := pgSearch.RunCursorQueryForSchema[storage.TestGGrandChild1](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
...
func dropTableTestGGrandChild1(ctx context.Context, db postgres.DB) {
