...
func New(db postgres.DB) Store {
	return pgSearch.NewGloballyScopedGenericStore[storeType, *storeType](
		...
		schema,
		pkGetter,
		insertIntoTestGGrandChild1,
		copyFromTestGGrandChild1,
...
func insertIntoTestGGrandChild1(batch *pgx.Batch, pool pgSearch.BufferPool, obj *storage.TestGGrandChild1) (*[]byte, error) {
...
func copyFromTestGGrandChild1(ctx context.Context, s pgSearch.Deleter, tx *postgres.Tx, pool pgSearch.BufferPool, objs ...*storage.TestGGrandChild1) error {
	if len(objs) == 0 {
		return nil
	}
	...
	for objBatch := range slices.Chunk(objs, batchSize) {
		...
		if _, err := tx.CopyFrom(ctx, pgx.Identifier{"test_g_grand_child1"}, copyCols, pgx.CopyFromRows(inputRows)); err != nil {
			return err
		}
