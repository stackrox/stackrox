...
func (s *storeImpl) Count(ctx context.Context) (int, error) {
	defer metrics.SetPostgresOperationDurationTime(time.Now(), ops.Count, "TestShortCircuit")
...
func (s *storeImpl) Exists(ctx context.Context, id string) (bool, error) {
	defer metrics.SetPostgresOperationDurationTime(time.Now(), ops.Exists, "TestShortCircuit")
...
func (s *storeImpl) Get(ctx context.Context, id string) (*storage.TestShortCircuit, bool, error) {
	defer metrics.SetPostgresOperationDurationTime(time.Now(), ops.Get, "TestShortCircuit")

	q := search.NewQueryBuilder().AddDocIDs(id).ProtoQuery()

	data, err := pgSearch.RunGetQueryForSchema[storage.TestShortCircuit](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetByQuery(ctx context.Context, query *v1.Query) ([]*storage.TestShortCircuit, error) {
	...
	rows, err := pgSearch.RunGetManyQueryForSchema[storage.TestShortCircuit](ctx, schema, query, s.db)
	if err != nil {
...
func (s *storeImpl) GetIDs(ctx context.Context) ([]string, error) {
	defer metrics.SetPostgresOperationDurationTime(time.Now(), ops.GetAll, "storage.TestShortCircuitIDs")
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.TestShortCircuit) error) error {
	fetcher, closer, err := pgSearch.RunCursorQueryForSchema[storage.TestShortCircuit](ctx, schema, search.EmptyQuery(), s.db)
	if err != nil {
		return err
	}
	defer closer()
