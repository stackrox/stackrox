...
const createTableQuery = "create table if not exists {{.Table}} (id varchar primary key, value jsonb)"
...
func (s *storeImpl) Get(id string) (*storage.{{.Type}}, bool, error) {
	...
	log.Infof("Took %d to query a {{.Type}}", time.Since(t).Milliseconds())
	...
	log.Infof("Took %d to scan a {{.Type}}", time.Since(t).Milliseconds())
	...
	log.Infof("Took %d to unmarshal a {{.Type}}", time.Since(t).Milliseconds())
