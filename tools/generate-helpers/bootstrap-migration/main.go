...
func main() {
	...
	var storeObjects string
	c.Flags().StringVar(&storeObjects, "storeObjects", "", "the objects to migrate (comma separated)")
	dataAccess := "gorm"
	c.Flags().StringVar(&dataAccess, "dataAccess", "gorm", "the data access method (gorm or store)")
	...
	c.RunE = func(*cobra.Command, []string) error {
		...
		// If we are not building schemas just write the template.
		if !buildStores {
		...
		if buildStores {
			...
			storeMigrationDirPath := path.Join(fullMigrationDirPath, "schema")
			...
			templateMap["dataAccess"] = dataAccess
			...
			for _, store := range stores {
				...
				if len(storeTable) == 2 {
					...
					templateMap["tableProvided"] = true
				}

				// Create store directory
				storeMigrationDirPath := path.Join(fullMigrationDirPath, strings.ToLower(rawObjects[1]), "postgres")
				err = os.MkdirAll(storeMigrationDirPath, 0755)
				if err != nil {
					return err
				}
				...
				templateMap["tableProvided"] = false
			}
		}

		// Overwrite sequence number file
