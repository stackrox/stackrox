...
func verifyImportsFromAllowedPackagesOnly(path, validImportRoot, packageName string) (errs []error) {
	...
	if validImportRoot == "migrator" {
		allowedPackages = append(allowedPackages, "pkg/env", "pkg/rocksdb", "pkg/process/id", "pkg/migrations", "pkg/testutils", "pkg/batcher", "pkg/config", "pkg/features", "pkg/grpc/routes")
	}
