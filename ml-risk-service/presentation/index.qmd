...
- **Cons:** Manually tuned, doesn't learn from data
- **Gap:** Can't adapt to changing threat landscape
:::
...
Model ranking: [10, 8, 9, 7, 6, 5, 4, 3, 2, 1]
                    ↑ Small error at top
NDCG ≈ 0.98  (98% as good as perfect ranking)
```
:::
...
1. `policy_violation_score` → +2.1
2. `max_vulnerability_score` → +1.8
3. `privileged_container_ratio` → +1.2
4. `external_exposure` → +0.6
:::
::::
...
POST /api/v1/training/central/train-full
  → Returns: model_version, validation_ndcg
...
POST /api/v1/training/central/validate-predictions
  → Returns: mae, rmse, correlation, ndcg

# List available models
GET /api/v1/models
  → Returns: model metadata, versions
```
