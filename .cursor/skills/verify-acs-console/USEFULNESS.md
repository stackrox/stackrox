...
- Doctor that fails closed when this machine has no cluster, kubectl, or Central, so this run reports **inconclusive** instead of shipping a component screenshot.
...
- Full Central + k8s will not start on a machine with no cluster, kubectl, or container engine. Doctor fails; that is a real limit, not a pass.
