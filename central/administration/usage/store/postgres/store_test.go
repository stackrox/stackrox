...
func (s *SecuredUnitsStoreSuite) TestStore() {
	...
	s.NoError(err)
	s.True(exists)
	protoassert.Equal(s.T(), securedUnits, foundSecuredUnits)

	securedUnitsCount, err := store.Count(ctx, search.EmptyQuery())
	s.NoError(err)
	s.Equal(1, securedUnitsCount)
	securedUnitsCount, err = store.Count(withNoAccessCtx, search.EmptyQuery())
	s.NoError(err)
	...
	foundSecuredUnitss, missing, err := store.GetMany(ctx, securedUnitsIDs)
	s.NoError(err)
	s.Empty(missing)
	protoassert.ElementsMatch(s.T(), securedUnitss, foundSecuredUnitss)
