...
func init() {
	...
	utils.Must(
		...
		schema.AddQuery("imageCVEFlat(cve: String, subfieldScopeQuery: String): ImageCVEFlat"),
	)
}

type imageCVEFlatResolver struct {
	ctx  context.Context
	root *Resolver
...
func (resolver *Resolver) wrapImageCVEFlatWithContext(ctx context.Context, value imagecve.CveCore, err error) (*imageCVEFlatResolver, error) {
	if err != nil || value == nil {
		return nil, err
	}
	return &imageCVEFlatResolver{ctx: ctx, root: resolver, data: value}, nil
}

func (resolver *Resolver) wrapImageCVEsFlatWithContext(ctx context.Context, values []imagecve.CveCore, err error) ([]*imageCVEFlatResolver, error) {
	if err != nil || len(values) == 0 {
		return nil, err
	}
	output := make([]*imageCVEFlatResolver, len(values))
	for i, v := range values {
		output[i] = &imageCVEFlatResolver{ctx: ctx, root: resolver, data: v}
	}
	return output, nil
}

// ImageCVEFlatCount returns the count of image cves satisfying the specified query.
// Note: Client must explicitly pass observed/deferred CVEs.
func (resolver *Resolver) ImageCVEFlatCount(ctx context.Context, q RawQuery) (int32, error) {
...
// ImageCVEsFlat returns graphQL resolver for image cves satisfying the specified query.
// Note: Client must explicitly pass observed/deferred CVEs.
func (resolver *Resolver) ImageCVEsFlat(ctx context.Context, q PaginatedQuery) ([]*imageCVEFlatResolver, error) {
	defer metrics.SetGraphQLOperationDurationTime(time.Now(), pkgMetrics.Root, "ImageCVEs")
	...
	ret, err := resolver.wrapImageCVEsFlatWithContext(ctx, cves, err)
	if err != nil {
		return nil, err
	}
	for _, r := range ret {
...
func (resolver *imageCVEFlatResolver) AffectedImageCount(_ context.Context) int32 {
	return int32(resolver.data.GetAffectedImageCount())
}

func (resolver *imageCVEFlatResolver) AffectedImageCountBySeverity(ctx context.Context) (*resourceCountBySeverityResolver, error) {
	return resolver.root.wrapResourceCountByCVESeverityWithContext(ctx, resolver.data.GetImagesBySeverity(), nil)
}

func (resolver *imageCVEFlatResolver) CVE(_ context.Context) string {
...
func (resolver *imageCVEFlatResolver) PublishedOn(_ context.Context) *graphql.Time {
...
func (resolver *imageCVEFlatResolver) ExceptionCount(ctx context.Context, args struct{ RequestStatus *[]*string }) (int32, error) {
...
func (resolver *imageCVEFlatResolver) TopCVSS(_ context.Context) float64 {
	return float64(resolver.data.GetTopCVSS())
}

func (resolver *imageCVEFlatResolver) TopNVDCVSS(_ context.Context) float64 {
	return float64(resolver.data.GetTopNVDCVSS())
}

// ImageCVEFlat returns graphQL resolver for specified image cve.
func (resolver *Resolver) ImageCVEFlat(ctx context.Context, args struct {
	Cve                *string
	SubfieldScopeQuery *string
}) (*imageCVEFlatResolver, error) {
	...
	ret, err := resolver.wrapImageCVEFlatWithContext(ctx, cves[0], err)
	if err != nil {
		return nil, err
	}
