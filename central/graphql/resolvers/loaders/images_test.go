...
func (suite *ImageLoaderTestSuite) TestFromID() {
	...
	suite.NoError(err)
	suite.True(protocompat.Equal(loader.loaded[sha1], image))
	...
	suite.NoError(err)
	suite.True(protocompat.Equal(thirdImage, image))

	// Above call should now be preloaded.
	image, err = loader.FromID(suite.ctx, sha3)
	suite.NoError(err)
	suite.True(protocompat.Equal(loader.loaded[sha3], image))
}
...
func (suite *ImageLoaderTestSuite) TestFullImageWithID() {
	...
	suite.NoError(err)
	suite.True(protocompat.Equal(thirdImageFull, image))
