...
func (suite *PolicyLoaderTestSuite) TestFromID() {
	...
	suite.NoError(err)
	suite.True(protocompat.Equal(loader.loaded[policy1], policy))
	...
	suite.NoError(err)
	suite.True(protocompat.Equal(thirdPolicy, policy))

	// Above call should now be preloaded.
	policy, err = loader.FromID(suite.ctx, policy3)
	suite.NoError(err)
	suite.True(protocompat.Equal(loader.loaded[policy3], policy))
}
