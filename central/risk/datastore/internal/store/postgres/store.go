...
const (
	...
	getWithRollupStmt = "select row_to_json((select record from (select table0.Id as Id, table0.Subject_Id as Subject_Id, table0.Subject_Namespace as Subject_Namespace, table0.Subject_ClusterId as Subject_ClusterId, table0.Subject_Type as Subject_Type, table0.Score as Score, to_json(join0)->'array' as join0 from risk table0 left join lateral (select array(select json_build_object('idx', table1.idx, 'Name', table1.Name, 'Score', table1.Score, 'join0', to_json(join0)->'array') from risk_Results table1 left join lateral (select array(select json_build_object('idx', table2.idx, 'Message', table2.Message, 'Url', table2.Url) from risk_Results_Factors table2 where (table0.Id = table2.risk_Id and table1.idx = table2.risk_Results_idx))) join0 on true where (table0.Id = table1.risk_Id))) join0 on true where (table0.Id = $1)) record ))"
