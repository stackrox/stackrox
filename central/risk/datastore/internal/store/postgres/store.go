...
func (s *storeImpl) Get(ctx context.Context, id string) (*storage.Risk, bool, error) {
	...
	data, err := postgres.RunGetQueryForSchemaType[storage.Risk](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetMany(ctx context.Context, ids []string) ([]*storage.Risk, []int, error) {
	...
	rows, err := postgres.RunGetManyQueryForSchemaType[storage.Risk](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.Risk) error) error {
	...
	fetcher, closer, err := postgres.RunCursorQueryForSchemaType[storage.Risk](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
