...
func (s *storeImpl) GetByQuery(ctx context.Context, query *v1.Query) ([]*storage.Risk, error) {
	...
	rows, err := pgSearch.RunGetManyQueryForSchema[storage.Risk](ctx, schema, query, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.Risk) error) error {
	fetcher, closer, err := pgSearch.RunCursorQueryForSchema[storage.Risk](ctx, schema, search.EmptyQuery(), s.db)
	if err != nil {
		return err
	}
	defer closer()
