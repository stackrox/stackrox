...
const (
	...
	getWithRollupStmt = "select row_to_json((select table0_record from (select table0.Id as Id, table0.Subject_Id as Subject_Id, table0.Subject_Namespace as Subject_Namespace, table0.Subject_ClusterId as Subject_ClusterId, table0.Subject_Type as Subject_Type, table0.Score as Score, to_json(join0)->'array' as join0 from risk table0 left join lateral (select array(select row_to_json((select table1_record from (select table1.idx as idx, table1.Name as Name, table1.Score as Score, to_json(join0)->'array' as join0 from risk_Results table1 left join lateral (select array(select row_to_json((select table2_record from (select table2.idx as idx, table2.Message as Message, table2.Url as Url from risk_Results_Factors table2 where (table2) ) table0.Id = table2.risk_Id and table1.idx = table2.risk_Results_idx_record )))) join0 on true where (table1) ) table0.Id = table1.risk_Id_record )))) join0 on true where (table0) ) table0.Id = $1_record ))"
