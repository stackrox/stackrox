...
func (s *RisksStoreSuite) TestStore() {
	...
	s.NoError(err)
	s.Equal(1, riskCount)
	riskCount, err = store.Count(withNoAccessCtx)
	s.NoError(err)
	s.Zero(riskCount)
...
func (s *RisksStoreSuite) TestSACUpsertMany() {
	...
	for name, testCase := range testCases {
		s.T().Run(fmt.Sprintf("with %s", name), func(t *testing.T) {
			assert.ErrorIs(t, s.store.UpsertMany(testCase.context, []*storage.Risk{obj}), testCase.expectedWriteError)
		})
	}
}
...
func (s *RisksStoreSuite) TestSACGetMany() {
	objA := &storage.Risk{}
	s.NoError(testutils.FullInit(objA, testutils.UniqueInitializer(), testutils.JSONFieldsFilter))
	...
	for name, expected := range map[string]struct {
		...
		withAllAccess:           {elems: []*storage.Risk{objA, objB}, missingIndices: []int{}},
		withNoAccess:            {elems: []*storage.Risk{}, missingIndices: []int{0, 1}},
		withNoAccessToCluster:   {elems: []*storage.Risk{}, missingIndices: []int{0, 1}},
		withAccessToDifferentNs: {elems: []*storage.Risk{}, missingIndices: []int{0, 1}},
		withAccess:              {elems: []*storage.Risk{objA}, missingIndices: []int{1}},
		withAccessToCluster:     {elems: []*storage.Risk{objA}, missingIndices: []int{1}},
	} {
		s.T().Run(fmt.Sprintf("with %s", name), func(t *testing.T) {
