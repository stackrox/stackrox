...
func (s *serviceAccountSACSuite) runSearchServiceAccountTest(c testutils.SACSearchTestCase) {
	...
	for i := range results {
		obj, found, err := s.datastore.GetServiceAccount(s.testContexts[testutils.UnrestrictedReadCtx], results[i].GetId())
		if found && err == nil {
...
func (s *serviceAccountSACSuite) runSearchTest(c testutils.SACSearchTestCase) {
	...
	resultObjs := make([]sac.NamespaceScopedObject, 0, len(results))
	for i := range results {
		obj, found, err := s.datastore.GetServiceAccount(s.testContexts[testutils.UnrestrictedReadCtx], results[i].ID)
		if found && err == nil {
			resultObjs = append(resultObjs, obj)
		}
	}
