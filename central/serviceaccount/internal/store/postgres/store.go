...
type Store interface {
	...
	GetMany(ctx context.Context, identifiers []string) ([]*storage.ServiceAccount, []int, error)
...
type storeImpl struct {
	*pgSearch.GenericStore[storage.ServiceAccount, *storage.ServiceAccount]
	db    postgres.DB
	mutex sync.RWMutex
}
...
func New(db postgres.DB) Store {
	return &storeImpl{
		db: db,
		GenericStore: pgSearch.NewGenericStore[storage.ServiceAccount, *storage.ServiceAccount](
			db,
			targetResource,
			schema,
...
func pkGetter(obj *storage.ServiceAccount) string {
	return obj.GetId()
}

func metricsSetPostgresOperationDurationTime(start time.Time, op ops.Op) {
	metrics.SetPostgresOperationDurationTime(start, op, "ServiceAccount")
}

func metricsSetAcquireDBConnDuration(start time.Time, op ops.Op) {
	metrics.SetAcquireDBConnDuration(start, op, "ServiceAccount")
}
...
func (s *storeImpl) copyFromServiceAccounts(ctx context.Context, tx *postgres.Tx, objs ...*storage.ServiceAccount) error {
...
func (s *storeImpl) upsert(ctx context.Context, objs ...*storage.ServiceAccount) error {
	...
	for _, obj := range objs {
		batch := &pgx.Batch{}
		if err := insertIntoServiceAccounts(ctx, batch, obj); err != nil {
			return err
		}
...
func (s *storeImpl) Get(ctx context.Context, id string) (*storage.ServiceAccount, bool, error) {
	...
	data, err := pgSearch.RunGetQueryForSchema[storage.ServiceAccount](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetByQuery(ctx context.Context, query *v1.Query) ([]*storage.ServiceAccount, error) {
	...
	rows, err := pgSearch.RunGetManyQueryForSchema[storage.ServiceAccount](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetMany(ctx context.Context, identifiers []string) ([]*storage.ServiceAccount, []int, error) {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.ServiceAccount) error) error {
	...
	fetcher, closer, err := pgSearch.RunCursorQueryForSchema[storage.ServiceAccount](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
