...
func (s *storeImpl) GetByQuery(ctx context.Context, query *v1.Query) ([]*storage.ServiceAccount, error) {
	...
	rows, err := pgSearch.RunGetManyQueryForSchema[storage.ServiceAccount](ctx, schema, query, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.ServiceAccount) error) error {
	fetcher, closer, err := pgSearch.RunCursorQueryForSchema[storage.ServiceAccount](ctx, schema, search.EmptyQuery(), s.db)
	if err != nil {
		return err
	}
	defer closer()
