...
func (s *storeImpl) Get(ctx context.Context, id string) (*storage.ImageComponentEdge, bool, error) {
	...
	data, err := postgres.RunGetQueryForSchemaType[storage.ImageComponentEdge](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetMany(ctx context.Context, ids []string) ([]*storage.ImageComponentEdge, []int, error) {
	...
	rows, err := postgres.RunGetManyQueryForSchemaType[storage.ImageComponentEdge](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.ImageComponentEdge) error) error {
	var sacQueryFilter *v1.Query
	fetcher, closer, err := postgres.RunCursorQueryForSchemaType[storage.ImageComponentEdge](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
