...
func (s *storeImpl) GetByQuery(ctx context.Context, query *v1.Query) ([]*storage.ImageComponentEdge, error) {
	...
	rows, err := pgSearch.RunGetManyQueryForSchema[storage.ImageComponentEdge](ctx, schema, query, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.ImageComponentEdge) error) error {
	fetcher, closer, err := pgSearch.RunCursorQueryForSchema[storage.ImageComponentEdge](ctx, schema, search.EmptyQuery(), s.db)
	if err != nil {
		return err
	}
	defer closer()
