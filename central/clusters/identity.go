...
// These certificates are used to register new clusters at central.
// All certificates share the same init bundle UUID written in the OU subject field.
func IssueSecuredClusterCRSCertificates(validUntil time.Time, validFor time.Duration) (*mtls.IssuedCert, uuid.UUID, error) {
	crsID := uuid.NewV4()
	cert, err := mtls.IssueNewCrsCert(crsID, validUntil, validFor)
	if err != nil {
