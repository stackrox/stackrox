...
func (s *ListeningEndpointsStoreSuite) TestStore() {
	...
	s.NoError(err)
	s.True(exists)
	s.True(protocompat.Equal(processListeningOnPortStorage, foundProcessListeningOnPortStorage))

	processListeningOnPortStorageCount, err := store.Count(ctx, search.EmptyQuery())
	s.NoError(err)
	s.Equal(1, processListeningOnPortStorageCount)
	processListeningOnPortStorageCount, err = store.Count(withNoAccessCtx, search.EmptyQuery())
	s.NoError(err)
...
type testCase struct {
	...
	expectedObjects        []*storage.ProcessListeningOnPortStorage
	expectedWriteError     error
}

func (s *ListeningEndpointsStoreSuite) getTestData(access ...storage.Access) (*storage.ProcessListeningOnPortStorage, *storage.ProcessListeningOnPortStorage, map[string]testCase) {
	objA := &storage.ProcessListeningOnPortStorage{}
	s.NoError(testutils.FullInit(objA, testutils.UniqueInitializer(), testutils.JSONFieldsFilter))

	objB := &storage.ProcessListeningOnPortStorage{}
	...
	testCases := map[string]testCase{
		withAllAccess: {
			...
			expectedMissingIndices: []int{},
			expectedObjects:        []*storage.ProcessListeningOnPortStorage{objA, objB},
		...
		withAccess: {
			...
			expectedObjects:        []*storage.ProcessListeningOnPortStorage{objA},
			expectedWriteError:     nil,
		},
		withAccessToCluster: {
			context: sac.WithGlobalAccessScopeChecker(context.Background(),
...
func (s *ListeningEndpointsStoreSuite) TestSACUpsert() {
...
func (s *ListeningEndpointsStoreSuite) TestSACUpsertMany() {
	obj, _, testCases := s.getTestData(storage.Access_READ_WRITE_ACCESS)
	for name, testCase := range testCases {
		s.T().Run(fmt.Sprintf("with %s", name), func(t *testing.T) {
			assert.ErrorIs(t, s.store.UpsertMany(testCase.context, []*storage.ProcessListeningOnPortStorage{obj}), testCase.expectedWriteError)
		})
	}
}

func (s *ListeningEndpointsStoreSuite) TestSACCount() {
...
func (s *ListeningEndpointsStoreSuite) TestSACWalk() {
	objA, objB, testCases := s.getTestData(storage.Access_READ_ACCESS)
	s.Require().NoError(s.store.Upsert(withAllAccessCtx, objA))
	s.Require().NoError(s.store.Upsert(withAllAccessCtx, objB))
...
func (s *ListeningEndpointsStoreSuite) TestSACExists() {
...
func (s *ListeningEndpointsStoreSuite) TestSACGet() {
	objA, _, testCases := s.getTestData(storage.Access_READ_ACCESS)
	s.Require().NoError(s.store.Upsert(withAllAccessCtx, objA))
...
func (s *ListeningEndpointsStoreSuite) TestSACDelete() {
...
func (s *ListeningEndpointsStoreSuite) TestSACDeleteMany() {
	objA, objB, testCases := s.getTestData(storage.Access_READ_ACCESS, storage.Access_READ_WRITE_ACCESS)
	for name, testCase := range testCases {
		s.T().Run(fmt.Sprintf("with %s", name), func(t *testing.T) {
			s.SetupTest()
...
func (s *ListeningEndpointsStoreSuite) TestSACGetMany() {
	objA, objB, testCases := s.getTestData(storage.Access_READ_ACCESS)
	s.Require().NoError(s.store.Upsert(withAllAccessCtx, objA))
	s.Require().NoError(s.store.Upsert(withAllAccessCtx, objB))
