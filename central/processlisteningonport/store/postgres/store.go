...
func (s *storeImpl) Get(ctx context.Context, id string) (*storage.ProcessListeningOnPortStorage, bool, error) {
	...
	data, err := pgSearch.RunGetQueryForSchema[storage.ProcessListeningOnPortStorage](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetByQuery(ctx context.Context, query *v1.Query) ([]*storage.ProcessListeningOnPortStorage, error) {
	...
	rows, err := pgSearch.RunGetManyQueryForSchema[storage.ProcessListeningOnPortStorage](ctx, schema, query, s.db)
	if err != nil {
...
func (s *storeImpl) GetMany(ctx context.Context, identifiers []string) ([]*storage.ProcessListeningOnPortStorage, []int, error) {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.ProcessListeningOnPortStorage) error) error {
	fetcher, closer, err := pgSearch.RunCursorQueryForSchema[storage.ProcessListeningOnPortStorage](ctx, schema, search.EmptyQuery(), s.db)
	if err != nil {
		return err
	}
	defer closer()
