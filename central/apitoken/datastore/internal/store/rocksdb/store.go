...
func (b *storeImpl) Count() (int, error) {
	defer metrics.SetRocksDBOperationDurationTime(time.Now(), ops.Count, "TokenMetadata")
...
func (b *storeImpl) Exists(id string) (bool, error) {
	defer metrics.SetRocksDBOperationDurationTime(time.Now(), ops.Exists, "TokenMetadata")
...
func (b *storeImpl) GetIDs() ([]string, error) {
	defer metrics.SetRocksDBOperationDurationTime(time.Now(), ops.GetAll, "TokenMetadataIDs")
...
func (b *storeImpl) Get(id string) (*storage.TokenMetadata, bool, error) {
	defer metrics.SetRocksDBOperationDurationTime(time.Now(), ops.Get, "TokenMetadata")
...
func (b *storeImpl) GetMany(ids []string) ([]*storage.TokenMetadata, []int, error) {
	defer metrics.SetRocksDBOperationDurationTime(time.Now(), ops.GetMany, "TokenMetadata")

	msgs, missingIndices, err := b.crud.GetMany(ids)
	if err != nil {
		return nil, nil, err
	}
	objs := make([]*storage.TokenMetadata, 0, len(msgs))
	for _, m := range msgs {
		objs = append(objs, m.(*storage.TokenMetadata))
	}
	return objs, missingIndices, nil
}

// Upsert inserts the object into the DB
func (b *storeImpl) Upsert(obj *storage.TokenMetadata) error {
	defer metrics.SetRocksDBOperationDurationTime(time.Now(), ops.Add, "TokenMetadata")

	return b.crud.Upsert(obj)
}

// UpsertMany batches objects into the DB
func (b *storeImpl) UpsertMany(objs []*storage.TokenMetadata) error {
	defer metrics.SetRocksDBOperationDurationTime(time.Now(), ops.AddMany, "TokenMetadata")
...
func (b *storeImpl) Delete(id string) error {
	defer metrics.SetRocksDBOperationDurationTime(time.Now(), ops.Remove, "TokenMetadata")
...
func (b *storeImpl) DeleteMany(ids []string) error {
	defer metrics.SetRocksDBOperationDurationTime(time.Now(), ops.RemoveMany, "TokenMetadata")

	return b.crud.DeleteMany(ids)
}

// Walk iterates over all of the objects in the store and applies the closure
func (b *storeImpl) Walk(fn func(obj *storage.TokenMetadata) error) error {
