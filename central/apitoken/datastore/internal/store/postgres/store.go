...
func (s *storeImpl) Get(ctx context.Context, id string) (*storage.TokenMetadata, bool, error) {
	...
	data, err := postgres.RunGetQueryForSchema[storage.TokenMetadata](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetMany(ctx context.Context, identifiers []string) ([]*storage.TokenMetadata, []int, error) {
	...
	rows, err := postgres.RunGetManyQueryForSchema[storage.TokenMetadata](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.TokenMetadata) error) error {
	...
	fetcher, closer, err := postgres.RunCursorQueryForSchema[storage.TokenMetadata](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
