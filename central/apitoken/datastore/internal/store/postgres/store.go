...
const createTableQuery = "create table if not exists apiTokens (id varchar primary key, value jsonb)"
...
//
	return &storeImpl{
		...
		countStmt: compileStmtOrPanic(db, "select count(*) from apiTokens"),
		existsStmt: compileStmtOrPanic(db, "select exists(select 1 from apiTokens where id = $1)"),
		getIDsStmt: compileStmtOrPanic(db, "select id from apiTokens"),
		getStmt: compileStmtOrPanic(db, "select value from apiTokens where id = $1"),
		getManyStmt: compileStmtOrPanic(db, "select value from apiTokens where id = ANY($1::text[])"),
		upsertStmt: compileStmtOrPanic(db, "insert into apiTokens(id, value) values($1, $2) on conflict(id) do update set value=$2"),
		deleteStmt: compileStmtOrPanic(db, "delete from apiTokens where id = $1"),
		deleteManyStmt: compileStmtOrPanic(db, "delete from apiTokens where id = ANY($1::text[])"),
		walkStmt: compileStmtOrPanic(db, "select value from apiTokens"),
		walkWithIDStmt: compileStmtOrPanic(db, "select id, value from apiTokens"),
	}
//
}
...
func (s *storeImpl) Get(id string) (*storage.TokenMetadata, bool, error) {
	...
	log.Infof("Took %d to query a TokenMetadata", time.Since(t).Milliseconds())
	...
	log.Infof("Took %d to scan a TokenMetadata", time.Since(t).Milliseconds())
	...
	log.Infof("Took %d to unmarshal a TokenMetadata", time.Since(t).Milliseconds())
