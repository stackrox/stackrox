...
func (s *storeImpl) GetByQuery(ctx context.Context, query *v1.Query) ([]*storage.TokenMetadata, error) {
	...
	rows, err := pgSearch.RunGetManyQueryForSchema[storage.TokenMetadata](ctx, schema, query, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.TokenMetadata) error) error {
	fetcher, closer, err := pgSearch.RunCursorQueryForSchema[storage.TokenMetadata](ctx, schema, search.EmptyQuery(), s.db)
	if err != nil {
		return err
	}
	defer closer()
