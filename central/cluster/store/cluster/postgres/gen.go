package postgres

// Cluster store does not use Postgres CopyFrom operation to copy data into DB. This is because copyFrom requires an
// explicit delete prior to copy consequently prohibiting references to clusters table.
//go:generate pg-table-bindings-wrapper --type=storage.Cluster --cached-store --for-sac --search-category CLUSTERS --no-copy-from --default-sort search.Cluster.String() --generate-data-model-helpers
