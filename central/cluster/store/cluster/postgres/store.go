...
func (s *storeImpl) GetByQuery(ctx context.Context, query *v1.Query) ([]*storage.Cluster, error) {
	...
	rows, err := pgSearch.RunGetManyQueryForSchema[storage.Cluster](ctx, schema, query, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.Cluster) error) error {
	fetcher, closer, err := pgSearch.RunCursorQueryForSchema[storage.Cluster](ctx, schema, search.EmptyQuery(), s.db)
	if err != nil {
		return err
	}
	defer closer()
