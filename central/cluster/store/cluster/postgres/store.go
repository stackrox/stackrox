...
const createTableQuery = "create table if not exists clusters (id varchar primary key, value jsonb)"
...
//
	return &storeImpl{
		...
		countStmt: compileStmtOrPanic(db, "select count(*) from clusters"),
		existsStmt: compileStmtOrPanic(db, "select exists(select 1 from clusters where id = $1)"),
		...
		walkWithIDStmt: compileStmtOrPanic(db, "select id, value from clusters"),
	}
//
}
...
func (s *storeImpl) Get(id string) (*storage.Cluster, bool, error) {
	...
	log.Infof("Took %d to query a Cluster", time.Since(t).Milliseconds())
	...
	log.Infof("Took %d to scan a Cluster", time.Since(t).Milliseconds())
	...
	log.Infof("Took %d to unmarshal a Cluster", time.Since(t).Milliseconds())
