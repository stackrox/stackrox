...
func (s *storeImpl) GetByQuery(ctx context.Context, query *v1.Query) ([]*storage.ClusterHealthStatus, error) {
	...
	rows, err := pgSearch.RunGetManyQueryForSchema[storage.ClusterHealthStatus](ctx, schema, query, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.ClusterHealthStatus) error) error {
	fetcher, closer, err := pgSearch.RunCursorQueryForSchema[storage.ClusterHealthStatus](ctx, schema, search.EmptyQuery(), s.db)
	if err != nil {
		return err
	}
	defer closer()
