...
func (s *storeImpl) Get(ctx context.Context, id string) (*storage.ClusterHealthStatus, bool, error) {
	...
	data, err := postgres.RunGetQueryForSchemaType[storage.ClusterHealthStatus](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetMany(ctx context.Context, ids []string) ([]*storage.ClusterHealthStatus, []int, error) {
	...
	rows, err := postgres.RunGetManyQueryForSchemaType[storage.ClusterHealthStatus](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.ClusterHealthStatus) error) error {
	var sacQueryFilter *v1.Query
	fetcher, closer, err := postgres.RunCursorQueryForSchemaType[storage.ClusterHealthStatus](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
