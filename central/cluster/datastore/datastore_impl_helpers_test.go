...
func TestSensorCapabilitiesEqual(t *testing.T) {
	t.Run("returns true for identical capabilities", func(t *testing.T) {
		cluster := &storage.Cluster{
			SensorCapabilities: []string{"cap1", "cap2"},
		}

		equal := sensorCapabilitiesEqual(cluster, []string{"cap1", "cap2"})
		assert.True(t, equal)
	})

	t.Run("returns true for capabilities in different order", func(t *testing.T) {
		cluster := &storage.Cluster{
			SensorCapabilities: []string{"cap1", "cap2"},
		}

		equal := sensorCapabilitiesEqual(cluster, []string{"cap2", "cap1"})
		assert.True(t, equal)
	})

	t.Run("returns false for different capabilities", func(t *testing.T) {
		cluster := &storage.Cluster{
			SensorCapabilities: []string{"cap1", "cap2"},
		}

		equal := sensorCapabilitiesEqual(cluster, []string{"cap1", "cap3"})
		assert.False(t, equal)
	})

	t.Run("returns false for different number of capabilities", func(t *testing.T) {
		cluster := &storage.Cluster{
			SensorCapabilities: []string{"cap1", "cap2"},
		}

		equal := sensorCapabilitiesEqual(cluster, []string{"cap1"})
		assert.False(t, equal)
	})
	...
	t.Run("returns true for empty capabilities", func(t *testing.T) {
		...
		equal := sensorCapabilitiesEqual(cluster, []string{})
		assert.True(t, equal)
	})

	t.Run("returns true for nil vs empty slice", func(t *testing.T) {
		cluster := &storage.Cluster{
			SensorCapabilities: nil,
		}

		equal := sensorCapabilitiesEqual(cluster, []string{})
		assert.True(t, equal, "nil and empty slice should be treated as equivalent")
	})

	t.Run("returns true for empty slice vs nil", func(t *testing.T) {
		cluster := &storage.Cluster{
			SensorCapabilities: []string{},
		}

		equal := sensorCapabilitiesEqual(cluster, nil)
		assert.True(t, equal, "empty slice and nil should be treated as equivalent")
	})
}
