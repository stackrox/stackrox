...
type Store interface {
	...
	Walk(ctx context.Context, fn func(obj *storage.ReportSnapshot) error) error
}
...
func metricsSetPostgresOperationDurationTime(start time.Time, op ops.Op) {
	metrics.SetPostgresOperationDurationTime(start, op, "ReportSnapshot")
}

func metricsSetAcquireDBConnDuration(start time.Time, op ops.Op) {
	metrics.SetAcquireDBConnDuration(start, op, "ReportSnapshot")
}
...
func (s *storeImpl) copyFromReportSnapshots(ctx context.Context, tx *postgres.Tx, objs ...*storage.ReportSnapshot) error {
	...
	for idx, obj := range objs {
		...
		if (idx+1)%batchSize == 0 || idx == len(objs)-1 {
			...
			_, err = tx.CopyFrom(ctx, pgx.Identifier{"report_snapshots"}, copyCols, pgx.CopyFromRows(inputRows))
...
func (s *storeImpl) copyFrom(ctx context.Context, objs ...*storage.ReportSnapshot) error {
	...
	if err := s.copyFromReportSnapshots(ctx, tx, objs...); err != nil {
...
func (s *storeImpl) upsert(ctx context.Context, objs ...*storage.ReportSnapshot) error {
	...
	for _, obj := range objs {
		batch := &pgx.Batch{}
		if err := insertIntoReportSnapshots(ctx, batch, obj); err != nil {
			return err
		}
...
func (s *storeImpl) Upsert(ctx context.Context, obj *storage.ReportSnapshot) error {
	defer metrics.SetPostgresOperationDurationTime(time.Now(), ops.Upsert, "ReportSnapshot")
...
func (s *storeImpl) UpsertMany(ctx context.Context, objs []*storage.ReportSnapshot) error {
	defer metrics.SetPostgresOperationDurationTime(time.Now(), ops.UpdateMany, "ReportSnapshot")
...
func (s *storeImpl) DeleteMany(ctx context.Context, identifiers []string) error {
	defer metrics.SetPostgresOperationDurationTime(time.Now(), ops.RemoveMany, "ReportSnapshot")
...
func (s *storeImpl) Count(ctx context.Context) (int, error) {
	defer metrics.SetPostgresOperationDurationTime(time.Now(), ops.Count, "ReportSnapshot")
...
func (s *storeImpl) GetByQuery(ctx context.Context, query *v1.Query) ([]*storage.ReportSnapshot, error) {
	defer metrics.SetPostgresOperationDurationTime(time.Now(), ops.GetByQuery, "ReportSnapshot")
	...
	rows, err := pgSearch.RunGetManyQueryForSchema[storage.ReportSnapshot](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetMany(ctx context.Context, identifiers []string) ([]*storage.ReportSnapshot, []int, error) {
	defer metrics.SetPostgresOperationDurationTime(time.Now(), ops.GetMany, "ReportSnapshot")
	...
	resultsByID := make(map[string]*storage.ReportSnapshot, len(rows))
	for _, msg := range rows {
		resultsByID[msg.GetReportId()] = msg
	}
	...
	elems := make([]*storage.ReportSnapshot, 0, len(resultsByID))
