...
var (
	downloadLog = logging.LoggerForModule()
)
...
// NewDownloadHandler creates an HTTP handler for downloading node vulnerability reports
...
func (h *downloadHandler) handle(w http.ResponseWriter, r *http.Request) {
	...
	if rep.GetType() != storage.ReportSnapshot_NODE_VULNERABILITY {
		httputil.WriteGRPCStyleError(w, codes.InvalidArgument,
			errors.Errorf("Report snapshot '%s' is not a node vulnerability report", id))
		return
	}
	...
	if err == nil && status.GetRunState() == storage.ReportStatus_GENERATED {
		...
		if err != nil {
			downloadLog.Error("Error setting report state to DELIVERED")
		}
	}
}
