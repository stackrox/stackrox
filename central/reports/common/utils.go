...
// For the purpose of vulnerability reporting, nil/empty list of access scope rules would mean allow access to all clusters/namespaces.
func ExtractAccessScopeRules(identity authn.Identity) []*storage.SimpleAccessScope_Rules {
	...
	for _, role := range roles {
		...
		//  The reason for doing this that the access scope "AccessScopeIncludeAll" which allows all clusters/namespaces
		//  has scope rules = nil. However, for any other access scope, nil/empty scope rules translates to deny all access scope.
		...
		if accessScope == nil || accessScope.Id == rolePkg.AccessScopeExcludeAll.Id {
			...
			// nil list of access scope rules means include all
			return nil
