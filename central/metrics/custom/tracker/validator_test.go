...
func TestTranslateConfiguration(t *testing.T) {
	...
	// Test that the parsed filters are equal to the test filters.
	assert.Equal(t, len(testFilters), len(lf))
	for metric, filters := range lf {
...
func Test_noLabels(t *testing.T) {
	...
	md, _, err := tracker.translateStorageConfiguration(nil)
	assert.NoError(t, err)
	assert.Empty(t, md)
}
