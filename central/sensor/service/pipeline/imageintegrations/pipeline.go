...
func setUpIntegrationParams(ctx context.Context, imageIntegration *storage.ImageIntegration, includePathInDescription bool) (description string, matches matchingFunc, err error) {
	switch config := imageIntegration.WhichIntegrationConfig(); config {
	case storage.ImageIntegration_Docker_case:
		dockerCfg := imageIntegration.GetDocker()
		origEndpoint := dockerCfg.GetEndpoint()
		dockerCfg.SetEndpoint(parseEndpointForURL(origEndpoint))
	...
	case storage.ImageIntegration_Ecr_case:
		description = imageIntegration.GetEcr().GetEndpoint()
		if description == "" {
			description = fmt.Sprintf("AWS account %s and region %s", imageIntegration.GetEcr().GetRegistryId(), imageIntegration.GetEcr().GetRegion())
		}
		matches = matchesECRAuth
	default:
		err = errors.Errorf("unsupported autogenerated integration type: %v", config)
	}
	return
}
...
func (s *pipelineImpl) updateIntegrationInsecureFlag(ctx context.Context, imageIntegration *storage.ImageIntegration) {
	switch imageIntegration.WhichIntegrationConfig() {
	case storage.ImageIntegration_Docker_case:
		dockerCfg := imageIntegration.GetDocker()
		...
		if tlsErr != nil || skip {
			...
			dockerCfg.SetInsecure(false)
		} else {
			dockerCfg.SetInsecure(!validTLS)
		}
	}
}
...
func (s *pipelineImpl) legacyRun(ctx context.Context, imageIntegration *storage.ImageIntegration, matches matchingFunc) error {
	...
	imageIntegration.SetId(integrationToUpdate.GetId())
	imageIntegration.SetName(integrationToUpdate.GetName())
	if err := s.integrationManager.Upsert(imageIntegration); err != nil {
		return errors.Wrap(err, "notifying of image integration update")
	}
...
func (s *pipelineImpl) Run(ctx context.Context, clusterID string, msg *central.MsgFromSensor, _ common.MessageInjector) error {
	...
	imageIntegration.SetClusterId(clusterID)
	...
	if !sourcedIntegration {
		imageIntegration.SetName(fmt.Sprintf("Autogenerated %s for cluster %s", description, clusterName))
	...
	imageIntegration.SetName(fmt.Sprintf("Autogenerated %s for cluster %s from %s/%s",
