...
func (s *pipelineImpl) Run(ctx context.Context, clusterID string, msg *central.MsgFromSensor, _ common.MessageInjector) error {
	...
	requiresShortCircuit, err := s.upsertImageIntegration(ctx, imageIntegration)
	if err != nil {
		return err
	}

	if requiresShortCircuit {
		s.enrichAndDetectLoop.ShortCircuit()
	}
...
func (s *pipelineImpl) upsertImageIntegration(ctx context.Context, imageIntegration *storage.ImageIntegration) (bool, error) {
	...
	if err != nil {
		return exists, errors.Wrapf(err, "retrieving image integration %q", imageIntegration.GetId())
	}

	if _, err := s.datastore.AddImageIntegration(ctx, imageIntegration); err != nil {
		return exists, errors.Wrapf(err, "adding image integration %q", imageIntegration.GetId())
	}
	if err := s.integrationManager.Upsert(imageIntegration); err != nil {
