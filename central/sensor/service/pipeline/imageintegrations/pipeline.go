...
func (s *pipelineImpl) Reconcile(ctx context.Context, clusterID string, storeMap *reconciliation.StoreMap) error {
	if !features.SourcedAutogeneratedIntegrations.Enabled() && !env.AutogenerateGlobalPullSecRegistries.BooleanSetting() {
		return nil
	}
	...
	for _, integration := range integrations {
		...
		// If only generating sourced integrations for the OCP global pull secret, reconcile only the integrations
		// that represent the OCP global pull secret, skip everything else.
		if !features.SourcedAutogeneratedIntegrations.Enabled() &&
			env.AutogenerateGlobalPullSecRegistries.BooleanSetting() &&
			!openshift.GlobalPullSecretIntegration(integration) {
			continue
		}
...
func (s *pipelineImpl) Run(ctx context.Context, clusterID string, msg *central.MsgFromSensor, _ common.MessageInjector) error {
	...
	sourcedIntegration := features.SourcedAutogeneratedIntegrations.Enabled() ||
		(isGlobalPullSecret && env.AutogenerateGlobalPullSecRegistries.BooleanSetting())
	...
	if !sourcedIntegration || source == nil {
