...
func (suite *PipelineTestSuite) TestMatch_VirtualMachineMessage() {
	msg := createVMMessage("vm-1", "test-vm", central.ResourceAction_CREATE_RESOURCE)
	result := suite.pipeline.Match(msg)
	suite.True(result, "Should match virtual machine messages")
}

func (suite *PipelineTestSuite) TestMatch_NonVirtualMachineMessage() {
	msg := createNonVMMessage()
	result := suite.pipeline.Match(msg)
	suite.False(result, "Should not match non-virtual machine messages")
}

func (suite *PipelineTestSuite) TestMatch_NilEvent() {
	...
	result := suite.pipeline.Match(msg)
	suite.False(result, "Should not match messages without events")
}

func (suite *PipelineTestSuite) TestRun_CreateVirtualMachine() {
	vmID := "vm-1"
	vmName := "test-vm"
	msg := createVMMessage(vmID, vmName, central.ResourceAction_CREATE_RESOURCE)

	// Expect the VM to be upserted
	suite.vmDatastore.EXPECT().
		UpsertVirtualMachine(ctx, gomock.Any()).
		Do(func(ctx context.Context, vm *storage.VirtualMachine) {
			suite.Equal(vmID, vm.Id)
			suite.Equal(vmName, vm.Name)
		}).
		Return(nil)
...
func (suite *PipelineTestSuite) TestRun_UpdateVirtualMachine() {
	vmID := "vm-1"
	vmName := "updated-vm"
	msg := createVMMessage(vmID, vmName, central.ResourceAction_UPDATE_RESOURCE)
...
func (suite *PipelineTestSuite) TestRun_UnsetAction() {
	vmID := "vm-1"
	vmName := "test-vm"
	msg := createVMMessage(vmID, vmName, central.ResourceAction_UNSET_ACTION_RESOURCE)
...
func (suite *PipelineTestSuite) TestRun_RemoveVirtualMachine() {
	vmID := "vm-1"
	vmName := "test-vm"
	msg := createVMMessage(vmID, vmName, central.ResourceAction_REMOVE_RESOURCE)

	// No expectations for datastore operations since removal is unsupported
	// The pipeline should log a warning and return without error

	err := suite.pipeline.Run(ctx, testClusterID, msg, nil)
	suite.NoError(err, "Should handle removal action gracefully")
}
...
func TestPipelineRun_DifferentActions(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			msg := createVMMessage("vm-1", "test-vm", tt.action)
...
func TestPipelineImplementsFragment(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()

	vmDatastore := vmDatastoreMocks.NewMockDataStore(ctrl)
	pipeline := newPipeline(vmDatastore)
	
	// Verify that pipelineImpl implements pipeline.Fragment interface
	// This is already verified at compile time by the var _ pipeline.Fragment = (*pipelineImpl)(nil) line
	// in the main pipeline.go file, so we just need to verify it returns the expected type
	_, ok := pipeline.(*pipelineImpl)
	assert.True(t, ok, "Should return pipelineImpl instance")
}
