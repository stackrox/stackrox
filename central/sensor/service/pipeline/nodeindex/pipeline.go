...
func (p *pipelineImpl) Run(ctx context.Context, _ string, msg *central.MsgFromSensor, injector common.MessageInjector) error {
	...
	nodeId := event.GetId()
	log.Debugf("Processing node index report: will query DB searching for node %s", nodeId)
	...
	log.Debugf("Processing node index report: found node %s in the DB", nodeId)
