...
//   - sensor's policy version is less than the minimum supported by central (see policyversion.MinimumSupportedVersionForSensor()), send version 1.1
...
func (c *sensorConnection) getPolicySyncMsgFromPolicies(policies []*storage.Policy) (*central.MsgToSensor, error) {
	...
	if sensorPolicyVersion, err := policyversion.FromString(sensorPolicyVersionStr); err != nil {
		...
		if policyversion.Compare(sensorPolicyVersion, policyversion.MinimumSupportedVersionForSensor()) < 0 {
