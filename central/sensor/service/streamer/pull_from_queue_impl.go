...
func (s *pullFromQueueImpl) Start(toPull pullable, outputChannel chan<- *central.MsgFromSensor) {
	go s.pullLoop(toPull, outputChannel)
}

// pullLoop grabs the next available output and pushes it to the channel when able.
func (s *pullFromQueueImpl) pullLoop(toPull pullable, outputChannel chan<- *central.MsgFromSensor) {
...
func (s *pullFromQueueImpl) pullUntilEmpty(toPull pullable, outputChannel chan<- *central.MsgFromSensor) {
	for {
