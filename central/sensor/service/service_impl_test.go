...
func (s *crsTestSuite) SetupSuite() {
	...
	s.context = sac.WithGlobalAccessScopeChecker(context.Background(),
...
func (s *crsTestSuite) TestCrsFlowFailsAfterLastContact() {
	...
	err = mockServer.clusterDataStore.UpdateCluster(s.context, cluster)
	s.NoError(err)
...
func (s *mockServer) getClusters() ([]*storage.Cluster, error) {
	ctx := sac.WithGlobalAccessScopeChecker(context.Background(), sac.AllowFixedScopes(
		sac.AccessModeScopeKeys(storage.Access_READ_ACCESS),
		sac.ResourceScopeKeys(resources.Cluster, resources.Administration, resources.Integration)))
	return s.clusterDataStore.GetClusters(ctx)
}
...
func (s *crsTestSuite) newSensorService(ctx context.Context, ctrl *gomock.Controller, crsMeta *storage.InitBundleMeta) (Service, *mockServer) {
	...
	clusterInitStore := clusterInitStore.GetTestPostgresDataStore(s.T(), s.db.DB)

	if crsMeta != nil {
		err := clusterInitStore.Add(s.context, crsMeta)
	...
	s.Require().NoError(err, "failed to obtain Postgres-backed cluster data store")
...
func newCrsMeta(maxRegistrations uint64) *storage.InitBundleMeta {
