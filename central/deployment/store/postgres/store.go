...
func (s *storeImpl) GetByQuery(ctx context.Context, query *v1.Query) ([]*storage.Deployment, error) {
	...
	rows, err := pgSearch.RunGetManyQueryForSchema[storage.Deployment](ctx, schema, query, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.Deployment) error) error {
	fetcher, closer, err := pgSearch.RunCursorQueryForSchema[storage.Deployment](ctx, schema, search.EmptyQuery(), s.db)
	if err != nil {
		return err
	}
	defer closer()
