...
func (ds *datastoreImpl) SearchListDeployments(ctx context.Context, q *v1.Query) ([]*storage.ListDeployment, error) {
	...
	deployments, _, err := ds.deploymentStore.GetManyListDeployments(ctx, ids...)
	if err != nil {
		return nil, err
	}

	ds.updateListDeploymentPriority(deployments...)
...
func (ds *datastoreImpl) SearchDeployments(ctx context.Context, q *v1.Query) ([]*v1.SearchResult, error) {
	...
	// TODO:  Refactor this and the list one above
	results, err := ds.Search(ctx, q)
	if err != nil {
		return nil, err
	}
	...
	ids := pkgSearch.ResultsToIDs(results)
	deployments, missingIndices, err := ds.deploymentStore.GetManyListDeployments(ctx, ids...)
	if err != nil {
		return nil, err
	}
...
func convertDeployment(deployment *storage.ListDeployment, result pkgSearch.Result) *v1.SearchResult {
