...
func (suite *SecretServiceTestSuite) TestGetSecret() {
	...
	suite.NoError(err)
	suite.True(protocompat.Equal(expectedSecret, actualSecretAndRelationship))
	suite.True(protocompat.Equal(expectedRelationship, actualSecretAndRelationship.GetRelationship()))
}
