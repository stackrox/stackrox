...
func (s *storeImpl) GetByQuery(ctx context.Context, query *v1.Query) ([]*storage.Secret, error) {
	...
	rows, err := pgSearch.RunGetManyQueryForSchema[storage.Secret](ctx, schema, query, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.Secret) error) error {
	fetcher, closer, err := pgSearch.RunCursorQueryForSchema[storage.Secret](ctx, schema, search.EmptyQuery(), s.db)
	if err != nil {
		return err
	}
	defer closer()
