...
const (
	...
	getWithRollupStmt = "select row_to_json((select record from (select table0.Id as Id, table0.Name as Name, table0.ClusterId as ClusterId, table0.ClusterName as ClusterName, table0.Namespace as Namespace, table0.Type as Type, table0.Labels as Labels, table0.Annotations as Annotations, table0.CreatedAt as CreatedAt, table0.Relationship_Id as Relationship_Id, to_json(join0)->'array' as join0, to_json(join1)->'array' as join1, to_json(join2)->'array' as join2 from secrets table0 left join lateral (select array(select json_build_object('idx', table1.idx, 'Name', table1.Name, 'Type', table1.Type, 'Cert_Subject_CommonName', table1.Cert_Subject_CommonName, 'Cert_Subject_Country', table1.Cert_Subject_Country, 'Cert_Subject_Organization', table1.Cert_Subject_Organization, 'Cert_Subject_OrganizationUnit', table1.Cert_Subject_OrganizationUnit, 'Cert_Subject_Locality', table1.Cert_Subject_Locality, 'Cert_Subject_Province', table1.Cert_Subject_Province, 'Cert_Subject_StreetAddress', table1.Cert_Subject_StreetAddress, 'Cert_Subject_PostalCode', table1.Cert_Subject_PostalCode, 'Cert_Subject_Names', table1.Cert_Subject_Names, 'Cert_Issuer_CommonName', table1.Cert_Issuer_CommonName, 'Cert_Issuer_Country', table1.Cert_Issuer_Country, 'Cert_Issuer_Organization', table1.Cert_Issuer_Organization, 'Cert_Issuer_OrganizationUnit', table1.Cert_Issuer_OrganizationUnit, 'Cert_Issuer_Locality', table1.Cert_Issuer_Locality, 'Cert_Issuer_Province', table1.Cert_Issuer_Province, 'Cert_Issuer_StreetAddress', table1.Cert_Issuer_StreetAddress, 'Cert_Issuer_PostalCode', table1.Cert_Issuer_PostalCode, 'Cert_Issuer_Names', table1.Cert_Issuer_Names, 'Cert_Sans', table1.Cert_Sans, 'Cert_StartDate', table1.Cert_StartDate, 'Cert_EndDate', table1.Cert_EndDate, 'Cert_Algorithm', table1.Cert_Algorithm, 'join0', to_json(join0)->'array') from secrets_Files table1 left join lateral (select array(select json_build_object('idx', table2.idx, 'Name', table2.Name, 'Username', table2.Username) from secrets_Files_Registries table2 where (table0.Id = table2.secrets_Id and table1.idx = table2.secrets_Files_idx))) join0 on true where (table0.Id = table1.secrets_Id))) join0 on true left join lateral (select array(select json_build_object('idx', table3.idx, 'Id', table3.Id, 'Path', table3.Path) from secrets_ContainerRelationships table3 where (table0.Id = table3.secrets_Id))) join1 on true left join lateral (select array(select json_build_object('idx', table4.idx, 'Id', table4.Id, 'Name', table4.Name) from secrets_DeploymentRelationships table4 where (table0.Id = table4.secrets_Id))) join2 on true where (table0.Id = $1)) record ))"
