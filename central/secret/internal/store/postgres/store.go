...
const createTableQuery = "create table if not exists secrets (id varchar primary key, value jsonb)"
...
//
	return &storeImpl{
		...
		countStmt: compileStmtOrPanic(db, "select count(*) from secrets"),
		existsStmt: compileStmtOrPanic(db, "select exists(select 1 from secrets where id = $1)"),
		...
		walkWithIDStmt: compileStmtOrPanic(db, "select id, value from secrets"),
	}
//
}
...
func (s *storeImpl) Get(id string) (*storage.Secret, bool, error) {
	...
	log.Infof("Took %d to query a Secret", time.Since(t).Milliseconds())
	...
	log.Infof("Took %d to scan a Secret", time.Since(t).Milliseconds())
	...
	log.Infof("Took %d to unmarshal a Secret", time.Since(t).Milliseconds())
