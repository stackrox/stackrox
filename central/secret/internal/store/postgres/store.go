...
const (
	...
	getWithRollupStmt = "select row_to_json((select table0_record from (select table0.Id as Id, table0.Name as Name, table0.ClusterId as ClusterId, table0.ClusterName as ClusterName, table0.Namespace as Namespace, table0.Type as Type, table0.Labels as Labels, table0.Annotations as Annotations, table0.CreatedAt as CreatedAt, table0.Relationship_Id as Relationship_Id, to_json(join0)->'array' as join0, to_json(join1)->'array' as join1, to_json(join2)->'array' as join2 from secrets table0 left join lateral (select array(select row_to_json((select table1_record from (select table1.idx as idx, table1.Name as Name, table1.Type as Type, table1.Cert_Subject_CommonName as Cert_Subject_CommonName, table1.Cert_Subject_Country as Cert_Subject_Country, table1.Cert_Subject_Organization as Cert_Subject_Organization, table1.Cert_Subject_OrganizationUnit as Cert_Subject_OrganizationUnit, table1.Cert_Subject_Locality as Cert_Subject_Locality, table1.Cert_Subject_Province as Cert_Subject_Province, table1.Cert_Subject_StreetAddress as Cert_Subject_StreetAddress, table1.Cert_Subject_PostalCode as Cert_Subject_PostalCode, table1.Cert_Subject_Names as Cert_Subject_Names, table1.Cert_Issuer_CommonName as Cert_Issuer_CommonName, table1.Cert_Issuer_Country as Cert_Issuer_Country, table1.Cert_Issuer_Organization as Cert_Issuer_Organization, table1.Cert_Issuer_OrganizationUnit as Cert_Issuer_OrganizationUnit, table1.Cert_Issuer_Locality as Cert_Issuer_Locality, table1.Cert_Issuer_Province as Cert_Issuer_Province, table1.Cert_Issuer_StreetAddress as Cert_Issuer_StreetAddress, table1.Cert_Issuer_PostalCode as Cert_Issuer_PostalCode, table1.Cert_Issuer_Names as Cert_Issuer_Names, table1.Cert_Sans as Cert_Sans, table1.Cert_StartDate as Cert_StartDate, table1.Cert_EndDate as Cert_EndDate, table1.Cert_Algorithm as Cert_Algorithm, to_json(join0)->'array' as join0 from secrets_Files table1 left join lateral (select array(select row_to_json((select table2_record from (select table2.idx as idx, table2.Name as Name, table2.Username as Username from secrets_Files_Registries table2 where (table2) ) table0.Id = table2.secrets_Id and table1.idx = table2.secrets_Files_idx_record )))) join0 on true where (table1) ) table0.Id = table1.secrets_Id_record )))) join0 on true left join lateral (select array(select row_to_json((select table3_record from (select table3.idx as idx, table3.Id as Id, table3.Path as Path from secrets_ContainerRelationships table3 where (table3) ) table0.Id = table3.secrets_Id_record )))) join1 on true left join lateral (select array(select row_to_json((select table4_record from (select table4.idx as idx, table4.Id as Id, table4.Name as Name from secrets_DeploymentRelationships table4 where (table4) ) table0.Id = table4.secrets_Id_record )))) join2 on true where (table0) ) table0.Id = $1_record ))"
