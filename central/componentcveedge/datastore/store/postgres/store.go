...
func (s *storeImpl) Get(ctx context.Context, id string) (*storage.ComponentCVEEdge, bool, error) {
	...
	data, err := postgres.RunGetQueryForSchemaType[storage.ComponentCVEEdge](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetMany(ctx context.Context, ids []string) ([]*storage.ComponentCVEEdge, []int, error) {
	...
	rows, err := postgres.RunGetManyQueryForSchemaType[storage.ComponentCVEEdge](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.ComponentCVEEdge) error) error {
	var sacQueryFilter *v1.Query
	fetcher, closer, err := postgres.RunCursorQueryForSchemaType[storage.ComponentCVEEdge](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
