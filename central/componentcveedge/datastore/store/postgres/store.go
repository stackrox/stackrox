...
func (s *storeImpl) GetByQuery(ctx context.Context, query *v1.Query) ([]*storage.ComponentCVEEdge, error) {
	...
	rows, err := pgSearch.RunGetManyQueryForSchema[storage.ComponentCVEEdge](ctx, schema, query, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.ComponentCVEEdge) error) error {
	fetcher, closer, err := pgSearch.RunCursorQueryForSchema[storage.ComponentCVEEdge](ctx, schema, search.EmptyQuery(), s.db)
	if err != nil {
		return err
	}
	defer closer()
