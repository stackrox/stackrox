...
const (
	...
	existsStmt = "SELECT EXISTS(SELECT 1 FROM WatchedImage WHERE )"

	getStmt    = "SELECT serialized FROM WatchedImage WHERE "
	deleteStmt = "DELETE FROM WatchedImage WHERE "
...
type Store interface {
	Count() (int, error)
	Exists() (bool, error)
	Get() (*storage.WatchedImage, bool, error)
...
func New(db *pgxpool.Pool) Store {
	...
	for _, table := range []string{
		"create table if not exists WatchedImage(serialized jsonb not null, PRIMARY KEY ());",
	} {
...
func (s *storeImpl) Get() (*storage.WatchedImage, bool, error) {
...
func (s *storeImpl) Upsert(obj0 *storage.WatchedImage) error {
	...
	localQuery := "insert into WatchedImage(serialized) values($1) on conflict() do update set serialized = EXCLUDED.serialized"
