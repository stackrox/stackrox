...
type Store interface {
	...
	GetMany(ctx context.Context, identifiers []string) ([]*storage.WatchedImage, []int, error)
...
type storeImpl struct {
	*pgSearch.GenericStore[storage.WatchedImage, *storage.WatchedImage]
	db    postgres.DB
	mutex sync.RWMutex
}
...
func New(db postgres.DB) Store {
	return &storeImpl{
		db: db,
		GenericStore: pgSearch.NewGenericStore[storage.WatchedImage, *storage.WatchedImage](
			db,
			schema,
			pkGetter,
...
func pkGetter(obj *storage.WatchedImage) string {
	return obj.GetName()
}

func metricsSetPostgresOperationDurationTime(start time.Time, op ops.Op) {
	metrics.SetPostgresOperationDurationTime(start, op, "WatchedImage")
}

func metricsSetAcquireDBConnDuration(start time.Time, op ops.Op) {
	metrics.SetAcquireDBConnDuration(start, op, "WatchedImage")
}
...
func (s *storeImpl) Get(ctx context.Context, name string) (*storage.WatchedImage, bool, error) {
	...
	data, err := pgSearch.RunGetQueryForSchema[storage.WatchedImage](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetMany(ctx context.Context, identifiers []string) ([]*storage.WatchedImage, []int, error) {
	...
	rows, err := pgSearch.RunGetManyQueryForSchema[storage.WatchedImage](ctx, schema, q, s.db)
	if err != nil {
	...
	resultsByID := make(map[string]*storage.WatchedImage, len(rows))
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.WatchedImage) error) error {
	...
	fetcher, closer, err := pgSearch.RunCursorQueryForSchema[storage.WatchedImage](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
...
func dropTableWatchedImages(ctx context.Context, db postgres.DB) {
