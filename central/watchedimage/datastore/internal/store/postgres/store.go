...
func (s *storeImpl) Get(ctx context.Context, name string) (*storage.WatchedImage, bool, error) {
	...
	data, err := postgres.RunGetQueryForSchemaType[storage.WatchedImage](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetMany(ctx context.Context, ids []string) ([]*storage.WatchedImage, []int, error) {
	...
	rows, err := postgres.RunGetManyQueryForSchemaType[storage.WatchedImage](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.WatchedImage) error) error {
	...
	fetcher, closer, err := postgres.RunCursorQueryForSchemaType[storage.WatchedImage](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
