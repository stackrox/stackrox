...
func (s *PolicyServiceTestSuite) SetupTest() {
	...
	s.networkPolicies = networkPolicyMocks.NewMockDataStore(s.mockCtrl)
	...
	for _, c := range cases {
		s.T().Run(c.name, func(t *testing.T) {
			s.deployments.EXPECT().
				GetDeploymentIDs(gomock.Any()).
				Return([]string{"deployment-1"}, nil)

			s.deployments.EXPECT().
				GetDeployment(gomock.Any(), "deployment-1").
				Return(testDeployment, true, nil)

			s.deployments.EXPECT().
				GetImagesForDeployment(gomock.Any(), testDeployment).
				Return(nil, nil)

			s.networkPolicies.EXPECT().
				GetNetworkPolicies(gomock.Any(), "cluster-1", "namespace-1").
				Return(nil, nil)
