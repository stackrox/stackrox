...
func (s *storeImpl) GetByQuery(ctx context.Context, query *v1.Query) ([]*storage.Policy, error) {
	...
	rows, err := pgSearch.RunGetManyQueryForSchema[storage.Policy](ctx, schema, query, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.Policy) error) error {
	fetcher, closer, err := pgSearch.RunCursorQueryForSchema[storage.Policy](ctx, schema, search.EmptyQuery(), s.db)
	if err != nil {
		return err
	}
	defer closer()
