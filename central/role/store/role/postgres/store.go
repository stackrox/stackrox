...
func (s *storeImpl) Get(ctx context.Context, name string) (*storage.Role, bool, error) {
	...
	data, err := postgres.RunGetQueryForSchemaType[storage.Role](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetMany(ctx context.Context, ids []string) ([]*storage.Role, []int, error) {
	...
	rows, err := postgres.RunGetManyQueryForSchemaType[storage.Role](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.Role) error) error {
	...
	fetcher, closer, err := postgres.RunCursorQueryForSchemaType[storage.Role](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
