...
const (
	...
	getStmt        = "SELECT t1.serialized FROM roles t1 WHERE t1.Name = $1"
	deleteStmt     = "DELETE FROM roles t1 WHERE t1.Name = $1"
	walkStmt       = "SELECT serialized FROM roles"
	existsStmt     = "SELECT EXISTS(SELECT 1 FROM roles t1 WHERE t1.Name = $1)"
	getIDsStmt     = "SELECT distinct Name FROM roles"
