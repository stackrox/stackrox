...
const (
	countStmt  = "SELECT COUNT(*) FROM Role"
	existsStmt = "SELECT EXISTS(SELECT 1 FROM Role WHERE )"

	getStmt    = "SELECT serialized FROM Role WHERE "
	deleteStmt = "DELETE FROM Role WHERE "
	walkStmt   = "SELECT serialized FROM Role"
)
...
func New(db *pgxpool.Pool) Store {
	...
	for _, table := range []string{
		"create table if not exists Role(serialized jsonb not null, PRIMARY KEY ());",
	} {
...
func (s *storeImpl) Get() (*storage.Role, bool, error) {
	defer metrics.SetPostgresOperationDurationTime(time.Now(), ops.Get, "Role")

	conn, release := s.acquireConn(ops.Get, "Role")
	defer release()
	...
	defer metrics.SetJSONPBOperationDurationTime(time.Now(), "Unmarshal", "Role")
...
func (s *storeImpl) Upsert(obj0 *storage.Role) error {
	...
	metrics.SetJSONPBOperationDurationTime(t, "Marshal", "Role")
	conn, release := s.acquireConn(ops.Add, "Role")
	defer release()
	...
	localQuery := "insert into Role(serialized) values($1) on conflict() do update set serialized = EXCLUDED.serialized"
...
func (s *storeImpl) UpsertMany(objs []*storage.Role) error {
	...
	conn, release := s.acquireConn(ops.AddMany, "Role")
	defer release()
...
func (s *storeImpl) Delete() error {
	...
	conn, release := s.acquireConn(ops.Remove, "Role")
	defer release()
