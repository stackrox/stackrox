...
const (
	...
	getWithRollupStmt = "select row_to_json((select record from (select table0.Id as Id, table0.Name as Name, table0.Description as Description, table0.Rules_IncludedClusters as Rules_IncludedClusters, to_json(join0)->'array' as join0, to_json(join1)->'array' as join1, to_json(join2)->'array' as join2 from simpleaccessscopes table0 left join lateral (select array(select json_build_object('idx', table1.idx, 'ClusterName', table1.ClusterName, 'NamespaceName', table1.NamespaceName) from simpleaccessscopes_IncludedNamespaces table1 where (table0.Id = table1.simpleaccessscopes_Id))) join0 on true left join lateral (select array(select json_build_object('idx', table2.idx, 'join0', to_json(join0)->'array') from simpleaccessscopes_ClusterLabelSelectors table2 left join lateral (select array(select json_build_object('idx', table3.idx, 'Key', table3.Key, 'Op', table3.Op, 'Values', table3.Values) from simpleaccessscopes_ClusterLabelSelectors_Requirements table3 where (table0.Id = table3.simpleaccessscopes_Id and table2.idx = table3.simpleaccessscopes_ClusterLabelSelectors_idx))) join0 on true where (table0.Id = table2.simpleaccessscopes_Id))) join1 on true left join lateral (select array(select json_build_object('idx', table4.idx, 'join0', to_json(join0)->'array') from simpleaccessscopes_NamespaceLabelSelectors table4 left join lateral (select array(select json_build_object('idx', table5.idx, 'Key', table5.Key, 'Op', table5.Op, 'Values', table5.Values) from simpleaccessscopes_NamespaceLabelSelectors_Requirements table5 where (table0.Id = table5.simpleaccessscopes_Id and table4.idx = table5.simpleaccessscopes_NamespaceLabelSelectors_idx))) join0 on true where (table0.Id = table4.simpleaccessscopes_Id))) join2 on true where (table0.Id = $1)) record ))"
