...
const createTableQuery = "create table if not exists simple_access_scopes (id varchar primary key, value jsonb)"
...
func (s *storeImpl) Get(id string) (*storage.SimpleAccessScope, bool, error) {
	...
	log.Infof("Took %d to query a SimpleAccessScope", time.Since(t).Milliseconds())
	...
	log.Infof("Took %d to scan a SimpleAccessScope", time.Since(t).Milliseconds())
	...
	log.Infof("Took %d to unmarshal a SimpleAccessScope", time.Since(t).Milliseconds())
