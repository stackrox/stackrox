...
func (b *storeImpl) Count(_ context.Context) (int, error) {
	defer metrics.SetRocksDBOperationDurationTime(time.Now(), ops.Count, "SimpleAccessScope")
...
func (b *storeImpl) Exists(_ context.Context, id string) (bool, error) {
	defer metrics.SetRocksDBOperationDurationTime(time.Now(), ops.Exists, "SimpleAccessScope")
...
func (b *storeImpl) GetIDs(_ context.Context) ([]string, error) {
	defer metrics.SetRocksDBOperationDurationTime(time.Now(), ops.GetAll, "SimpleAccessScopeIDs")

	return b.crud.GetKeys()
}

// Get returns the object, if it exists from the store
func (b *storeImpl) Get(_ context.Context, id string) (*storage.SimpleAccessScope, bool, error) {
	defer metrics.SetRocksDBOperationDurationTime(time.Now(), ops.Get, "SimpleAccessScope")
...
func (b *storeImpl) GetMany(_ context.Context, ids []string) ([]*storage.SimpleAccessScope, []int, error) {
	defer metrics.SetRocksDBOperationDurationTime(time.Now(), ops.GetMany, "SimpleAccessScope")

	msgs, missingIndices, err := b.crud.GetMany(ids)
	if err != nil {
		return nil, nil, err
	}
	objs := make([]*storage.SimpleAccessScope, 0, len(msgs))
	for _, m := range msgs {
		objs = append(objs, m.(*storage.SimpleAccessScope))
	}
	return objs, missingIndices, nil
}

// Upsert inserts the object into the DB
func (b *storeImpl) Upsert(_ context.Context, obj *storage.SimpleAccessScope) error {
	defer metrics.SetRocksDBOperationDurationTime(time.Now(), ops.Add, "SimpleAccessScope")
...
func (b *storeImpl) UpsertMany(_ context.Context, objs []*storage.SimpleAccessScope) error {
	defer metrics.SetRocksDBOperationDurationTime(time.Now(), ops.AddMany, "SimpleAccessScope")
...
func (b *storeImpl) Delete(_ context.Context, id string) error {
	defer metrics.SetRocksDBOperationDurationTime(time.Now(), ops.Remove, "SimpleAccessScope")
...
func (b *storeImpl) DeleteMany(_ context.Context, ids []string) error {
	defer metrics.SetRocksDBOperationDurationTime(time.Now(), ops.RemoveMany, "SimpleAccessScope")

	return b.crud.DeleteMany(ids)
}

// Walk iterates over all of the objects in the store and applies the closure
func (b *storeImpl) Walk(ctx context.Context, fn func(obj *storage.SimpleAccessScope) error) error {
