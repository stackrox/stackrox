...
func New(db *rocksdb.RocksDB) (Store, error) {
	globaldb.RegisterBucket(bucket, "PermissionSet")
...
func (b *storeImpl) Count() (int, error) {
	defer metrics.SetRocksDBOperationDurationTime(time.Now(), ops.Count, "PermissionSet")
...
func (b *storeImpl) Exists(id string) (bool, error) {
	defer metrics.SetRocksDBOperationDurationTime(time.Now(), ops.Exists, "PermissionSet")
...
func (b *storeImpl) GetIDs() ([]string, error) {
	defer metrics.SetRocksDBOperationDurationTime(time.Now(), ops.GetAll, "PermissionSetIDs")

	return b.crud.GetKeys()
}

// Get returns the object, if it exists from the store
func (b *storeImpl) Get(id string) (*storage.PermissionSet, bool, error) {
	defer metrics.SetRocksDBOperationDurationTime(time.Now(), ops.Get, "PermissionSet")
...
func (b *storeImpl) GetMany(ids []string) ([]*storage.PermissionSet, []int, error) {
	defer metrics.SetRocksDBOperationDurationTime(time.Now(), ops.GetMany, "PermissionSet")

	msgs, missingIndices, err := b.crud.GetMany(ids)
	if err != nil {
		return nil, nil, err
	}
	objs := make([]*storage.PermissionSet, 0, len(msgs))
	for _, m := range msgs {
		objs = append(objs, m.(*storage.PermissionSet))
	}
	return objs, missingIndices, nil
}

// Upsert inserts the object into the DB
func (b *storeImpl) Upsert(obj *storage.PermissionSet) error {
	defer metrics.SetRocksDBOperationDurationTime(time.Now(), ops.Add, "PermissionSet")

	return b.crud.Upsert(obj)
}

// UpsertMany batches objects into the DB
func (b *storeImpl) UpsertMany(objs []*storage.PermissionSet) error {
	defer metrics.SetRocksDBOperationDurationTime(time.Now(), ops.AddMany, "PermissionSet")
...
func (b *storeImpl) Delete(id string) error {
	defer metrics.SetRocksDBOperationDurationTime(time.Now(), ops.Remove, "PermissionSet")
...
func (b *storeImpl) DeleteMany(ids []string) error {
	defer metrics.SetRocksDBOperationDurationTime(time.Now(), ops.RemoveMany, "PermissionSet")

	return b.crud.DeleteMany(ids)
}

// Walk iterates over all of the objects in the store and applies the closure
func (b *storeImpl) Walk(fn func(obj *storage.PermissionSet) error) error {
