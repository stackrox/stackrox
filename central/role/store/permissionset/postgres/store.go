...
func (s *storeImpl) Get(ctx context.Context, id string) (*storage.PermissionSet, bool, error) {
	...
	data, err := pgSearch.RunGetQueryForSchema[storage.PermissionSet](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetMany(ctx context.Context, identifiers []string) ([]*storage.PermissionSet, []int, error) {
	...
	rows, err := pgSearch.RunGetManyQueryForSchema[storage.PermissionSet](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.PermissionSet) error) error {
	fetcher, closer, err := pgSearch.RunCursorQueryForSchema[storage.PermissionSet](ctx, schema, search.EmptyQuery(), s.db)
	if err != nil {
		return err
	}
	defer closer()
