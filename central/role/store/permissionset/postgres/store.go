...
const (
	countStmt  = "SELECT COUNT(*) FROM PermissionSet"
	existsStmt = "SELECT EXISTS(SELECT 1 FROM PermissionSet WHERE )"

	getStmt    = "SELECT serialized FROM PermissionSet WHERE "
	deleteStmt = "DELETE FROM PermissionSet WHERE "
	walkStmt   = "SELECT serialized FROM PermissionSet"
)
...
type Store interface {
	Count() (int, error)
	Exists() (bool, error)
	Get() (*storage.PermissionSet, bool, error)
...
func New(db *pgxpool.Pool) Store {
	...
	for _, table := range []string{
		"create table if not exists PermissionSet(serialized jsonb not null, PRIMARY KEY ());",
	} {
...
func (s *storeImpl) Get() (*storage.PermissionSet, bool, error) {
	defer metrics.SetPostgresOperationDurationTime(time.Now(), ops.Get, "PermissionSet")

	conn, release := s.acquireConn(ops.Get, "PermissionSet")
	defer release()
	...
	defer metrics.SetJSONPBOperationDurationTime(time.Now(), "Unmarshal", "PermissionSet")
...
func (s *storeImpl) Upsert(obj0 *storage.PermissionSet) error {
	...
	metrics.SetJSONPBOperationDurationTime(t, "Marshal", "PermissionSet")
	conn, release := s.acquireConn(ops.Add, "PermissionSet")
	defer release()
	...
	localQuery := "insert into PermissionSet(serialized) values($1) on conflict() do update set serialized = EXCLUDED.serialized"
...
func (s *storeImpl) UpsertMany(objs []*storage.PermissionSet) error {
	...
	conn, release := s.acquireConn(ops.AddMany, "PermissionSet")
	defer release()
...
func (s *storeImpl) Delete() error {
	...
	conn, release := s.acquireConn(ops.Remove, "PermissionSet")
	defer release()
