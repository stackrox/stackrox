...
func TestFilterCurrent(t *testing.T) {
	...
	assert.Equal(t, int32(0), bm.Sr.Nodes)
	assert.Equal(t, int32(0), bm.Sr.Cores)

	inject(c)

	ids.Add("test1")
	bm = c.FilterCurrent(ids) // removes test2 values, as not present in ids
	assert.Equal(t, int32(1), bm.Sr.Nodes)
	assert.Equal(t, int32(10), bm.Sr.Cores)
	ids.Add("test2")
	bm = c.FilterCurrent(ids)
	assert.Equal(t, int32(1), bm.Sr.Nodes)
	assert.Equal(t, int32(10), bm.Sr.Cores)
	...
	bm = c.FilterCurrent(ids)
	assert.Equal(t, int32(3), bm.Sr.Nodes)
	assert.Equal(t, int32(30), bm.Sr.Cores)
}
