...
func (s *storeImpl) Insert(ctx context.Context, obj *storage.Usage) error {
...
func (s *storeImpl) retryableInsert(ctx context.Context, rec *storage.Usage) error {
	...
	defer release()
	_, err = conn.Exec(ctx, insertStmt, protoconv.ConvertTimestampToTimeOrNow(rec.GetTimestamp()), serialized)
	return err
}
