...
func (s *storeImpl) Get(ctx context.Context, id string) (*storage.NodeCVE, bool, error) {
	...
	data, err := postgres.RunGetQueryForSchemaType[storage.NodeCVE](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetMany(ctx context.Context, ids []string) ([]*storage.NodeCVE, []int, error) {
	...
	rows, err := postgres.RunGetManyQueryForSchemaType[storage.NodeCVE](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.NodeCVE) error) error {
	var sacQueryFilter *v1.Query
	fetcher, closer, err := postgres.RunCursorQueryForSchemaType[storage.NodeCVE](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
