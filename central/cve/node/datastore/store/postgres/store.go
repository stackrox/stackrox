...
func (s *storeImpl) GetByQuery(ctx context.Context, query *v1.Query) ([]*storage.NodeCVE, error) {
	...
	rows, err := pgSearch.RunGetManyQueryForSchema[storage.NodeCVE](ctx, schema, query, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.NodeCVE) error) error {
	fetcher, closer, err := pgSearch.RunCursorQueryForSchema[storage.NodeCVE](ctx, schema, search.EmptyQuery(), s.db)
	if err != nil {
		return err
	}
	defer closer()
