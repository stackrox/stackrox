...
func insertIntoClusterCves(batch *pgx.Batch, pool pgSearch.BufferPool, obj *storage.ClusterCVE) (*[]byte, error) {
...
func copyFromClusterCves(ctx context.Context, s pgSearch.Deleter, tx *postgres.Tx, pool pgSearch.BufferPool, objs ...*storage.ClusterCVE) error {
	if len(objs) == 0 {
		return nil
	}
