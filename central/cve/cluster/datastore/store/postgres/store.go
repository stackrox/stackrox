...
func (s *storeImpl) GetByQuery(ctx context.Context, query *v1.Query) ([]*storage.ClusterCVE, error) {
	...
	rows, err := pgSearch.RunGetManyQueryForSchema[storage.ClusterCVE](ctx, schema, query, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.ClusterCVE) error) error {
	fetcher, closer, err := pgSearch.RunCursorQueryForSchema[storage.ClusterCVE](ctx, schema, search.EmptyQuery(), s.db)
	if err != nil {
		return err
	}
	defer closer()
