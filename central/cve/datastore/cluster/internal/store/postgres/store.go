...
create table if not exists cluster_cves_References (
    ...
    cluster_cves_OperatingSystem varchar,
    idx integer,
    URI varchar,
    Tags text[],
    PRIMARY KEY(cluster_cves_Id, cluster_cves_OperatingSystem, idx),
    CONSTRAINT fk_parent_table_0 FOREIGN KEY (cluster_cves_Id, cluster_cves_OperatingSystem) REFERENCES cluster_cves(Id, OperatingSystem) ON DELETE CASCADE
)
`
...
func insertIntoClusterCves(ctx context.Context, tx pgx.Tx, obj *storage.CVE) error {
	...
	for childIdx, child := range obj.GetReferences() {
		if err := insertIntoClusterCvesReferences(ctx, tx, child, obj.GetId(), obj.GetOperatingSystem(), childIdx); err != nil {
			return err
		}
	}

	query = "delete from cluster_cves_References where cluster_cves_Id = $1 AND cluster_cves_OperatingSystem = $2 AND idx >= $3"
...
func insertIntoClusterCvesReferences(ctx context.Context, tx pgx.Tx, obj *storage.CVE_Reference, cluster_cves_Id string, cluster_cves_OperatingSystem string, idx int) error {

	values := []interface{}{
		// parent primary keys start
		cluster_cves_Id,
		cluster_cves_OperatingSystem,
		idx,
		obj.GetURI(),
		obj.GetTags(),
	}

	finalStr := "INSERT INTO cluster_cves_References (cluster_cves_Id, cluster_cves_OperatingSystem, idx, URI, Tags) VALUES($1, $2, $3, $4, $5) ON CONFLICT(cluster_cves_Id, cluster_cves_OperatingSystem, idx) DO UPDATE SET cluster_cves_Id = EXCLUDED.cluster_cves_Id, cluster_cves_OperatingSystem = EXCLUDED.cluster_cves_OperatingSystem, idx = EXCLUDED.idx, URI = EXCLUDED.URI, Tags = EXCLUDED.Tags"
	_, err := tx.Exec(ctx, finalStr, values...)
	if err != nil {
		return err
	}
...
func (s *storeImpl) copyFromClusterCves(ctx context.Context, tx pgx.Tx, objs ...*storage.CVE) error {
	...
	for _, obj := range objs {
		...
		if err = s.copyFromClusterCvesReferences(ctx, tx, obj.GetId(), obj.GetOperatingSystem(), obj.GetReferences()...); err != nil {
			return err
		}
	}
...
func (s *storeImpl) copyFromClusterCvesReferences(ctx context.Context, tx pgx.Tx, cluster_cves_Id string, cluster_cves_OperatingSystem string, objs ...*storage.CVE_Reference) error {
