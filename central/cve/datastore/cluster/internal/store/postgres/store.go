...
const (
	...
	getStmt    = "SELECT t1.serialized FROM cluster_cves t1 WHERE t1.Id = $1 AND t1.OperatingSystem = $2"
	deleteStmt = "DELETE FROM cluster_cves t1 WHERE t1.Id = $1 AND t1.OperatingSystem = $2"
	walkStmt   = "SELECT serialized FROM cluster_cves"
	existsStmt = "SELECT EXISTS(SELECT 1 FROM cluster_cves t1 WHERE t1.Id = $1 AND t1.OperatingSystem = $2)"
