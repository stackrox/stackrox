...
create table if not exists cluster_cves_References (
    ...
    CONSTRAINT fk_parent_table_0 FOREIGN KEY (cveid) REFERENCES cluster_cves(id) ON DELETE CASCADE
)
`
...
func insertIntoClusterCves(ctx context.Context, tx pgx.Tx, obj *storage.CVE) error {
	...
	query = "delete from cluster_cves_References where cveid = $1 AND idx >= $2"
...
func insertIntoClusterCvesReferences(ctx context.Context, tx pgx.Tx, obj *storage.CVE_Reference, cveid string, idx int) error {
	...
	finalStr := "INSERT INTO cluster_cves_References (cveid, idx, uri, tags) VALUES($1, $2, $3, $4) ON CONFLICT(cveid, idx) DO UPDATE SET cveid = EXCLUDED.cveid, idx = EXCLUDED.idx, uri = EXCLUDED.uri, tags = EXCLUDED.tags"
	_, err := tx.Exec(ctx, finalStr, values...)
	if err != nil {
		return err
	}
...
func (s *storeImpl) copyFromClusterCvesReferences(ctx context.Context, tx pgx.Tx, cveid string, objs ...*storage.CVE_Reference) error {
