...
const (
	...
	getStmt    = "SELECT t1.serialized FROM image_cves t1 WHERE t1.Id = $1 AND t1.OperatingSystem = $2"
	deleteStmt = "DELETE FROM image_cves t1 WHERE t1.Id = $1 AND t1.OperatingSystem = $2"
	walkStmt   = "SELECT serialized FROM image_cves"
	existsStmt = "SELECT EXISTS(SELECT 1 FROM image_cves t1 WHERE t1.Id = $1 AND t1.OperatingSystem = $2)"
