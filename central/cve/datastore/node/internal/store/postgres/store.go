...
create table if not exists node_cves_References (
    ...
    node_cves_OperatingSystem varchar,
    idx integer,
    URI varchar,
    Tags text[],
    PRIMARY KEY(node_cves_Id, node_cves_OperatingSystem, idx),
    CONSTRAINT fk_parent_table_0 FOREIGN KEY (node_cves_Id, node_cves_OperatingSystem) REFERENCES node_cves(Id, OperatingSystem) ON DELETE CASCADE
)
`
...
func insertIntoNodeCves(ctx context.Context, tx pgx.Tx, obj *storage.CVE) error {
	...
	for childIdx, child := range obj.GetReferences() {
		if err := insertIntoNodeCvesReferences(ctx, tx, child, obj.GetId(), obj.GetOperatingSystem(), childIdx); err != nil {
			return err
		}
	}

	query = "delete from node_cves_References where node_cves_Id = $1 AND node_cves_OperatingSystem = $2 AND idx >= $3"
...
func insertIntoNodeCvesReferences(ctx context.Context, tx pgx.Tx, obj *storage.CVE_Reference, node_cves_Id string, node_cves_OperatingSystem string, idx int) error {

	values := []interface{}{
		// parent primary keys start
		node_cves_Id,
		node_cves_OperatingSystem,
		idx,
		obj.GetURI(),
		obj.GetTags(),
	}

	finalStr := "INSERT INTO node_cves_References (node_cves_Id, node_cves_OperatingSystem, idx, URI, Tags) VALUES($1, $2, $3, $4, $5) ON CONFLICT(node_cves_Id, node_cves_OperatingSystem, idx) DO UPDATE SET node_cves_Id = EXCLUDED.node_cves_Id, node_cves_OperatingSystem = EXCLUDED.node_cves_OperatingSystem, idx = EXCLUDED.idx, URI = EXCLUDED.URI, Tags = EXCLUDED.Tags"
	_, err := tx.Exec(ctx, finalStr, values...)
	if err != nil {
		return err
	}
...
func (s *storeImpl) copyFromNodeCves(ctx context.Context, tx pgx.Tx, objs ...*storage.CVE) error {
	...
	for _, obj := range objs {
		...
		if err = s.copyFromNodeCvesReferences(ctx, tx, obj.GetId(), obj.GetOperatingSystem(), obj.GetReferences()...); err != nil {
			return err
		}
	}
...
func (s *storeImpl) copyFromNodeCvesReferences(ctx context.Context, tx pgx.Tx, node_cves_Id string, node_cves_OperatingSystem string, objs ...*storage.CVE_Reference) error {
