...
func insertIntoImageCves(batch *pgx.Batch, pool pgSearch.BufferPool, obj *storage.ImageCVE) (*[]byte, error) {
...
func copyFromImageCves(ctx context.Context, s pgSearch.Deleter, tx *postgres.Tx, pool pgSearch.BufferPool, objs ...*storage.ImageCVE) error {
	if len(objs) == 0 {
		return nil
	}
