...
func (s *storeImpl) Get(ctx context.Context, id string) (*storage.ImageCVE, bool, error) {
	...
	data, err := postgres.RunGetQueryForSchemaType[storage.ImageCVE](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetMany(ctx context.Context, ids []string) ([]*storage.ImageCVE, []int, error) {
	...
	rows, err := postgres.RunGetManyQueryForSchemaType[storage.ImageCVE](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.ImageCVE) error) error {
	var sacQueryFilter *v1.Query
	fetcher, closer, err := postgres.RunCursorQueryForSchemaType[storage.ImageCVE](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
