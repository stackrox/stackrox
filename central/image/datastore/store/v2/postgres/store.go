...
func (s *storeImpl) retryableUpdateVulnState(ctx context.Context, cve string, imageIDs []string, state storage.VulnerabilityState) error {
	...
	rows, err := tx.Query(ctx, "SELECT image_component_v2_id, idx, serialized FROM "+imageComponentsV2CVEsTable+" "+
		"WHERE "+imageComponentsV2CVEsTable+".imageid = ANY($1::text[]) AND "+imageComponentsV2CVEsTable+".cvebaseinfo_cve = $2", imageIDs, cve)
	if err != nil {
		return err
	}
	defer rows.Close()
	var imageCVEs []*componentCVE
	for rows.Next() {
		var componentID string
		var cveIndex int
		var serialized []byte

		if err := rows.Scan(&componentID, &cveIndex, &serialized); err != nil {
			return pgutils.ErrNilIfNoRows(err)
		}
		...
		compCVE := &componentCVE{
			componentID: componentID,
			cveIndex:    cveIndex,
			cve:         &msg,
		}

		imageCVEs = append(imageCVEs, compCVE)
	}

	// Update state.
	componentIDs := make([]string, 0, len(imageCVEs))
	for _, compCVE := range imageCVEs {
		compCVE.cve.State = state
		componentIDs = append(componentIDs, compCVE.componentID)
	}

	// Construct keys to lock.
	keys := make([][]byte, 0, len(componentIDs)+len(imageIDs))
...
func (s *storeImpl) updateCVEVulnState(ctx context.Context, tx *postgres.Tx, objs ...*componentCVE) error {
	batch := &pgx.Batch{}
	for _, obj := range objs {
		if err := s.insertIntoImageComponentV2Cves(batch, obj.cve, obj.componentID, obj.cveIndex); err != nil {
...
func (s *storeImpl) insertIntoImageComponentV2Cves(batch *pgx.Batch, obj *storage.ImageCVEV2, imageComponentV2ID string, idx int) error {
