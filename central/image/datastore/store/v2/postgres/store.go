...
const (
	...
	imageComponentsV2CVEsTable = pkgSchema.ImageComponentV2CvesTableName
...
func (s *storeImpl) insertIntoImages(
	...
	for _, component := range existingComponents {
		...
		for _, cve := range component.GetCves() {
	...
	return copyFromImageComponentsV2(ctx, tx, iTime, componentCVEMap, parts.componentsV2...)
}
...
// TODO(ROX-27402) Can we just use the method in the image component store?  Need to check to see if the transaction object
// is exposed in the public funcs
// TODO(ROX-27402) Verify the parameters if we have to keep it.  Probably have to keep it to make sure various times and stats are passed correctly
func copyFromImageComponentsV2(ctx context.Context, tx *postgres.Tx, iTime time.Time, componentCVEMap map[string]map[string]*storage.ImageCVEV2, objs ...*storage.ImageComponentV2) error {
	...
	for idx, obj := range objs {
		...
		if err := copyFromImageComponentV2Cves(ctx, tx, iTime, componentCVEMap[obj.GetId()], obj.GetId(), obj.GetCves()...); err != nil {
			return err
		}
	}
...
// TODO(ROX-27402) Can we just use the method in the image component store?  Need to check to see if the transaction object
// is exposed in the public funcs
// TODO(ROX-27402) Verify the parameters if we have to keep it.
func copyFromImageComponentV2Cves(ctx context.Context, tx *postgres.Tx, iTime time.Time, existingCVEs map[string]*storage.ImageCVEV2, imageComponentV2ID string, objs ...*storage.ImageCVEV2) error {
	batchSize := pgSearch.MaxBatchSize
	if len(objs) < batchSize {
		batchSize = len(objs)
	}
...
// TODO(ROX-27402): make sure populating all the data appropriately
...
func getImageComponents(ctx context.Context, tx *postgres.Tx, imageIDs string) ([]*storage.ImageComponentV2, error) {
	defer metrics.SetPostgresOperationDurationTime(time.Now(), ops.Get, "ImageComponents")

	rows, err := tx.Query(ctx, "SELECT serialized FROM "+imageComponentsV2Table+" WHERE image_id = $1", imageIDs)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func getCVEs(ctx context.Context, tx *postgres.Tx, componentID string) (map[string]*storage.ImageCVEV2, error) {
	defer metrics.SetPostgresOperationDurationTime(time.Now(), ops.GetMany, "ImageCVEs")

	rows, err := tx.Query(ctx, "SELECT serialized FROM "+imageComponentsV2CVEsTable+" WHERE image_component_v2_id = $1", componentID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *storeImpl) deleteImageTree(ctx context.Context, tx *postgres.Tx, imageID string) error {
	...
	// TODO(ROX-27402): Put in delete if we don't just use referential integrity
	return nil
}
...
// TODO(ROX-27402): work through this one to update vuln status.  Also need to ensure we don't overwrite it, maybe.
func (s *storeImpl) retryableUpdateVulnState(ctx context.Context, cve string, imageIDs []string, state storage.VulnerabilityState) error {
	...
	//	rows, err := tx.Query(ctx, "SELECT "+imageComponentsV2CVEsTable+".serialized FROM "+imageComponentsV2CVEsTable+" "+
	...
	//	imageCVEs, err = pgutils.ScanRows[storage.ImageCVEV2](rows)
	//	if err != nil || len(imageCVEs) == 0 {
	//		return err
	//	}
	//
	//	// Update state.
	...
	//		cve.State = state
	//		cveIDs = append(cveIDs, cve.GetId())
	//	}
	//
...
// // Used for testing
// TODO(ROX-27402) remove V2 tables.
