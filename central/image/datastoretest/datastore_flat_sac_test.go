...
type imageDatastoreFlatSACSuite struct {
	suite.Suite
...
func (s *imageDatastoreFlatSACSuite) SetupSuite() {
	var err error
...
func (s *imageDatastoreFlatSACSuite) TearDownSuite() {
	s.pgtestbase.Close()
}

func (s *imageDatastoreFlatSACSuite) SetupTest() {
	s.testImageIDs = make([]string, 0)
}

func (s *imageDatastoreFlatSACSuite) TearDownTest() {
	for _, id := range s.testImageIDs {
		s.deleteImage(id)
	}
}

func (s *imageDatastoreFlatSACSuite) deleteImage(id string) {
	s.Require().NoError(s.datastore.DeleteImages(s.testContexts[testutils.UnrestrictedReadWriteCtx], id))
}

func (s *imageDatastoreFlatSACSuite) deleteDeployment(clusterid, id string) {
	s.Require().NoError(s.deploymentDatastore.RemoveDeployment(sac.WithAllAccess(context.Background()), clusterid, id))
}

func (s *imageDatastoreFlatSACSuite) deleteNamespace(id string) {
...
func (s *imageDatastoreFlatSACSuite) verifyRawImagesEqual(image1, image2 *storage.Image) {
	s.Equal(image1.GetId(), image2.GetId())
	s.Equal(image1.GetComponents(), image2.GetComponents())
	s.Equal(image1.GetCves(), image2.GetCves())
}

func (s *imageDatastoreFlatSACSuite) TestUpsertImage() {
...
func (s *imageDatastoreFlatSACSuite) TestUpdateVulnerabilityState() {
		...
		s.Run(name, func() {
			...
			for _, cve := range foundCVEs {
				query := searchPkg.NewQueryBuilder().AddExactMatches(searchPkg.CVE, cve.GetCve()).
					AddExactMatches(searchPkg.ImageSHA, image.GetId()).ProtoQuery()
...
func (s *imageDatastoreFlatSACSuite) TestDeleteImages() {
...
func (s *imageDatastoreFlatSACSuite) setupReadTest() ([]*storage.Image, func(), error) {
	var setupErr error
...
func (s *imageDatastoreFlatSACSuite) TestExists() {
...
func (s *imageDatastoreFlatSACSuite) TestListImage() {
	images, cleanup, setupErr := s.setupReadTest()
	defer cleanup()
	s.Require().NoError(setupErr)
	s.Require().NotZero(len(images))
...
func (s *imageDatastoreFlatSACSuite) TestGetImage() {
	images, cleanup, setupErr := s.setupReadTest()
	defer cleanup()
	s.Require().NoError(setupErr)
	s.Require().NotZero(len(images))
...
func (s *imageDatastoreFlatSACSuite) TestGetImageMetadata() {
	images, cleanup, setupErr := s.setupReadTest()
	defer cleanup()
	s.Require().NoError(setupErr)
	s.Require().NotZero(len(images))
...
func (s *imageDatastoreFlatSACSuite) TestGetImagesBatch() {
	images, cleanup, setupErr := s.setupReadTest()
	defer cleanup()
	s.Require().NoError(setupErr)
	s.Require().True(len(images) > 1)
...
func (s *imageDatastoreFlatSACSuite) TestWalkByQuery() {
	images, cleanup, setupErr := s.setupReadTest()
	defer cleanup()
	s.Require().NoError(setupErr)
	s.Require().True(len(images) > 1)
...
func (s *imageDatastoreFlatSACSuite) runReadTest(testName string, prefix string, testFunc func(c testutils.SACCrudTestCase)) {
...
func (s *imageDatastoreFlatSACSuite) getSearchTestCases() map[string]map[string]bool {
...
func (s *imageDatastoreFlatSACSuite) setupSearchTest() (func(), error) {
	var setupErr error
...
func (s *imageDatastoreFlatSACSuite) TestCountImages() {
...
func (s *imageDatastoreFlatSACSuite) TestCount() {
	cleanup, setupErr := s.setupSearchTest()
	defer cleanup()
	s.Require().NoError(setupErr)
...
func (s *imageDatastoreFlatSACSuite) TestSearch() {
	cleanup, setupErr := s.setupSearchTest()
	defer cleanup()
	s.Require().NoError(setupErr)
...
func (s *imageDatastoreFlatSACSuite) TestSearchImages() {
	cleanup, setupErr := s.setupSearchTest()
	defer cleanup()
	s.Require().NoError(setupErr)
...
func (s *imageDatastoreFlatSACSuite) TestSearchRawImages() {
	cleanup, setupErr := s.setupSearchTest()
	defer cleanup()
	s.Require().NoError(setupErr)
...
func (s *imageDatastoreFlatSACSuite) TestSearchListImages() {
	cleanup, setupErr := s.setupSearchTest()
	defer cleanup()
	s.Require().NoError(setupErr)
	refImages := map[string]*storage.Image{
...
func (s *imageDatastoreFlatSACSuite) runSearchTest(testName string, testFunc func(key string, c map[string]bool)) {
	imageGraphBefore := graphDBTestUtils.GetImageGraph(
		sac.WithAllAccess(context.Background()),
		s.T(),
		s.pgtestbase.DB,
