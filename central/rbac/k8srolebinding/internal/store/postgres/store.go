...
const (
		...
		upsertStmt = "insert into rolebindings (id, value, Name, Namespace, ClusterId, ClusterName, RoleId) values($1, $2, $3, $4, $5, $6, $7) on conflict(id) do update set value = EXCLUDED.value, Name = EXCLUDED.Name, Namespace = EXCLUDED.Namespace, ClusterId = EXCLUDED.ClusterId, ClusterName = EXCLUDED.ClusterName, RoleId = EXCLUDED.RoleId"
...
const (
	...
	batchInsertTemplate = "insert into rolebindings (id, value, Name, Namespace, ClusterId, ClusterName, RoleId) values %s on conflict(id) do update set value = EXCLUDED.value, Name = EXCLUDED.Name, Namespace = EXCLUDED.Namespace, ClusterId = EXCLUDED.ClusterId, ClusterName = EXCLUDED.ClusterName, RoleId = EXCLUDED.RoleId"
)
...
func (s *storeImpl) upsert(id string, obj *storage.K8SRoleBinding) error {
	...
	_, err = conn.Exec(context.Background(), upsertStmt, id, value, obj.GetName(), obj.GetNamespace(), obj.GetClusterId(), obj.GetClusterName(), obj.GetRoleId())
	return err
}
...
func (s *storeImpl) UpsertMany(objs []*storage.K8SRoleBinding) error {
	...
	for start, end, ok := batch.Next(); ok; start, end, ok = batch.Next() {
		...
		for i, obj := range objs[start:end] {
			...
			metrics.SetJSONPBOperationDurationTime(t, "Marshal", "K8SRoleBinding")
			id := keyFunc(obj)
			data = append(data, id, value, obj.GetName(), obj.GetNamespace(), obj.GetClusterId(), obj.GetClusterName(), obj.GetRoleId())
		}
