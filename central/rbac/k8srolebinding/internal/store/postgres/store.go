...
func (s *storeImpl) Get(ctx context.Context, id string) (*storage.K8SRoleBinding, bool, error) {
	...
	data, err := postgres.RunGetQueryForSchemaType[storage.K8SRoleBinding](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetMany(ctx context.Context, ids []string) ([]*storage.K8SRoleBinding, []int, error) {
	...
	rows, err := postgres.RunGetManyQueryForSchemaType[storage.K8SRoleBinding](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.K8SRoleBinding) error) error {
	...
	fetcher, closer, err := postgres.RunCursorQueryForSchemaType[storage.K8SRoleBinding](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
