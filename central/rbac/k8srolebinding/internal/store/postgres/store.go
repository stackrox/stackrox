...
const createTableQuery = "create table if not exists rolebindings (id varchar primary key, value jsonb)"
...
//
	return &storeImpl{
		...
		countStmt: compileStmtOrPanic(db, "select count(*) from rolebindings"),
		existsStmt: compileStmtOrPanic(db, "select exists(select 1 from rolebindings where id = $1)"),
		...
		walkWithIDStmt: compileStmtOrPanic(db, "select id, value from rolebindings"),
	}
//
}
...
func (s *storeImpl) Get(id string) (*storage.K8SRoleBinding, bool, error) {
	...
	log.Infof("Took %d to query a K8SRoleBinding", time.Since(t).Milliseconds())
	...
	log.Infof("Took %d to scan a K8SRoleBinding", time.Since(t).Milliseconds())
	...
	log.Infof("Took %d to unmarshal a K8SRoleBinding", time.Since(t).Milliseconds())
