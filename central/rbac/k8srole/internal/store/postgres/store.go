...
func (s *storeImpl) Get(ctx context.Context, id string) (*storage.K8SRole, bool, error) {
	...
	data, err := postgres.RunGetQueryForSchemaType[storage.K8SRole](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetMany(ctx context.Context, ids []string) ([]*storage.K8SRole, []int, error) {
	...
	rows, err := postgres.RunGetManyQueryForSchemaType[storage.K8SRole](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.K8SRole) error) error {
	...
	fetcher, closer, err := postgres.RunCursorQueryForSchemaType[storage.K8SRole](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
