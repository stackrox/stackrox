...
const (
	...
	getWithRollupStmt = "select row_to_json((select record from (select table0.Id as Id, table0.Name as Name, table0.Namespace as Namespace, table0.ClusterId as ClusterId, table0.ClusterName as ClusterName, table0.ClusterRole as ClusterRole, table0.Labels as Labels, table0.Annotations as Annotations, table0.CreatedAt as CreatedAt, to_json(join0)->'array' as join0 from k8sroles table0 left join lateral (select array(select json_build_object('idx', table1.idx, 'Verbs', table1.Verbs, 'ApiGroups', table1.ApiGroups, 'Resources', table1.Resources, 'NonResourceUrls', table1.NonResourceUrls, 'ResourceNames', table1.ResourceNames) from k8sroles_Rules table1 where (table0.Id = table1.k8sroles_Id))) join0 on true where (table0.Id = $1)) record ))"
