...
const createTableQuery = "create table if not exists k8sroles (id varchar primary key, value jsonb)"
...
func (s *storeImpl) Get(id string) (*storage.K8SRole, bool, error) {
	...
	log.Infof("Took %d to query a K8SRole", time.Since(t).Milliseconds())
	...
	log.Infof("Took %d to scan a K8SRole", time.Since(t).Milliseconds())
	...
	log.Infof("Took %d to unmarshal a K8SRole", time.Since(t).Milliseconds())
