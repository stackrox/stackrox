...
func (s *storeImpl) Get(ctx context.Context, infoId string) (*storage.NetworkEntity, bool, error) {
	...
	data, err := postgres.RunGetQueryForSchemaType[storage.NetworkEntity](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetMany(ctx context.Context, ids []string) ([]*storage.NetworkEntity, []int, error) {
	...
	rows, err := postgres.RunGetManyQueryForSchemaType[storage.NetworkEntity](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.NetworkEntity) error) error {
	...
	fetcher, closer, err := postgres.RunCursorQueryForSchemaType[storage.NetworkEntity](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
