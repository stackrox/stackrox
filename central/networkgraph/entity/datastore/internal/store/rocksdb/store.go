...
func (b *storeImpl) Count() (int, error) {
	defer metrics.SetRocksDBOperationDurationTime(time.Now(), ops.Count, "NetworkEntity")
...
func (b *storeImpl) Get(id string) (*storage.NetworkEntity, bool, error) {
...
func (b *storeImpl) GetMany(ids []string) ([]*storage.NetworkEntity, []int, error) {
	defer metrics.SetRocksDBOperationDurationTime(time.Now(), ops.GetMany, "NetworkEntity")

	msgs, missingIndices, err := b.crud.GetMany(ids)
	if err != nil {
		return nil, nil, err
	}
	objs := make([]*storage.NetworkEntity, 0, len(msgs))
	for _, m := range msgs {
		objs = append(objs, m.(*storage.NetworkEntity))
	}
	return objs, missingIndices, nil
}

// Upsert inserts the object into the DB
func (b *storeImpl) Upsert(obj *storage.NetworkEntity) error {
...
func (b *storeImpl) UpsertMany(objs []*storage.NetworkEntity) error {
	defer metrics.SetRocksDBOperationDurationTime(time.Now(), ops.AddMany, "NetworkEntity")
...
func (b *storeImpl) Walk(fn func(obj *storage.NetworkEntity) error) error {
