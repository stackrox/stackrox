...
func copyFromNetworkGraphConfigs(ctx context.Context, s pgSearch.Deleter, tx *postgres.Tx, objs ...*storage.NetworkGraphConfig) error {
	...
	for idx, obj := range objs {
		...
		if (idx+1)%batchSize == 0 || idx == len(objs)-1 {
			...
			if _, err := tx.CopyFrom(ctx, pgx.Identifier{"network_graph_configs"}, copyCols, pgx.CopyFromRows(inputRows)); err != nil {
				return err
			}
...
func dropTableNetworkGraphConfigs(ctx context.Context, db postgres.DB) {
	_, _ = db.Exec(ctx, "TRUNCATE network_graph_configs CASCADE")
