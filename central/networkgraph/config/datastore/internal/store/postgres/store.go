...
type Store interface {
	...
	GetMany(ctx context.Context, identifiers []string) ([]*storage.NetworkGraphConfig, []int, error)
...
type storeImpl struct {
	*pgSearch.GenericStore[storage.NetworkGraphConfig, *storage.NetworkGraphConfig]
	db    postgres.DB
	mutex sync.RWMutex
}
...
func New(db postgres.DB) Store {
	return &storeImpl{
		db: db,
		GenericStore: pgSearch.NewGenericStore[storage.NetworkGraphConfig, *storage.NetworkGraphConfig](
			db,
			targetResource,
			schema,
...
func metricsSetAcquireDBConnDuration(start time.Time, op ops.Op) {
	metrics.SetAcquireDBConnDuration(start, op, "NetworkGraphConfig")
}
...
func (s *storeImpl) GetMany(ctx context.Context, identifiers []string) ([]*storage.NetworkGraphConfig, []int, error) {
