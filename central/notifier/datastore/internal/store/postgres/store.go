...
type storeImpl struct {
	*pgSearch.GenericStore[storage.Notifier, *storage.Notifier]
	db    postgres.DB
	mutex sync.RWMutex
}
...
func New(db postgres.DB) Store {
	return &storeImpl{
		db: db,
		GenericStore: pgSearch.NewGenericStore[storage.Notifier, *storage.Notifier](
			db,
			schema,
			pkGetter,
...
func (s *storeImpl) Get(ctx context.Context, id string) (*storage.Notifier, bool, error) {
	...
	data, err := pgSearch.RunGetQueryForSchema[storage.Notifier](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetMany(ctx context.Context, identifiers []string) ([]*storage.Notifier, []int, error) {
	...
	rows, err := pgSearch.RunGetManyQueryForSchema[storage.Notifier](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.Notifier) error) error {
	...
	fetcher, closer, err := pgSearch.RunCursorQueryForSchema[storage.Notifier](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
