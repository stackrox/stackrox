...
const (
	baseTable  = "notifiers"
	countStmt  = "SELECT COUNT(*) FROM notifiers"
...
func New(ctx context.Context, db *pgxpool.Pool) Store {
	pgutils.CreateTable(ctx, db, pkgSchema.CreateTableNotifiersStmt)
...
func insertIntoNotifiers(ctx context.Context, tx pgx.Tx, obj *storage.Notifier) error {
...
func (s *storeImpl) upsert(ctx context.Context, objs ...*storage.Notifier) error {
	...
	for _, obj := range objs {
		...
		if err := insertIntoNotifiers(ctx, tx, obj); err != nil {
			if err := tx.Rollback(ctx); err != nil {
				return err
			}
			return err
...
func (s *storeImpl) Delete(ctx context.Context, id string) error {
	...
	conn, release, err := s.acquireConn(ctx, ops.Remove, "Notifier")
	if err != nil {
		return err
	}
	defer release()
...
func (s *storeImpl) DeleteMany(ctx context.Context, ids []string) error {
	...
	conn, release, err := s.acquireConn(ctx, ops.RemoveMany, "Notifier")
	if err != nil {
		return err
	}
	defer release()
