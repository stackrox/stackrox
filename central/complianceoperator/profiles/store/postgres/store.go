...
type storeImpl struct {
	*pgSearch.GenericStore[storage.ComplianceOperatorProfile, *storage.ComplianceOperatorProfile]
	db    postgres.DB
	mutex sync.RWMutex
}
...
func New(db postgres.DB) Store {
	return &storeImpl{
		db: db,
		GenericStore: pgSearch.NewGenericStore[storage.ComplianceOperatorProfile, *storage.ComplianceOperatorProfile](
			db,
			schema,
			pkGetter,
...
func pkGetter(obj *storage.ComplianceOperatorProfile) string {
	return obj.GetId()
}

func metricsSetPostgresOperationDurationTime(start time.Time, op ops.Op) {
	metrics.SetPostgresOperationDurationTime(start, op, "ComplianceOperatorProfile")
}

func metricsSetAcquireDBConnDuration(start time.Time, op ops.Op) {
	metrics.SetAcquireDBConnDuration(start, op, "ComplianceOperatorProfile")
}
...
func (s *storeImpl) DeleteMany(ctx context.Context, identifiers []string) error {
	defer metrics.SetPostgresOperationDurationTime(time.Now(), ops.RemoveMany, "ComplianceOperatorProfile")
...
func (s *storeImpl) Count(ctx context.Context) (int, error) {
	defer metrics.SetPostgresOperationDurationTime(time.Now(), ops.Count, "ComplianceOperatorProfile")
...
func (s *storeImpl) Get(ctx context.Context, id string) (*storage.ComplianceOperatorProfile, bool, error) {
	defer metrics.SetPostgresOperationDurationTime(time.Now(), ops.Get, "ComplianceOperatorProfile")
	...
	data, err := pgSearch.RunGetQueryForSchema[storage.ComplianceOperatorProfile](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetIDs(ctx context.Context) ([]string, error) {
	defer metrics.SetPostgresOperationDurationTime(time.Now(), ops.GetAll, "storage.ComplianceOperatorProfileIDs")
	var sacQueryFilter *v1.Query
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.ComplianceOperatorProfile) error) error {
	var sacQueryFilter *v1.Query
	sacQueryFilter, err := pgSearch.GetReadSACQuery(ctx, targetResource)
	if err != nil {
		return err
	}
	fetcher, closer, err := pgSearch.RunCursorQueryForSchema[storage.ComplianceOperatorProfile](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
