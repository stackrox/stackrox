...
func Singleton() Manager {
	once.Do(func() {
		var err error
		manager, err = NewManager(profileDatastore.Singleton(), scansDatastore.Singleton(), scanSettingBindingDatastore.Singleton(), rulesDatastore.Singleton(), checkResultsDatastore.Singleton())
		utils.Must(err)
	})
	return manager
}
