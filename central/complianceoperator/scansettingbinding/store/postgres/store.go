...
func New(db postgres.DB) Store {
	return pgSearch.NewGloballyScopedGenericStore[storeType, *storeType](
		...
		schema,
		pkGetter,
		insertIntoComplianceOperatorScanSettingBindings,
		copyFromComplianceOperatorScanSettingBindings,
...
func insertIntoComplianceOperatorScanSettingBindings(batch *pgx.Batch, pool pgSearch.BufferPool, obj *storage.ComplianceOperatorScanSettingBinding) (*[]byte, error) {
...
func copyFromComplianceOperatorScanSettingBindings(ctx context.Context, s pgSearch.Deleter, tx *postgres.Tx, pool pgSearch.BufferPool, objs ...*storage.ComplianceOperatorScanSettingBinding) error {
	if len(objs) == 0 {
		return nil
	}
	...
	for objBatch := range slices.Chunk(objs, batchSize) {
		...
		if _, err := tx.CopyFrom(ctx, pgx.Identifier{"compliance_operator_scan_setting_bindings"}, copyCols, pgx.CopyFromRows(inputRows)); err != nil {
			return err
		}
