...
const (
	baseTable = "compliance_operator_remediation_v2"
	storeName = "ComplianceOperatorRemediationV2"
)
...
func New(db postgres.DB) Store {
	return pgSearch.NewGenericStore[storeType, *storeType](
		...
		schema,
		pkGetter,
		insertIntoComplianceOperatorRemediationV2,
		copyFromComplianceOperatorRemediationV2,
...
func isUpsertAllowed(ctx context.Context, objs ...*storeType) error {
	...
	if len(deniedIDs) != 0 {
		return errors.Wrapf(sac.ErrResourceAccessDenied, "modifying complianceOperatorRemediationV2s with IDs [%s] was denied", strings.Join(deniedIDs, ", "))
	}
	return nil
}
...
func copyFromComplianceOperatorRemediationV2(ctx context.Context, s pgSearch.Deleter, tx *postgres.Tx, objs ...*storage.ComplianceOperatorRemediationV2) error {
	...
	for objBatch := range slices.Chunk(objs, batchSize) {
		...
		if _, err := tx.CopyFrom(ctx, pgx.Identifier{"compliance_operator_remediation_v2"}, copyColsComplianceOperatorRemediationV2, pgx.CopyFromRows(inputRows)); err != nil {
			return err
		}
...
func Destroy(ctx context.Context, db postgres.DB) {
	dropTableComplianceOperatorRemediationV2(ctx, db)
}

func dropTableComplianceOperatorRemediationV2(ctx context.Context, db postgres.DB) {
	_, _ = db.Exec(ctx, "DROP TABLE IF EXISTS compliance_operator_remediation_v2 CASCADE")
