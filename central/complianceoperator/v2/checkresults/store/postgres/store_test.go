...
func (s *ComplianceOperatorCheckResultV2StoreSuite) TestStore() {
	...
	complianceOperatorCheckResultV2Count, err = store.Count(withNoAccessCtx, search.EmptyQuery())
	s.NoError(err)
	...
	s.False(exists)
	s.Nil(foundComplianceOperatorCheckResultV2)
	s.NoError(store.Delete(withNoAccessCtx, complianceOperatorCheckResultV2.GetId()))
	...
	foundComplianceOperatorCheckResultV2s, missing, err := store.GetMany(ctx, complianceOperatorCheckResultV2IDs)
	s.NoError(err)
	s.Empty(missing)
	protoassert.ElementsMatch(s.T(), complianceOperatorCheckResultV2s, foundComplianceOperatorCheckResultV2s)
...
type testCase struct {
	...
	expectedObjects        []*storage.ComplianceOperatorCheckResultV2
	expectedWriteError     error
}

func (s *ComplianceOperatorCheckResultV2StoreSuite) getTestData(access ...storage.Access) (*storage.ComplianceOperatorCheckResultV2, *storage.ComplianceOperatorCheckResultV2, map[string]testCase) {
	objA := &storage.ComplianceOperatorCheckResultV2{}
	s.NoError(testutils.FullInit(objA, testutils.UniqueInitializer(), testutils.JSONFieldsFilter))

	objB := &storage.ComplianceOperatorCheckResultV2{}
	...
	testCases := map[string]testCase{
		withAllAccess: {
			...
			expectedMissingIndices: []int{},
			expectedObjects:        []*storage.ComplianceOperatorCheckResultV2{objA, objB},
		...
		withAccess: {
			...
			expectedObjects:        []*storage.ComplianceOperatorCheckResultV2{objA},
			expectedWriteError:     nil,
		},
		withAccessToCluster: {
			context: sac.WithGlobalAccessScopeChecker(context.Background(),
...
func (s *ComplianceOperatorCheckResultV2StoreSuite) TestSACUpsert() {
...
func (s *ComplianceOperatorCheckResultV2StoreSuite) TestSACUpsertMany() {
	obj, _, testCases := s.getTestData(storage.Access_READ_WRITE_ACCESS)
	for name, testCase := range testCases {
		s.T().Run(fmt.Sprintf("with %s", name), func(t *testing.T) {
			assert.ErrorIs(t, s.store.UpsertMany(testCase.context, []*storage.ComplianceOperatorCheckResultV2{obj}), testCase.expectedWriteError)
		})
	}
}

func (s *ComplianceOperatorCheckResultV2StoreSuite) TestSACCount() {
...
func (s *ComplianceOperatorCheckResultV2StoreSuite) TestSACWalk() {
	objA, objB, testCases := s.getTestData(storage.Access_READ_ACCESS)
	s.Require().NoError(s.store.Upsert(withAllAccessCtx, objA))
	s.Require().NoError(s.store.Upsert(withAllAccessCtx, objB))
...
func (s *ComplianceOperatorCheckResultV2StoreSuite) TestSACExists() {
...
func (s *ComplianceOperatorCheckResultV2StoreSuite) TestSACGet() {
	objA, _, testCases := s.getTestData(storage.Access_READ_ACCESS)
	s.Require().NoError(s.store.Upsert(withAllAccessCtx, objA))
...
func (s *ComplianceOperatorCheckResultV2StoreSuite) TestSACDelete() {
...
func (s *ComplianceOperatorCheckResultV2StoreSuite) TestSACDeleteMany() {
	objA, objB, testCases := s.getTestData(storage.Access_READ_ACCESS, storage.Access_READ_WRITE_ACCESS)
	for name, testCase := range testCases {
		s.T().Run(fmt.Sprintf("with %s", name), func(t *testing.T) {
			s.SetupTest()
...
func (s *ComplianceOperatorCheckResultV2StoreSuite) TestSACGetMany() {
	objA, objB, testCases := s.getTestData(storage.Access_READ_ACCESS)
	s.Require().NoError(s.store.Upsert(withAllAccessCtx, objA))
	s.Require().NoError(s.store.Upsert(withAllAccessCtx, objB))
