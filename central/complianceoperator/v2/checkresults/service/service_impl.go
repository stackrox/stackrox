...
func (s *serviceImpl) GetComplianceScanCheckResult(ctx context.Context, req *v2.ResourceByID) (*v2.ComplianceClusterCheckStatus, error) {
	...
	// Build resolver for outdated detection (feature-flag gated; nil ⇒ UNKNOWN).
...
func (s *serviceImpl) GetComplianceProfileResults(ctx context.Context, request *v2.ComplianceProfileResultsRequest) (*v2.ListComplianceProfileResults, error) {
	...
	// below stay at their zero values (UNKNOWN / 0) and no extra datastore queries
	// run, so behavior is identical to before this feature.
	var checkDataStates map[string]v2.ComplianceDataState
...
func (s *serviceImpl) GetComplianceProfileCheckResult(ctx context.Context, request *v2.ComplianceProfileCheckRequest) (*v2.ListComplianceCheckClusterResponse, error) {
	...
	// Outdated-data detection is feature-flag gated; with the flag off resolver is
	// nil (⇒ UNKNOWN) and outdatedCount is 0, matching prior behavior.
	var resolver *compliancedata.ConfigResolver
	var outdatedCount int32
	if features.ComplianceSurfaceStaleData.Enabled() {
		// Build resolver for outdated detection over the returned results' configs.
		configNames := make(map[string]struct{})
		for _, result := range scanResults {
...
func (s *serviceImpl) GetComplianceProfileCheckDetails(ctx context.Context, request *v2.ComplianceCheckDetailRequest) (*v2.ComplianceClusterCheckStatus, error) {
	...
	// Outdated-data detection is feature-flag gated; with the flag off resolver is
	// nil ⇒ per-cluster data state defaults to UNKNOWN (prior behavior).
	var resolver *compliancedata.ConfigResolver
	if features.ComplianceSurfaceStaleData.Enabled() {
		configNames := make(map[string]struct{})
		for _, result := range scanResults {
