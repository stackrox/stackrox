...
func (s *ComplianceResultsServiceTestSuite) SetupSuite() {
	...
	// Individual flag-off cases override this via t.Setenv.
...
// TestGetComplianceProfileResults_FlagOff verifies that with
// ROX_COMPLIANCE_SURFACE_STALE_DATA disabled the outdated-data fields stay at their
// zero values (UNKNOWN / 0) and none of the outdated-detection datastore queries
// (MinLastStartedTime* / GetScanConfigurationByName) run — i.e. behavior is
// identical to before the feature. The strict gomock controller fails the test
// if any of those calls happen, since no expectations are registered for them.
func (s *ComplianceResultsServiceTestSuite) TestGetComplianceProfileResults_FlagOff() {
	...
	resp, err := s.service.GetComplianceProfileResults(s.ctx, &apiV2.ComplianceProfileResultsRequest{
		ProfileName: "ocp4-cis",
		Query:       &apiV2.RawQuery{Query: ""},
	})
	s.Require().NoError(err)
	// Fields are byte-for-byte the pre-feature zero values.
	expected := convertUtils.GetComplianceProfileResultsV2(s.T(), "ocp4-cis", apiV2.ComplianceDataState_COMPLIANCE_DATA_STATE_UNKNOWN, 0)
	protoassert.Equal(s.T(), expected, resp)
}
