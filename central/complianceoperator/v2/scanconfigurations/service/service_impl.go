...
func (s *serviceImpl) DeleteComplianceScanConfiguration(ctx context.Context, req *v2.ResourceByID) (*v2.Empty, error) {
	...
	if err != nil {
		return nil, errors.Wrapf(errox.InvariantViolation, "Unable to find the Report Snapshots asociated with the scan config")
	}
	...
	for _, snapshot := range snapshots {
		...
		if err := s.blobDS.Delete(blobCtx, blobName); err != nil {
			return nil, errors.Wrapf(errox.InvariantViolation, "Unable to delete the report asociated with the scan config")
		}
	}
...
func (s *serviceImpl) DeleteReport(ctx context.Context, req *v2.ResourceByID) (*v2.Empty, error) {
	...
	if err = s.blobDS.Delete(ctx, blobName); err != nil {
		return nil, errors.Wrapf(errox.InvariantViolation, "Unable to delete the downloadable report")
	}
