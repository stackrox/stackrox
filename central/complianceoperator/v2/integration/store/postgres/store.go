...
func New(db postgres.DB) Store {
	return pgSearch.NewGenericStore[storeType, *storeType](
		...
		schema,
		pkGetter,
		insertIntoComplianceIntegrations,
...
func insertIntoComplianceIntegrations(batch *pgx.Batch, obj *storage.ComplianceIntegration) error {
	...
	finalStr := "INSERT INTO compliance_integrations (Id, Version, ClusterId, OperatorInstalled, Status, serialized) VALUES($1, $2, $3, $4, $5, $6) ON CONFLICT(Id) DO UPDATE SET Id = EXCLUDED.Id, Version = EXCLUDED.Version, ClusterId = EXCLUDED.ClusterId, OperatorInstalled = EXCLUDED.OperatorInstalled, Status = EXCLUDED.Status, serialized = EXCLUDED.serialized"
...
func copyFromComplianceIntegrations(ctx context.Context, s pgSearch.Deleter, tx *postgres.Tx, objs ...*storage.ComplianceIntegration) error {
	...
	for idx, obj := range objs {
		...
		if (idx+1)%batchSize == 0 || idx == len(objs)-1 {
			...
			if _, err := tx.CopyFrom(ctx, pgx.Identifier{"compliance_integrations"}, copyCols, pgx.CopyFromRows(inputRows)); err != nil {
				return err
			}
...
func Destroy(ctx context.Context, db postgres.DB) {
	dropTableComplianceIntegrations(ctx, db)
}

func dropTableComplianceIntegrations(ctx context.Context, db postgres.DB) {
	_, _ = db.Exec(ctx, "DROP TABLE IF EXISTS compliance_integrations CASCADE")
