...
type ComplainceReportingTestSuite struct {
	...
	reportDataGenerator *mocks.MockReportDataGenerator
}
...
func (s *ComplainceReportingTestSuite) SetupSuite() {
	...
	s.reportDataGenerator = mocks.NewMockReportDataGenerator(s.mockCtrl)
	...
	s.reportGen = &complianceReportGeneratorImpl{
		...
		reportResultsAggregator: s.reportDataGenerator,
	}
}
...
func (s *ComplainceReportingTestSuite) TestProcessReportRequest() {
	...
	s.Run("Fail to upsert Snapshot", func() {
		...
		s.reportDataGenerator.EXPECT().GetReportData(gomock.Any()).Times(1).Return(&types.Results{})
