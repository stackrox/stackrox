...
type newResponseHandler[T any] func(func(T) error, func(), <-chan T) (reportUtils.AsyncResponseHandler[T], error)
...
func (rg *complianceReportGeneratorImpl) ProcessReportRequest(req *report.Request) error {
	...
	handler, err := rg.newHandlerFn(func(err error) error {
		...
		if dbErr := reportUtils.UpdateSnapshotOnError(req.Ctx, snapshot, reportUtils.ErrSendingEmail, rg.snapshotDS); dbErr != nil {
	...
	// rg.watchSender(reportGenCtx, snapshot, rg.reportSender.SendEmail(reportGenCtx, reportName, zipData, reportData, req.Notifiers))
	return nil
}
