...
func (s *ComplianceOperatorSuiteV2StoreSuite) TestStore() {
	...
	s.NoError(err)
	s.True(exists)
	protoassert.Equal(s.T(), complianceOperatorSuiteV2, foundComplianceOperatorSuiteV2)

	complianceOperatorSuiteV2Count, err := store.Count(ctx, search.EmptyQuery())
	s.NoError(err)
	s.Equal(1, complianceOperatorSuiteV2Count)
	complianceOperatorSuiteV2Count, err = store.Count(withNoAccessCtx, search.EmptyQuery())
	s.NoError(err)
	...
	foundComplianceOperatorSuiteV2s, missing, err := store.GetMany(ctx, complianceOperatorSuiteV2IDs)
	s.NoError(err)
	s.Empty(missing)
	protoassert.ElementsMatch(s.T(), complianceOperatorSuiteV2s, foundComplianceOperatorSuiteV2s)
