...
func (s *ComplianceOperatorProfileV2StoreSuite) TestStore() {
	...
	s.NoError(err)
	s.True(exists)
	protoassert.Equal(s.T(), complianceOperatorProfileV2, foundComplianceOperatorProfileV2)

	complianceOperatorProfileV2Count, err := store.Count(ctx, search.EmptyQuery())
	s.NoError(err)
	s.Equal(1, complianceOperatorProfileV2Count)
	complianceOperatorProfileV2Count, err = store.Count(withNoAccessCtx, search.EmptyQuery())
	s.NoError(err)
	...
	foundComplianceOperatorProfileV2s, missing, err := store.GetMany(ctx, complianceOperatorProfileV2IDs)
	s.NoError(err)
	s.Empty(missing)
	protoassert.ElementsMatch(s.T(), complianceOperatorProfileV2s, foundComplianceOperatorProfileV2s)
