...
func New(db postgres.DB) Store {
	return pgSearch.NewGenericStore[storeType, *storeType](
		...
		schema,
		pkGetter,
		insertIntoComplianceOperatorScans,
		copyFromComplianceOperatorScans,
...
func insertIntoComplianceOperatorScans(ctx context.Context, batch *pgx.Batch, obj *storage.ComplianceOperatorScan) error {
	...
	finalStr := "INSERT INTO compliance_operator_scans (Id, serialized, tenant_id) VALUES($1, $2, $3) ON CONFLICT(Id) DO UPDATE SET Id = EXCLUDED.Id, serialized = EXCLUDED.serialized, tenant_id = EXCLUDED.tenant_id"
...
func copyFromComplianceOperatorScans(ctx context.Context, s pgSearch.Deleter, tx *postgres.Tx, objs ...*storage.ComplianceOperatorScan) error {
	...
	for idx, obj := range objs {
		...
		if (idx+1)%batchSize == 0 || idx == len(objs)-1 {
			...
			if _, err := tx.CopyFrom(ctx, pgx.Identifier{"compliance_operator_scans"}, copyCols, pgx.CopyFromRows(inputRows)); err != nil {
				return err
			}
