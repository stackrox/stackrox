...
func (s *storeImpl) Get(ctx context.Context, id string) (*storage.ImageComponent, bool, error) {
	...
	data, err := postgres.RunGetQueryForSchema[storage.ImageComponent](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetByQuery(ctx context.Context, query *v1.Query) ([]*storage.ImageComponent, error) {
	...
	rows, err := postgres.RunGetManyQueryForSchema[storage.ImageComponent](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.ImageComponent) error) error {
	var sacQueryFilter *v1.Query
	fetcher, closer, err := postgres.RunCursorQueryForSchema[storage.ImageComponent](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
