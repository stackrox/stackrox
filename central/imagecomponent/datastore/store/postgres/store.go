...
func (s *storeImpl) Get(ctx context.Context, id string) (*storage.ImageComponent, bool, error) {
	...
	data, err := postgres.RunGetQueryForSchemaType[storage.ImageComponent](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetMany(ctx context.Context, ids []string) ([]*storage.ImageComponent, []int, error) {
	...
	rows, err := postgres.RunGetManyQueryForSchemaType[storage.ImageComponent](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.ImageComponent) error) error {
	var sacQueryFilter *v1.Query
	fetcher, closer, err := postgres.RunCursorQueryForSchemaType[storage.ImageComponent](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
