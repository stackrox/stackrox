...
func New(db postgres.DB) Store {
	return pgSearch.NewGloballyScopedGenericStore[storeType, *storeType](
		...
		schema,
		pkGetter,
		insertIntoImageComponents,
		copyFromImageComponents,
...
func insertIntoImageComponents(batch *pgx.Batch, pool pgSearch.BufferPool, obj *storage.ImageComponent) (*[]byte, error) {
...
func copyFromImageComponents(ctx context.Context, s pgSearch.Deleter, tx *postgres.Tx, pool pgSearch.BufferPool, objs ...*storage.ImageComponent) error {
	if len(objs) == 0 {
		return nil
	}
	...
	for objBatch := range slices.Chunk(objs, batchSize) {
		...
		if _, err := tx.CopyFrom(ctx, pgx.Identifier{"image_components"}, copyCols, pgx.CopyFromRows(inputRows)); err != nil {
			return err
		}
