...
func (s *storeImpl) GetByQuery(ctx context.Context, query *v1.Query) ([]*storage.ImageComponent, error) {
	...
	rows, err := pgSearch.RunGetManyQueryForSchema[storage.ImageComponent](ctx, schema, query, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.ImageComponent) error) error {
	fetcher, closer, err := pgSearch.RunCursorQueryForSchema[storage.ImageComponent](ctx, schema, search.EmptyQuery(), s.db)
	if err != nil {
		return err
	}
	defer closer()
