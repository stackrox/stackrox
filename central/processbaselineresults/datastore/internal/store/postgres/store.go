...
const (
	...
	getWithRollupStmt = "select row_to_json((select record from (select table0.DeploymentId as DeploymentId, table0.ClusterId as ClusterId, table0.Namespace as Namespace, to_json(join0)->'array' as join0 from processwhitelistresults table0 left join lateral (select array(select json_build_object('idx', table1.idx, 'ContainerName', table1.ContainerName, 'BaselineStatus', table1.BaselineStatus, 'AnomalousProcessesExecuted', table1.AnomalousProcessesExecuted) from processwhitelistresults_BaselineStatuses table1 where (table0.DeploymentId = table1.processwhitelistresults_DeploymentId))) join0 on true where (table0.DeploymentId = $1)) record ))"
