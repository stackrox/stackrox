...
func (s *storeImpl) Get(ctx context.Context, deploymentId string) (*storage.ProcessBaselineResults, bool, error) {
	...
	data, err := postgres.RunGetQueryForSchemaType[storage.ProcessBaselineResults](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetMany(ctx context.Context, ids []string) ([]*storage.ProcessBaselineResults, []int, error) {
	...
	rows, err := postgres.RunGetManyQueryForSchemaType[storage.ProcessBaselineResults](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.ProcessBaselineResults) error) error {
	...
	fetcher, closer, err := postgres.RunCursorQueryForSchemaType[storage.ProcessBaselineResults](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
