...
func ignoreSigningHeaders(o *awsS3.Options, headers []string) {
		...
		func(ctx context.Context, in middleware.FinalizeInput, next middleware.FinalizeHandler) (
			...
			if !ok {
				return out, metadata, &v4.SigningError{
					Err: fmt.Errorf("(ignoreHeaders) unexpected request middleware type %T", in.Request),
				}
			}
...
func restoreIgnored() middleware.FinalizeMiddleware {
	return middleware.FinalizeMiddlewareFunc(
		...
		func(ctx context.Context, in middleware.FinalizeInput, next middleware.FinalizeHandler) (
			...
			if !ok {
				return out, metadata, &v4.SigningError{
					Err: fmt.Errorf("(restoreIgnored) unexpected request middleware type %T", in.Request),
				}
			}
