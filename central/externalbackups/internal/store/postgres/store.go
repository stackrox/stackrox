...
func New(db postgres.DB) Store {
	return pgSearch.NewGloballyScopedGenericStore[storeType, *storeType](
		...
		schema,
		pkGetter,
		insertIntoExternalBackups,
		copyFromExternalBackups,
...
func insertIntoExternalBackups(batch *pgx.Batch, pool pgSearch.BufferPool, obj *storage.ExternalBackup) (*[]byte, error) {
...
func copyFromExternalBackups(ctx context.Context, s pgSearch.Deleter, tx *postgres.Tx, pool pgSearch.BufferPool, objs ...*storage.ExternalBackup) error {
	if len(objs) == 0 {
		return nil
	}
	...
	for objBatch := range slices.Chunk(objs, batchSize) {
		...
		if _, err := tx.CopyFrom(ctx, pgx.Identifier{"external_backups"}, copyCols, pgx.CopyFromRows(inputRows)); err != nil {
			return err
		}
