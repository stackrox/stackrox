...
func (s *storeImpl) GetMany(ctx context.Context, identifiers []string) ([]*storage.ExternalBackup, []int, error) {
	...
	rows, err := postgres.RunGetManyQueryForSchema[storage.ExternalBackup](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.ExternalBackup) error) error {
	...
	fetcher, closer, err := postgres.RunCursorQueryForSchema[storage.ExternalBackup](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
