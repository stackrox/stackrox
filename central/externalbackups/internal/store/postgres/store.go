...
func New(ctx context.Context, db *pgxpool.Pool) Store {
	pgutils.CreateTable(ctx, db, pkgSchema.CreateTableExternalBackupsStmt)
...
func insertIntoExternalBackups(ctx context.Context, tx pgx.Tx, obj *storage.ExternalBackup) error {
...
func (s *storeImpl) upsert(ctx context.Context, objs ...*storage.ExternalBackup) error {
	...
	for _, obj := range objs {
		...
		if err := insertIntoExternalBackups(ctx, tx, obj); err != nil {
			if err := tx.Rollback(ctx); err != nil {
				return err
			}
			return err
