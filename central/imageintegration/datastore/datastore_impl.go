...
func (ds *datastoreImpl) AddImageIntegration(ctx context.Context, integration *storage.ImageIntegration) (string, error) {
	...
	sourcedIntegration := features.SourcedAutogeneratedIntegrations.Enabled() ||
		(openshift.GlobalPullSecretIntegration(integration) && env.AutogenerateGlobalPullSecRegistries.BooleanSetting())

	// Sourced integrations have a predictable ID that is used for reconciliation, deletes, etc.,
	// therefore do not generate a random ID for sourced integrations.
