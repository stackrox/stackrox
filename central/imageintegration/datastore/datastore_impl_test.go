  ...
  func (suite *ImageIntegrationDataStoreTestSuite) SetupTest() {
      ...
      suite.datastore = NewForTestOnly(suite.store, adminEventsDS.GetTestPostgresDataStore(suite.T(), suite.testDB.DB))
  }
  ...
  func (suite *ImageIntegrationDataStoreTestSuite) TestRemoveImageIntegrationClearsEvents() {
      ...
      integration := suite.storeIntegration("Test Events Integration")

      gotInt, exists, err := suite.datastore.GetImageIntegration(ctx, integration.GetId())
      suite.NoError(err)
      suite.True(exists)
      protoassert.Equal(suite.T(), integration, gotInt)

      adminEventsStore := adminEventsDS.GetTestPostgresDataStore(suite.T(), suite.testDB.DB)
      event := &storage.AdministrationEvent{
          Id:         uuid.NewV4().String(),
          ResourceId: integration.GetId(),
      }
      err = adminEventsDS.UpsertTestEvents(ctx, suite.T(), adminEventsStore, event)
      suite.NoError(err)

      gotEvent, err := adminEventsStore.GetEvent(ctx, event.GetId())
      suite.NoError(err)
      suite.NotNil(gotEvent)

      err = suite.datastore.RemoveImageIntegration(ctx, integration.GetId())
      suite.NoError(err)

      _, exists, err = suite.datastore.GetImageIntegration(ctx, integration.GetId())
      suite.NoError(err)
      suite.False(exists)

      _, err = adminEventsStore.GetEvent(ctx, event.GetId())
      suite.Error(err)
  }
  ...
  func (suite *ImageIntegrationDataStoreTestSuite) TestDataStoreSearch() {
      ...
      ds := New(suite.store, adminEventsDS.GetTestPostgresDataStore(suite.T(), suite.testDB.DB))
