...
func (s *serviceImpl) DeleteImageIntegration(ctx context.Context, request *v1.ResourceByID) (*v1.Empty, error) {
	...
	if ii, exists, err := s.datastore.GetImageIntegration(ctx, request.GetId()); err == nil && exists {
		doBroadcast = validIntegrationForBroadcast(ii)
		name = ii.GetName()
	}
...
func validIntegrationForBroadcast(ii *storage.ImageIntegration) bool {
...
func validConnForBroadcast(conn connection.SensorConnection) bool {
