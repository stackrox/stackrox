...
type storeImpl struct {
	*pgSearch.GenericStore[storage.ImageIntegration, *storage.ImageIntegration]
	db    postgres.DB
	mutex sync.RWMutex
}
...
func New(db postgres.DB) Store {
	return &storeImpl{
		db: db,
		GenericStore: pgSearch.NewGenericStore[storage.ImageIntegration, *storage.ImageIntegration](
			db,
			targetResource,
			schema,
...
func (s *storeImpl) Count(ctx context.Context) (int, error) {
	defer metrics.SetPostgresOperationDurationTime(time.Now(), ops.Count, "ImageIntegration")
...
func (s *storeImpl) Get(ctx context.Context, id string) (*storage.ImageIntegration, bool, error) {
	defer metrics.SetPostgresOperationDurationTime(time.Now(), ops.Get, "ImageIntegration")
	...
	data, err := pgSearch.RunGetQueryForSchema[storage.ImageIntegration](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetByQuery(ctx context.Context, query *v1.Query) ([]*storage.ImageIntegration, error) {
	...
	rows, err := pgSearch.RunGetManyQueryForSchema[storage.ImageIntegration](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetMany(ctx context.Context, identifiers []string) ([]*storage.ImageIntegration, []int, error) {
	defer metrics.SetPostgresOperationDurationTime(time.Now(), ops.GetMany, "ImageIntegration")
	...
	resultsByID := make(map[string]*storage.ImageIntegration, len(rows))
	...
	elems := make([]*storage.ImageIntegration, 0, len(resultsByID))
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.ImageIntegration) error) error {
	var sacQueryFilter *v1.Query
	sacQueryFilter, err := pgSearch.GetReadSACQuery(ctx, targetResource)
	if err != nil {
		return err
	}
	fetcher, closer, err := pgSearch.RunCursorQueryForSchema[storage.ImageIntegration](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
