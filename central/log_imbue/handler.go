...
// Handler returns an http handler for the logimbue endpoint.
func Handler(storage db.LogsStorage) http.Handler {
	return &logImbueHandler{
		storage:            storage,
		compressorProvider: getCompressor,
	}
}

type logImbueHandler struct {
	storage            db.LogsStorage
	compressorProvider func() (Compressor, error)
}

// ServeHTTP adds or retrieves logs from the backend
func (l logImbueHandler) ServeHTTP(resp http.ResponseWriter, req *http.Request) {
...
func (l logImbueHandler) post(resp http.ResponseWriter, req *http.Request) {
	// If we panic unpacking the contents, we want to return an HTTP error for a bad request.
	defer recoverFromBuffPanic(resp)
...
func (l logImbueHandler) get(resp http.ResponseWriter, req *http.Request) {
	...
	for _, log := range logs {
		if _, err = compressor.Write([]byte(log)); err != nil {
			logger.Error(err)
			anyFailures = true
		} else {
			anySuccesses = true
