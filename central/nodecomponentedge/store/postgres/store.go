...
const (
	...
	deleteManyStmt = "DELETE FROM node_component_edges WHERE Id = ANY($1::text[])"
...
type Store interface {
	...
	Exists(ctx context.Context, id string, nodeId string, nodeComponentId string) (bool, error)
	Get(ctx context.Context, id string, nodeId string, nodeComponentId string) (*storage.NodeComponentEdge, bool, error)
...
func (s *storeImpl) GetMany(ctx context.Context, ids []string) ([]*storage.NodeComponentEdge, []int, error) {
	...
	conn, release, err := s.acquireConn(ctx, ops.GetMany, "NodeComponentEdge")
	if err != nil {
		return nil, nil, err
	}
	defer release()
