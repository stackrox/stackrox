...
func (s *storeImpl) GetByQuery(ctx context.Context, query *v1.Query) ([]*storage.NodeComponentEdge, error) {
	...
	rows, err := pgSearch.RunGetManyQueryForSchema[storage.NodeComponentEdge](ctx, schema, query, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.NodeComponentEdge) error) error {
	fetcher, closer, err := pgSearch.RunCursorQueryForSchema[storage.NodeComponentEdge](ctx, schema, search.EmptyQuery(), s.db)
	if err != nil {
		return err
	}
	defer closer()
