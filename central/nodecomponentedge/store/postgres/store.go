...
func (s *storeImpl) Get(ctx context.Context, id string) (*storage.NodeComponentEdge, bool, error) {
	...
	data, err := postgres.RunGetQueryForSchema[storage.NodeComponentEdge](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetMany(ctx context.Context, ids []string) ([]*storage.NodeComponentEdge, []int, error) {
	...
	rows, err := postgres.RunGetManyQueryForSchema[storage.NodeComponentEdge](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.NodeComponentEdge) error) error {
	var sacQueryFilter *v1.Query
	fetcher, closer, err := postgres.RunCursorQueryForSchema[storage.NodeComponentEdge](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
