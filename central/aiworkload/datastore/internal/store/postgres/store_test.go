...
type AiWorkloadsStoreSuite struct {
	suite.Suite
	store  Store
	testDB *pgtest.TestPostgres
}

func TestAiWorkloadsStore(t *testing.T) {
	suite.Run(t, new(AiWorkloadsStoreSuite))
}

func (s *AiWorkloadsStoreSuite) SetupSuite() {

	s.testDB = pgtest.ForT(s.T())
	s.store = New(s.testDB.DB)
}

func (s *AiWorkloadsStoreSuite) SetupTest() {
	ctx := sac.WithAllAccess(context.Background())
	tag, err := s.testDB.Exec(ctx, "TRUNCATE ai_workloads CASCADE")
	s.T().Log("ai_workloads", tag)
	s.store = New(s.testDB.DB)
	s.NoError(err)
}

func (s *AiWorkloadsStoreSuite) TestStore() {
	ctx := sac.WithAllAccess(context.Background())

	store := s.store

	aIWorkload := &storage.AIWorkload{}
	s.NoError(testutils.FullInit(aIWorkload, testutils.SimpleInitializer(), testutils.JSONFieldsFilter))

	foundAIWorkload, exists, err := store.Get(ctx, aIWorkload.GetId())
	s.NoError(err)
	s.False(exists)
	s.Nil(foundAIWorkload)

	withNoAccessCtx := sac.WithNoAccess(ctx)

	s.NoError(store.Upsert(ctx, aIWorkload))
	foundAIWorkload, exists, err = store.Get(ctx, aIWorkload.GetId())
	s.NoError(err)
	s.True(exists)
	protoassert.Equal(s.T(), aIWorkload, foundAIWorkload)

	aIWorkloadCount, err := store.Count(ctx, search.EmptyQuery())
	s.NoError(err)
	s.Equal(1, aIWorkloadCount)
	aIWorkloadCount, err = store.Count(withNoAccessCtx, search.EmptyQuery())
	s.NoError(err)
	s.Zero(aIWorkloadCount)

	aIWorkloadExists, err := store.Exists(ctx, aIWorkload.GetId())
	s.NoError(err)
	s.True(aIWorkloadExists)
	s.NoError(store.Upsert(ctx, aIWorkload))
	s.ErrorIs(store.Upsert(withNoAccessCtx, aIWorkload), sac.ErrResourceAccessDenied)

	s.NoError(store.Delete(ctx, aIWorkload.GetId()))
	foundAIWorkload, exists, err = store.Get(ctx, aIWorkload.GetId())
	s.NoError(err)
	s.False(exists)
	s.Nil(foundAIWorkload)
	s.NoError(store.Delete(withNoAccessCtx, aIWorkload.GetId()))

	var aIWorkloads []*storage.AIWorkload
	var aIWorkloadIDs []string
	for i := 0; i < 200; i++ {
		aIWorkload := &storage.AIWorkload{}
		s.NoError(testutils.FullInit(aIWorkload, testutils.UniqueInitializer(), testutils.JSONFieldsFilter))
		aIWorkloads = append(aIWorkloads, aIWorkload)
		aIWorkloadIDs = append(aIWorkloadIDs, aIWorkload.GetId())
	}

	s.NoError(store.UpsertMany(ctx, aIWorkloads))

	foundAIWorkloads, missing, err := store.GetMany(ctx, aIWorkloadIDs)
	s.NoError(err)
	s.Empty(missing)
	protoassert.ElementsMatch(s.T(), aIWorkloads, foundAIWorkloads)

	aIWorkloadCount, err = store.Count(ctx, search.EmptyQuery())
	s.NoError(err)
	s.Equal(200, aIWorkloadCount)

	s.NoError(store.DeleteMany(ctx, aIWorkloadIDs))

	aIWorkloadCount, err = store.Count(ctx, search.EmptyQuery())
	s.NoError(err)
	s.Equal(0, aIWorkloadCount)
}
...
type testCase struct {
	...
	expectedObjects        []*storage.AIWorkload
	expectedWriteError     error
}

func (s *AiWorkloadsStoreSuite) getTestData(access ...storage.Access) (*storage.AIWorkload, *storage.AIWorkload, map[string]testCase) {
	objA := &storage.AIWorkload{}
	s.NoError(testutils.FullInit(objA, testutils.UniqueInitializer(), testutils.JSONFieldsFilter))

	objB := &storage.AIWorkload{}
	...
	testCases := map[string]testCase{
		withAllAccess: {
			...
			expectedMissingIndices: []int{},
			expectedObjects:        []*storage.AIWorkload{objA, objB},
		...
		withAccess: {
			...
			expectedObjects:        []*storage.AIWorkload{objA},
			expectedWriteError:     nil,
		},
		withAccessToCluster: {
			context: sac.WithGlobalAccessScopeChecker(context.Background(),
...
func (s *AiWorkloadsStoreSuite) TestSACUpsert() {
...
func (s *AiWorkloadsStoreSuite) TestSACUpsertMany() {
	obj, _, testCases := s.getTestData(storage.Access_READ_WRITE_ACCESS)
	for name, testCase := range testCases {
		s.T().Run(fmt.Sprintf("with %s", name), func(t *testing.T) {
			assert.ErrorIs(t, s.store.UpsertMany(testCase.context, []*storage.AIWorkload{obj}), testCase.expectedWriteError)
		})
	}
}

func (s *AiWorkloadsStoreSuite) TestSACCount() {
...
func (s *AiWorkloadsStoreSuite) TestSACWalk() {
	objA, objB, testCases := s.getTestData(storage.Access_READ_ACCESS)
	s.Require().NoError(s.store.Upsert(withAllAccessCtx, objA))
	s.Require().NoError(s.store.Upsert(withAllAccessCtx, objB))
	...
	for name, testCase := range testCases {
		s.T().Run(fmt.Sprintf("with %s", name), func(t *testing.T) {
			identifiers := []string{}
			getIDs := func(obj *storage.AIWorkload) error {
...
func (s *AiWorkloadsStoreSuite) TestSACGetByQueryFn() {
	objA, objB, testCases := s.getTestData(storage.Access_READ_ACCESS)
	s.Require().NoError(s.store.Upsert(withAllAccessCtx, objA))
	s.Require().NoError(s.store.Upsert(withAllAccessCtx, objB))
...
func (s *AiWorkloadsStoreSuite) TestSACGetIDs() {
	objA, objB, testCases := s.getTestData(storage.Access_READ_ACCESS)
	s.Require().NoError(s.store.Upsert(withAllAccessCtx, objA))
	s.Require().NoError(s.store.Upsert(withAllAccessCtx, objB))
...
func (s *AiWorkloadsStoreSuite) TestSACExists() {
...
func (s *AiWorkloadsStoreSuite) TestSACGet() {
	objA, _, testCases := s.getTestData(storage.Access_READ_ACCESS)
	s.Require().NoError(s.store.Upsert(withAllAccessCtx, objA))
...
func (s *AiWorkloadsStoreSuite) TestSACDelete() {
...
func (s *AiWorkloadsStoreSuite) TestSACDeleteMany() {
	objA, objB, testCases := s.getTestData(storage.Access_READ_ACCESS, storage.Access_READ_WRITE_ACCESS)
	for name, testCase := range testCases {
		s.T().Run(fmt.Sprintf("with %s", name), func(t *testing.T) {
			s.SetupTest()
...
func (s *AiWorkloadsStoreSuite) TestSACGetMany() {
	objA, objB, testCases := s.getTestData(storage.Access_READ_ACCESS)
	s.Require().NoError(s.store.Upsert(withAllAccessCtx, objA))
	s.Require().NoError(s.store.Upsert(withAllAccessCtx, objB))
