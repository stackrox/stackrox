...
type RuntimeConfigurationCollectionsStoreSuite struct {
	suite.Suite
	store  Store
	testDB *pgtest.TestPostgres
}

func TestRuntimeConfigurationCollectionsStore(t *testing.T) {
	suite.Run(t, new(RuntimeConfigurationCollectionsStoreSuite))
}

func (s *RuntimeConfigurationCollectionsStoreSuite) SetupSuite() {

	s.testDB = pgtest.ForT(s.T())
	s.store = New(s.testDB.DB)
}

func (s *RuntimeConfigurationCollectionsStoreSuite) SetupTest() {
	ctx := sac.WithAllAccess(context.Background())
	tag, err := s.testDB.Exec(ctx, "TRUNCATE runtime_configuration_collections CASCADE")
	s.T().Log("runtime_configuration_collections", tag)
	s.store = New(s.testDB.DB)
	s.NoError(err)
}

func (s *RuntimeConfigurationCollectionsStoreSuite) TearDownSuite() {
	s.testDB.Teardown(s.T())
}

func (s *RuntimeConfigurationCollectionsStoreSuite) TestStore() {
	ctx := sac.WithAllAccess(context.Background())
