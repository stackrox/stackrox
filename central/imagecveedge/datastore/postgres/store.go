...
func (s *storeImpl) GetByQuery(ctx context.Context, query *v1.Query) ([]*storage.ImageCVEEdge, error) {
	...
	rows, err := pgSearch.RunGetManyQueryForSchema[storage.ImageCVEEdge](ctx, schema, query, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.ImageCVEEdge) error) error {
	fetcher, closer, err := pgSearch.RunCursorQueryForSchema[storage.ImageCVEEdge](ctx, schema, search.EmptyQuery(), s.db)
	if err != nil {
		return err
	}
	defer closer()
