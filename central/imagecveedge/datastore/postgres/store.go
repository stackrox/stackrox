...
func (s *storeImpl) Get(ctx context.Context, id string) (*storage.ImageCVEEdge, bool, error) {
	...
	data, err := postgres.RunGetQueryForSchemaType[storage.ImageCVEEdge](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetMany(ctx context.Context, ids []string) ([]*storage.ImageCVEEdge, []int, error) {
	...
	rows, err := postgres.RunGetManyQueryForSchemaType[storage.ImageCVEEdge](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.ImageCVEEdge) error) error {
	var sacQueryFilter *v1.Query
	fetcher, closer, err := postgres.RunCursorQueryForSchemaType[storage.ImageCVEEdge](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
