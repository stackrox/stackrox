...
func insertIntoBlobs(ctx context.Context, batch *pgx.Batch, obj *storage.Blob) error {
	...
	finalStr := "INSERT INTO blobs (Name, Length, serialized) VALUES($1, $2, $3) ON CONFLICT(Name) DO UPDATE SET Name = EXCLUDED.Name, Length = EXCLUDED.Length, serialized = EXCLUDED.serialized"
	batch.Queue(finalStr, values...)

	return nil
}

func (s *storeImpl) copyFromBlobs(ctx context.Context, tx *postgres.Tx, objs ...*storage.Blob) error {
	...
	for idx, obj := range objs {
		...
		if (idx+1)%batchSize == 0 || idx == len(objs)-1 {
			...
			_, err = tx.CopyFrom(ctx, pgx.Identifier{"blobs"}, copyCols, pgx.CopyFromRows(inputRows))
...
func (s *storeImpl) copyFrom(ctx context.Context, objs ...*storage.Blob) error {
	conn, release, err := s.acquireConn(ctx, ops.Get, "Blob")
	if err != nil {
		return err
	}
	defer release()
	...
	if err := s.copyFromBlobs(ctx, tx, objs...); err != nil {
...
func (s *storeImpl) upsert(ctx context.Context, objs ...*storage.Blob) error {
	...
	for _, obj := range objs {
		batch := &pgx.Batch{}
		if err := insertIntoBlobs(ctx, batch, obj); err != nil {
			return err
		}
...
func (s *storeImpl) Delete(ctx context.Context, name string) error {
	defer metrics.SetPostgresOperationDurationTime(time.Now(), ops.Remove, "Blob")
...
func (s *storeImpl) DeleteMany(ctx context.Context, identifiers []string) error {
	defer metrics.SetPostgresOperationDurationTime(time.Now(), ops.RemoveMany, "Blob")
...
func (s *storeImpl) Count(ctx context.Context) (int, error) {
	defer metrics.SetPostgresOperationDurationTime(time.Now(), ops.Count, "Blob")
...
func (s *storeImpl) Exists(ctx context.Context, name string) (bool, error) {
	defer metrics.SetPostgresOperationDurationTime(time.Now(), ops.Exists, "Blob")
...
func (s *storeImpl) Get(ctx context.Context, name string) (*storage.Blob, bool, error) {
	defer metrics.SetPostgresOperationDurationTime(time.Now(), ops.Get, "Blob")
	...
	data, err := pgSearch.RunGetQueryForSchema[storage.Blob](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetMany(ctx context.Context, identifiers []string) ([]*storage.Blob, []int, error) {
	defer metrics.SetPostgresOperationDurationTime(time.Now(), ops.GetMany, "Blob")
	...
	resultsByID := make(map[string]*storage.Blob, len(rows))
	...
	elems := make([]*storage.Blob, 0, len(resultsByID))
...
func (s *storeImpl) GetIDs(ctx context.Context) ([]string, error) {
	defer metrics.SetPostgresOperationDurationTime(time.Now(), ops.GetAll, "storage.BlobIDs")
	var sacQueryFilter *v1.Query
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.Blob) error) error {
	var sacQueryFilter *v1.Query
	scopeChecker := sac.GlobalAccessScopeChecker(ctx).AccessMode(storage.Access_READ_ACCESS).Resource(targetResource)
	if !scopeChecker.IsAllowed() {
		return nil
	}
	fetcher, closer, err := pgSearch.RunCursorQueryForSchema[storage.Blob](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
