...
const createTableQuery = "create table if not exists namespaces (id varchar primary key, value jsonb)"
...
//
	return &storeImpl{
		...
		upsertStmt: compileStmtOrPanic(db, "insert into namespaces(id, value) values($1, $2) on conflict(id) do update set value=$2"),
...
func (s *storeImpl) Get(id string) (*storage.NamespaceMetadata, bool, error) {
	...
	log.Infof("Took %d to query a NamespaceMetadata", time.Since(t).Milliseconds())
	...
	log.Infof("Took %d to scan a NamespaceMetadata", time.Since(t).Milliseconds())
