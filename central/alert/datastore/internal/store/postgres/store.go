...
func (s *storeImpl) GetByQuery(ctx context.Context, query *v1.Query) ([]*storage.Alert, error) {
	...
	rows, err := pgSearch.RunGetManyQueryForSchema[storage.Alert](ctx, schema, query, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.Alert) error) error {
	fetcher, closer, err := pgSearch.RunCursorQueryForSchema[storage.Alert](ctx, schema, search.EmptyQuery(), s.db)
	if err != nil {
		return err
	}
	defer closer()
