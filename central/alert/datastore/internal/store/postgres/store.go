...
func insertIntoAlerts(batch *pgx.Batch, pool pgSearch.BufferPool, obj *storage.Alert) (*[]byte, error) {
...
func copyFromAlerts(ctx context.Context, s pgSearch.Deleter, tx *postgres.Tx, pool pgSearch.BufferPool, objs ...*storage.Alert) error {
	if len(objs) == 0 {
		return nil
	}
