...
type storeImpl struct {
	*pgSearch.GenericSingleIDStore[storage.Alert, *storage.Alert]
	db    postgres.DB
	mutex sync.RWMutex
}
...
func New(db postgres.DB) Store {
	return &storeImpl{
		GenericSingleIDStore: pgSearch.NewGenericSingleIDStore[storage.Alert, *storage.Alert](
			db,
			targetResource,
			schema,
...
func metricsSetPostgresOperationDurationTime(start time.Time, op ops.Op) {
	metrics.SetPostgresOperationDurationTime(start, op, "Alert")
}
