...
const (
	...
	batchInsertTemplate = "insert into alerts (id, value, LifecycleStage, Time, State, Policy_Id, Policy_Name, Policy_Description, Policy_Severity) values %s on conflict(id) do update set value = EXCLUDED.value, LifecycleStage = EXCLUDED.LifecycleStage, Time = EXCLUDED.Time, State = EXCLUDED.State, Policy_Id = EXCLUDED.Policy_Id, Policy_Name = EXCLUDED.Policy_Name, Policy_Description = EXCLUDED.Policy_Description, Policy_Severity = EXCLUDED.Policy_Severity"
)
...
func (s *storeImpl) upsert(id string, obj *storage.Alert) error {
	...
	_, err = s.db.Exec(context.Background(), upsertStmt, id, value, obj.GetLifecycleStage(), obj.GetTime().String(), obj.GetState(), obj.GetPolicy().GetId(), obj.GetPolicy().GetName(), obj.GetPolicy().GetDescription(), obj.GetPolicy().GetSeverity())
	return err
}
...
func (s *storeImpl) UpsertMany(objs []*storage.Alert) error {
	...
	for start, end, ok := batch.Next(); ok; start, end, ok = batch.Next() {
		...
		for i, obj := range objs[start:end] {
			...
			id := keyFunc(obj)
			data = append(data, id, value, obj.GetLifecycleStage(), obj.GetTime().String(), obj.GetState(), obj.GetPolicy().GetId(), obj.GetPolicy().GetName(), obj.GetPolicy().GetDescription(), obj.GetPolicy().GetSeverity())
		}
