...
func (s *storeImpl) Delete(ctx context.Context, serialStr string) error {
	...
	q := search.NewQueryBuilder().AddDocIDs(serialStr).ProtoQuery()
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.ServiceIdentity) error) error {
	fetcher, closer, err := pgSearch.RunCursorQueryForSchema[storage.ServiceIdentity](ctx, schema, search.EmptyQuery(), s.db)
	if err != nil {
		return err
	}
	defer closer()
