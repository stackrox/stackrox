...
func (s *storeImpl) GetMany(ctx context.Context, ids []string) ([]*storage.ServiceIdentity, []int, error) {
	...
	rows, err := postgres.RunGetManyQueryForSchemaType[storage.ServiceIdentity](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.ServiceIdentity) error) error {
	...
	fetcher, closer, err := postgres.RunCursorQueryForSchemaType[storage.ServiceIdentity](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
