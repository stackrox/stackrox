...
type Store interface {
	...
	GetMany(ctx context.Context, identifiers []string) ([]*storage.ServiceIdentity, []int, error)
...
type storeImpl struct {
	*pgSearch.GenericStore[storage.ServiceIdentity, *storage.ServiceIdentity]
	db    postgres.DB
	mutex sync.RWMutex
}
...
func New(db postgres.DB) Store {
	return &storeImpl{
		db: db,
		GenericStore: pgSearch.NewGenericStore[storage.ServiceIdentity, *storage.ServiceIdentity](
			db,
			schema,
			pkGetter,
...
func pkGetter(obj *storage.ServiceIdentity) string {
...
func (s *storeImpl) Get(ctx context.Context, serialStr string) (*storage.ServiceIdentity, bool, error) {
	...
	data, err := pgSearch.RunGetQueryForSchema[storage.ServiceIdentity](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetMany(ctx context.Context, identifiers []string) ([]*storage.ServiceIdentity, []int, error) {
	...
	rows, err := pgSearch.RunGetManyQueryForSchema[storage.ServiceIdentity](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.ServiceIdentity) error) error {
	...
	fetcher, closer, err := pgSearch.RunCursorQueryForSchema[storage.ServiceIdentity](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
