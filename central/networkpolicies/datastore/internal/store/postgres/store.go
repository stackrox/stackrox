...
func (s *storeImpl) Get(ctx context.Context, id string) (*storage.NetworkPolicy, bool, error) {
	...
	data, err := postgres.RunGetQueryForSchemaType[storage.NetworkPolicy](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetMany(ctx context.Context, ids []string) ([]*storage.NetworkPolicy, []int, error) {
	...
	rows, err := postgres.RunGetManyQueryForSchemaType[storage.NetworkPolicy](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.NetworkPolicy) error) error {
	...
	fetcher, closer, err := postgres.RunCursorQueryForSchemaType[storage.NetworkPolicy](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
