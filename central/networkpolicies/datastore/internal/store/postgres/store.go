...
func (s *storeImpl) Get(ctx context.Context, id string) (*storage.NetworkPolicy, bool, error) {
	...
	data, err := postgres.RunGetQueryForSchema[storage.NetworkPolicy](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetByQuery(ctx context.Context, query *v1.Query) ([]*storage.NetworkPolicy, error) {
	...
	rows, err := postgres.RunGetManyQueryForSchema[storage.NetworkPolicy](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.NetworkPolicy) error) error {
	...
	fetcher, closer, err := postgres.RunCursorQueryForSchema[storage.NetworkPolicy](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
