...
type Store interface {
	...
	GetMany(ctx context.Context, identifiers []string) ([]*storage.NetworkPolicy, []int, error)
...
type storeImpl struct {
	*pgSearch.GenericStore[storage.NetworkPolicy, *storage.NetworkPolicy]
	db    postgres.DB
	mutex sync.RWMutex
}
...
func New(db postgres.DB) Store {
	return &storeImpl{
		db: db,
		GenericStore: pgSearch.NewGenericStore[storage.NetworkPolicy, *storage.NetworkPolicy](
			db,
			schema,
			pkGetter,
...
func pkGetter(obj *storage.NetworkPolicy) string {
	return obj.GetId()
}
...
func (s *storeImpl) GetMany(ctx context.Context, identifiers []string) ([]*storage.NetworkPolicy, []int, error) {
