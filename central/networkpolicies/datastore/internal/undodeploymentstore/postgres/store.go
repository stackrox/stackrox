...
func (s *storeImpl) GetMany(ctx context.Context, ids []string) ([]*storage.NetworkPolicyApplicationUndoDeploymentRecord, []int, error) {
	...
	rows, err := postgres.RunGetManyQueryForSchemaType[storage.NetworkPolicyApplicationUndoDeploymentRecord](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.NetworkPolicyApplicationUndoDeploymentRecord) error) error {
	var sacQueryFilter *v1.Query
	fetcher, closer, err := postgres.RunCursorQueryForSchemaType[storage.NetworkPolicyApplicationUndoDeploymentRecord](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
