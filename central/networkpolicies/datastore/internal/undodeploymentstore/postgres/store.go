...
const createTableQuery = "create table if not exists networkpolicies-undodeployment (id varchar primary key, value jsonb)"
...
//
	return &storeImpl{
		...
		countStmt: compileStmtOrPanic(db, "select count(*) from networkpolicies-undodeployment"),
		existsStmt: compileStmtOrPanic(db, "select exists(select 1 from networkpolicies-undodeployment where id = $1)"),
		getIDsStmt: compileStmtOrPanic(db, "select id from networkpolicies-undodeployment"),
		getStmt: compileStmtOrPanic(db, "select value from networkpolicies-undodeployment where id = $1"),
		getManyStmt: compileStmtOrPanic(db, "select value from networkpolicies-undodeployment where id = ANY($1::text[])"),
		upsertStmt: compileStmtOrPanic(db, "insert into networkpolicies-undodeployment(id, value) values($1, $2) on conflict(id) do update set value=$2"),
		deleteStmt: compileStmtOrPanic(db, "delete from networkpolicies-undodeployment where id = $1"),
		deleteManyStmt: compileStmtOrPanic(db, "delete from networkpolicies-undodeployment where id = ANY($1::text[])"),
		walkStmt: compileStmtOrPanic(db, "select value from networkpolicies-undodeployment"),
		walkWithIDStmt: compileStmtOrPanic(db, "select id, value from networkpolicies-undodeployment"),
	}
//
}
...
func (s *storeImpl) Get(id string) (*storage.NetworkPolicyApplicationUndoDeploymentRecord, bool, error) {
	...
	log.Infof("Took %d to query a NetworkPolicyApplicationUndoDeploymentRecord", time.Since(t).Milliseconds())
	...
	log.Infof("Took %d to scan a NetworkPolicyApplicationUndoDeploymentRecord", time.Since(t).Milliseconds())
	...
	log.Infof("Took %d to unmarshal a NetworkPolicyApplicationUndoDeploymentRecord", time.Since(t).Milliseconds())
