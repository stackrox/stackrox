...
func TestReprocessImagesAndResyncDeployments_SkipBrokenSensor(t *testing.T) {
	...
	updatedImageTypeCond := gomock.Cond(func(x any) bool {
		msg, ok := x.(*central.MsgToSensor)
		require.True(t, ok)
		return event.GetEventTypeWithoutPrefix(msg.GetMsg()) == "UpdatedImage"
	})
	reprocessDeploymentsTypeCond := gomock.Cond(func(x any) bool {
		msg, ok := x.(*central.MsgToSensor)
		require.True(t, ok)
...
func TestInjectMessage(t *testing.T) {
	...
	contextWithTimeout := gomock.Cond(func(x any) bool {
		ctx, ok := x.(context.Context)
		require.True(t, ok)
		_, hasTimeout := ctx.Deadline()
		return hasTimeout
	})
	contextWithoutTimeout := gomock.Cond(func(x any) bool {
		ctx, ok := x.(context.Context)
		require.True(t, ok)
		_, hasTimeout := ctx.Deadline()
		return !hasTimeout
