...
func New(db *badger.DB) store.Store {
	globaldb.RegisterBucket(podBucket, "Pod")
	return &storeImpl{
...
func (b *storeImpl) GetPod(id string) (pod *storage.Pod, exists bool, err error) {
	defer metrics.SetBadgerOperationDurationTime(time.Now(), ops.Get, "Pod")
...
func (b *storeImpl) GetPods() ([]*storage.Pod, error) {
	defer metrics.SetBadgerOperationDurationTime(time.Now(), ops.GetMany, "Pod")
...
func (b *storeImpl) CountPods() (int, error) {
	defer metrics.SetBadgerOperationDurationTime(time.Now(), ops.Count, "Pod")
...
func (b *storeImpl) RemovePod(id string) error {
	defer metrics.SetBadgerOperationDurationTime(time.Now(), ops.Remove, "Pod")
