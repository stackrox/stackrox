...
func (s *storeImpl) Get(ctx context.Context, id string) (*storage.Pod, bool, error) {
	...
	data, err := postgres.RunGetQueryForSchemaType[storage.Pod](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetMany(ctx context.Context, ids []string) ([]*storage.Pod, []int, error) {
	...
	rows, err := postgres.RunGetManyQueryForSchemaType[storage.Pod](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.Pod) error) error {
	...
	fetcher, closer, err := postgres.RunCursorQueryForSchemaType[storage.Pod](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
