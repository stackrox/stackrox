...
func (s *storeImpl) GetByQuery(ctx context.Context, query *v1.Query) ([]*storage.Pod, error) {
	...
	rows, err := pgSearch.RunGetManyQueryForSchema[storage.Pod](ctx, schema, query, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.Pod) error) error {
	fetcher, closer, err := pgSearch.RunCursorQueryForSchema[storage.Pod](ctx, schema, search.EmptyQuery(), s.db)
	if err != nil {
		return err
	}
	defer closer()
