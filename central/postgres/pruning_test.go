...
func (s *PostgresPruningSuite) TestGetOrphanedProcessIDs() {
	cases := []struct {
		name              string
	...
	for _, c := range cases {
		s.T().Run(c.name, func(t *testing.T) {
			...
			idsToPrune, err := GetOrphanedProcessIndicatorIDs(s.ctx, s.testDB.DB, orphanWindow)
			log.Infof("IDs to prune %s", idsToPrune)
			log.Infof("expected IDs to prune %s", c.expectedDeletions)
			s.Nil(err)
			s.ElementsMatch(c.expectedDeletions, idsToPrune)
