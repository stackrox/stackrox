...
func keyToID(key *storage.ProcessWhitelistKey) (string, error) {
	if stringutils.AllNotEmpty(key.GetDeploymentId(), key.GetContainerName()) {
		return fmt.Sprintf("%s:%s:%s", deploymentContainerKeyPrefix, key.GetDeploymentId(), key.GetContainerName()), nil
	}
