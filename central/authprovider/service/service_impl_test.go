...
func (s *authProviderServiceTestSuite) TestPostDuplicateAuthProvider() {
	...
	// register a backend, hit an error when storing the auth provider
	// in DB and deregister its backend.
	// Currently, the deregister part is missing and will be added
	// in a follow-up PR.
	_, err = s.service.PostAuthProvider(ctx, postRequest)
	s.Error(err)
	s.Equal(2, openshiftAuth.GetRegisteredBackendCount())
