...
type storeImpl struct {
	*pgSearch.GenericStore[storage.AuthProvider, *storage.AuthProvider]
	db    postgres.DB
	mutex sync.RWMutex
}
...
func New(db postgres.DB) Store {
	return &storeImpl{
		db: db,
		GenericStore: pgSearch.NewGenericStore[storage.AuthProvider, *storage.AuthProvider](
			db,
			schema,
			pkGetter,
...
func pkGetter(obj *storage.AuthProvider) string {
	return obj.GetId()
}

func metricsSetPostgresOperationDurationTime(start time.Time, op ops.Op) {
	metrics.SetPostgresOperationDurationTime(start, op, "AuthProvider")
}

func metricsSetAcquireDBConnDuration(start time.Time, op ops.Op) {
	metrics.SetAcquireDBConnDuration(start, op, "AuthProvider")
}
...
func (s *storeImpl) upsert(ctx context.Context, objs ...*storage.AuthProvider) error {
	...
	for _, obj := range objs {
		batch := &pgx.Batch{}
		if err := insertIntoAuthProviders(ctx, batch, obj); err != nil {
			return err
		}
...
func (s *storeImpl) Get(ctx context.Context, id string) (*storage.AuthProvider, bool, error) {
	...
	data, err := pgSearch.RunGetQueryForSchema[storage.AuthProvider](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetMany(ctx context.Context, identifiers []string) ([]*storage.AuthProvider, []int, error) {
	...
	rows, err := pgSearch.RunGetManyQueryForSchema[storage.AuthProvider](ctx, schema, q, s.db)
	if err != nil {
	...
	elems := make([]*storage.AuthProvider, 0, len(resultsByID))
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.AuthProvider) error) error {
	...
	fetcher, closer, err := pgSearch.RunCursorQueryForSchema[storage.AuthProvider](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
