...
func (s *storeImpl) GetByQuery(ctx context.Context, query *v1.Query) ([]*storage.NodeComponentCVEEdge, error) {
	...
	rows, err := pgSearch.RunGetManyQueryForSchema[storage.NodeComponentCVEEdge](ctx, schema, query, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.NodeComponentCVEEdge) error) error {
	fetcher, closer, err := pgSearch.RunCursorQueryForSchema[storage.NodeComponentCVEEdge](ctx, schema, search.EmptyQuery(), s.db)
	if err != nil {
		return err
	}
	defer closer()
