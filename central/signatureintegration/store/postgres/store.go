...
func (s *storeImpl) Get(ctx context.Context, id string) (*storage.SignatureIntegration, bool, error) {
	...
	data, err := postgres.RunGetQueryForSchemaType[storage.SignatureIntegration](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetMany(ctx context.Context, ids []string) ([]*storage.SignatureIntegration, []int, error) {
	...
	rows, err := postgres.RunGetManyQueryForSchemaType[storage.SignatureIntegration](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.SignatureIntegration) error) error {
	...
	fetcher, closer, err := postgres.RunCursorQueryForSchemaType[storage.SignatureIntegration](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
