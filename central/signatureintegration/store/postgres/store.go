...
type Store interface {
	...
	GetMany(ctx context.Context, identifiers []string) ([]*storage.SignatureIntegration, []int, error)
...
func (s *storeImpl) Get(ctx context.Context, id string) (*storage.SignatureIntegration, bool, error) {
	...
	data, err := postgres.RunGetQueryForSchema[storage.SignatureIntegration](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetMany(ctx context.Context, identifiers []string) ([]*storage.SignatureIntegration, []int, error) {
	...
	rows, err := postgres.RunGetManyQueryForSchema[storage.SignatureIntegration](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.SignatureIntegration) error) error {
	...
	fetcher, closer, err := postgres.RunCursorQueryForSchema[storage.SignatureIntegration](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
