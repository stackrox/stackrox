...
//value of singlePK
//{0xc000169600 Id {false GetId()} Id  string string varchar {false  true false false} {  false false false false}}
...
func (s *storeImpl) copyIntoSignatureintegrations(ctx context.Context, tx pgx.Tx, objs ...*storage.SignatureIntegration) error {
	...
	lowerTable := strings.ToLower("signatureintegrations")
...
func (s *storeImpl) copyIntoSignatureintegrationsPublicKeys(ctx context.Context, tx pgx.Tx, signatureintegrations_Id string, objs ...*storage.CosignPublicKeyVerification_PublicKey) error {
	...
	// Todo: I'm sure there is a cleaner way to do this.
	copyCols := []string{}
	columns := "signatureintegrations_Id, idx, Name, PublicKeyPemEnc"
	columns = strings.ToLower(columns)
	...
	lowerTable := strings.ToLower("signatureintegrations_PublicKeys")
