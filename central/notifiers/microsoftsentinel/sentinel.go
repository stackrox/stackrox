...
func (s sentinel) AlertNotify(ctx context.Context, alert *storage.Alert) error {
	...
	_, err = s.azlogsClient.Upload(ctx, s.sentinel().GetDataCollectionRuleConfig().GetDataCollectionRuleId(), s.sentinel().GetDataCollectionRuleConfig().GetStreamName(), data, &azlogs.UploadOptions{})
	if err != nil {
...
func Validate(sentinel *storage.MicrosoftSentinel, validateSecret bool) error {
	...
	if sentinel.GetDataCollectionRuleConfig().GetDataCollectionRuleId() == "" {
		errorList.AddString("Data Collection Rule Id must be specified")
	}

	if sentinel.GetDataCollectionRuleConfig().GetStreamName() == "" {
