...
func (s *storeImpl) GetByQuery(ctx context.Context, query *v1.Query) ([]*storage.NetworkBaseline, error) {
	...
	rows, err := pgSearch.RunGetManyQueryForSchema[storage.NetworkBaseline](ctx, schema, query, s.db)
	if err != nil {
...
func (s *storeImpl) GetMany(ctx context.Context, identifiers []string) ([]*storage.NetworkBaseline, []int, error) {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.NetworkBaseline) error) error {
	fetcher, closer, err := pgSearch.RunCursorQueryForSchema[storage.NetworkBaseline](ctx, schema, search.EmptyQuery(), s.db)
	if err != nil {
		return err
	}
	defer closer()
