...
func keyFunc(msg proto.Message) string {
	return msg.(*storage.NetworkBaseline).GetDeploymentId()
}
...
const createTableQuery = "create table if not exists networkbaseline (id varchar primary key, value jsonb)"
...
//
	return &storeImpl{
		...
		countStmt: compileStmtOrPanic(db, "select count(*) from networkbaseline"),
		existsStmt: compileStmtOrPanic(db, "select exists(select 1 from networkbaseline where id = $1)"),
		getIDsStmt: compileStmtOrPanic(db, "select id from networkbaseline"),
		getStmt: compileStmtOrPanic(db, "select value from networkbaseline where id = $1"),
		getManyStmt: compileStmtOrPanic(db, "select value from networkbaseline where id = ANY($1::text[])"),
		upsertStmt: compileStmtOrPanic(db, "insert into networkbaseline(id, value) values($1, $2) on conflict(id) do update set value=$2"),
		deleteStmt: compileStmtOrPanic(db, "delete from networkbaseline where id = $1"),
		deleteManyStmt: compileStmtOrPanic(db, "delete from networkbaseline where id = ANY($1::text[])"),
		walkStmt: compileStmtOrPanic(db, "select value from networkbaseline"),
		walkWithIDStmt: compileStmtOrPanic(db, "select id, value from networkbaseline"),
	}
//
}
...
func (s *storeImpl) Get(id string) (*storage.NetworkBaseline, bool, error) {
	...
	log.Infof("Took %d to query a NetworkBaseline", time.Since(t).Milliseconds())
	...
	log.Infof("Took %d to scan a NetworkBaseline", time.Since(t).Milliseconds())
	...
	log.Infof("Took %d to unmarshal a NetworkBaseline", time.Since(t).Milliseconds())
...
func (s *storeImpl) GetMany(ids []string) ([]*storage.NetworkBaseline, []int, error) {
	...
	for rows.Next() {
		...
		elem := msg.(*storage.NetworkBaseline)
