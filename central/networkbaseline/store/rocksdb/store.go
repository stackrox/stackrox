...
func (b *storeImpl) Count(_ context.Context) (int, error) {
	defer metrics.SetRocksDBOperationDurationTime(time.Now(), ops.Count, "NetworkBaseline")
...
func (b *storeImpl) Exists(_ context.Context, id string) (bool, error) {
	defer metrics.SetRocksDBOperationDurationTime(time.Now(), ops.Exists, "NetworkBaseline")
...
func (b *storeImpl) GetIDs(_ context.Context) ([]string, error) {
	defer metrics.SetRocksDBOperationDurationTime(time.Now(), ops.GetAll, "NetworkBaselineIDs")
...
func (b *storeImpl) GetMany(_ context.Context, ids []string) ([]*storage.NetworkBaseline, []int, error) {
	defer metrics.SetRocksDBOperationDurationTime(time.Now(), ops.GetMany, "NetworkBaseline")

	msgs, missingIndices, err := b.crud.GetMany(ids)
	if err != nil {
		return nil, nil, err
	}
	objs := make([]*storage.NetworkBaseline, 0, len(msgs))
	for _, m := range msgs {
		objs = append(objs, m.(*storage.NetworkBaseline))
	}
	return objs, missingIndices, nil
}

// Upsert inserts the object into the DB
func (b *storeImpl) Upsert(_ context.Context, obj *storage.NetworkBaseline) error {
	defer metrics.SetRocksDBOperationDurationTime(time.Now(), ops.Add, "NetworkBaseline")

	return b.crud.Upsert(obj)
}

// UpsertMany batches objects into the DB
func (b *storeImpl) UpsertMany(_ context.Context, objs []*storage.NetworkBaseline) error {
	defer metrics.SetRocksDBOperationDurationTime(time.Now(), ops.AddMany, "NetworkBaseline")
...
func (b *storeImpl) Delete(_ context.Context, id string) error {
	defer metrics.SetRocksDBOperationDurationTime(time.Now(), ops.Remove, "NetworkBaseline")
...
func (b *storeImpl) DeleteMany(_ context.Context, ids []string) error {
	defer metrics.SetRocksDBOperationDurationTime(time.Now(), ops.RemoveMany, "NetworkBaseline")

	return b.crud.DeleteMany(ids)
}

// Walk iterates over all of the objects in the store and applies the closure
func (b *storeImpl) Walk(ctx context.Context, fn func(obj *storage.NetworkBaseline) error) error {
	return b.crud.Walk(func(msg proto.Message) error {
		return fn(msg.(*storage.NetworkBaseline))
	})
}
