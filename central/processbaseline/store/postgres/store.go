...
func (s *storeImpl) GetByQuery(ctx context.Context, query *v1.Query) ([]*storage.ProcessBaseline, error) {
	...
	rows, err := pgSearch.RunGetManyQueryForSchema[storage.ProcessBaseline](ctx, schema, query, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.ProcessBaseline) error) error {
	fetcher, closer, err := pgSearch.RunCursorQueryForSchema[storage.ProcessBaseline](ctx, schema, search.EmptyQuery(), s.db)
	if err != nil {
		return err
	}
	defer closer()
