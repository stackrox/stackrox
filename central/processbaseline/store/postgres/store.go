...
const (
	...
	getWithRollupStmt = "select row_to_json((select record from (select table0.Id as Id, table0.Key_DeploymentId as Key_DeploymentId, table0.Key_ContainerName as Key_ContainerName, table0.Key_ClusterId as Key_ClusterId, table0.Key_Namespace as Key_Namespace, table0.Created as Created, table0.UserLockedTimestamp as UserLockedTimestamp, table0.StackRoxLockedTimestamp as StackRoxLockedTimestamp, table0.LastUpdate as LastUpdate, to_json(join0)->'array' as join0, to_json(join1)->'array' as join1 from processbaselines table0 left join lateral (select array(select json_build_object('idx', table1.idx, 'Element_ProcessName', table1.Element_ProcessName, 'Auto', table1.Auto) from processbaselines_Elements table1 where (table0.Id = table1.processbaselines_Id))) join0 on true left join lateral (select array(select json_build_object('idx', table2.idx, 'Element_ProcessName', table2.Element_ProcessName, 'Auto', table2.Auto) from processbaselines_ElementGraveyard table2 where (table0.Id = table2.processbaselines_Id))) join1 on true where (table0.Id = $1)) record ))"
