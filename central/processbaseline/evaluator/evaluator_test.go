...
func TestProcessBaselineEvaluator(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			if c.indicators != nil {
				mockIndicators.EXPECT().GetByQueryFn(gomock.Any(), gomock.Any(), gomock.Any()).Do(
					func(_ context.Context, _ *v1.Query, fn func(indicator *storage.ProcessIndicator) error) {
