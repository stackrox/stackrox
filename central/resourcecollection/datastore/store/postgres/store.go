...
func (s *storeImpl) Delete(ctx context.Context, id string) error {
	defer metrics.SetPostgresOperationDurationTime(time.Now(), ops.Remove, "ResourceCollection")
...
func (s *storeImpl) DeleteMany(ctx context.Context, identifiers []string) error {
	defer metrics.SetPostgresOperationDurationTime(time.Now(), ops.RemoveMany, "ResourceCollection")
...
func (s *storeImpl) Count(ctx context.Context) (int, error) {
	defer metrics.SetPostgresOperationDurationTime(time.Now(), ops.Count, "ResourceCollection")
...
func (s *storeImpl) Exists(ctx context.Context, id string) (bool, error) {
	defer metrics.SetPostgresOperationDurationTime(time.Now(), ops.Exists, "ResourceCollection")
...
func (s *storeImpl) Get(ctx context.Context, id string) (*storage.ResourceCollection, bool, error) {
	defer metrics.SetPostgresOperationDurationTime(time.Now(), ops.Get, "ResourceCollection")
...
func (s *storeImpl) GetByQuery(ctx context.Context, query *v1.Query) ([]*storage.ResourceCollection, error) {
	defer metrics.SetPostgresOperationDurationTime(time.Now(), ops.GetByQuery, "ResourceCollection")

	rows, err := pgSearch.RunGetManyQueryForSchema[storage.ResourceCollection](ctx, schema, query, s.db)
	if err != nil {
...
func (s *storeImpl) GetMany(ctx context.Context, identifiers []string) ([]*storage.ResourceCollection, []int, error) {
	defer metrics.SetPostgresOperationDurationTime(time.Now(), ops.GetMany, "ResourceCollection")
	...
	rows, err := pgSearch.RunGetManyQueryForSchema[storage.ResourceCollection](ctx, schema, q, s.db)
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
	...
	resultsByID := make(map[string]*storage.ResourceCollection, len(rows))
	...
	elems := make([]*storage.ResourceCollection, 0, len(resultsByID))
...
func (s *storeImpl) GetIDs(ctx context.Context) ([]string, error) {
	defer metrics.SetPostgresOperationDurationTime(time.Now(), ops.GetAll, "storage.ResourceCollectionIDs")
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.ResourceCollection) error) error {
	fetcher, closer, err := pgSearch.RunCursorQueryForSchema[storage.ResourceCollection](ctx, schema, search.EmptyQuery(), s.db)
	if err != nil {
		return err
	}
	defer closer()
