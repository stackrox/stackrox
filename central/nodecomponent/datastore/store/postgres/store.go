...
func (s *storeImpl) Get(ctx context.Context, id string) (*storage.NodeComponent, bool, error) {
	...
	data, err := postgres.RunGetQueryForSchemaType[storage.NodeComponent](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetMany(ctx context.Context, ids []string) ([]*storage.NodeComponent, []int, error) {
	...
	rows, err := postgres.RunGetManyQueryForSchemaType[storage.NodeComponent](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.NodeComponent) error) error {
	var sacQueryFilter *v1.Query
	fetcher, closer, err := postgres.RunCursorQueryForSchemaType[storage.NodeComponent](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
