...
type Store interface {
	...
	GetByQuery(ctx context.Context, query *v1.Query) ([]*storage.NodeComponent, error)
	GetMany(ctx context.Context, identifiers []string) ([]*storage.NodeComponent, []int, error)
...
func (s *storeImpl) Get(ctx context.Context, id string) (*storage.NodeComponent, bool, error) {
	...
	data, err := pgSearch.RunGetQueryForSchema[storage.NodeComponent](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetByQuery(ctx context.Context, query *v1.Query) ([]*storage.NodeComponent, error) {
	defer metrics.SetPostgresOperationDurationTime(time.Now(), ops.GetByQuery, "NodeComponent")

	rows, err := pgSearch.RunGetManyQueryForSchema[storage.NodeComponent](ctx, schema, query, s.db)
	if err != nil {
...
func (s *storeImpl) GetMany(ctx context.Context, identifiers []string) ([]*storage.NodeComponent, []int, error) {
	...
	rows, err := pgSearch.RunGetManyQueryForSchema[storage.NodeComponent](ctx, schema, q, s.db)
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
	...
	resultsByID := make(map[string]*storage.NodeComponent, len(rows))
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.NodeComponent) error) error {
	fetcher, closer, err := pgSearch.RunCursorQueryForSchema[storage.NodeComponent](ctx, schema, search.EmptyQuery(), s.db)
	if err != nil {
		return err
	}
	defer closer()
...
func dropTableNodeComponents(ctx context.Context, db postgres.DB) {
