...
func (s *storeImpl) Get(ctx context.Context, id string) (*storage.InitBundleMeta, bool, error) {
	...
	data, err := postgres.RunGetQueryForSchema[storage.InitBundleMeta](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetMany(ctx context.Context, identifiers []string) ([]*storage.InitBundleMeta, []int, error) {
	...
	rows, err := postgres.RunGetManyQueryForSchema[storage.InitBundleMeta](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.InitBundleMeta) error) error {
	...
	fetcher, closer, err := postgres.RunCursorQueryForSchema[storage.InitBundleMeta](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
