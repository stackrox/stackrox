...
//
	return &storeImpl{
		...
		countStmt: compileStmtOrPanic(db, "select count(*) from clusterinitbundles"),
		existsStmt: compileStmtOrPanic(db, "select exists(select 1 from clusterinitbundles where id = $1)"),
		...
		walkWithIDStmt: compileStmtOrPanic(db, "select id, value from clusterinitbundles"),
	}
//
}
