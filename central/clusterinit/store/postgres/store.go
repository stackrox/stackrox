...
const (
		countStmt = "select count(*) from InitBundleMeta"
		existsStmt = "select exists(select 1 from InitBundleMeta where id = $1)"
		getIDsStmt = "select id from InitBundleMeta"
		getStmt = "select serialized from InitBundleMeta where id = $1"
		getManyStmt = "select serialized from InitBundleMeta where id = ANY($1::text[])"
		upsertStmt = "insert into InitBundleMeta (id, value) values($1, $2) on conflict(id) do update set value = EXCLUDED.value"
		deleteStmt = "delete from InitBundleMeta where id = $1"
		deleteManyStmt = "delete from InitBundleMeta where id = ANY($1::text[])"
		walkStmt = "select serialized from InitBundleMeta"
		walkWithIDStmt = "select id, serialized from InitBundleMeta"
)

var (
	log = logging.LoggerForModule()

	table = "InitBundleMeta"
...
func keyFunc(msg proto.Message) string {
	return msg.(*storage.InitBundleMeta).GetId()
}

const (
	createTableQuery = "create table if not exists InitBundleMeta (id varchar primary key, value jsonb)"
	createIDIndexQuery = "create index if not exists InitBundleMeta_id on InitBundleMeta using hash ((id))"

	batchInsertTemplate = "insert into InitBundleMeta (id, value) values %s on conflict(id) do update set value = EXCLUDED.value"
)
...
func New(db *pgxpool.Pool) Store {
	...
	for _, table := range []string {
		"create table if not exists InitBundleMeta(serialized jsonb not null, PRIMARY KEY ());",
...
func (s *storeImpl) GetIDs() ([]string, error) {
	defer metrics.SetPostgresOperationDurationTime(time.Now(), ops.GetAll, "InitBundleMetaIDs")
...
func (s *storeImpl) Get(id string) (*storage.InitBundleMeta, bool, error) {
	...
	defer metrics.SetJSONPBOperationDurationTime(time.Now(), "Unmarshal", "InitBundleMeta")
	if err := jsonpb.Unmarshal(buf, msg); err != nil {
		return nil, false, err
	}
	return msg.(*storage.InitBundleMeta), true, nil
}
...
func (s *storeImpl) GetMany(ids []string) ([]*storage.InitBundleMeta, []int, error) {
	...
	for rows.Next() {
		...
		metrics.SetJSONPBOperationDurationTime(t, "Unmarshal", "InitBundleMeta")
		elem := msg.(*storage.InitBundleMeta)
...
func (s *storeImpl) upsert(id string, obj0 *storage.InitBundleMeta) error {
	t := time.Now()
	serialized, err := marshaler.MarshalToString(obj0)
	if err != nil {
		return err
	}
	metrics.SetJSONPBOperationDurationTime(t, "Marshal", "InitBundleMeta")
	conn, release := s.acquireConn(ops.Add, "InitBundleMeta")
	defer release()
...
//		}
	...
	localQuery := "insert into InitBundleMeta(serialized) values($1) on conflict() do update set serialized = EXCLUDED.serialized"
...
func (s *storeImpl) Upsert(obj *storage.InitBundleMeta) error {
	defer metrics.SetPostgresOperationDurationTime(time.Now(), ops.Add, "InitBundleMeta")
...
func (s *storeImpl) UpsertMany(objs []*storage.InitBundleMeta) error {
	...
	conn, release := s.acquireConn(ops.AddMany, "InitBundleMeta")
	defer release()

	defer metrics.SetPostgresOperationDurationTime(time.Now(), ops.AddMany, "InitBundleMeta")
	numElems := 2
...
func (s *storeImpl) Walk(fn func(obj *storage.InitBundleMeta) error) error {
	...
	for rows.Next() {
		...
		return fn(msg.(*storage.InitBundleMeta))
	}
	return nil
}
