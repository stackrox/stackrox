...
const createTableQuery = "create table if not exists clusterinitbundles (id varchar primary key, value jsonb)"
...
func (s *storeImpl) Get(id string) (*storage.InitBundleMeta, bool, error) {
	...
	log.Infof("Took %d to query a InitBundleMeta", time.Since(t).Milliseconds())
	...
	log.Infof("Took %d to scan a InitBundleMeta", time.Since(t).Milliseconds())
	...
	log.Infof("Took %d to unmarshal a InitBundleMeta", time.Since(t).Milliseconds())
