...
var (
	...
	bucket = []byte("clusterinitbundles")
)
...
func (b *storeImpl) GetMany(_ context.Context, ids []string) ([]*storage.InitBundleMeta, []int, error) {
	...
	objs := make([]*storage.InitBundleMeta, 0, len(msgs))
	for _, m := range msgs {
		objs = append(objs, m.(*storage.InitBundleMeta))
	}
...
func (b *storeImpl) Walk(ctx context.Context, fn func(obj *storage.InitBundleMeta) error) error {
