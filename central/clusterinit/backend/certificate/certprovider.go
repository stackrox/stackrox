...
type Provider interface {
	...
	GetCRSCert(validFor time.Time, validUntil time.Duration) (*mtls.IssuedCert, uuid.UUID, error)
}
...
func (c *certProviderImpl) GetCRSCert(validUntil time.Time, validFor time.Duration) (*mtls.IssuedCert, uuid.UUID, error) {
	cert, id, err := clusters.IssueSecuredClusterCRSCertificates(validUntil, validFor)
	if err != nil {
