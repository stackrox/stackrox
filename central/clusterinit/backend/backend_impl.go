...
func (b *backendImpl) InitiateClusterRegistration(ctx context.Context, initArtifactId, clusterName string) error {
	if err := access.CheckAccess(ctx, storage.Access_READ_WRITE_ACCESS); err != nil {
		return err
	}

	return b.store.InitiateClusterRegistration(ctx, initArtifactId, clusterName)
}

func (b *backendImpl) MarkClusterRegistrationComplete(ctx context.Context, initArtifactId, clusterName string) error {
	if err := access.CheckAccess(ctx, storage.Access_READ_WRITE_ACCESS); err != nil {
		return err
	}

	return b.store.MarkClusterRegistrationComplete(ctx, initArtifactId, clusterName)
}
