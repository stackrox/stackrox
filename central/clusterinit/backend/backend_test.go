...
func (s *clusterInitBackendTestSuite) TestCrsAutoRevocationOneShot() {
	...
	crsWithMeta, err := s.backend.IssueCRS(s.ctx, crsName, time.Time{}, 1)
	s.Require().NoError(err)
	id := crsWithMeta.Meta.Id

	s.Require().NoErrorf(s.backend.CheckRevoked(s.ctx, id), "CRS %q is revoked", id)

	s.Require().NoErrorf(s.backend.InitiateClusterRegistration(s.ctx, id, clusterName), "recording initiated registration for CRS %q failed", id)
	s.Require().Errorf(s.backend.InitiateClusterRegistration(s.ctx, id, clusterName), "cluster registration still possible")
	s.Require().NoErrorf(s.backend.MarkClusterRegistrationComplete(s.ctx, id, clusterName), "recording completed registration for CRS %q failed", id)
	s.Require().Errorf(s.backend.CheckRevoked(s.ctx, id), "CRS %q is not revoked", id)
}
...
func (s *clusterInitBackendTestSuite) TestCrsAutoRevocationAfterTwoRegistrations() {
	...
	crsWithMeta, err := s.backend.IssueCRS(s.ctx, crsName, time.Time{}, 2)
	s.Require().NoError(err)
	id := crsWithMeta.Meta.Id
	...
	clusterName := fmt.Sprintf("some-cluster-%s", uuid.NewV4().String())
	s.Require().NoErrorf(s.backend.InitiateClusterRegistration(s.ctx, id, clusterName), "recording initiated registration for CRS %q failed", id)
	s.Require().NoErrorf(s.backend.InitiateClusterRegistration(s.ctx, id, clusterName), "cluster registration not possible")
	s.Require().NoErrorf(s.backend.MarkClusterRegistrationComplete(s.ctx, id, clusterName), "recording completed registration for CRS %q failed", id)
	s.Require().NoErrorf(s.backend.CheckRevoked(s.ctx, id), "CRS %q is revoked", id)
