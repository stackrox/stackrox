...
func (s *storeImpl) GetByQuery(ctx context.Context, query *v1.Query) ([]*storage.ProcessIndicator, error) {
	...
	rows, err := pgSearch.RunGetManyQueryForSchema[storage.ProcessIndicator](ctx, schema, query, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.ProcessIndicator) error) error {
	fetcher, closer, err := pgSearch.RunCursorQueryForSchema[storage.ProcessIndicator](ctx, schema, search.EmptyQuery(), s.db)
	if err != nil {
		return err
	}
	defer closer()
