...
func (s *storeImpl) Get(ctx context.Context, id string) (*storage.ProcessIndicator, bool, error) {
	...
	data, err := postgres.RunGetQueryForSchemaType[storage.ProcessIndicator](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetMany(ctx context.Context, ids []string) ([]*storage.ProcessIndicator, []int, error) {
	...
	rows, err := postgres.RunGetManyQueryForSchemaType[storage.ProcessIndicator](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.ProcessIndicator) error) error {
	...
	fetcher, closer, err := postgres.RunCursorQueryForSchemaType[storage.ProcessIndicator](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
