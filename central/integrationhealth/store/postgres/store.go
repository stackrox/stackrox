...
func New(db postgres.DB) Store {
	return pgSearch.NewGloballyScopedGenericStore[storeType, *storeType](
		...
		schema,
		pkGetter,
		insertIntoIntegrationHealths,
		copyFromIntegrationHealths,
...
func insertIntoIntegrationHealths(batch *pgx.Batch, pool pgSearch.BufferPool, obj *storage.IntegrationHealth) (*[]byte, error) {
...
func copyFromIntegrationHealths(ctx context.Context, s pgSearch.Deleter, tx *postgres.Tx, pool pgSearch.BufferPool, objs ...*storage.IntegrationHealth) error {
	if len(objs) == 0 {
		return nil
	}
	...
	for objBatch := range slices.Chunk(objs, batchSize) {
		...
		if _, err := tx.CopyFrom(ctx, pgx.Identifier{"integration_healths"}, copyCols, pgx.CopyFromRows(inputRows)); err != nil {
			return err
		}
