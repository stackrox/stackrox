...
type Store interface {
	...
	GetMany(ctx context.Context, identifiers []string) ([]*storage.IntegrationHealth, []int, error)
...
type storeImpl struct {
	*pgSearch.GenericStore[storage.IntegrationHealth, *storage.IntegrationHealth]
	db    postgres.DB
	mutex sync.RWMutex
}
...
func New(db postgres.DB) Store {
	return &storeImpl{
		db: db,
		GenericStore: pgSearch.NewGenericStore[storage.IntegrationHealth, *storage.IntegrationHealth](
			db,
			schema,
			pkGetter,
...
func pkGetter(obj *storage.IntegrationHealth) string {
	return obj.GetId()
}
...
func (s *storeImpl) GetMany(ctx context.Context, identifiers []string) ([]*storage.IntegrationHealth, []int, error) {
