...
func (s *storeImpl) Get(ctx context.Context, id string) (*storage.IntegrationHealth, bool, error) {
	...
	data, err := postgres.RunGetQueryForSchema[storage.IntegrationHealth](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetMany(ctx context.Context, identifiers []string) ([]*storage.IntegrationHealth, []int, error) {
	...
	rows, err := postgres.RunGetManyQueryForSchema[storage.IntegrationHealth](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.IntegrationHealth) error) error {
	...
	fetcher, closer, err := postgres.RunCursorQueryForSchema[storage.IntegrationHealth](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
