...
func New(db *rocksdb.RocksDB) (Store, error) {
	globaldb.RegisterBucket(bucket, "SignatureIntegration")
...
func (b *storeImpl) Count() (int, error) {
	defer metrics.SetRocksDBOperationDurationTime(time.Now(), ops.Count, "SignatureIntegration")
...
func (b *storeImpl) Exists(id string) (bool, error) {
	defer metrics.SetRocksDBOperationDurationTime(time.Now(), ops.Exists, "SignatureIntegration")
...
func (b *storeImpl) GetIDs() ([]string, error) {
	defer metrics.SetRocksDBOperationDurationTime(time.Now(), ops.GetAll, "SignatureIntegrationIDs")

	return b.crud.GetKeys()
}

// Get returns the object, if it exists from the store
func (b *storeImpl) Get(id string) (*storage.SignatureIntegration, bool, error) {
	defer metrics.SetRocksDBOperationDurationTime(time.Now(), ops.Get, "SignatureIntegration")
...
func (b *storeImpl) GetMany(ids []string) ([]*storage.SignatureIntegration, []int, error) {
	...
	objs := make([]*storage.SignatureIntegration, 0, len(msgs))
	for _, m := range msgs {
		objs = append(objs, m.(*storage.SignatureIntegration))
	}
	return objs, missingIndices, nil
}

// Upsert inserts the object into the DB
func (b *storeImpl) Upsert(obj *storage.SignatureIntegration) error {
	defer metrics.SetRocksDBOperationDurationTime(time.Now(), ops.Add, "SignatureIntegration")

	return b.crud.Upsert(obj)
}

// UpsertMany batches objects into the DB
func (b *storeImpl) UpsertMany(objs []*storage.SignatureIntegration) error {
	defer metrics.SetRocksDBOperationDurationTime(time.Now(), ops.AddMany, "SignatureIntegration")
...
func (b *storeImpl) Delete(id string) error {
	defer metrics.SetRocksDBOperationDurationTime(time.Now(), ops.Remove, "SignatureIntegration")
...
func (b *storeImpl) DeleteMany(ids []string) error {
	defer metrics.SetRocksDBOperationDurationTime(time.Now(), ops.RemoveMany, "SignatureIntegration")

	return b.crud.DeleteMany(ids)
}

// Walk iterates over all of the objects in the store and applies the closure
func (b *storeImpl) Walk(fn func(obj *storage.SignatureIntegration) error) error {
