...
func insertIntoPolicyCategoryEdges(ctx context.Context, batch *pgx.Batch, obj *storage.PolicyCategoryEdge) error {
	...
	finalStr := "INSERT INTO policy_category_edges (Id, PolicyId, CategoryId, serialized, tenant_id) VALUES($1, $2, $3, $4, $5) ON CONFLICT(Id) DO UPDATE SET Id = EXCLUDED.Id, PolicyId = EXCLUDED.PolicyId, CategoryId = EXCLUDED.CategoryId, serialized = EXCLUDED.serialized, tenant_id = EXCLUDED.tenant_id"
...
func copyFromPolicyCategoryEdges(ctx context.Context, s pgSearch.Deleter, tx *postgres.Tx, objs ...*storage.PolicyCategoryEdge) error {
	...
	for idx, obj := range objs {
		...
		if (idx+1)%batchSize == 0 || idx == len(objs)-1 {
			...
			if _, err := tx.CopyFrom(ctx, pgx.Identifier{"policy_category_edges"}, copyCols, pgx.CopyFromRows(inputRows)); err != nil {
				return err
			}
