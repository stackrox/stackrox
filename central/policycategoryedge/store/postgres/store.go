...
func (s *storeImpl) Get(ctx context.Context, id string) (*storage.PolicyCategoryEdge, bool, error) {
	...
	data, err := pgSearch.RunGetQueryForSchema[storage.PolicyCategoryEdge](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetByQuery(ctx context.Context, query *v1.Query) ([]*storage.PolicyCategoryEdge, error) {
	...
	rows, err := pgSearch.RunGetManyQueryForSchema[storage.PolicyCategoryEdge](ctx, schema, query, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.PolicyCategoryEdge) error) error {
	fetcher, closer, err := pgSearch.RunCursorQueryForSchema[storage.PolicyCategoryEdge](ctx, schema, search.EmptyQuery(), s.db)
	if err != nil {
		return err
	}
	defer closer()
