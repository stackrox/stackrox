...
func (s *storeImpl) GetByQuery(ctx context.Context, query *v1.Query) ([]*storage.ClusterCVEEdge, error) {
	...
	rows, err := pgSearch.RunGetManyQueryForSchema[storage.ClusterCVEEdge](ctx, schema, query, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.ClusterCVEEdge) error) error {
	fetcher, closer, err := pgSearch.RunCursorQueryForSchema[storage.ClusterCVEEdge](ctx, schema, search.EmptyQuery(), s.db)
	if err != nil {
		return err
	}
	defer closer()
