...
var (
	...
	mainVersionPattern = regexp.MustCompile(`^\d+\.\d+\.\d+`)

	// mainVersionNum is the version number of central's version tag.
	mainVersionNum string
)
...
func init() {
	...
	// We try to detect and take into account only the version
	// number (ie. X.Y.Z), otherwise copy the full version.
	mainVersion := version.GetMainVersion()
	mainVersionNum = mainVersionPattern.FindString(mainVersion)
	if mainVersionNum == "" {
		mainVersionNum = mainVersion
	}
}
...
func (h *httpHandler) openDefinitions(ctx context.Context, t updaterType, opts openOpts) (*vulDefFile, error) {
	...
	if !h.online {
		log.Debugf("offline bundle %s is available: %v", opts.offlineBlobName, offline)
...
func (h *httpHandler) openOfflineDefinitions(ctx context.Context, t updaterType, opts openOpts) (*vulDefFile, error) {
	...
	case vulnerabilityUpdaterType:
		...
		if buildinfo.ReleaseBuild && mv.VulnerabilityVersion != opts.vulnVersion {
			log.Debugf("v4 offline bundle vulnerability version mismatch: "+
...
func (h *httpHandler) handleZipContentsFromVulnDump(ctx context.Context) error {
	...
	checkV4Manifest := func(zipF *zip.File) (bool, error) {
		...
		return mv.supportsRelease(mainVersionNum), nil
	}
...
// supportsRelease returns true if this manifest supports a given release as
// provided by its version number.
func (m *manifest) supportsRelease(versionNum string) bool {
	for _, v := range m.releaseVersionsList {
		if versionNum == v {
			return true
		}
	}
