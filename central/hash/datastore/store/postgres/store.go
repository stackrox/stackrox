...
func (s *storeImpl) copyFromHashes(ctx context.Context, tx *postgres.Tx, objs ...*storage.Hash) error {
	...
	for idx, obj := range objs {
		...
		if (idx+1)%batchSize == 0 || idx == len(objs)-1 {
			...
			_, err = tx.CopyFrom(ctx, pgx.Identifier{"hashes"}, copyCols, pgx.CopyFromRows(inputRows))
...
func (s *storeImpl) copyFrom(ctx context.Context, objs ...*storage.Hash) error {
	conn, release, err := s.acquireConn(ctx, ops.Get, "Hash")
	if err != nil {
		return err
	}
	defer release()
	...
	if err := s.copyFromHashes(ctx, tx, objs...); err != nil {
...
func (s *storeImpl) upsert(ctx context.Context, objs ...*storage.Hash) error {
	...
	for _, obj := range objs {
		batch := &pgx.Batch{}
		if err := insertIntoHashes(ctx, batch, obj); err != nil {
			return err
		}
...
func (s *storeImpl) Delete(ctx context.Context, clusterID string) error {
	...
	q := search.NewQueryBuilder().AddDocIDs(clusterID).ProtoQuery()
...
func (s *storeImpl) DeleteMany(ctx context.Context, identifiers []string) error {
	defer metrics.SetPostgresOperationDurationTime(time.Now(), ops.RemoveMany, "Hash")
...
func (s *storeImpl) Count(ctx context.Context) (int, error) {
	defer metrics.SetPostgresOperationDurationTime(time.Now(), ops.Count, "Hash")
...
func (s *storeImpl) GetMany(ctx context.Context, identifiers []string) ([]*storage.Hash, []int, error) {
	defer metrics.SetPostgresOperationDurationTime(time.Now(), ops.GetMany, "Hash")
	...
	rows, err := pgSearch.RunGetManyQueryForSchema[storage.Hash](ctx, schema, q, s.db)
	if err != nil {
	...
	resultsByID := make(map[string]*storage.Hash, len(rows))
	...
	elems := make([]*storage.Hash, 0, len(resultsByID))
...
func (s *storeImpl) GetIDs(ctx context.Context) ([]string, error) {
	defer metrics.SetPostgresOperationDurationTime(time.Now(), ops.GetAll, "storage.HashIDs")
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.Hash) error) error {
	fetcher, closer, err := pgSearch.RunCursorQueryForSchema[storage.Hash](ctx, schema, search.EmptyQuery(), s.db)
	if err != nil {
		return err
	}
	defer closer()
