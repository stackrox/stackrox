...
func (s *datastorePostgresTestSuite) TestUpsertTokenExchangerFailureRollsBackTransaction() {
	...
	mockIssuerFetcher := mocks.NewMockServiceAccountIssuerFetcher(controller)
	mockIssuerFetcher.EXPECT().GetServiceAccountIssuer().Return("https://localhost", nil).AnyTimes()

	authDataStore := New(authStore, mockSet, mockIssuerFetcher)
	...
	readIssuerFetcher := mocks.NewMockServiceAccountIssuerFetcher(controller)
	readIssuerFetcher.EXPECT().GetServiceAccountIssuer().Return("https://localhost", nil).AnyTimes()

	readDataStore := New(authStore, readMockSet, readIssuerFetcher)
