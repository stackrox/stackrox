...
func init() {
	framework.MustRegisterChecks(
		...
		kubeletCommandLineCheck("CIS_Kubernetes_v1_5:4_2_5", "streaming-connection-idle-timeout", "0", "4h", common.NotMatches),
		...
		kubeletCommandLineCheck("CIS_Kubernetes_v1_5:4_2_9", "event-qps", "0", "5", common.Info),
		multipleFlagsSetCheck("CIS_Kubernetes_v1_5:4_2_10", "kubelet", kubeletCommandLineOverride, "tls-cert-file", "tls-private-key-file"),
		...
		kubeletCommandLineCheck("CIS_Kubernetes_v1_5:4_2_13", "tls-cipher-suites", tlsCiphers, defaultTLSCiphers, common.OnlyContains),
	)
}

func kubeletCommandLineOverride(ctx framework.ComplianceContext, msg string) {
	framework.Notef(ctx, "%s. Please check the kubelet config file to verify this result.", msg)
}
...
func kubeletCommandLineCheck(name string, key, target, defaultVal string, evalFunc common.CommandEvaluationFunc) framework.Check {
	return genericKubernetesCommandlineCheck(name, "kubelet", key, target, defaultVal, evalFunc, kubeletCommandLineOverride)
}
