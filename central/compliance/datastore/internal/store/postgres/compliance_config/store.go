...
type storeImpl struct {
	*pgSearch.GenericStore[storage.ComplianceConfig, *storage.ComplianceConfig]
	db    postgres.DB
	mutex sync.RWMutex
}
...
func New(db postgres.DB) Store {
	return &storeImpl{
		db: db,
		GenericStore: pgSearch.NewGenericStore[storage.ComplianceConfig, *storage.ComplianceConfig](
			db,
			schema,
			pkGetter,
...
func metricsSetAcquireDBConnDuration(start time.Time, op ops.Op) {
	metrics.SetAcquireDBConnDuration(start, op, "ComplianceConfig")
}
...
func (s *storeImpl) DeleteMany(ctx context.Context, identifiers []string) error {
	defer metrics.SetPostgresOperationDurationTime(time.Now(), ops.RemoveMany, "ComplianceConfig")
...
func (s *storeImpl) Count(ctx context.Context) (int, error) {
	defer metrics.SetPostgresOperationDurationTime(time.Now(), ops.Count, "ComplianceConfig")
...
func (s *storeImpl) GetMany(ctx context.Context, identifiers []string) ([]*storage.ComplianceConfig, []int, error) {
	defer metrics.SetPostgresOperationDurationTime(time.Now(), ops.GetMany, "ComplianceConfig")
	...
	rows, err := pgSearch.RunGetManyQueryForSchema[storage.ComplianceConfig](ctx, schema, q, s.db)
	if err != nil {
	...
	elems := make([]*storage.ComplianceConfig, 0, len(resultsByID))
...
func (s *storeImpl) GetIDs(ctx context.Context) ([]string, error) {
	defer metrics.SetPostgresOperationDurationTime(time.Now(), ops.GetAll, "storage.ComplianceConfigIDs")
	var sacQueryFilter *v1.Query
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.ComplianceConfig) error) error {
	var sacQueryFilter *v1.Query
	sacQueryFilter, err := pgSearch.GetReadSACQuery(ctx, targetResource)
	if err != nil {
		return err
	}
	fetcher, closer, err := pgSearch.RunCursorQueryForSchema[storage.ComplianceConfig](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
