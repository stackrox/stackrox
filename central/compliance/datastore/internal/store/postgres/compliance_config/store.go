...
func (s *storeImpl) Delete(ctx context.Context, standardID string) error {
	...
	q := search.NewQueryBuilder().AddDocIDs(standardID).ProtoQuery()
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.ComplianceConfig) error) error {
	fetcher, closer, err := pgSearch.RunCursorQueryForSchema[storage.ComplianceConfig](ctx, schema, search.EmptyQuery(), s.db)
	if err != nil {
		return err
	}
	defer closer()
