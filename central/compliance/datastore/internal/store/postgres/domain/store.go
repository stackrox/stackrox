...
func (s *storeImpl) GetByQuery(ctx context.Context, query *v1.Query) ([]*storage.ComplianceDomain, error) {
	...
	rows, err := pgSearch.RunGetManyQueryForSchema[storage.ComplianceDomain](ctx, schema, query, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.ComplianceDomain) error) error {
	fetcher, closer, err := pgSearch.RunCursorQueryForSchema[storage.ComplianceDomain](ctx, schema, search.EmptyQuery(), s.db)
	if err != nil {
		return err
	}
	defer closer()
