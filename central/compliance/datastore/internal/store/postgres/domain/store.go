...
func (s *storeImpl) Get(ctx context.Context, id string) (*storage.ComplianceDomain, bool, error) {
	...
	data, err := postgres.RunGetQueryForSchema[storage.ComplianceDomain](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetByQuery(ctx context.Context, query *v1.Query) ([]*storage.ComplianceDomain, error) {
	...
	rows, err := postgres.RunGetManyQueryForSchema[storage.ComplianceDomain](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.ComplianceDomain) error) error {
	var sacQueryFilter *v1.Query
	fetcher, closer, err := postgres.RunCursorQueryForSchema[storage.ComplianceDomain](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
