...
func (s *storeImpl) Get(ctx context.Context, id string) (*storage.ComplianceDomain, bool, error) {
	...
	data, err := postgres.RunGetQueryForSchemaType[storage.ComplianceDomain](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetMany(ctx context.Context, ids []string) ([]*storage.ComplianceDomain, []int, error) {
	...
	rows, err := postgres.RunGetManyQueryForSchemaType[storage.ComplianceDomain](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.ComplianceDomain) error) error {
	var sacQueryFilter *v1.Query
	fetcher, closer, err := postgres.RunCursorQueryForSchemaType[storage.ComplianceDomain](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
