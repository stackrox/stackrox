...
func metricsSetPostgresOperationDurationTime(start time.Time, op ops.Op) {
	metrics.SetPostgresOperationDurationTime(start, op, "ComplianceDomain")
}

func metricsSetAcquireDBConnDuration(start time.Time, op ops.Op) {
	metrics.SetAcquireDBConnDuration(start, op, "ComplianceDomain")
}
...
func (s *storeImpl) Get(ctx context.Context, id string) (*storage.ComplianceDomain, bool, error) {
	...
	data, err := pgSearch.RunGetQueryForSchema[storage.ComplianceDomain](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetByQuery(ctx context.Context, query *v1.Query) ([]*storage.ComplianceDomain, error) {
	...
	rows, err := pgSearch.RunGetManyQueryForSchema[storage.ComplianceDomain](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetMany(ctx context.Context, identifiers []string) ([]*storage.ComplianceDomain, []int, error) {
	...
	resultsByID := make(map[string]*storage.ComplianceDomain, len(rows))
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.ComplianceDomain) error) error {
	...
	fetcher, closer, err := pgSearch.RunCursorQueryForSchema[storage.ComplianceDomain](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
