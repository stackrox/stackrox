...
func (s *ComplianceRunResultsStoreSuite) TestStore() {
	...
	complianceRunResultsCount, err = store.Count(withNoAccessCtx, search.EmptyQuery())
	s.NoError(err)
	...
	foundComplianceRunResultss, missing, err := store.GetMany(ctx, complianceRunResultsIDs)
	s.NoError(err)
	s.Empty(missing)
	protoassert.ElementsMatch(s.T(), complianceRunResultss, foundComplianceRunResultss)
