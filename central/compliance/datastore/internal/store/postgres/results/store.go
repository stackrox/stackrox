...
func insertIntoComplianceRunResults(ctx context.Context, batch *pgx.Batch, obj *storage.ComplianceRunResults) error {
	...
	values := []interface{}{
		...
		obj.GetRunMetadata().GetClusterId(),
...
func (s *storeImpl) copyFrom(ctx context.Context, objs ...*storage.ComplianceRunResults) error {
	conn, release, err := s.acquireConn(ctx, ops.Get, "ComplianceRunResults")
	if err != nil {
		return err
	}
	defer release()
...
func (s *storeImpl) Get(ctx context.Context, runMetadataRunId string) (*storage.ComplianceRunResults, bool, error) {
	...
	data, err := postgres.RunGetQueryForSchemaType[storage.ComplianceRunResults](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetMany(ctx context.Context, ids []string) ([]*storage.ComplianceRunResults, []int, error) {
	defer metrics.SetPostgresOperationDurationTime(time.Now(), ops.GetMany, "ComplianceRunResults")
	...
	rows, err := postgres.RunGetManyQueryForSchemaType[storage.ComplianceRunResults](ctx, schema, q, s.db)
	if err != nil {
	...
	resultsByID := make(map[string]*storage.ComplianceRunResults, len(rows))
	...
	elems := make([]*storage.ComplianceRunResults, 0, len(resultsByID))
	for i, id := range ids {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.ComplianceRunResults) error) error {
	...
	fetcher, closer, err := postgres.RunCursorQueryForSchemaType[storage.ComplianceRunResults](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
...
func dropTableComplianceRunResults(ctx context.Context, db *pgxpool.Pool) {
