...
func (s *storeImpl) getDomain(ctx context.Context, domainID string) (*storage.ComplianceDomain, error) {
	...
	cachedDomain, ok := domainCache.Get(domainID)
	if ok {
...
func getRunResultsCache() *expirable.LRU[string, *storage.ComplianceRunResults] {
	onceforRunResults.Do(func() {
		runResultsCache = expirable.NewLRU[string, *storage.ComplianceRunResults](
			runResultsCacheSize,
			nil,
