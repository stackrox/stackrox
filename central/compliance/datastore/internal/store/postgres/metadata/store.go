...
func (s *storeImpl) Delete(ctx context.Context, runID string) error {
	...
	q := search.NewQueryBuilder().AddDocIDs(runID).ProtoQuery()
...
func (s *storeImpl) GetByQuery(ctx context.Context, query *v1.Query) ([]*storage.ComplianceRunMetadata, error) {
	...
	rows, err := pgSearch.RunGetManyQueryForSchema[storage.ComplianceRunMetadata](ctx, schema, query, s.db)
	if err != nil {
...
func (s *storeImpl) GetMany(ctx context.Context, identifiers []string) ([]*storage.ComplianceRunMetadata, []int, error) {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.ComplianceRunMetadata) error) error {
	fetcher, closer, err := pgSearch.RunCursorQueryForSchema[storage.ComplianceRunMetadata](ctx, schema, search.EmptyQuery(), s.db)
	if err != nil {
		return err
	}
	defer closer()
