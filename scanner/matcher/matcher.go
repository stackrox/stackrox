...
func NewMatcher(ctx context.Context, cfg config.MatcherConfig) (Matcher, error) {
	...
	// VersionRange) created by the migration above. With pool_min_conns > 0,
	// the pool eagerly opens connections before migrations run, so those
	// connections miss the AfterConnect type registration. Resetting ensures
