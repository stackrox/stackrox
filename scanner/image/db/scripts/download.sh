...
if [[ "$arch" == "s390x" ]]; then
  ...
  postgres_minor=$(dnf list ${dnf_list_args[@]+"${dnf_list_args[@]}"} --disablerepo='*' --enablerepo="pgdg${postgres_major}" -y "postgresql${postgres_major}-server.$arch" | tee >(cat >&2)| tail -n 1 | awk '{print $2}')
