...
var nameFromSeverity = map[severity]string{
	severityLow:       "Low",
...
func (e *Enricher) ParseEnrichment(ctx context.Context, contents io.ReadCloser) ([]driver.EnrichmentRecord, error) {
	...
	for b, err := r.ReadBytes('\n'); err == nil; b, err = r.ReadBytes('\n') {
		...
		// The current csaf type from Claircore does not enable us
		// to fetch c.Document.AggregateSeverity.Text which would have the official severity rating.
		// However, this rating is simply the highest severity rating of all related CVEs.
		// So, we calculate this below.
		//
		...
		// but Moderate for subscription-manager in RHEL 7.
		...
		for _, v := range c.Vulnerabilities {
			...
			for _, threat := range v.Threats {
				if threat.Category == "impact" {
					sev = max(sev, severityFromName[threat.Details])
				}
			}
		}
