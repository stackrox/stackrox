...
func NewUpdater(ctx context.Context, client *http.Client, url, filePath string) (*RepositoryToCPEUpdater, error) {
	...
	if filePath != "" {
		...
		if err == nil {
			...
			if err := json.NewDecoder(f).Decode(initValue); err != nil {
				slog.WarnContext(ctx, "failed to decode initial repo-to-CPE mapping file", "reason", err)
				initValue = nil
			}
			defer utils.IgnoreError(f.Close)
		} else {
			slog.WarnContext(ctx, "failed to open repo-to-CPE mapping file", "reason", err, "file", filePath)
		}
	}
	...
	if len(initValue.Data) == 0 {
		if _, err := updater.Get(ctx, client); err != nil {
			slog.WarnContext(ctx, "failed to fetch initial repo-to-CPE mapping", "reason", err)
		}
	}
