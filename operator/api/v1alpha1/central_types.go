...
type ExposureRouteReencryptTLS struct {
	...
	// The PEM encoded certificate that is served on the route.
	// If left empty, a certificate signed by the OpenShift certificate authority is used.
	// Must be a single serving certificate instead of a certificate chain.
	//+operator-sdk:csv:customresourcedefinitions:type=spec,order=2
	Certificate *string `json:"certificate,omitempty"`

	// The CA certificate of the final destination.
	// Should be provided because the OpenShift router uses it for health checks on the secure connection.
	//+operator-sdk:csv:customresourcedefinitions:type=spec,order=3
