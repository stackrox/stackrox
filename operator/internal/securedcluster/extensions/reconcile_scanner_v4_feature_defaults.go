...
func reconcileScannerV4FeatureDefaults(
	...
	spec := &platform.LocalScannerV4ComponentSpec{}
	if securedCluster.Spec.ScannerV4 != nil {
		spec = securedCluster.Spec.ScannerV4.DeepCopy()
	}

	componentPolicy, usedDefaulting := defaulting.SecuredClusterScannerV4ComponentPolicy(logger, &securedCluster.Status, securedCluster.GetAnnotations(), spec)
	if !usedDefaulting {
	...
	securedCluster.Spec.ScannerV4 = spec
	return nil
}
