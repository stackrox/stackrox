...
var (
	thisPredicate = SkipStatusControllerUpdates[*platform.Central]{}
)
...
func TestCentralStatusPredicate_OwnedConditionsUnchanged_ShouldAllow(t *testing.T) {
	...
	result := thisPredicate.Update(event.TypedUpdateEvent[*platform.Central]{
		ObjectOld: old,
		ObjectNew: new,
	})

	if !result {
		t.Error("Expected predicate to allow update when status-controller conditions unchanged but other fields changed")
	}
}
...
func TestCentralStatusPredicate_BothOwnedChanged_ShouldBlock(t *testing.T) {
	...
	if result {
		t.Error("Expected predicate to block update when both status-controller conditions changed")
	}
}
