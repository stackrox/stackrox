...
func CheckStruct(t *testing.T, s any, schema chartutil.Values) {
	...
	for i := 0; i < structValue.NumField(); i++ {
		...
		t.Run(structField.Name, func(t *testing.T) {
			...
			case reflect.Struct:
				CheckStruct(t, field.Interface(), fieldSchema)
			case reflect.Ptr:
			...
			case reflect.String:
				checkLeafField(t, field, fieldSchema)
			case reflect.Map, reflect.Slice:
				// Currently we keep maps and slices empty by default.
				// TODO(ROX-29467): look at this closer
				t.Skip()
			default:
				t.Fatalf("unsupported field type %q", field.Type().Kind())
			}
		})
	}
}
...
func requireNoDefaultProperty(t *testing.T, schema chartutil.Values) {
	t.Run("check absence of default in schema", func(t *testing.T) {
		// TODO(ROX-30279): remove this bypass when closing this ticket, and the t.Run indirection
		if _, hasDefault := schema["default"]; hasDefault && strings.Contains(t.Name(), "/AdmissionControl/") {
			t.Skip("ROX-30279")
		}
