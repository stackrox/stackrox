...
func RegisterNewReconciler(mgr ctrl.Manager, selector string) error {
	...
	opts := []pkgReconciler.Option{
		...
		pkgReconciler.WithPreExtension(extensions.ReconcilePVCExtension(mgr.GetClient(), mgr.GetAPIReader(), extensions.PVCTargetCentral,
			extensions.DefaultCentralPVCName, extensions.WithDefaultClaimSize(extensions.DefaultPVCSize))),
		pkgReconciler.WithPreExtension(extensions.ReconcilePVCExtension(mgr.GetClient(), mgr.GetAPIReader(), extensions.PVCTargetCentralDB,
			extensions.DefaultCentralDBPVCName, extensions.WithDefaultClaimSize(extensions.DefaultPVCSize))),
		pkgReconciler.WithPreExtension(extensions.ReconcilePVCExtension(mgr.GetClient(), mgr.GetAPIReader(), extensions.PVCTargetCentralDBBackup,
			extensions.DefaultCentralDBBackupPVCName, extensions.WithDefaultClaimSize(extensions.DefaultPVCSize))),
