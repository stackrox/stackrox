...
func reconcileScannerV4FeatureDefaults(ctx context.Context, client ctrlClient.Client, u *unstructured.Unstructured, logger logr.Logger) error {
	...
	for _, flow := range defaultingFlows {
		if err := executeDefaultingFlow(ctx, logger, &central, client, flow); err != nil {
			return err
		}
	}
...
func executeDefaultingFlow(ctx context.Context, logger logr.Logger, central *platform.Central, client ctrlClient.Client, flow defaulting.CentralDefaultingFlow) error {
