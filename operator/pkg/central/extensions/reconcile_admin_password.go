...
func (r *reconcileAdminPasswordExtensionRun) updateStatus(status *platform.CentralStatus) bool {
	...
	if r.autoGenerated && len(r.passwordSecretName) == 0 {
		status.Central.AdminPassword.AdminPasswordSecret = defaultPasswordSecretName
	} else {
		status.Central.AdminPassword.AdminPasswordSecret = r.passwordSecretName
	}
	fmt.Printf("Password secret name: %s\n", r.passwordSecretName)
