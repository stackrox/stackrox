...
function main() {
  ...
  5)
    ...
    ;;
  *)
    echo -e "Usage:\n\t$0 [--allow-dirty-tag | -d] <operator_ns> <index-image-repo> <index-version> [<install-version> [<install-channel>]]" >&2
  ...
  apply_operator_manifests "${operator_ns}" "${index_image_repo}" "${index_version}" "${operator_version}" "${operator_channel}"
