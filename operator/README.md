...
while ! kubectl -n index-test patch clusterserviceversions.operators.coreos.com rhacs-operator.v$(make --quiet tag) --type json -p '[ { "op": "add", "path": "/spec/install/spec/deployments/0/spec/template/spec/imagePullSecrets", "value": [{"name": "my-opm-image-pull-secrets"}] } ]'; do echo retrying...; sleep 1; done
