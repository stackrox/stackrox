...
while true; do
  deployment_count="$(curl --location --silent --user "admin:${ROX_PASSWORD}" --request GET "${API_ENDPOINT}/v1/deploymentscount" -k | jq .count)"
  if [[ "$deployment_count" -ge "$max_deployments" ]]; then
