...
@test "check_for_stackrox_OOMs()" {
    ...
    with_oomkilled_test="${ARTIFACT_DIR}/junit-misc/junit-OOMCheck-central-84bf956f94-bg6hr.xml"
    assert [ -f "$with_oomkilled_test" ]
    run grep -q "was OOMKilled" "$with_oomkilled_test"
    assert_success

    without_oomkilled_test="${ARTIFACT_DIR}/junit-misc/junit-OOMCheck-sensor-67d98c67bf-v688m.xml"
