...
deploy_stackrox_with_roxie() {
    ...
    if [[ -n "${TRUSTED_CA_FILE:-}" ]]; then
        info "Adding ${TRUSTED_CA_FILE} as trusted CA to roxie overrides"
        merge_yaml "$override_file" << EOF
spec:
  tls:
    additionalCAs:
    - {"name": "additional-ca", "content": "$(cat "${TRUSTED_CA_FILE}")"}
EOF
    fi

    if [[ -n "$ROX_DEFAULT_TLS_KEY_FILE" && -n "$ROX_DEFAULT_TLS_CERT_FILE" ]]; then
        info "Adding default TLS cert to roxie overrides"
        # Create default TLS Secret with test certificates
        # These certificates are generated by setup_default_TLS_certs() in config_part_1()
        # and are required by tests (see tests/cert_test.go and tests/endpoints_test.go)
...
metadata:
  ...
  labels:
    app.kubernetes.io/managed-by: acs-tests
data:
...
EOF
    ...
    "")
        ;;
    lb)
        echo "Enabling load balancer exposure in roxie overrides"
        merge_yaml "$override_file" << EOF
spec:
  central:
    exposure:
...
EOF
        ;;
    route)
        echo "Enabling route exposure in roxie overrides"
        merge_yaml "$override_file" << EOF
spec:
  central:
    exposure:
...
EOF
    ...
    if [[ "${CGO_CHECKS:-}" == "true" ]]; then
        ...
        patch_yaml "$override_file" '.spec.customize.envVars += {"name": "MUTEX_WATCHDOG_TIMEOUT_SECS", "value": "15"}'
    fi
    ...
    source "$roxie_envrc"
    export ROX_USERNAME="admin" # roxie currently doesn't persist the admin username in the envrc file.
    ci_export t"ROX_USERNAME" "$ROX_USERNAME"
