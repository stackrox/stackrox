...
patch_resources_for_test() {
    ...
    kubectl -n stackrox patch svc central-loadbalancer --patch "$(cat "$TEST_ROOT"/tests/yaml/endpoints-test-lb-patch.yaml)"
    kubectl -n stackrox apply -f "$TEST_ROOT/tests/yaml/endpoints-test-netpol.yaml"
