...
deploy_central() {
    ...
    info "Deploying central to namespace $namespace"
    ...
    if [[ "${DEPLOY_STACKROX_VIA_OPERATOR}" == "true" ]]; then
        ...
        CENTRAL_NAMESPACE="$namespace" "$ROOT/${DEPLOY_DIR}/central.sh"
    fi
}
...
wait_for_api() {
    ...
    if [[ "${USE_MIDSTREAM_IMAGES}" == "true" ]]; then
        API_HOSTNAME=$(kubectl get routes/central -n "$namespace" -o json | jq -r '.spec.host')
        API_PORT=443
    else
        ...
        if [[ "${LOAD_BALANCER}" == "lb" ]]; then
            API_HOSTNAME=$(./scripts/k8s/get-lb-ip.sh "$namespace")
            API_PORT=443
        fi
    fi
