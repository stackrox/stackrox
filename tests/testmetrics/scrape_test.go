...
func TestScrapePodViaPortForward_Validation(t *testing.T) {
	ctx := context.Background()
	_, err := ScrapePodViaPortForward(ctx, nil, "ns", "pod", 9090, "/metrics")
	require.Error(t, err)
	_, err = ScrapePodViaPortForward(ctx, &rest.Config{}, "", "pod", 9090, "")
	require.Error(t, err)
	_, err = ScrapePodViaPortForward(ctx, &rest.Config{}, "ns", "pod", 0, "")
	require.Error(t, err)
}
