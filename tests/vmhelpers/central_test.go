...
func TestWaitForVMPresentInCentral_UsesListVirtualMachines(t *testing.T) {
	...
	vm, err := WaitForVMPresentInCentralWithOptions(ctx, client, opts, "ns1", "vm1")
...
func TestWaitForVMPresentInCentral_PlanSignatureUsesDefaults(t *testing.T) {
	ctx := context.Background()
	client := &stubVirtualMachineClient{
		listFn: func(ctx context.Context, _ *v2.ListVirtualMachinesRequest) (*v2.ListVirtualMachinesResponse, error) {
			return &v2.ListVirtualMachinesResponse{
				VirtualMachines: []*v2.VirtualMachine{{Id: "id-2", Namespace: "ns", Name: "vm"}},
			}, nil
		},
	}
	vm, err := WaitForVMPresentInCentral(ctx, client, "ns", "vm")
	require.NoError(t, err)
	require.Equal(t, "id-2", vm.GetId())
}
...
func TestWaitForVMScanTimestamp(t *testing.T) {
	...
	vm, err := WaitForVMScanTimestampWithOptions(ctx, client, opts, "vid")
	require.NoError(t, err)
	require.NotNil(t, vm.GetScan().GetScanTime())
	require.GreaterOrEqual(t, calls, 3)
}

func TestCentralWaitWithOptionsStubClients(t *testing.T) {
	ctx := context.Background()
	opts := WaitOptions{Timeout: 150 * time.Millisecond, PollInterval: 5 * time.Millisecond}
	...
	for _, tc := range []struct {
		...
		{
			name: "WaitForVMIdentityFieldsWithOptions",
			run: func(t *testing.T) {
				var calls int
				client := &stubVirtualMachineClient{
				...
				vm, err := WaitForVMIdentityFieldsWithOptions(ctx, client, opts, "vid", "ns-x", "vm-x")
				require.NoError(t, err)
		...
		{
			name: "WaitForVMRunningInCentralWithOptions",
			run: func(t *testing.T) {
				var calls int
				client := &stubVirtualMachineClient{
					getFn: func(_ context.Context, _ *v2.GetVirtualMachineRequest) (*v2.VirtualMachine, error) {
				...
				vm, err := WaitForVMRunningInCentralWithOptions(ctx, client, opts, "r1")
				require.NoError(t, err)
		...
		{
			name: "WaitForVMScanNonNilWithOptions",
			run: func(t *testing.T) {
				var calls int
				client := &stubVirtualMachineClient{
				...
				vm, err := WaitForVMScanNonNilWithOptions(ctx, client, opts, "s1")
				require.NoError(t, err)
		...
		{
			name: "WaitForVMComponentsReportedWithOptions",
			run: func(t *testing.T) {
				var calls int
				client := &stubVirtualMachineClient{
					getFn: func(_ context.Context, req *v2.GetVirtualMachineRequest) (*v2.VirtualMachine, error) {
				...
				vm, err := WaitForVMComponentsReportedWithOptions(ctx, client, opts, "cvm")
				require.NoError(t, err)
		...
		{
			name: "WaitForAllVMComponentsScannedWithOptions",
			run: func(t *testing.T) {
				var calls int
				client := &stubVirtualMachineClient{
					getFn: func(_ context.Context, req *v2.GetVirtualMachineRequest) (*v2.VirtualMachine, error) {
				...
				vm, err := WaitForAllVMComponentsScannedWithOptions(ctx, client, opts, "all1")
				require.NoError(t, err)
				require.True(t, allComponentsScanned(vm))
				require.GreaterOrEqual(t, calls, 2)
			},
