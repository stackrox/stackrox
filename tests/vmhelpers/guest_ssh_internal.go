...
// Kept as a named function (rather than inlining errors.Is) for symmetry with the other isSSH*Failure
// classifiers in the sshFailureRules table.
...
// WaitForSSHReachableWithPolicy is like WaitForSSHReachable but uses the
// supplied policy for poll cadence and failure thresholds.
func WaitForSSHReachableWithPolicy(t testing.TB, ctx context.Context, virt Virtctl, namespace, vm string, policy SSHReachabilityPolicy) error {
	t.Helper()
	attempts := 0
