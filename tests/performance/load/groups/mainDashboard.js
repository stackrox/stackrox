...
export function mainDashboard(host, headers, tags) {
    group('main dashboard', function () {
         ...
         *  select count(*)
         *   from nodes
         *   where nodes.ClusterId = $1
         *
         *  - violationCount ($12 = 0, $13 = 3)
         *  select count(*)
         *   from alerts
         *   where (
         *     (alerts.ClusterId = $1 and (alerts.Namespace = $2 or ... or alerts.Namespace = $11))
         *     and ((alerts.State = $12) or (alerts.State = $13))
         *   )
         *
         ...
         *   from images
         *   inner join deployments_containers on images.Id = deployments_containers.Image_Id
         *   inner join deployments on deployments_containers.deployments_Id = deployments.Id
         *   where (
         *     deployments.ClusterId = $1 and (deployments.Namespace = $2 or ... or deployments.Namespace = $11)
         *   )
         *
         *  - secretCount
         *  select count(*)
         *   from secrets
         *   where (
         *     secrets.ClusterId = $1 and (secrets.Namespace = $2 or ... or secrets.Namespace = $11)
         *   )
         **/
        ...
        /** DB Queries
         *  - clusters
         *  select clusters.Id::text as Cluster_ID
         *   from clusters
         *   where clusters.Id = $1
         *   order by clusters.Name asc
         *
         *  $2 = q1.clusters.Id
         *  select "cluster_health_statuses".serialized
         *   from cluster_health_statuses
         *   inner join clusters on cluster_health_statuses.Id = clusters.Id
         *   where (
         *     clusters.Id = $1 and cluster_health_statuses.Id = ANY($2::uuid[])
         *   )
         *
         *  - namespaces
         *  $12 = q1.clusters.Id
         *  select namespaces.Id::text as Namespace_ID
         *   from namespaces where (
         *     (namespaces.ClusterId = $1 and (namespaces.Name = $2 or ... or namespaces.Name = $11))
         *     and namespaces.ClusterId = $12
         *   )
         ...
         *   where (
         *     (namespaces.ClusterId = $1 and (namespaces.Name = $2 or ... or namespaces.Name = $11))
         *     and (namespaces.Id = ANY($12::uuid[]) and namespaces.ClusterId = $13)
         *   )
         *   order by namespaces.Name asc
         *
         ...
         *   where (
         *     (alerts.ClusterId = $1 and (alerts.Namespace = $2 or ... or alerts.Namespace = $11))
         *     and ((alerts.Policy_Severity = $12) and ((alerts.State = $13) or (alerts.State = $14)))
         *   )
         */
         ...
         *  select "alerts".serialized
         *   from alerts
         *   where (
         *     (alerts.ClusterId = $1 and (alerts.Namespace = $2 or ... or alerts.Namespace = $11))
         *     and ((alerts.Policy_Severity = $12) and ((alerts.State = $13) or (alerts.State = $14)))
         *   )
         *   order by alerts.Time desc LIMIT 3
         *
        ...
        /** DB Queries
         *  - images
         *  select distinct(images.Id) as Image_Sha, images.RiskScore as image_risk_score
         *   from images
         *   inner join deployments_containers on images.Id = deployments_containers.Image_Id
         *   inner join deployments on deployments_containers.deployments_Id = deployments.Id
         *   where (
         *     deployments.ClusterId = $1 and (deployments.Namespace = $2 or … or deployments.Namespace = $11)
         *   )
         *   order by images.RiskScore desc LIMIT 6
         *
         *  ($23..$26 = q1.images.Id)
         ...
         *     (deployments.ClusterId = $1 and (deployments.Namespace = $2 or … or deployments.Namespace = $11))
         *     and (
         *       (deployments.ClusterId = $12 and (deployments.Namespace = $13 or … or deployments.Namespace = $22))
         *       and (images.Id = $23 or images.Id = $24 or images.Id = $25 or images.Id = $26 or images.Id = $27 or images.Id = $28)
         *     )
         *   )
         *
         *  - imageVulnerabilityCounter
         ...
         *   where (
         *     (deployments.ClusterId = $1 and (deployments.Namespace = $2 or … or deployments.Namespace = $11))
         *     and ((image_cves.Snoozed = $12 and image_component_cve_edges.IsFixable = $13) and images.Id = $14)
         *   )
         *
         *  ($12 = q3.image_cves.Id)
         *  select "image_cves".serialized
         *   from image_cves
         *   inner join image_component_cve_edges on image_cves.Id = image_component_cve_edges.ImageCveId
         *   inner join image_component_edges on image_component_cve_edges.ImageComponentId = image_component_edges.ImageComponentId
         *   inner join deployments_containers on image_component_edges.ImageId = deployments_containers.Image_Id
         *   inner join deployments on deployments_containers.deployments_Id = deployments.Id
         *   where (
         *     (deployments.ClusterId = $1 and (deployments.Namespace = $2 or … or deployments.Namespace = $11))
         *     and image_cves.Id = ANY($12::text[])
         *   )
         */
         ...
         *  select deployments.Id::text as Deployment_ID
         *   from deployments
         *   where
         *     (deployments.ClusterId = $$ and (deployments.Namespace = $$ or ... or deployments.Namespace = $$))
         *   order by deployments.RiskScore desc LIMIT 6
         *
         *  - associated process data
         *  select "process_baseline_results".serialized
         *   from process_baseline_results
         *   where process_baseline_results.DeploymentId = ANY($$::uuid[])
         */
         ...
         *   from deployments
         *   where
         *     (deployments.ClusterId = $$ and (deployments.Namespace = $$ or deployments.Namespace = $$ or deployments.Namespace = $$ or deployments.Namespace = $$ or deployments.Namespace = $$ or deployments.Namespace = $$ or deployments.Namespace = $$ or deployments.Namespace = $$ or deployments.Namespace = $$ or deployments.Namespace = $$))
         *   order by deployments.RiskScore desc LIMIT 6
         */
         ...
         *   where (
         *     (deployments.ClusterId = $1 and (deployments.Namespace = $2 or ... or deployments.Namespace = $11))
         *     and images.Metadata_V1_Created > $12 and images.Metadata_V1_Created < $13
         *   )
         *
         *  (query3 older than)
         *  select count(distinct(images.Id))
         ...
         *   where (
         *     (deployments.ClusterId = $1 and (deployments.Namespace = $2 or ... or deployments.Namespace = $11))
         *     and images.Metadata_V1_Created <= $12
         *   )
         */
        ...
        /** DB Queries
         *
         *  select alerts.Id::text as Alert_ID, alerts.ClusterName, alerts.Policy_Severity, alerts.Policy_Categories
         *  from alerts
         *  where (
         *    (alerts.ClusterId = $$ and (alerts.Namespace = $$ or ... or alerts.Namespace = $$))
         *    and (
         *      (
         *        (alerts.ClusterName is not null and alerts.Policy_Severity is not null)
         *        and alerts.Policy_Categories is not null
         *      )
         *      and ((alerts.State = $$) or (alerts.State = $$))
         *    )
         *  )
         */
        ...
        /** DB Queries
         *  - complianceStandards ?
         *  select "compliance_configs".serialized
         *   from compliance_configs where
         *   compliance_configs.StandardId = ANY($1::text[])
         *
