...
func (c *endpointsTestCase) runHTTPTest(t *testing.T, testCtx *endpointsTestContext, useHTTP2 bool) {
	...
	assert.NoError(t, protocompat.UnmarshalProtoJSON(resp.Body, &pong), "expected response for ping request to be unmarshalable into Pong protobuf")
	...
	if !assert.NoError(t, protocompat.UnmarshalProtoJSON(resp.Body, &authStatus), "expected response for auth status request to be unmarshalable into auth status PB") {
		return
	}
	c.verifyAuthStatus(t, testCtx, &authStatus)
}
