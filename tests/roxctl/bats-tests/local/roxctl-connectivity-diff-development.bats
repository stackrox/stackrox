...
@test "roxctl-development connectivity-diff produces no output when all yamls are templated" {
  mkdir -p "$out_dir"
...
@test "roxctl-development connectivity-diff produces errors when some yamls are templated" {
  mkdir -p "$out_dir"
  write_yaml_to_file "$templated_fragment" "$(mktemp "$out_dir/templated-XXXXXX.yaml")"
...
@test "roxctl-development connectivity-diff produces errors when all yamls are not K8s resources" {
  mkdir -p "$out_dir"
...
@test "roxctl-development connectivity-diff treats warning as error with strict when some yamls are not valid" {
...
@test "roxctl-development connectivity-diff generates conns diff report between resources from two directories to output file" {
  dir1="${diff_tests_dir}/acs-security-demos/"
  dir2="${diff_tests_dir}/acs-security-demos-new-version/"
  # assert files exist in dir1
  assert_file_exist "${dir1}/backend/catalog/deployment.yaml"
...
@test "roxctl-development connectivity-diff generates conns diff report between resources from another two directories md output" {
  dir1="${diff_tests_dir}/netpol-analysis-example-minimal/"
  dir2="${diff_tests_dir}/netpol-diff-example-minimal/"
  # assert files exist in dir1
  assert_file_exist "${dir1}/backend.yaml"
