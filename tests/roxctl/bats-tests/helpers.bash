...
helm_template_central() {
  ...
  assert_output --partial "wrote $out_dir/rendered/stackrox-central-services/templates/02-scanner-v4-07-indexer-deployment.yaml"
}
  ...
  for component in "${@}"; do
    ...
    case $component in
      ...
      central-db)
        ...
        ;;
      scanner-v4-indexer)
        assert_registry_version_file "${dir}/02-scanner-v4-07-indexer-deployment.yaml" 0 "indexer" "$regex"
        ;;
      scanner-v4-db)
        assert_registry_version_file "${dir}/02-scanner-v4-07-db-deployment.yaml" 0 "db" "$regex"
        ;;
      collector) # only when generated by helm output
...
# $4 - registry-slug for expected scanner-v4 registries (see 'image_reference_regex()' for the list of currently supported registry-slugs)
...
run_image_defaults_registry_test() {
  ...
  assert_components_registry "$out_dir/scanner-v4" "$expected_scanner_registry" "$any_version" 'scanner-v4-indexer' 'scanner-v4-db'
