...
func TestClientCAAuthWithMultipleVerifiedChains(t *testing.T) {
	...
	// Compare auth provider ignoring config
	a := createdAuthProvider.Clone()
	b := authStatus.GetAuthProvider().Clone()
	a.Config = b.Config
	assert.True(t, protocompat.Equal(a, b))
	...
	// Compare auth provider ignoring config, validate, active and last updated
	a = createdAuthProvider.Clone()
	b = authStatus.GetAuthProvider().Clone()
	a.Config = b.Config
