...
type Store interface {
	...
	Get(ctx context.Context, id string) (*storage.ComplianceOperatorCheckResult, bool, error)
...
func insertIntoComplianceOperatorCheckResults(ctx context.Context, batch *pgx.Batch, obj *storage.ComplianceOperatorCheckResult) error {
...
func (s *storeImpl) copyFromComplianceOperatorCheckResults(ctx context.Context, tx *postgres.Tx, objs ...*storage.ComplianceOperatorCheckResult) error {
	...
	for idx, obj := range objs {
		...
		if (idx+1)%batchSize == 0 || idx == len(objs)-1 {
			...
			_, err = tx.CopyFrom(ctx, pgx.Identifier{"compliance_operator_check_results"}, copyCols, pgx.CopyFromRows(inputRows))
...
func (s *storeImpl) copyFrom(ctx context.Context, objs ...*storage.ComplianceOperatorCheckResult) error {
	...
	if err := s.copyFromComplianceOperatorCheckResults(ctx, tx, objs...); err != nil {
...
func (s *storeImpl) upsert(ctx context.Context, objs ...*storage.ComplianceOperatorCheckResult) error {
	conn, release, err := s.acquireConn(ctx, ops.Get, "ComplianceOperatorCheckResult")
	if err != nil {
		return err
	}
	...
	for _, obj := range objs {
		batch := &pgx.Batch{}
		if err := insertIntoComplianceOperatorCheckResults(ctx, batch, obj); err != nil {
			return err
		}
...
func (s *storeImpl) Get(ctx context.Context, id string) (*storage.ComplianceOperatorCheckResult, bool, error) {
	...
	data, err := pgSearch.RunGetQueryForSchema[storage.ComplianceOperatorCheckResult](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetMany(ctx context.Context, ids []string) ([]*storage.ComplianceOperatorCheckResult, []int, error) {
	...
	rows, err := pgSearch.RunGetManyQueryForSchema[storage.ComplianceOperatorCheckResult](ctx, schema, q, s.db)
	if err != nil {
	...
	resultsByID := make(map[string]*storage.ComplianceOperatorCheckResult, len(rows))
	...
	elems := make([]*storage.ComplianceOperatorCheckResult, 0, len(resultsByID))
	for i, id := range ids {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.ComplianceOperatorCheckResult) error) error {
	var sacQueryFilter *v1.Query
	fetcher, closer, err := pgSearch.RunCursorQueryForSchema[storage.ComplianceOperatorCheckResult](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
