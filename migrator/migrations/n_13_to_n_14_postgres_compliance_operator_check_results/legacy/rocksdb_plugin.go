...
var (
	bucket = []byte("complianceoperatorcheckresults")
)
...
func keyFunc(msg protocompat.Message) []byte {
	return []byte(msg.(*storage.ComplianceOperatorCheckResult).GetId())
}
...
func (b *storeImpl) UpsertMany(_ context.Context, objs []*storage.ComplianceOperatorCheckResult) error {
...
func (b *storeImpl) Walk(_ context.Context, fn func(obj *storage.ComplianceOperatorCheckResult) error) error {
	return b.crud.Walk(func(msg protocompat.Message) error {
		return fn(msg.(*storage.ComplianceOperatorCheckResult))
	})
}
