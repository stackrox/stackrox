...
func (s *storeImpl) Get(ctx context.Context, id string) (*storage.InitBundleMeta, bool, error) {
	...
	data, err := postgres.RunGetQueryForSchemaType[storage.InitBundleMeta](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetMany(ctx context.Context, ids []string) ([]*storage.InitBundleMeta, []int, error) {
	...
	rows, err := postgres.RunGetManyQueryForSchemaType[storage.InitBundleMeta](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.InitBundleMeta) error) error {
	var sacQueryFilter *v1.Query
	fetcher, closer, err := postgres.RunCursorQueryForSchemaType[storage.InitBundleMeta](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
