...
var (
	migration = types.Migration{
		...
		Run: func(databases *types.Databases) error {
			if err := moveAuthProviders(databases.PkgRocksDB, databases.GormDB, databases.PostgresDB); err != nil {
	...
	rocksdbBucket = []byte("authProviders")
	batchSize     = 10000
	schema        = pkgSchema.AuthProvidersSchema
	log           = loghelper.LogWrapper{}
)

func moveAuthProviders(rocksDB *rocksdb.RocksDB, gormDB *gorm.DB, postgresDB *pgxpool.Pool) error {
	...
	store.Walk(ctx, func(obj *storage.AuthProvider) error {
		authProviders = append(authProviders, obj)
		if len(authProviders) == 10*batchSize {
			if err := store.copyFrom(ctx, authProviders...); err != nil {
				log.WriteToStderrf("failed to persist auth_providers to store %v", err)
				return err
			}
			authProviders = authProviders[:0]
		}
		return nil
	})
