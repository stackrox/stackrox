...
func (s *storeImpl) Get(ctx context.Context, id string) (*storage.Policy, bool, error) {
	...
	data, err := postgres.RunGetQueryForSchemaType[storage.Policy](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetMany(ctx context.Context, ids []string) ([]*storage.Policy, []int, error) {
	...
	rows, err := postgres.RunGetManyQueryForSchemaType[storage.Policy](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.Policy) error) error {
	var sacQueryFilter *v1.Query
	fetcher, closer, err := postgres.RunCursorQueryForSchemaType[storage.Policy](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
