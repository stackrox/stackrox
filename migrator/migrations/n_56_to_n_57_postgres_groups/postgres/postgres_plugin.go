...
func (s *storeImpl) Get(ctx context.Context, propsId string) (*storage.Group, bool, error) {
	...
	data, err := postgres.RunGetQueryForSchemaType[storage.Group](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetMany(ctx context.Context, ids []string) ([]*storage.Group, []int, error) {
	...
	rows, err := postgres.RunGetManyQueryForSchemaType[storage.Group](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.Group) error) error {
	var sacQueryFilter *v1.Query
	fetcher, closer, err := postgres.RunCursorQueryForSchemaType[storage.Group](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
