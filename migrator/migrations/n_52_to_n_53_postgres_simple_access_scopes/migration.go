...
var (
	...
	batchSize               = 1000
	permissionSetSchema     = pkgSchema.PermissionSetsSchema
	roleSchema              = pkgSchema.RolesSchema
	simpleAccessScopeSchema = pkgSchema.SimpleAccessScopesSchema
...
func migrateAll(rocksDatabase *rocksdb.RocksDB, gormDB *gorm.DB, postgresDB *pgxpool.Pool) error {
	...
	// This function call was added in 3.73.3 in order to cover an overlooked reference to access scope IDs.
	// Users who migrated to postgres with earlier 3.73 may have report configurations relying on incorrect scope IDs.
...
func migrateAccessScopes(gormDB *gorm.DB, postgresDB *pgxpool.Pool, legacyStore legacysimpleaccessscopes.Store) error {
	...
	pkgSchema.ApplySchemaForTable(context.Background(), gormDB, simpleAccessScopeSchema.Table)
...
func migratePermissionSets(gormDB *gorm.DB, postgresDB *pgxpool.Pool, legacyStore legacypermissionsets.Store) error {
	pkgSchema.ApplySchemaForTable(context.Background(), gormDB, permissionSetSchema.Table)
	ctx := sac.WithAllAccess(context.Background())
...
func migrateRoles(gormDB *gorm.DB, postgresDB *pgxpool.Pool, legacyStore legacyroles.Store) error {
	pkgSchema.ApplySchemaForTable(context.Background(), gormDB, roleSchema.Table)
	ctx := sac.WithAllAccess(context.Background())
...
// This function was added in 3.73.3 in order to cover an overlooked reference to access scope IDs.
// Users who migrated to postgres with earlier 3.73 may have report configurations relying on incorrect scope IDs.
