...
func (s *storeImpl) Get(ctx context.Context, id string) (*storage.SimpleAccessScope, bool, error) {
	...
	data, err := postgres.RunGetQueryForSchemaType[storage.SimpleAccessScope](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetMany(ctx context.Context, ids []string) ([]*storage.SimpleAccessScope, []int, error) {
	...
	rows, err := postgres.RunGetManyQueryForSchemaType[storage.SimpleAccessScope](ctx, schema, q, s.db)
	if err != nil {
	...
	resultsByID := make(map[string]*storage.SimpleAccessScope, len(rows))
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.SimpleAccessScope) error) error {
	var sacQueryFilter *v1.Query
	fetcher, closer, err := postgres.RunCursorQueryForSchemaType[storage.SimpleAccessScope](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
