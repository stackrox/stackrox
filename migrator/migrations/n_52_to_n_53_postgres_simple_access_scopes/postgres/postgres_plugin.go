...
func (s *storeImpl) GetMany(ctx context.Context, ids []string) ([]*storage.SimpleAccessScope, []int, error) {
	...
	rows, err := postgres.RunGetManyQueryForSchema[storage.SimpleAccessScope](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.SimpleAccessScope) error) error {
	var sacQueryFilter *v1.Query
	fetcher, closer, err := postgres.RunCursorQueryForSchema[storage.SimpleAccessScope](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
