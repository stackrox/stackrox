...
func (s *postgresMigrationSuite) TestMigration() {
	...
	for i := 0; i < 200; i++ {
		...
		bytes, err := proto.Marshal(networkEntity)
		s.NoError(err, "failed to marshal data")
		rocksWriteBatch.Put(rocksdbmigration.GetPrefixedKey(rocksdbBucket, keyFunc(networkEntity)), bytes)
		networkEntitys = append(networkEntitys, networkEntity)
	}

	s.NoError(s.db.Write(gorocksdb.NewDefaultWriteOptions(), rocksWriteBatch))
	s.NoError(moveNetworkEntities(s.rocksDB, s.gormDB, s.pool))
	var count int64
	s.gormDB.Model(pkgSchema.CreateTableNetworkEntitiesStmt.GormModel).Count(&count)
	s.Equal(int64(len(networkEntitys)), count)
	for _, networkEntity := range networkEntitys {
		s.Equal(networkEntity, s.get(networkEntity.GetInfo().GetId()))
	}
}

func (s *postgresMigrationSuite) get(infoId string) *storage.NetworkEntity {
