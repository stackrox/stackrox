...
import (
	...
	migStore "github.com/stackrox/rox/migrator/migrations/m_191_to_m_192_cluster_with_store/cluster/postgres"
	"github.com/stackrox/rox/migrator/migrations/m_191_to_m_192_cluster_with_store/schema"
...
func migrate(database *types.Databases) error {
	...
	ctx := database.DBCtx
	db := database.GormDB
	pgutils.CreateTableFromModel(database.DBCtx, db, schema.CreateTableClustersStmt)

	store := migStore.New(database.PostgresDB)
	var convertedRecords []*storage.Cluster
	var count int

	err := store.Walk(ctx, func(obj *storage.Cluster) error {
