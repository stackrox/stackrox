...
func (s *storeImpl) copyFromLogImbues(ctx context.Context, tx *postgres.Tx, objs ...*storage.LogImbue) error {
	...
	for idx, obj := range objs {
		...
		if (idx+1)%batchSize == 0 || idx == len(objs)-1 {
			...
			_, err = tx.CopyFrom(ctx, pgx.Identifier{"log_imbues"}, copyCols, pgx.CopyFromRows(inputRows))
...
func (s *storeImpl) copyFrom(ctx context.Context, objs ...*storage.LogImbue) error {
	conn, release, err := s.acquireConn(ctx, ops.Get, "LogImbue")
	if err != nil {
		return err
	}
	defer release()
	...
	if err := s.copyFromLogImbues(ctx, tx, objs...); err != nil {
...
func (s *storeImpl) upsert(ctx context.Context, objs ...*storage.LogImbue) error {
	...
	for _, obj := range objs {
		batch := &pgx.Batch{}
		if err := insertIntoLogImbues(ctx, batch, obj); err != nil {
			return err
		}
...
func (s *storeImpl) GetMany(ctx context.Context, ids []string) ([]*storage.LogImbue, []int, error) {
	...
	rows, err := pgSearch.RunGetManyQueryForSchema[storage.LogImbue](ctx, schema, q, s.db)
	if err != nil {
	...
	elems := make([]*storage.LogImbue, 0, len(resultsByID))
	for i, id := range ids {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.LogImbue) error) error {
	var sacQueryFilter *v1.Query
	fetcher, closer, err := pgSearch.RunCursorQueryForSchema[storage.LogImbue](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
