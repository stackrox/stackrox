...
func (s *postgresMigrationSuite) TearDownTest() {
	...
	_ = s.gormDB.Migrator().DropTable(pkgSchema.CreateTableComplianceOperatorScanSettingBindingsStmt.GormModel)
	pgtest.CleanUpDB(s.T(), s.ctx, s.pool)
...
func (s *postgresMigrationSuite) TestMigration() {
	...
	for i := 0; i < 200; i++ {
		...
		bytes, err := proto.Marshal(complianceOperatorScanSettingBinding)
		s.NoError(err, "failed to marshal data")
		rocksWriteBatch.Put(rocksdbmigration.GetPrefixedKey(rocksdbBucket, keyFunc(complianceOperatorScanSettingBinding)), bytes)
	...
	s.NoError(moveComplianceOperatorScanSettingBindings(s.rocksDB, s.gormDB, s.pool))
	var count int64
	s.gormDB.Model(pkgSchema.CreateTableComplianceOperatorScanSettingBindingsStmt.GormModel).Count(&count)
	s.Equal(int64(len(complianceOperatorScanSettingBindings)), count)
	for _, complianceOperatorScanSettingBinding := range complianceOperatorScanSettingBindings {
		s.Equal(complianceOperatorScanSettingBinding, s.get(complianceOperatorScanSettingBinding.GetId()))
	}
}

func (s *postgresMigrationSuite) get(id string) *storage.ComplianceOperatorScanSettingBinding {
