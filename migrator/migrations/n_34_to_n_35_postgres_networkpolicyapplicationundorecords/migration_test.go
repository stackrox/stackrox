...
func (s *postgresMigrationSuite) TearDownTest() {
	...
	_ = s.gormDB.Migrator().DropTable(pkgSchema.CreateTableNetworkpolicyapplicationundorecordsStmt.GormModel)
	pgtest.CleanUpDB(s.ctx, s.T(), s.pool)
...
func (s *postgresMigrationSuite) TestMigration() {
	...
	var count int64
	s.gormDB.Model(pkgSchema.CreateTableNetworkpolicyapplicationundorecordsStmt.GormModel).Count(&count)
	s.Equal(int64(len(networkPolicyApplicationUndoRecords)), count)
	for _, networkPolicyApplicationUndoRecord := range networkPolicyApplicationUndoRecords {
		s.Equal(networkPolicyApplicationUndoRecord, s.get(networkPolicyApplicationUndoRecord.GetClusterId()))
	}
}

func (s *postgresMigrationSuite) get(clusterId string) *storage.NetworkPolicyApplicationUndoRecord {
