...
func (s *storeImpl) Get(ctx context.Context, clusterId string) (*storage.NetworkPolicyApplicationUndoRecord, bool, error) {
	...
	data, err := postgres.RunGetQueryForSchemaType[storage.NetworkPolicyApplicationUndoRecord](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetMany(ctx context.Context, ids []string) ([]*storage.NetworkPolicyApplicationUndoRecord, []int, error) {
	...
	rows, err := postgres.RunGetManyQueryForSchemaType[storage.NetworkPolicyApplicationUndoRecord](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.NetworkPolicyApplicationUndoRecord) error) error {
	var sacQueryFilter *v1.Query
	fetcher, closer, err := postgres.RunCursorQueryForSchemaType[storage.NetworkPolicyApplicationUndoRecord](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
