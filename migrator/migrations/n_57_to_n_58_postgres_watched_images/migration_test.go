...
func (s *postgresMigrationSuite) TestMigration() {
	...
	for i := 0; i < 200; i++ {
		...
		bytes, err := proto.Marshal(watchedImage)
		s.NoError(err, "failed to marshal data")
		rocksWriteBatch.Put(rocksdbmigration.GetPrefixedKey(rocksdbBucket, keyFunc(watchedImage)), bytes)
		watchedImages = append(watchedImages, watchedImage)
	}

	s.NoError(s.db.Write(gorocksdb.NewDefaultWriteOptions(), rocksWriteBatch))
	s.NoError(moveWatchedImages(s.rocksDB, s.gormDB, s.pool))
	var count int64
	s.gormDB.Model(pkgSchema.CreateTableWatchedImagesStmt.GormModel).Count(&count)
	s.Equal(int64(len(watchedImages)), count)
	for _, watchedImage := range watchedImages {
		s.Equal(watchedImage, s.get(watchedImage.GetName()))
	}
}

func (s *postgresMigrationSuite) get(name string) *storage.WatchedImage {
