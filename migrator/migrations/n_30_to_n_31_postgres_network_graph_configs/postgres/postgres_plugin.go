...
func (s *storeImpl) copyFrom(ctx context.Context, objs ...*storage.NetworkGraphConfig) error {
	conn, release, err := s.acquireConn(ctx, ops.Get, "NetworkGraphConfig")
	if err != nil {
		return err
	}
	defer release()
...
func (s *storeImpl) upsert(ctx context.Context, objs ...*storage.NetworkGraphConfig) error {
	conn, release, err := s.acquireConn(ctx, ops.Get, "NetworkGraphConfig")
	if err != nil {
		return err
	}
...
func (s *storeImpl) Upsert(ctx context.Context, obj *storage.NetworkGraphConfig) error {
	defer metrics.SetPostgresOperationDurationTime(time.Now(), ops.Upsert, "NetworkGraphConfig")
...
func (s *storeImpl) UpsertMany(ctx context.Context, objs []*storage.NetworkGraphConfig) error {
	defer metrics.SetPostgresOperationDurationTime(time.Now(), ops.UpdateMany, "NetworkGraphConfig")
...
func (s *storeImpl) Get(ctx context.Context, id string) (*storage.NetworkGraphConfig, bool, error) {
...
func (s *storeImpl) GetIDs(ctx context.Context) ([]string, error) {
	defer metrics.SetPostgresOperationDurationTime(time.Now(), ops.GetAll, "storage.NetworkGraphConfigIDs")
	var sacQueryFilter *v1.Query
...
func (s *storeImpl) GetMany(ctx context.Context, ids []string) ([]*storage.NetworkGraphConfig, []int, error) {
	...
	elems := make([]*storage.NetworkGraphConfig, 0, len(resultsByID))
	for i, id := range ids {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.NetworkGraphConfig) error) error {
	var sacQueryFilter *v1.Query
	scopeChecker := sac.GlobalAccessScopeChecker(ctx).AccessMode(storage.Access_READ_ACCESS).Resource(targetResource)
	if ok, err := scopeChecker.Allowed(ctx); err != nil {
		return err
