...
var (
	migration = types.Migration{
		...
		Run: func(databases *types.Databases) error {
			if err := moveNamespaces(databases.PkgRocksDB, databases.GormDB, databases.PostgresDB); err != nil {
	...
	rocksdbBucket = []byte("namespaces")
	batchSize     = 10000
	schema        = pkgSchema.NamespacesSchema
	log           = loghelper.LogWrapper{}
)

func moveNamespaces(rocksDB *rocksdb.RocksDB, gormDB *gorm.DB, postgresDB *pgxpool.Pool) error {
	...
	store.Walk(ctx, func(obj *storage.NamespaceMetadata) error {
		namespaces = append(namespaces, obj)
		if len(namespaces) == 10*batchSize {
			if err := store.copyFrom(ctx, namespaces...); err != nil {
