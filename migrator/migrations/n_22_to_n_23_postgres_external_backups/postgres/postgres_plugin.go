...
func insertIntoExternalBackups(ctx context.Context, batch *pgx.Batch, obj *storage.ExternalBackup) error {
	...
	finalStr := "INSERT INTO external_backups (Id, serialized) VALUES($1, $2) ON CONFLICT(Id) DO UPDATE SET Id = EXCLUDED.Id, serialized = EXCLUDED.serialized"
...
func (s *storeImpl) copyFromExternalBackups(ctx context.Context, tx *postgres.Tx, objs ...*storage.ExternalBackup) error {
	...
	for idx, obj := range objs {
		...
		if (idx+1)%batchSize == 0 || idx == len(objs)-1 {
			...
			_, err = tx.CopyFrom(ctx, pgx.Identifier{"external_backups"}, copyCols, pgx.CopyFromRows(inputRows))
...
func (s *storeImpl) copyFrom(ctx context.Context, objs ...*storage.ExternalBackup) error {
	conn, release, err := s.acquireConn(ctx, ops.Get, "ExternalBackup")
	if err != nil {
		return err
	}
	defer release()
	...
	if err := s.copyFromExternalBackups(ctx, tx, objs...); err != nil {
...
func (s *storeImpl) upsert(ctx context.Context, objs ...*storage.ExternalBackup) error {
	...
	for _, obj := range objs {
		batch := &pgx.Batch{}
		if err := insertIntoExternalBackups(ctx, batch, obj); err != nil {
			return err
		}
...
func (s *storeImpl) Get(ctx context.Context, id string) (*storage.ExternalBackup, bool, error) {
	...
	data, err := pgSearch.RunGetQueryForSchema[storage.ExternalBackup](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetAll(ctx context.Context) ([]*storage.ExternalBackup, error) {
	...
	var objs []*storage.ExternalBackup
	err := s.Walk(ctx, func(obj *storage.ExternalBackup) error {
...
func (s *storeImpl) GetMany(ctx context.Context, ids []string) ([]*storage.ExternalBackup, []int, error) {
	...
	rows, err := pgSearch.RunGetManyQueryForSchema[storage.ExternalBackup](ctx, schema, q, s.db)
	if err != nil {
	...
	resultsByID := make(map[string]*storage.ExternalBackup, len(rows))
	...
	elems := make([]*storage.ExternalBackup, 0, len(resultsByID))
	for i, id := range ids {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.ExternalBackup) error) error {
	var sacQueryFilter *v1.Query
	fetcher, closer, err := pgSearch.RunCursorQueryForSchema[storage.ExternalBackup](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
...
func Destroy(ctx context.Context, db postgres.DB) {
	dropTableExternalBackups(ctx, db)
}
