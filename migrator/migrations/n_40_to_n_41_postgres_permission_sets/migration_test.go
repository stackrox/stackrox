...
func (s *postgresMigrationSuite) TestMigration() {
	...
	for i := 0; i < 200; i++ {
		...
		bytes, err := proto.Marshal(permissionSet)
		s.NoError(err, "failed to marshal data")
		rocksWriteBatch.Put(rocksdbmigration.GetPrefixedKey(rocksdbBucket, keyFunc(permissionSet)), bytes)
		permissionSets = append(permissionSets, permissionSet)
	}

	s.NoError(s.db.Write(gorocksdb.NewDefaultWriteOptions(), rocksWriteBatch))
	s.NoError(movePermissionSets(s.rocksDB, s.gormDB, s.pool))
	var count int64
