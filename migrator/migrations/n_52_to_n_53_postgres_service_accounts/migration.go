...
var (
	migration = types.Migration{
		...
		Run: func(databases *types.Databases) error {
			...
			if err := moveServiceAccounts(databases.PkgRocksDB, databases.GormDB, databases.PostgresDB, legacyStore); err != nil {
...
func moveServiceAccounts(legacyDB *rocksdb.RocksDB, gormDB *gorm.DB, postgresDB *pgxpool.Pool, legacyStore legacy.Store) error {
	...
	legacyStore.Walk(ctx, func(obj *storage.ServiceAccount) error {
		...
		if len(serviceAccounts) == 10*batchSize {
			if err := store.copyFrom(ctx, serviceAccounts...); err != nil {
