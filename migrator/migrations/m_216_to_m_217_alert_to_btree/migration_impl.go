...
const (
	tableName   = "alerts"
	index       = "alerts_deployment_id"
	tmpIndex    = "alerts_deployment_id_tmp"
	indexColumn = "deployment_id"
)
...
func migrate(database *types.Databases) error {
	...
	err = tx.Commit(ctx)
	if err != nil {
		return postgreshelper.WrapRollback(ctx, tx, errors.Wrapf(err, "unable to update alerts indexes in migration %d", startSeqNum))
	}

	log.Infof("Alerts indexes migration complete")
	return nil
}
