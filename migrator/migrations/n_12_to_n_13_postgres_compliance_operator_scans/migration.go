...
var (
	migration = types.Migration{
		...
		Run: func(databases *types.Databases) error {
			if err := moveComplianceOperatorScans(databases.PkgRocksDB, databases.GormDB, databases.PostgresDB); err != nil {
	...
	rocksdbBucket = []byte("complianceoperatorscans")
	batchSize     = 10000
	schema        = pkgSchema.ComplianceOperatorScansSchema
	log           = loghelper.LogWrapper{}
)

func moveComplianceOperatorScans(rocksDB *rocksdb.RocksDB, gormDB *gorm.DB, postgresDB *pgxpool.Pool) error {
	...
	store.Walk(ctx, func(obj *storage.ComplianceOperatorScan) error {
		complianceOperatorScans = append(complianceOperatorScans, obj)
		if len(complianceOperatorScans) == 10*batchSize {
			if err := store.copyFrom(ctx, complianceOperatorScans...); err != nil {
