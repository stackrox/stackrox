...
func (s *storeImpl) Get(ctx context.Context, id string) (*storage.Alert, bool, error) {
	...
	data, err := postgres.RunGetQueryForSchemaType[storage.Alert](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetMany(ctx context.Context, ids []string) ([]*storage.Alert, []int, error) {
	...
	rows, err := postgres.RunGetManyQueryForSchemaType[storage.Alert](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.Alert) error) error {
	var sacQueryFilter *v1.Query
	fetcher, closer, err := postgres.RunCursorQueryForSchemaType[storage.Alert](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
