...
func (s *storeImpl) upsert(ctx context.Context, objs ...*storage.NetworkGraphConfig) error {
	...
	for _, obj := range objs {
		batch := &pgx.Batch{}
		if err := insertIntoNetworkGraphConfigs(ctx, batch, obj); err != nil {
			return err
		}
...
func (s *storeImpl) Get(ctx context.Context, id string) (*storage.NetworkGraphConfig, bool, error) {
	...
	data, err := pgSearch.RunGetQueryForSchema[storage.NetworkGraphConfig](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetMany(ctx context.Context, ids []string) ([]*storage.NetworkGraphConfig, []int, error) {
	...
	rows, err := pgSearch.RunGetManyQueryForSchema[storage.NetworkGraphConfig](ctx, schema, q, s.db)
	if err != nil {
	...
	resultsByID := make(map[string]*storage.NetworkGraphConfig, len(rows))
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.NetworkGraphConfig) error) error {
	var sacQueryFilter *v1.Query
	fetcher, closer, err := pgSearch.RunCursorQueryForSchema[storage.NetworkGraphConfig](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
...
func dropTableNetworkGraphConfigs(ctx context.Context, db postgres.DB) {
