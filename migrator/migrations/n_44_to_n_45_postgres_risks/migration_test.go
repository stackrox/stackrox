...
func (s *postgresMigrationSuite) TearDownTest() {
	...
	_ = s.gormDB.Migrator().DropTable(pkgSchema.CreateTableRisksStmt.GormModel)
	pgtest.CleanUpDB(s.ctx, s.T(), s.pool)
...
func (s *postgresMigrationSuite) TestMigration() {
	...
	s.NoError(legacyStore.UpsertMany(s.ctx, risks))
	s.NoError(move(s.gormDB, s.pool, legacyStore))
	var count int64
	s.gormDB.Model(pkgSchema.CreateTableRisksStmt.GormModel).Count(&count)
	s.Equal(int64(len(risks)), count)
	for _, risk := range risks {
		s.Equal(risk, s.get(risk.GetId()))
	}
}

func (s *postgresMigrationSuite) get(id string) *storage.Risk {
