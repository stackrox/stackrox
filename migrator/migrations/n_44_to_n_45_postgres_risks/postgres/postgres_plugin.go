...
func (s *storeImpl) Get(ctx context.Context, id string) (*storage.Risk, bool, error) {
	...
	data, err := pgSearch.RunGetQueryForSchema[storage.Risk](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetMany(ctx context.Context, ids []string) ([]*storage.Risk, []int, error) {
	...
	rows, err := pgSearch.RunGetManyQueryForSchema[storage.Risk](ctx, schema, q, s.db)
	if err != nil {
	...
	resultsByID := make(map[string]*storage.Risk, len(rows))
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.Risk) error) error {
	var sacQueryFilter *v1.Query
	fetcher, closer, err := pgSearch.RunCursorQueryForSchema[storage.Risk](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
