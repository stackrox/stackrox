...
func (s *postgresMigrationSuite) TestMigration() {
	...
	for i := 0; i < 200; i++ {
		...
		bytes, err := proto.Marshal(complianceOperatorProfile)
		s.NoError(err, "failed to marshal data")
		rocksWriteBatch.Put(rocksdbmigration.GetPrefixedKey(rocksdbBucket, keyFunc(complianceOperatorProfile)), bytes)
	...
	s.NoError(moveComplianceOperatorProfiles(s.rocksDB, s.gormDB, s.pool))
	var count int64
	s.gormDB.Model(pkgSchema.CreateTableComplianceOperatorProfilesStmt.GormModel).Count(&count)
	s.Equal(int64(len(complianceOperatorProfiles)), count)
	for _, complianceOperatorProfile := range complianceOperatorProfiles {
		s.Equal(complianceOperatorProfile, s.get(complianceOperatorProfile.GetId()))
	}
}

func (s *postgresMigrationSuite) get(id string) *storage.ComplianceOperatorProfile {
