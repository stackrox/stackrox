...
func (s *storeImpl) Get(ctx context.Context, runId string) (*storage.ComplianceRunMetadata, bool, error) {
	...
	data, err := pgSearch.RunGetQueryForSchema[storage.ComplianceRunMetadata](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetMany(ctx context.Context, ids []string) ([]*storage.ComplianceRunMetadata, []int, error) {
	...
	rows, err := pgSearch.RunGetManyQueryForSchema[storage.ComplianceRunMetadata](ctx, schema, q, s.db)
	if err != nil {
	...
	resultsByID := make(map[string]*storage.ComplianceRunMetadata, len(rows))
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.ComplianceRunMetadata) error) error {
	var sacQueryFilter *v1.Query
	fetcher, closer, err := pgSearch.RunCursorQueryForSchema[storage.ComplianceRunMetadata](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
