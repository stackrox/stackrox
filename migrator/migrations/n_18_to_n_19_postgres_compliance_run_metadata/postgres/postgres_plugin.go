...
func (s *storeImpl) Get(ctx context.Context, runId string) (*storage.ComplianceRunMetadata, bool, error) {
	...
	data, err := postgres.RunGetQueryForSchemaType[storage.ComplianceRunMetadata](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetMany(ctx context.Context, ids []string) ([]*storage.ComplianceRunMetadata, []int, error) {
	...
	rows, err := postgres.RunGetManyQueryForSchemaType[storage.ComplianceRunMetadata](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.ComplianceRunMetadata) error) error {
	var sacQueryFilter *v1.Query
	fetcher, closer, err := postgres.RunCursorQueryForSchemaType[storage.ComplianceRunMetadata](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
