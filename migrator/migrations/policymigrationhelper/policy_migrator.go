...
type PolicyUpdates struct {
	...
	ExclusionsToAdd map[int]*storage.Exclusion
...
func (u *PolicyUpdates) applyToPolicy(policy *storage.Policy) {
	...
	if u.ExclusionsToAdd != nil {
		for index, exclusion := range u.ExclusionsToAdd {
			insertExclusion(exclusion, index, policy)
		}
	}
...
func insertExclusion(exclusion *storage.Exclusion, index int, policy *storage.Policy) {
	if index >= len(policy.GetExclusions()) { // append to end
		policy.Exclusions = append(policy.Exclusions, exclusion)
		return
	}

	exclusions := append(policy.Exclusions[:index+1], policy.Exclusions[index:]...)
