...
func (suite *policyMigratorTestSuite) TestAllUnmodifiedPoliciesGetUpdated() {
	...
	for i := 0; i < 10; i++ {
		...
		suite.NoError(insertPolicy(bucket, policy.Id, &policy))
		...
		policyID: {
			...
			ToChange: PolicyUpdates{
				ExclusionsToAdd: map[int]*storage.Exclusion{
					1: {Name: "exclusion1-changed", Deployment: &storage.Exclusion_Deployment{Scope: &storage.Scope{Namespace: "namespace-1"}}},
					2: {Name: "NEW exclusion", Deployment: &storage.Exclusion_Deployment{Scope: &storage.Scope{Namespace: "namespace-NEW"}}},
					5: {Name: "NEW exclusion2", Deployment: &storage.Exclusion_Deployment{Scope: &storage.Scope{Namespace: "namespace-NEW2"}}}, // should get added to end
				},
...
func (suite *policyMigratorTestSuite) TestExclusionAreAddedEvenIfPolicyHadNoneBefore() {
	...
	policiesToMigrate := map[string]PolicyChanges{
		policyID: {
			...
			ToChange: PolicyUpdates{
				ExclusionsToAdd: map[int]*storage.Exclusion{
					0: {Name: "exclusion1-added", Deployment: &storage.Exclusion_Deployment{Scope: &storage.Scope{Namespace: "namespace"}}},
				},
			},
		},
	}
