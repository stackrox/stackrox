...
func (s *storeImpl) upsert(ctx context.Context, objs ...*storage.NetworkPolicy) error {
	...
	for _, obj := range objs {
		batch := &pgx.Batch{}
		if err := insertIntoNetworkpolicies(ctx, batch, obj); err != nil {
			return err
		}
...
func (s *storeImpl) Get(ctx context.Context, id string) (*storage.NetworkPolicy, bool, error) {
	...
	data, err := pgSearch.RunGetQueryForSchema[storage.NetworkPolicy](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetMany(ctx context.Context, ids []string) ([]*storage.NetworkPolicy, []int, error) {
	...
	rows, err := pgSearch.RunGetManyQueryForSchema[storage.NetworkPolicy](ctx, schema, q, s.db)
	if err != nil {
	...
	resultsByID := make(map[string]*storage.NetworkPolicy, len(rows))
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.NetworkPolicy) error) error {
	var sacQueryFilter *v1.Query
	fetcher, closer, err := pgSearch.RunCursorQueryForSchema[storage.NetworkPolicy](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
