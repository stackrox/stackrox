// Frozen pre-migration schema for the alerts table (without deployment_type and enforcementcount columns).
...
var (
	// CreateTableAlertsStmt holds the create statement for table `alerts` (pre-migration).
...
// Alerts holds the pre-migration Gorm model (no deployment_type, no enforcementcount).
type Alerts struct {
