...
var (
	migration = types.Migration{
		...
		Run: func(databases *types.Databases) error {
			if err := moveNetworkpoliciesundodeployments(databases.PkgRocksDB, databases.GormDB, databases.PostgresDB); err != nil {
	...
	rocksdbBucket = []byte("networkpolicies-undodeployment")
	batchSize     = 10000
	schema        = pkgSchema.NetworkpoliciesundodeploymentsSchema
	log           = loghelper.LogWrapper{}
)

func moveNetworkpoliciesundodeployments(rocksDB *rocksdb.RocksDB, gormDB *gorm.DB, postgresDB *pgxpool.Pool) error {
	...
	store.Walk(ctx, func(obj *storage.NetworkPolicyApplicationUndoDeploymentRecord) error {
		networkpoliciesundodeployments = append(networkpoliciesundodeployments, obj)
		if len(networkpoliciesundodeployments) == 10*batchSize {
			if err := store.copyFrom(ctx, networkpoliciesundodeployments...); err != nil {
