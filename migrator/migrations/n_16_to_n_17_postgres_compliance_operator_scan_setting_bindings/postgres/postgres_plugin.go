...
func (s *storeImpl) upsert(ctx context.Context, objs ...*storage.ComplianceOperatorScanSettingBinding) error {
	conn, release, err := s.acquireConn(ctx, ops.Get, "ComplianceOperatorScanSettingBinding")
	if err != nil {
		return err
	}
	...
	for _, obj := range objs {
		batch := &pgx.Batch{}
		if err := insertIntoComplianceOperatorScanSettingBindings(ctx, batch, obj); err != nil {
			return err
		}
...
func (s *storeImpl) Get(ctx context.Context, id string) (*storage.ComplianceOperatorScanSettingBinding, bool, error) {
	...
	data, err := pgSearch.RunGetQueryForSchema[storage.ComplianceOperatorScanSettingBinding](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetMany(ctx context.Context, ids []string) ([]*storage.ComplianceOperatorScanSettingBinding, []int, error) {
	...
	rows, err := pgSearch.RunGetManyQueryForSchema[storage.ComplianceOperatorScanSettingBinding](ctx, schema, q, s.db)
	if err != nil {
	...
	resultsByID := make(map[string]*storage.ComplianceOperatorScanSettingBinding, len(rows))
	...
	elems := make([]*storage.ComplianceOperatorScanSettingBinding, 0, len(resultsByID))
	for i, id := range ids {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.ComplianceOperatorScanSettingBinding) error) error {
	var sacQueryFilter *v1.Query
	fetcher, closer, err := pgSearch.RunCursorQueryForSchema[storage.ComplianceOperatorScanSettingBinding](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
...
func dropTableComplianceOperatorScanSettingBindings(ctx context.Context, db postgres.DB) {
