...
func (s *postgresMigrationSuite) TestMigration() {
	...
	for i := 0; i < 200; i++ {
		...
		bytes, err := proto.Marshal(complianceOperatorCheckResult)
		s.NoError(err, "failed to marshal data")
		rocksWriteBatch.Put(rocksdbmigration.GetPrefixedKey(rocksdbBucket, keyFunc(complianceOperatorCheckResult)), bytes)
	...
	s.NoError(moveComplianceOperatorCheckResults(s.rocksDB, s.gormDB, s.pool))
	var count int64
