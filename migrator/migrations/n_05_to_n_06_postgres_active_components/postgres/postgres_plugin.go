...
func (s *storeImpl) Get(ctx context.Context, id string) (*storage.ActiveComponent, bool, error) {
	...
	data, err := postgres.RunGetQueryForSchemaType[storage.ActiveComponent](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetMany(ctx context.Context, ids []string) ([]*storage.ActiveComponent, []int, error) {
	...
	rows, err := postgres.RunGetManyQueryForSchemaType[storage.ActiveComponent](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.ActiveComponent) error) error {
	var sacQueryFilter *v1.Query
	fetcher, closer, err := postgres.RunCursorQueryForSchemaType[storage.ActiveComponent](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
