...
func (s *postgresMigrationSuite) SetupTest() {
	...
	_ = s.gormDB.Migrator().DropTable(pkgSchema.CreateTableIntegrationHealthsStmt.GormModel)
}
...
func (s *postgresMigrationSuite) TestMigration() {
	...
	for i := 0; i < 200; i++ {
		...
		bytes, err := proto.Marshal(integrationHealth)
		s.NoError(err, "failed to marshal data")
		rocksWriteBatch.Put(rocksdbmigration.GetPrefixedKey(rocksdbBucket, keyFunc(integrationHealth)), bytes)
	...
	s.NoError(moveIntegrationHealths(s.rocksDB, s.gormDB, s.pool))
	var count int64
	s.gormDB.Model(pkgSchema.CreateTableIntegrationHealthsStmt.GormModel).Count(&count)
	s.Equal(int64(len(integrationHealths)), count)
	for _, integrationHealth := range integrationHealths {
		s.Equal(integrationHealth, s.get(integrationHealth.GetId()))
	}
}

func (s *postgresMigrationSuite) get(id string) *storage.IntegrationHealth {
