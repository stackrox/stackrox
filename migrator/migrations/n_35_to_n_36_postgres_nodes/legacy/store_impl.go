...
// //go:generate cp // ../../../../central/node/datastore/internal/store/dackbox/store_impl.go .
...
func New(dacky *dackbox.DackBox, keyFence concurrency.KeyFence, noUpdateTimestamps bool) Store {
	return &storeImpl{
		dacky:              dacky,
		keyFence:           keyFence,
...
func (b *storeImpl) toUpsert(node *storage.Node) (*storage.Node, bool, bool, error) {
	...
	if oldNode.GetScan().GetScanTime().AsTime().After(nodeToUpsert.GetScan().GetScanTime().AsTime()) {
		scanUpdated = false
	...
	if oldNode.GetK8SUpdated().AsTime().After(nodeToUpsert.GetK8SUpdated().AsTime()) {
		nodeUpdated = false
