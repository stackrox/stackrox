...
func (s *postgresMigrationSuite) TearDownTest() {
	...
	_ = s.gormDB.Migrator().DropTable(pkgSchema.CreateTableComplianceStringsStmt.GormModel)
	pgtest.CleanUpDB(s.ctx, s.T(), s.pool)
...
func (s *postgresMigrationSuite) TestMigration() {
	...
	s.NoError(legacyStore.UpsertMany(s.ctx, complianceStringss))
	s.NoError(move(s.gormDB, s.pool, legacyStore))
	var count int64
	s.gormDB.Model(pkgSchema.CreateTableComplianceStringsStmt.GormModel).Count(&count)
	s.Equal(int64(len(complianceStringss)), count)
	for _, complianceStrings := range complianceStringss {
		s.Equal(complianceStrings, s.get(complianceStrings.GetId()))
	}
}

func (s *postgresMigrationSuite) get(id string) *storage.ComplianceStrings {
