...
func (s *storeImpl) Get(ctx context.Context, id string) (*storage.ProcessBaseline, bool, error) {
	...
	data, err := postgres.RunGetQueryForSchemaType[storage.ProcessBaseline](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetMany(ctx context.Context, ids []string) ([]*storage.ProcessBaseline, []int, error) {
	...
	rows, err := postgres.RunGetManyQueryForSchemaType[storage.ProcessBaseline](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.ProcessBaseline) error) error {
	var sacQueryFilter *v1.Query
	fetcher, closer, err := postgres.RunCursorQueryForSchemaType[storage.ProcessBaseline](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
