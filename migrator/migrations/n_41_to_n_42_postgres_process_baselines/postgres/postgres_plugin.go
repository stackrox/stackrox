...
func (s *storeImpl) Get(ctx context.Context, id string) (*storage.ProcessBaseline, bool, error) {
	...
	data, err := pgSearch.RunGetQueryForSchema[storage.ProcessBaseline](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetMany(ctx context.Context, ids []string) ([]*storage.ProcessBaseline, []int, error) {
	...
	rows, err := pgSearch.RunGetManyQueryForSchema[storage.ProcessBaseline](ctx, schema, q, s.db)
	if err != nil {
	...
	resultsByID := make(map[string]*storage.ProcessBaseline, len(rows))
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.ProcessBaseline) error) error {
	var sacQueryFilter *v1.Query
	fetcher, closer, err := pgSearch.RunCursorQueryForSchema[storage.ProcessBaseline](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
