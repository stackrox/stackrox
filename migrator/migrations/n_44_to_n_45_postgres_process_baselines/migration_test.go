...
func (s *postgresMigrationSuite) SetupTest() {
	...
	_ = s.gormDB.Migrator().DropTable(pkgSchema.CreateTableProcessBaselinesStmt.GormModel)
}
...
func (s *postgresMigrationSuite) TestMigration() {
	...
	for i := 0; i < 200; i++ {
		...
		bytes, err := proto.Marshal(processBaseline)
		s.NoError(err, "failed to marshal data")
		rocksWriteBatch.Put(rocksdbmigration.GetPrefixedKey(rocksdbBucket, keyFunc(processBaseline)), bytes)
	...
	s.NoError(moveProcessBaselines(s.rocksDB, s.gormDB, s.pool))
	var count int64
