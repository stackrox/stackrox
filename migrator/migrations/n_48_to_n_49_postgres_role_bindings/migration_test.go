...
func (s *postgresMigrationSuite) SetupTest() {
	...
	_ = s.gormDB.Migrator().DropTable(pkgSchema.CreateTableRoleBindingsStmt.GormModel)
}
...
func (s *postgresMigrationSuite) TestMigration() {
	...
	for i := 0; i < 200; i++ {
		...
		bytes, err := proto.Marshal(k8SRoleBinding)
		s.NoError(err, "failed to marshal data")
		rocksWriteBatch.Put(rocksdbmigration.GetPrefixedKey(rocksdbBucket, keyFunc(k8SRoleBinding)), bytes)
	...
	s.NoError(moveRoleBindings(s.rocksDB, s.gormDB, s.pool))
	var count int64
	s.gormDB.Model(pkgSchema.CreateTableRoleBindingsStmt.GormModel).Count(&count)
	s.Equal(int64(len(k8SRoleBindings)), count)
	for _, k8SRoleBinding := range k8SRoleBindings {
		s.Equal(k8SRoleBinding, s.get(k8SRoleBinding.GetId()))
	}
}

func (s *postgresMigrationSuite) get(id string) *storage.K8SRoleBinding {
