...
func (s *storeImpl) Get(ctx context.Context, id string) (*storage.Secret, bool, error) {
	...
	data, err := postgres.RunGetQueryForSchemaType[storage.Secret](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetMany(ctx context.Context, ids []string) ([]*storage.Secret, []int, error) {
	...
	rows, err := postgres.RunGetManyQueryForSchemaType[storage.Secret](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.Secret) error) error {
	var sacQueryFilter *v1.Query
	fetcher, closer, err := postgres.RunCursorQueryForSchemaType[storage.Secret](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
