...
func (s *storeImpl) copyFromSecretsFiles(ctx context.Context, tx pgx.Tx, secretsID string, objs ...*storage.SecretDataFile) error {
...
func (s *storeImpl) copyFromSecretsFilesRegistries(ctx context.Context, tx pgx.Tx, secretsID string, secretsFilesIdx int, objs ...*storage.ImagePullSecret_Registry) error {
...
func (s *storeImpl) Get(ctx context.Context, id string) (*storage.Secret, bool, error) {
	...
	data, err := postgres.RunGetQueryForSchema[storage.Secret](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.Secret) error) error {
	var sacQueryFilter *v1.Query
	fetcher, closer, err := postgres.RunCursorQueryForSchema[storage.Secret](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
