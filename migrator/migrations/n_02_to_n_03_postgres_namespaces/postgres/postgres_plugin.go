...
func (s *storeImpl) Get(ctx context.Context, id string) (*storage.NamespaceMetadata, bool, error) {
	...
	data, err := postgres.RunGetQueryForSchemaType[storage.NamespaceMetadata](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetMany(ctx context.Context, ids []string) ([]*storage.NamespaceMetadata, []int, error) {
	...
	rows, err := postgres.RunGetManyQueryForSchemaType[storage.NamespaceMetadata](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.NamespaceMetadata) error) error {
	var sacQueryFilter *v1.Query
	fetcher, closer, err := postgres.RunCursorQueryForSchemaType[storage.NamespaceMetadata](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
