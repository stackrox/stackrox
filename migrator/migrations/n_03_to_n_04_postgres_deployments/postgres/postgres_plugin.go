...
func (s *storeImpl) Get(ctx context.Context, id string) (*storage.Deployment, bool, error) {
	...
	data, err := postgres.RunGetQueryForSchemaType[storage.Deployment](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetMany(ctx context.Context, ids []string) ([]*storage.Deployment, []int, error) {
	...
	rows, err := postgres.RunGetManyQueryForSchemaType[storage.Deployment](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.Deployment) error) error {
	var sacQueryFilter *v1.Query
	fetcher, closer, err := postgres.RunCursorQueryForSchemaType[storage.Deployment](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
