...
func (s *storeImpl) copyFromRoleBindingsSubjects(ctx context.Context, tx pgx.Tx, roleBindingsID string, objs ...*storage.Subject) error {
...
func (s *storeImpl) Get(ctx context.Context, id string) (*storage.K8SRoleBinding, bool, error) {
	...
	data, err := postgres.RunGetQueryForSchema[storage.K8SRoleBinding](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) GetMany(ctx context.Context, ids []string) ([]*storage.K8SRoleBinding, []int, error) {
	...
	rows, err := postgres.RunGetManyQueryForSchema[storage.K8SRoleBinding](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.K8SRoleBinding) error) error {
	var sacQueryFilter *v1.Query
	fetcher, closer, err := postgres.RunCursorQueryForSchema[storage.K8SRoleBinding](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
