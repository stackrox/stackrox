...
func insertIntoNetworkpolicies(ctx context.Context, tx pgx.Tx, obj *storage.NetworkPolicy) error {
	...
	finalStr := "INSERT INTO networkpolicies (Id, serialized) VALUES($1, $2) ON CONFLICT(Id) DO UPDATE SET Id = EXCLUDED.Id, serialized = EXCLUDED.serialized"
...
func (s *storeImpl) copyFromNetworkpolicies(ctx context.Context, tx pgx.Tx, objs ...*storage.NetworkPolicy) error {
	...
	for idx, obj := range objs {
		...
		if (idx+1)%batchSize == 0 || idx == len(objs)-1 {
			...
			_, err = tx.CopyFrom(ctx, pgx.Identifier{"networkpolicies"}, copyCols, pgx.CopyFromRows(inputRows))
...
func (s *storeImpl) copyFrom(ctx context.Context, objs ...*storage.NetworkPolicy) error {
	conn, release, err := s.acquireConn(ctx, ops.Get, "NetworkPolicy")
	if err != nil {
		return err
	}
	defer release()
	...
	if err := s.copyFromNetworkpolicies(ctx, tx, objs...); err != nil {
...
func (s *storeImpl) upsert(ctx context.Context, objs ...*storage.NetworkPolicy) error {
	conn, release, err := s.acquireConn(ctx, ops.Get, "NetworkPolicy")
	if err != nil {
		return err
	}
	...
	for _, obj := range objs {
		...
		if err := insertIntoNetworkpolicies(ctx, tx, obj); err != nil {
			if err := tx.Rollback(ctx); err != nil {
				return err
			}
			return err
...
func (s *storeImpl) Upsert(ctx context.Context, obj *storage.NetworkPolicy) error {
	defer metrics.SetPostgresOperationDurationTime(time.Now(), ops.Upsert, "NetworkPolicy")
...
func (s *storeImpl) Count(ctx context.Context) (int, error) {
	defer metrics.SetPostgresOperationDurationTime(time.Now(), ops.Count, "NetworkPolicy")
...
func (s *storeImpl) Exists(ctx context.Context, id string) (bool, error) {
	defer metrics.SetPostgresOperationDurationTime(time.Now(), ops.Exists, "NetworkPolicy")
...
func (s *storeImpl) Get(ctx context.Context, id string) (*storage.NetworkPolicy, bool, error) {
	defer metrics.SetPostgresOperationDurationTime(time.Now(), ops.Get, "NetworkPolicy")
...
func (s *storeImpl) GetIDs(ctx context.Context) ([]string, error) {
	defer metrics.SetPostgresOperationDurationTime(time.Now(), ops.GetAll, "storage.NetworkPolicyIDs")
	var sacQueryFilter *v1.Query
...
func (s *storeImpl) GetMany(ctx context.Context, ids []string) ([]*storage.NetworkPolicy, []int, error) {
	...
	elems := make([]*storage.NetworkPolicy, 0, len(resultsByID))
	for i, id := range ids {
...
func dropTableNetworkpolicies(ctx context.Context, db *pgxpool.Pool) {
	_, _ = db.Exec(ctx, "DROP TABLE IF EXISTS networkpolicies CASCADE")

}

func Destroy(ctx context.Context, db *pgxpool.Pool) {
	dropTableNetworkpolicies(ctx, db)
}
