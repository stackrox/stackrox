...
func (s *storeImpl) Get(ctx context.Context, deploymentId string) (*storage.NetworkBaseline, bool, error) {
	...
	data, err := postgres.RunGetQueryForSchemaType[storage.NetworkBaseline](ctx, schema, q, s.db)
	if err != nil {
...
func (s *storeImpl) Walk(ctx context.Context, fn func(obj *storage.NetworkBaseline) error) error {
	var sacQueryFilter *v1.Query
	fetcher, closer, err := postgres.RunCursorQueryForSchemaType[storage.NetworkBaseline](ctx, schema, sacQueryFilter, s.db)
	if err != nil {
		return err
	}
	defer closer()
