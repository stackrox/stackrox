...
function create_ns {
    kubectl get ns "{{.K8sConfig.Namespace}}" > /dev/null || kubectl create ns "{{.K8sConfig.Namespace}}"
}
function create_file {
    local file="$1"
    kubectl create secret -n "{{.K8sConfig.Namespace}}" generic additional-ca --from-file="ca.crt=$file"
}
...
function create_directory {
    ...
    kubectl create secret -n "{{.K8sConfig.Namespace}}" generic additional-ca --from-file="$dir/"
}
