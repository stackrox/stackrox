...
{{ else }}
  ...
  {{ if $generate }}
    ...
    {{ else }}
      {{ if not $._rox._ca }}
        {{ $msg := list
            (printf "Tried to generate certificate for %s, but no CA certificate is available." $spec.CN)
            ""
            "Providing the CA is required when certificates for new components need to be issued. This can happen, for instance, when enabling a new component such as Scanner V4 using 'helm upgrade'."
            ""
            "If the CA was generated by the Helm chart during initial installation, then you need to retrieve these auto-generated values, including the CA, from the cluster and provide them to the 'helm upgrade' command. A command for retrieving the auto-generated values was included in the post-installation notes of the central-services Helm chart."
            ""
            "If the CA was generated outside of the Helm chart and provided during the installation of the central-services chart, then the same must be done when executing 'helm upgrade'. This can be achived conveniently by specifying the '--reuse-values' flag to 'helm upgrade'."
            | join "\n"
        }}
