...
FULL_IMAGE="${ROX_IMAGE_REGISTRY}/${ROX_IMAGE_REPO}:${PREVENT_IMAGE_TAG}"
echo "Full image to pull is ${FULL_IMAGE}. Does that look correct? Hit any key to continue. "
read -s -n 1

if [ -z "$REGISTRY_USERNAME" ]; then
  echo -n "Registry username for StackRox Prevent image from $ROX_IMAGE_REGISTRY: "
  read REGISTRY_USERNAME
  echo
fi
if [ -z "$REGISTRY_PASSWORD" ]; then
  echo -n "Registry password for StackRox Prevent image from $ROX_IMAGE_REGISTRY: "
  read -s REGISTRY_PASSWORD
  echo
fi

OC_PROJECT="${OC_PROJECT:-stackrox}"
echo "docker login to $ROX_IMAGE_REGISTRY"

set +x
sudo docker login -u "$REGISTRY_USERNAME" -p "$REGISTRY_PASSWORD" "$ROX_IMAGE_REGISTRY"
set -x
