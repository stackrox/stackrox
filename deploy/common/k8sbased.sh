...
function launch_central {
    ...
    if [[ -x "$(command -v roxctl)" && "$(roxctl version)" == "$MAIN_IMAGE_TAG" ]]; then
       ...
       docker run --rm --volume "${k8s_dir}/../common/dev-license.lic":/input/dev-license.lic --env-file <(env | grep '^ROX_') "$MAIN_IMAGE" central generate ${ORCH} ${EXTRA_ARGS[@]} --license "/input/dev-license.lic" --output-format="${OUTPUT_FORMAT}" \
