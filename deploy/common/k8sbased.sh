...
function launch_central {
    ...
    if [[ -f "$unzip_dir/values-public.yaml" ]]; then
      ...
      if [[ "$SCANNER_SUPPORT" == "true" ]]; then
          ...
          if [[ -n "${REGISTRY_USERNAME}" ]]; then
            ROX_NAMESPACE="$namespace" $unzip_dir/scanner/scripts/setup.sh
          fi
    ...
    if [[ "${needs_monitoring}" == "true" ]]; then
      "${COMMON_DIR}/monitoring-central.sh" "$namespace"
    fi
...
function launch_sensor {
    ...
    if [[ "${SENSOR_HELM_DEPLOY:-}" == "true" ]]; then
      ...
      if [[ -n "$NAMESPACE_OVERRIDE" ]]; then
        ...
        ls $k8s_dir/sensor-deploy/*.yaml | while read file; do sed -i'.original' -e 's/namespace: stackrox/namespace: '"$namespace"'/g' $file; done
    ...
    if [[ "$MONITORING_SUPPORT" == "true" || ( "$(local_dev)" != "true" && -z "$MONITORING_SUPPORT" ) ]]; then
      "${COMMON_DIR}/monitoring-sensor.sh" "$namespace"
    fi
