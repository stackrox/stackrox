...
function launch_sensor {
    ...
    if [[ "${SENSOR_HELM_DEPLOY:-}" == "true" ]]; then
      ...
      if [[ -x "$(command -v roxctl)" && "$(roxctl version)" == "$MAIN_IMAGE_TAG" ]]; then
        ...
        export_central_cert
        roxctl --endpoint "${API_ENDPOINT}" sensor generate --insecure-skip-tls-verify --main-image-repository="${MAIN_IMAGE_REPO}" --central="$CLUSTER_API_ENDPOINT" --name="$CLUSTER" \
             --collection-method="$COLLECTION_METHOD" \
             "${ORCH}" \
             "${extra_config[@]+"${extra_config[@]}"}"
        mv "sensor-${CLUSTER}" "$k8s_dir/sensor-deploy"
        if [[ "${GENERATE_SCANNER_DEPLOYMENT_BUNDLE:-}" == "true" ]]; then
            roxctl --endpoint "${API_ENDPOINT}" scanner generate  --insecure-skip-tls-verify \
