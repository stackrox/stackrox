...
var ReadOnlyMode int32 = 0640
var PostgresUser int64 = 70
var CentralUser int64 = 4000
var TwoGigs resource.Quantity = resource.MustParse("2Gi")
...
func main() {
	...
	err = applyCentral(ctx, clientset)
...
func applyCentral(ctx context.Context, clientset *kubernetes.Clientset) error {
	...
	// if err != nil && !errors.IsAlreadyExists(err) {
	// 	return fmt.Errorf("Failed to create PVC: %w\n", err)
	// }
	// log("Created DB PVC")

	err = createCentralEndpointsConfig(ctx, clientset)
	if err != nil && !errors.IsAlreadyExists(err) {
		return fmt.Errorf("Failed to create central endpoints config: %w\n", err)
	}
	log("Created central endpoints config")

	err = createCentralConfig(ctx, clientset)
	if err != nil && !errors.IsAlreadyExists(err) {
		return fmt.Errorf("Failed to create central config: %w\n", err)
	}
	log("Created central config")

	err = createCentralDbConfig(ctx, clientset)
	if err != nil && !errors.IsAlreadyExists(err) {
		return fmt.Errorf("Failed to create central db config: %w\n", err)
	}
	log("Created central db config")

	err = createAdminPassword(ctx, clientset)
	if err != nil && !errors.IsAlreadyExists(err) {
		return fmt.Errorf("Failed to create admin password: %w\n", err)
	}
	log("Created admin password")

	err = createTlsSecrets(ctx, clientset)
	if err != nil && !errors.IsAlreadyExists(err) {
		return fmt.Errorf("Failed to create TLS secret: %w\n", err)
	}
	log("Created TLS secret")

	err = applyCentralDbDeployment(ctx, clientset)
	if err != nil {
		return err
	}

	err = applyCentralDeployment(ctx, clientset)
	if err != nil {
		return err
	}

	err = applyCentralServices(ctx, clientset)
	if err != nil {
		return err
	}
...
func createCentralDbConfig(ctx context.Context, client *kubernetes.Clientset) error {
	cm := v1.ConfigMap{
...
shared_preload_libraries = 'pg_stat_statements'`,
	...
	_, err := client.CoreV1().ConfigMaps("stackrox").Create(ctx, &cm, metav1.CreateOptions{})

	return err
}

func createCentralConfig(ctx context.Context, client *kubernetes.Clientset) error {
	cm := v1.ConfigMap{
		Data: map[string]string{
...
func createCentralEndpointsConfig(ctx context.Context, client *kubernetes.Clientset) error {
	cm := v1.ConfigMap{
		Data: map[string]string{
			"endpoints.yaml": "",
		},
...
func createTlsSecrets(ctx context.Context, client *kubernetes.Clientset) error {
	var secret v1.Secret

	ca, err := certgen.GenerateCA()
	if err != nil {
		return fmt.Errorf("creating new CA: %w\n", err)
	}
	...
	apply := func() error {
		...
		if errors.IsAlreadyExists(err) {
			_, err = client.CoreV1().Secrets("stackrox").Update(ctx, &secret, metav1.UpdateOptions{})
			log("Updated secret %s", secret.GetName())
		} else {
			log("Created secret %s", secret.GetName())
		}
	...
	if err := certgen.IssueCentralCert(fileMap, ca, mtls.WithNamespace("stackrox")); err != nil {
	...
	if err := certgen.IssueOtherServiceCerts(fileMap, ca, []mtls.Subject{mtls.CentralDBSubject}, mtls.WithNamespace("stackrox")); err != nil {
		return fmt.Errorf("issuing central service certificate: %w\n", err)
	}
...
func createAdminPassword(ctx context.Context, client *kubernetes.Clientset) error {
	...
	apply := func() error {
		...
		if errors.IsAlreadyExists(err) {
			_, err = client.CoreV1().Secrets("stackrox").Update(ctx, &secret, metav1.UpdateOptions{})
			if err == nil {
				log("Updated admin-pass")
			}
		} else if err == nil {
			log("Created admin-pass")
		}
		return err
	}
...
func applyCentralDbDeployment(ctx context.Context, client *kubernetes.Clientset) error {
	...
	if errors.IsAlreadyExists(err) {
		_, err = client.AppsV1().Deployments("stackrox").Update(ctx, &deployment, metav1.UpdateOptions{})
		log("Updated central deployment")
	} else {
		log("Created central deployment")
	}

	return err
}

func applyCentralDeployment(ctx context.Context, client *kubernetes.Clientset) error {
	deployment := apps.Deployment{
		Spec: apps.DeploymentSpec{
			Selector: &metav1.LabelSelector{
				MatchLabels: map[string]string{
...
func applyCentralServices(ctx context.Context, client *kubernetes.Clientset) error {
	// central
	...
	_, err := client.CoreV1().Services("stackrox").Create(ctx, &svc, metav1.CreateOptions{})

	if errors.IsAlreadyExists(err) {
		_, err = client.CoreV1().Services("stackrox").Update(ctx, &svc, metav1.UpdateOptions{})
		log("Updated central service")
	} else {
		log("Created central service")
	}
	...
	if errors.IsAlreadyExists(err) {
		_, err = client.CoreV1().Services("stackrox").Update(ctx, &svc, metav1.UpdateOptions{})
		log("Updated central-db service")
	} else {
		log("Created central-db service")
	}
