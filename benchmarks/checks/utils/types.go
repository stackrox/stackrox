...
func AddNotes(result *v1.BenchmarkCheckResult, notes ...string) {
	result.Notes = append(result.Notes, notes...)
}

// AddNotef allows Sprintf style formatting when adding a note
func AddNotef(result *v1.BenchmarkCheckResult, template string, args ...interface{}) {
	result.Notes = append(result.Notes, fmt.Sprintf(template, args...))
}

// Pass sets the test result to Pass
func Pass(result *v1.BenchmarkCheckResult) {
	result.Result = v1.BenchmarkCheckStatus_PASS
}

// Info sets the test result to Info
func Info(result *v1.BenchmarkCheckResult) {
	result.Result = v1.BenchmarkCheckStatus_INFO
}

// Warn sets the test result to Warn
func Warn(result *v1.BenchmarkCheckResult) {
	result.Result = v1.BenchmarkCheckStatus_WARN
}

// Note sets the test result to Note
func Note(result *v1.BenchmarkCheckResult) {
	result.Result = v1.BenchmarkCheckStatus_NOTE
}

// NotApplicable sets the test result to NotApplicable and should be used when the check applies to a specific type of node
func NotApplicable(result *v1.BenchmarkCheckResult) {
	result.Result = v1.BenchmarkCheckStatus_NOT_APPLICABLE
}
