...
class DashboardBenchmarks extends Component {
    ...
    renderBenchmarks = () => Object.keys(this.props.benchmarks).map((benchmarkName) => {
                ...
                .forEach((aggregatedResult) => {
                    if (results[aggregatedResult] !== undefined) {
                        const value = parseInt(
                            result.aggregatedResults[aggregatedResult],
                            10
                        );
        ...
        return (
            <div className="pb-3 flex w-full" key={benchmarkName}>
                <Link className="text-sm text-primary-500 tracking-wide underline w-1/3 text-left" to="/compliance">{benchmarkName}</Link>
                <div className="flex flex-1 w-2/3 h-2">
                    {
                        Object.keys(results).map((result) => {
                            ...
                            return (
                                <div className="border-r border-white" style={backgroundStyle} key={result} />
                            );
                        })
                    }
                </div>
    ...
    renderLegend = () => Object.keys(benchmarkResultsMap).map((result) => {
        const backgroundStyle = {
            backgroundColor: benchmarkResultsMap[result]
        };
        return (
