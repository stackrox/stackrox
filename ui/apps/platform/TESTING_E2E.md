...
Avoid raw PatternFly CSS classes (`.pf-v6-c-button`) — they change between PF versions. Avoid `data-testid`. Avoid deeply nested DOM selectors (`div div p > button`)
...
Globally applicable helper functions. Use these frequently, and create additional helpers when true cross cutting concerns are needed.
