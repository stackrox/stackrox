...
export type CancelDeferralModalProps = {
    isOpen: boolean;
    onRequestCancelDeferral: (values: CancelFormValues) => Promise<FormResponseMessage>;
    onCompleteCancelDeferral: () => void;
    onCancel: () => void;
};
...
function CancelDeferralModal({
    isOpen,
    onRequestCancelDeferral,
    onCompleteCancelDeferral,
    onCancel,
    ...
    const formik = useFormik<CancelFormValues>({
        ...
        onSubmit: (values: CancelFormValues) => {
            const response = onRequestCancelDeferral(values);
            return response;
        },
    });
