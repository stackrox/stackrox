...
function PolicyCriteriaFieldInput({
    ...
    switch (descriptor.type) {
        ...
        case 'select': {
            ...
            // selected resource.
            if (
                descriptor.name === 'Kubernetes API Verb' ||
                descriptor.name === 'Kubernetes Resource'
            ) {
                    ...
                    if (section) {
                        if (descriptor.name === 'Kubernetes API Verb') {
                            ...
                            const selectedResources = (resourceGroup?.values ?? [])
                                .map((v) =>
                                    typeof v.value === 'string' ? v.value.toUpperCase() : ''
                                )
                                .filter(Boolean);
                            ...
                            if (selectedVerbs.length > 0) {
                                filteredOptions = filteredOptions.filter((opt) => {
                                    const allowed =
                                        auditLogAllowedVerbsByResource[opt.value.toUpperCase()];
                                    return (
                                        !allowed || selectedVerbs.some((v) => allowed.includes(v))
                                    );
                                });
                            }
                        }
