...
import { getRequestQueryStringForSearchFilter, getUrlQueryStringForSearchFilter, generatePathWithQuery } from 'utils/searchUtils';
...
function SummaryStrip() {
    ...
    const items = [
        { label: 'Violations', value: data?.violationCount, href: violationsFullViewPath, isCritical: true },
        { label: 'Clusters', value: data?.clusterCount, href: clustersBasePath },
        { label: 'Deployments', value: data?.deploymentCount, href: `${configManagementPath}/${urlEntityListTypes[resourceTypes.DEPLOYMENT]}` },
        { label: 'Images', value: data?.imageCount, href: generatePathWithQuery(vulnerabilitiesAllImagesPath, {}, { customParams: { entityTab: 'Image' } }) },
        { label: 'Nodes', value: data?.nodeCount, href: `${configManagementPath}/${urlEntityListTypes[resourceTypes.NODE]}` },
        { label: 'Secrets', value: data?.secretCount, href: `${configManagementPath}/${urlEntityListTypes[resourceTypes.SECRET]}` },
    ];
    ...
    return (
        <div className="flex gap-px overflow-hidden rounded-xl bg-border-subtle">
            {items.map((item) => (
                ...
                >
                    <div className="text-2xs uppercase tracking-wide text-text-muted">{item.label}</div>
                    {isLoading ? (
                        <Skeleton className="mt-1 h-7 w-12" />
                    ) : (
                        <div className={`mt-0.5 font-mono text-xl font-600 ${item.isCritical ? 'text-severity-critical' : 'text-text-primary'}`}>
...
function ViolationsBySeverity() {
    ...
    const severityTiles = [
        { key: 'CRITICAL_SEVERITY', label: 'Critical', variant: 'critical' as const, borderClass: 'border-l-severity-critical' },
        { key: 'HIGH_SEVERITY', label: 'High', variant: 'high' as const, borderClass: 'border-l-severity-high' },
        { key: 'MEDIUM_SEVERITY', label: 'Medium', variant: 'medium' as const, borderClass: 'border-l-severity-medium' },
        { key: 'LOW_SEVERITY', label: 'Low', variant: 'low' as const, borderClass: 'border-l-severity-low' },
    ];
    ...
    return (
        <Card>
            <CardHeader>
                <CardTitle>
                    {isLoading ? 'Policy violations by severity' : `${totalCount} policy ${pluralize('violation', totalCount)} by severity`}
                </CardTitle>
                <Link to={violationsBasePath} className="text-2xs text-accent-blue hover:underline">View all &rarr;</Link>
            </CardHeader>
            <CardContent>
                {isLoading ? (
                    ...
                    <>
                        <div className="flex gap-2 mb-3">
                            {severityTiles.map((tile) => {
                                ...
                                const qs = getUrlQueryStringForSearchFilter({ ...searchFilter, Severity: tile.key });
                                return (
                                    <Link
                                        key={tile.key}
                                    ...
                                    >
                                        <div className={`font-mono text-2xl font-700 text-${tile.variant === 'critical' ? 'severity-critical' : tile.variant === 'high' ? 'severity-high' : tile.variant === 'medium' ? 'severity-medium' : 'severity-low'}`}>
                                            {count}
                                        </div>
                                        <div className={`mt-1 text-2xs uppercase tracking-wide text-${tile.variant === 'critical' ? 'severity-critical' : tile.variant === 'high' ? 'severity-high' : tile.variant === 'medium' ? 'severity-medium' : 'severity-low'}`}>
                                            {tile.label}
                                        </div>
                                    </Link>
                                );
                        ...
                        <div className="text-2xs text-text-muted mb-1.5">Most recent critical violations</div>
                        {recentData?.alerts?.map((alert) => (
                            <div key={alert.id} className="flex items-center gap-2.5 border-b border-border-subtle py-2 last:border-b-0">
                                <div className="h-2 w-2 shrink-0 rounded-full bg-severity-critical" />
                                <div className="min-w-0 flex-1">
                                    <div className="truncate text-xs text-text-primary">{alert.policy.name}</div>
                                    <div className="truncate font-mono text-2xs text-text-muted">
                                        {alert.deployment?.name ?? alert.resource?.name ?? 'Unknown'}
                                    </div>
                                </div>
    ...
    return (
        <Card>
            <CardHeader>
                ...
                <Link to={vulnerabilitiesAllImagesPath} className="text-2xs text-accent-blue hover:underline">View all &rarr;</Link>
            </CardHeader>
            <CardContent>
                {isLoading ? (
                    ...
                    <table className="w-full">
                        <thead>
                            <tr className="border-b border-border-subtle">
                                <th className="pb-1.5 text-left text-2xs font-500 uppercase tracking-wide text-text-muted">Image</th>
                                <th className="pb-1.5 text-center text-2xs font-500 uppercase tracking-wide text-text-muted">Critical</th>
                                <th className="pb-1.5 text-center text-2xs font-500 uppercase tracking-wide text-text-muted">Important</th>
                            </tr>
                        </thead>
                        <tbody>
                            {data?.images?.map((image) => (
                                <tr key={image.id} className="border-b border-border-subtle last:border-b-0 hover:bg-bg-hover">
                                    <td className="py-2 font-mono text-2xs text-text-primary">{image.name.fullName}</td>
                                    <td className="py-2 text-center">
                                        <Badge variant="critical">{image.imageVulnerabilityCounter.critical.total}</Badge>
                                    </td>
                                    <td className="py-2 text-center">
                                        <Badge variant="high">{image.imageVulnerabilityCounter.important.total}</Badge>
                                    </td>
                                </tr>
                            ))}
                        </tbody>
...
function DeploymentsAtMostRiskCC() {
    ...
    const { data: deployments, isLoading } = useServiceQuery(
        ...
        () => {
            ...
            return { request: request.then((results) => results.map(({ deployment }) => deployment)), cancel };
        }
    );
    ...
    return (
        <Card>
            <CardHeader>
                ...
                <Link to={riskBasePath} className="text-2xs text-accent-blue hover:underline">View all &rarr;</Link>
            </CardHeader>
            <CardContent>
                {isLoading ? (
                    ...
                    <table className="w-full">
                        <thead>
                            <tr className="border-b border-border-subtle">
                                <th className="pb-1.5 text-left text-2xs font-500 uppercase tracking-wide text-text-muted">Deployment</th>
                                <th className="pb-1.5 text-left text-2xs font-500 uppercase tracking-wide text-text-muted">Cluster</th>
                                <th className="pb-1.5 text-right text-2xs font-500 uppercase tracking-wide text-text-muted">Risk</th>
                            </tr>
                        </thead>
                        <tbody>
                            {deployments?.map((d) => {
                                ...
                                return (
                                    <tr key={d.id} className="border-b border-border-subtle last:border-b-0 hover:bg-bg-hover">
                                        <td className="py-2 text-xs text-text-primary">{d.name}</td>
                                        <td className="py-2 font-mono text-2xs text-text-muted">{d.cluster}</td>
                                        <td className="py-2 text-right">
                                            <Badge variant={p <= 2 ? 'critical' : p <= 4 ? 'high' : 'medium'}>
