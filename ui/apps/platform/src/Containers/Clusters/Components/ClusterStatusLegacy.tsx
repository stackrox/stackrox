...
type ClusterStatusLegacyProps = {
    healthStatus: ClusterHealthStatus;
    isList?: boolean;
};

function ClusterStatusLegacy({
    healthStatus,
    isList = false,
}: ClusterStatusLegacyProps): ReactElement {
    const { version } = useMetadata();

    const { overallHealthStatus = 'UNAVAILABLE' } = healthStatus ?? {};

    const { Icon, fgColor } = healthStatusStylesLegacy[overallHealthStatus];
...
export default ClusterStatusLegacy;
