...
const useIntegrations = ({ source, type }: UseIntegrationsParams): UseIntegrationsReturn => {
    const fetchFn = useCallback(() => {
        ...
        if (source === 'aiIntegrations') {
            // The framework's extractIntegrations expects a keyed object; the
            // service unwraps to a bare array, so re-wrap under `integrations`.
            return fetchAiIntegrations().then((integrations) => ({ integrations }));
        }
