...
/**
 * Allows custom proxy endpoints during development by setting the ACS_UI_CUSTOM_PROXIES environment
 * variable to a comma separated list of 'path,target' pairs.
 *
 * @example ACS_UI_CUSTOM_PROXIES='/v1/collections,http://localhost:3030,/v1/newapi,http://localhost:9000' npm run start
 * @returns {[string, string][]} An array of [path, target] pairs
 */
function parseCustomProxies() {
    const proxyString = process.env.ACS_UI_CUSTOM_PROXIES;
    if (!proxyString) {
        return [];
    }
    ...
    if (rawValues.length % 2 !== 0) {
        ...
        console.warn(
            'ACS_UI_CUSTOM_PROXIES must be set with an even number of comma delimited values',
