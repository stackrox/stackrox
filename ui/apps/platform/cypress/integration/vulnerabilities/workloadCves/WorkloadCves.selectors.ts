...
export const selectors = {
    ...
    severityDropdown: '.pf-v6-c-toolbar button[aria-label="CVE severity filter menu toggle"]',
    severityMenuItems: '.pf-v6-c-toolbar [aria-label="CVE severity filter menu items"] ul',
    severityMenuItem: (severity) => `${selectors.severityMenuItems} label:contains("${severity}")`,
    fixabilityDropdown: '.pf-v6-c-toolbar button[aria-label="CVE status filter menu toggle"]',
    fixabilityMenuItems: '.pf-v6-c-toolbar [aria-label="CVE status filter menu items"] ul',
