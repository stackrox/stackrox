...
describe('Clusters Sensor Compatibility Status', () => {
    ...
    it('should display the sensor compatibility status column and cell values', () => {
        ...
        cy.get(`td[data-label="Sensor compatibility status"]`).then(($cells) => {
            expect($cells.eq(0)).to.contain('Matched');
            expect($cells.eq(1)).to.contain('Compatible (Behind)');
            expect($cells.eq(2)).to.contain('Compatible (Ahead)');
            expect($cells.eq(3)).to.contain('Incompatible (Behind)');
            expect($cells.eq(4)).to.contain('Incompatible (Ahead)');
            expect($cells.eq(5)).to.contain('Unknown');
            // Unexpected enum value from server should fall back to Unknown
            expect($cells.eq(6)).to.contain('Unknown');
        });
    });
});
